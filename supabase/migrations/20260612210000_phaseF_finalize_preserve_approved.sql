-- PRD-025 Option A: engine_finalize_pod preserves 'approved' on no-op re-finalize.
-- Problem: the upsert unconditionally SET status='draft', so finalize running
-- AFTER approve silently un-approves every touched row and the next stitch
-- raises 'no approved rows' (seen in FE Commit, night of 2026-06-11/12:
-- approve 01:53:30.2 -> finalize 01:53:30.7 -> stitch failed).
-- Fix: approval survives when the incumbent row is approved and materially
-- unchanged (same qty + action); any material change still reverts to draft
-- and demands re-approval. action is part of the conflict key so the action
-- predicate is tautological on conflict; kept verbatim from PRD-025 for clarity.
--
-- Diff surface vs live v13_subset_aware_decision (md5 ec8ace36cc2b1a6527bc0eb8ea185b6d,
-- captured 2026-06-12, 2-arg overload; 1-arg wrapper untouched):
--  1. inserted CTE ON CONFLICT: status = CASE approved+unchanged THEN 'approved' ELSE 'draft' END
--  2. engine_version: v13_subset_aware_decision -> v14_preserve_approved
-- Rollback: redeploy the v13 body (restore status='draft' + version string).
