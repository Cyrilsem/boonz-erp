-- Refill System v2 / Phase 2 (#3) — stitch_pod_to_boonz: physical REMOVE/M2W fallback line.
--
-- ROOT CAUSE (investigated 2026-06-01): the REMOVE/M2W dispatch line is built in remove_lines_raw,
-- which INNER JOINs v_pod_inventory_latest (pil) with current_stock>0. VOX-sourced products sit
-- physically in the machine (WEIMI/v_live_shelf_stock sees them) but are NOT tracked in
-- pod_inventory, so pil is empty and the remove line is silently dropped (e.g. OMDCW A01 Popit
-- remove never reached the driver). It surfaces only in the diag array as 'no_inventory_to_remove',
-- which the driver never sees. NOTE: Popit is fully mapped (117 active mappings) and 0 pods are
-- physically-present-unmapped fleet-wide — so this is the pod_inventory gate, not a mapping gap.
--
-- FIX (verbatim reproduction of stitch_pod_to_boonz v12, diff-gated; intended changes only):
--   (A) new CTE remove_lines_physical_fallback — for REMOVE/M2W rows where the mapped+inventory
--       path (remove_lines_filtered) produced NOTHING for that (machine,shelf,pod) AND the row is
--       NOT internal_transfer (those must stay on the tracked path that the invariant checks), emit
--       a driver line keyed by pod with boonz_product_id NULL and qty = physical v_live_shelf_stock
--       current_stock (joined slot_name<->shelf_code, the corrected path). current_stock>0 only.
--   (B) all_lines UNION ALLs the fallback.
--   (C) comment CASE: new branch for action='REMOVE' AND boonz_product_id IS NULL flagging a
--       physical/untracked removal (clear shelf, no WH credit).
--   (D) engine_version 'v12_wh_decoupled_fanout_fix_diag' -> 'v13_physical_remove_fallback'.
-- The existing mapped path, the internal_transfer fan-out invariant, deviations, and procurement
-- alerts are untouched. Writes refill_plan_output only via the existing write_refill_plan(v_lines)
-- path; no new raw write path.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v13_physical_remove_fallback',
