-- PRD-053 Phase A (HEADLINE) - stitch REMOVE conservation: physical fallback sizes from the POD PLAN
-- total, never live shelf / pod_inventory stock.
--
-- Proven defect (rolled-back, 2026-06-23, VML-1004-0500-O1 shelf A03 Ice Tea): an approved REMOVE 13
-- where pod_inventory Ice Tea-Peach is Inactive (so the active-inventory path does not fire) fell through
-- to the physical-fallback CTEs, which sized variant_final from v_live_shelf_stock (4) -> emitted ONE line
-- of 4, leaking 7 units. v27's remove_lines (active path) already conserves to a.qty; only the
-- remove_phys_* fallback chain still capped to physical stock.
--
-- Fix (surgical, 5 references): remove_phys_base carries a.qty AS plan_qty (was vls.current_stock AS
-- phys_stock); remove_phys_qty / remove_phys_rem / remove_phys_final distribute plan_qty. Single-variant
-- Ice Tea: plan 13 -> 13. Multi-variant: plan_qty split by mapping. Everything else is v27 verbatim.
-- The per-expiry detail (6 known + 7 to-confirm) is added downstream at dispatch (PRD-053 Phase B); at the
-- stitch line level conservation means SUM(REMOVE lines) = plan total, which push_plan_to_dispatch then
-- asserts (stitch_leakage). Proven: rolled-back replay now yields REMOVE sum = 13.
--
-- Forward-only CREATE OR REPLACE. SECURITY DEFINER guards preserved (app.via_rpc/app.rpc_name/role).
-- engine_add_pod + engine_swap_pod untouched (md5 unchanged). No data loss. version v27 -> v28.
           ...
           CASE
             WHEN a.action IN ('REFILL','ADD_NEW') THEN
               CASE
                 ...
                 WHEN COALESCE((SELECT SUM(vp.warehouse_stock)::int FROM public.v_wh_pickable vp JOIN public.product_mapping pm2 ON pm2.boonz_product_id = vp.boonz_product_id AND pm2.pod_product_id = a.pod_product_id AND pm2.status = 'Active' AND (pm2.machine_id IS NULL OR pm2.machine_id = a.machine_id)), 0) = 0
