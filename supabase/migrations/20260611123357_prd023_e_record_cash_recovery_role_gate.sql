-- PRD-023e: record_cash_recovery was callable by ANY authenticated user (no role check).
-- The VOX-facing dashboard login (tracker_boonz) must be read-only: cash recovery is a
-- Boonz-internal write. Adds role allowlist + app.via_rpc audit config (Articles 3, 4, 8).
...
BEGIN
  ...
  -- Role gate: Boonz internal roles only. Client/tracker accounts are read-only.
  v_user_id := auth.uid();
  ...
  SELECT COALESCE(SUM(captured_amount_value), 0) INTO v_adyen_captured
  FROM adyen_transactions
  WHERE merchant_reference = p_merchant_reference AND status = 'SettledBulk';
  ...
  v_remaining_gap := GREATEST(v_txn_total - v_adyen_captured - v_total_cash, 0);
