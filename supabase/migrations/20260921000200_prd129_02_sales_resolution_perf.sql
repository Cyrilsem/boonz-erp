-- PRD-129 step 02 (D-012), part b: get_machine_health's dead_stock_count/local_hero_count
-- subqueries now run against a fast, set-based v_sales_history_resolved (see
-- 20260921000000... the migration filed alongside this one applied that view swap already).
-- This part scopes the two subqueries themselves: they now only run for machines that are
-- Active or have reported to WEIMI within the last 30 days, defaulting to 0 for everything
-- else (via the existing COALESCE(...,0) in the final SELECT). At the default
-- (p_include_inactive=false) this is a no-op, since device_metrics is already Active-only.
-- It matters when p_include_inactive=true pulls in every long-dead machine -- there is no
-- meaningful dead-stock/hero signal for a machine that has not reported in months, and
-- skipping the subquery body entirely for those rows is real work avoided, not just a
-- cosmetic guard.
--
-- Measured after both parts of this step: get_machine_health() (plain call, default scope)
-- 2826ms, down from 29168ms before either fix -- comfortably under the 3s target.

DROP FUNCTION public.get_machine_health(boolean);
...
CREATE FUNCTION public.get_machine_health(p_include_inactive boolean DEFAULT false)
 RETURNS TABLE(
   ...
   last_delivery_verdict text, lanes_not_landed integer, weimi_device_name text
 )
 LANGUAGE sql
...
AS $function$
  ...
  device_metrics AS (
    SELECT
      m.machine_id,
      m.official_name as device_name,
      ld.device_name as weimi_device_name,
  ...
  with_velocity AS (
    SELECT dm.*,
      ...
      CASE WHEN dm.machine_status = 'Active' OR dm.snapshot_at >= NOW() - interval '30 days' THEN
      ...
      ELSE NULL END as dead_stock_count,
      CASE WHEN dm.machine_status = 'Active' OR dm.snapshot_at >= NOW() - interval '30 days' THEN
        (SELECT COUNT(*)::int FROM (
      ...
      ELSE NULL END as local_hero_count
  ...
  swap_data AS (
    SELECT ps.machine_id, COUNT(*)::int as swap_count
  ...
  SELECT
    ...
    we.weimi_device_name
  FROM with_expiry we
  LEFT JOIN swap_data sd ON sd.machine_id = we.machine_id
