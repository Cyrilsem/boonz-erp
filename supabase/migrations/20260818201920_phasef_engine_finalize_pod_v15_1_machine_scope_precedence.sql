...
-- engine_finalize_pod v15.1: machine-scoped Active product_mapping rows, when present, become the
-- authoritative variant set for source_origin (globals ignored); vox_at_venue only when ALL rows
-- in the chosen set are venue_team. This is the current live body (engine_version literal below).
...
BEGIN
  ...
  unioned AS (
    ...
    UNION ALL
    SELECT plan_date, machine_id, shelf_id, pod_product_id, action, qty, reasoning,
           NULL::jsonb, NULL, swap_id FROM swap_remove_lines
    UNION ALL
    SELECT plan_date, machine_id, shelf_id, pod_product_id, action, qty, reasoning,
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version', 'v15_1_machine_scope_precedence',
