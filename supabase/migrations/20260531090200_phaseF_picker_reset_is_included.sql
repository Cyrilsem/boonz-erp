-- PRD-015 Phase A / AC#10 (part 2) — pick_machines_for_refill ON CONFLICT resets is_included=true.
-- Body is a VERBATIM copy of the live pg_get_functiondef (captured 2026-05-31) with EXACTLY ONE
-- change: `is_included = true` added to the ON CONFLICT DO UPDATE SET list (a fresh pick is a fresh
-- route, so a re-picked machine returns to included, consistent with the existing confirmed_at reset).
-- REVIEW NOTE for sign-off: diff this body against `SELECT pg_get_functiondef(...)` before apply;
-- the only intended delta is the single `is_included=true,` assignment near `confirmed_at=NULL`.
-- Depends on 20260531090000_phaseF_mtv_is_included. NOT YET APPLIED.
...
BEGIN
  ...
  WITH scored AS (
    SELECT s.*,
      CASE
        WHEN s.empty_shelves_count > 1 OR s.expired_skus_now >= 1 OR s.fill_pct < 50
          OR (s.runway_days IS NOT NULL AND s.runway_days < 2) THEN 'critical'
        WHEN s.empty_shelves_count = 1 OR s.expired_skus_3d >= 1
          OR (s.runway_days IS NOT NULL AND s.runway_days < 4) THEN 'urgent'
        WHEN s.expired_skus_7d >= 1 OR (s.fill_pct >= 50 AND s.fill_pct < 70) THEN 'high'
        WHEN s.days_since_visit >= 15 OR s.active_intent_count > 0 THEN 'medium'
        ELSE 'skip'
      END AS severity,
      ARRAY_REMOVE(ARRAY[
  ...
  with_score AS (
    SELECT sc.*,
      CASE sc.severity
        WHEN 'critical' THEN 100 WHEN 'urgent' THEN 75
        WHEN 'high' THEN 50 WHEN 'medium' THEN 25 ELSE 0
  ...
  sibling_picks AS (
    SELECT ws.* FROM with_score ws
     WHERE ws.r_cluster IN (SELECT r_cluster FROM primary_picks)
       ...
       AND (ws.empty_shelves_count > 0 OR ws.fill_pct < 70 OR ws.days_since_visit >= 7
         OR ws.expired_skus_7d > 0 OR ws.active_intent_count > 0)
  ),
  final_picks AS (
    ...
    SELECT sp.*, true AS sibling,
           ((CASE WHEN sp.empty_shelves_count > 0 THEN 8 ELSE 0 END) +
            ...
            (CASE WHEN sp.active_intent_count > 0 THEN 5 ELSE 0 END))::numeric(5,2) AS final_score
  ...
  ordered AS (
    SELECT fp.*,
      ...
      ROW_NUMBER() OVER (
        ORDER BY fp.r_cluster NULLS LAST, fp.final_score DESC,
                 fp.expired_skus_now DESC, fp.units_last_7d DESC, fp.official_name
      )::integer AS v_order
    FROM final_picks fp
  )
  ...
  ON CONFLICT (plan_date, machine_id) DO UPDATE
     SET official_name=EXCLUDED.official_name, location_type=EXCLUDED.location_type,
         ...
         confirmed_at=NULL, confirmed_by=NULL, is_included=true, updated_at=now();
