-- PRD-087: "last plan" clock means EXECUTED, so planned = visited.
--
-- get_machine_health(): the plan_data CTE now requires rpo.dispatched = true
-- (was: any approved plan <= today). Effect: last_plan_date = the last plan
-- actually pushed/executed; for a dispatched+complete refill it equals the
-- visit date. Approved-but-never-dispatched plans no longer inflate the clock.
-- Manual on-spot refills stay visible via days_since_visit (unchanged - it
-- reads v_machine_health_signals, which this migration does not touch).
--
-- ONLY the plan_data CTE changed; everything else byte-identical to the live
-- body (base md5 96ef4dd57267b1c47de5818faa72f9ce, base64-verified). Read-only
-- reader function: no write path, no data change, no view change.
-- Cody-reviewed (canonical reader, Articles 12/14/16 - see PRD-087 execution
-- log). Forward-only. Rollback = CREATE OR REPLACE without the one AND line.
