-- Refill System v2 / Phase 2 (#8 / F6) - swaps on/off flag (per-machine + global).
--
-- Lets CS toggle the autonomous swap engine off globally or per-machine (DONE-WHEN: "swaps
-- toggleable"). Flag lives in a new refill_settings KV table (CS chose this over altering the
-- protected machines table). engine_swap_pod (v9_2 -> v9_3) skips disabled machines in BOTH passes.
--   effective_swaps_enabled(machine) = per-machine override ?? global ?? true (default ON).
-- Parts: (A) refill_settings table + set_swaps_enabled DEFINER; (B) engine_swap_pod v9_3 (verbatim
-- repro of v9_2, diff-gated).
-- APPLIED 2026-06-01 (CS sign-off; Cody-approved Articles 1,2,4,12,14; diff-gated; verified pg_proc + RLS + seed).

-- ── (A) refill_settings KV table ────────────────────────────────────────────
...
DROP POLICY IF EXISTS refill_settings_select ON public.refill_settings;
CREATE POLICY refill_settings_select ON public.refill_settings
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.user_profiles
                 WHERE id = (SELECT auth.uid())
                   AND role = ANY (ARRAY['operator_admin','superadmin','manager'])));
-- No insert/update/delete policy: refill_settings is written only by set_swaps_enabled (DEFINER owner bypass).

-- Seed the global default ON (idempotent).
INSERT INTO public.refill_settings(setting_key, setting_value)
VALUES ('swaps_enabled', 'true'::jsonb)
ON CONFLICT (setting_key) DO NOTHING;

CREATE OR REPLACE FUNCTION public.set_swaps_enabled(
  p_enabled    boolean,
...
BEGIN
  ...
  PERFORM set_config('app.rpc_name', 'set_swaps_enabled', true);
  ...
  IF v_user_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'set_swaps_enabled: caller % lacks operator_admin/superadmin role', v_user_id;
  END IF;

  IF p_enabled IS NULL THEN RAISE EXCEPTION 'p_enabled required'; END IF;

  IF p_machine_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.machines WHERE machine_id = p_machine_id
  ) THEN
    RAISE EXCEPTION 'set_swaps_enabled: machine_id % not found', p_machine_id;
  END IF;

  v_key := CASE WHEN p_machine_id IS NULL THEN 'swaps_enabled'
                ELSE 'swaps_enabled:' || p_machine_id::text END;

  INSERT INTO public.refill_settings(setting_key, setting_value, updated_at, updated_by)
  VALUES (v_key, to_jsonb(p_enabled), now(), v_user_id)
  ON CONFLICT (setting_key) DO UPDATE
    SET setting_value = EXCLUDED.setting_value, updated_at = now(), updated_by = EXCLUDED.updated_by;

  RETURN jsonb_build_object('status','ok','scope',CASE WHEN p_machine_id IS NULL THEN 'global' ELSE 'machine' END,
                            'machine_id', p_machine_id, 'swaps_enabled', p_enabled);
END;
$function$;
GRANT EXECUTE ON FUNCTION public.set_swaps_enabled(boolean, uuid) TO authenticated;

-- ── (B) engine_swap_pod v9_3: skip swap-disabled machines (verbatim repro of v9_2) ──
...
BEGIN
  ...
  -- (A) Machine-present pods: every pod_product already placed on a machine, by the engine's own
  -- model (slot_lifecycle is_current) OR physically in WEIMI (current_stock>0). A substitute that
  -- appears here for a machine must NOT be introduced again on that machine. [#2 dedup-guard]
  ...
  SELECT p_plan_date, tr.machine_id, tr.shelf_id_final, tr.pod_out,
    ...
    jsonb_build_object('pass','1','source','strategic_machine_tag','tag_id', tr.tag_id,
                       ...
                       'engine_version','v9_3_swaps_flag')
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_cooldown', v_skip_cooldown, 'engine_version','v9_3_swaps_flag',
