-- PRD-110 · leg 164 · D-40 · THE CHIP SURFACE GAINS THE INTENTS CHIP
--
-- Cody's blocking revision on 20260809040500, Article 16, PRD-100 precedent
-- (METRICS_REGISTRY: "the chip surface (get_machine_health + consistency guard) gains the
-- holes chip" landed in the SAME unit that added s_holes).
--
-- ⛔⛔ WHY THIS COULD NOT WAIT FOR THE DAY THE DIAL IS TURNED. `urgency_breakdown` is a
--     registered surface whose invariant is "pts sum == v_machine_priority.urgency", and its
--     `runout` chip is computed as a RESIDUAL: urgency minus the six explicit terms. An eighth
--     term in `urgency` therefore lands inside the chip LABELLED RUNOUT. And the guard that
--     exists to catch exactly that, check_priority_surface_consistency(), derives its own
--     chip_runout by subtracting the SAME six - so both sides move together and the guard is
--     blind by construction (S-315: a constraint the guard could never reach). It is inert only
--     while w_intents = 0, i.e. it arms itself on the one day nobody is looking at chips.
--
-- ⭐ At w_intents = 0 both functions are output-identical: the residual loses a zero, and the
--    new chip is filtered out by the pre-existing `WHERE t.pts <> 0`. Proven below by identity.
--
-- get_machine_health                     prosrc md5 804a1a83 -> (post-image asserted below)
-- check_priority_surface_consistency     prosrc md5 5effaf44 -> (post-image asserted below)

CREATE TEMP TABLE _d40c_pre ON COMMIT DROP AS
  SELECT machine_name, priority_tier, priority_score, urgency_breakdown,
         COALESCE((SELECT round(sum((e->>'pts')::numeric),2)
                     FROM jsonb_array_elements(COALESCE(urgency_breakdown,'[]'::jsonb)) e), 0) AS bsum
    FROM public.get_machine_health();

CREATE TEMP TABLE _d40c_ctx ON COMMIT DROP AS
  SELECT (SELECT count(*) FROM _d40c_pre)                                         AS n_pre,
         (SELECT count(*) FROM public.check_priority_surface_consistency())        AS guard_pre,
         (SELECT proacl::text FROM pg_proc WHERE proname='get_machine_health')     AS acl_gmh_pre,
         (SELECT proacl::text FROM pg_proc WHERE proname='check_priority_surface_consistency') AS acl_grd_pre,
         (SELECT w_intents FROM public.pick_urgency_params ORDER BY id LIMIT 1)    AS w_pre;

DO $g$
BEGIN
  IF (SELECT w_pre FROM _d40c_ctx) <> 0 THEN
    RAISE EXCEPTION 'D-40 chip PRE: w_intents is % - this migration proves output-identity at 0 and cannot prove it at any other value', (SELECT w_pre FROM _d40c_ctx);
  END IF;
  IF (SELECT guard_pre FROM _d40c_ctx) <> 0 THEN
    RAISE EXCEPTION 'D-40 chip PRE: check_priority_surface_consistency already returns % rows - fix the surface before adding to it', (SELECT guard_pre FROM _d40c_ctx);
  END IF;
  IF (SELECT n_pre FROM _d40c_ctx) < 20 THEN
    RAISE EXCEPTION 'D-40 chip PRE: get_machine_health returned % rows', (SELECT n_pre FROM _d40c_ctx);
  END IF;
END $g$;
       ...
       FROM (VALUES
         ('runout',
          round(COALESCE(mp.urgency,0),2)
            ...
            - round(pup.w_intents  * COALESCE(mp.s_intents,0), 2)),
         ...
         ('intents',          round(pup.w_intents  * COALESCE(mp.s_intents,0), 2))
       ) t(l, pts)
       WHERE t.pts <> 0)
    END,
    mp.reasons_arr
  ...
  CROSS JOIN LATERAL (VALUES
    ...
    ('chip_runout',   round(ch.chip_runout,2)::text,
       round((round(COALESCE(mp.urgency,0),2)
        ...
        - round(pup.w_lowfill  * COALESCE(mp.s_lowfill,0), 2)
        - round(pup.w_holes    * COALESCE(mp.s_holes,0), 2)
        - round(pup.w_intents  * COALESCE(mp.s_intents,0), 2)), 2)::text),
    ('chip_holes',    round(ch.chip_holes,2)::text,    round(pup.w_holes    * COALESCE(mp.s_holes,0), 2)::text),
    ('chip_intents',  round(ch.chip_intents,2)::text,  round(pup.w_intents  * COALESCE(mp.s_intents,0), 2)::text)
  ) d(field, hv, cv)
...
DECLARE
  ...
  v_n     int;
BEGIN
  IF (SELECT prosrc FROM pg_proc WHERE proname='get_machine_health') NOT LIKE '%w_intents%' THEN
    RAISE EXCEPTION 'D-40 chip POST: get_machine_health does not carry the intents term';
  END IF;
  IF (SELECT prosrc FROM pg_proc WHERE proname='check_priority_surface_consistency') NOT LIKE '%chip_intents%' THEN
    RAISE EXCEPTION 'D-40 chip POST: the guard does not carry chip_intents';
  END IF;

  -- ⛔ OUTPUT IDENTITY AT w_intents = 0, per machine, on the surface FE renders verbatim.
  --    urgency_breakdown is compared as jsonb, not as a sum: a sum would be preserved by a
  --    chip that moved points from one label to another, which is the exact defect this unit
  --    exists to prevent.
  SELECT count(*) INTO v_diff
    FROM _d40c_pre a
    FULL OUTER JOIN public.get_machine_health() b ON b.machine_name = a.machine_name
   WHERE a.machine_name IS NULL OR b.machine_name IS NULL
      OR a.priority_tier      IS DISTINCT FROM b.priority_tier
      OR a.priority_score     IS DISTINCT FROM b.priority_score
      OR a.urgency_breakdown  IS DISTINCT FROM b.urgency_breakdown;

  IF v_diff <> 0 THEN
    RAISE EXCEPTION 'D-40 chip POST: % machines changed on the chip surface at w_intents = 0', v_diff;
  END IF;

  SELECT count(*) INTO v_guard FROM public.check_priority_surface_consistency();
  IF v_guard <> 0 THEN
    RAISE EXCEPTION 'D-40 chip POST: the consistency guard now reports % disagreements', v_guard;
  END IF;

  -- Article 4/13: the grant surface of both DEFINER functions is unchanged by the replace
  IF (SELECT proacl::text FROM pg_proc WHERE proname='get_machine_health')
     IS DISTINCT FROM (SELECT acl_gmh_pre FROM _d40c_ctx) THEN
    RAISE EXCEPTION 'D-40 chip POST: get_machine_health EXECUTE grants moved';
  END IF;
  IF (SELECT proacl::text FROM pg_proc WHERE proname='check_priority_surface_consistency')
     IS DISTINCT FROM (SELECT acl_grd_pre FROM _d40c_ctx) THEN
    RAISE EXCEPTION 'D-40 chip POST: guard EXECUTE grants moved';
  END IF;

  IF NOT (SELECT prosecdef FROM pg_proc WHERE proname='get_machine_health')
     OR (SELECT proconfig FROM pg_proc WHERE proname='get_machine_health')
        IS DISTINCT FROM ARRAY['search_path=public, pg_temp'] THEN
    RAISE EXCEPTION 'D-40 chip POST: get_machine_health lost SECURITY DEFINER or its search_path';
  END IF;

  SELECT count(*) INTO v_n FROM public.get_machine_health();
  RAISE NOTICE 'D-40 chip ok: % machines, breakdown byte-identical, guard clean', v_n;
END $g$;
