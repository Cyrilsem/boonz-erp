-- WS-A PRIMARY - stitch_pod_to_boonz v18: make mix_weight the canonical pod->SKU split source. STATUS: DRAFT -
-- NOT APPLIED. Cody-reviewed below. Builds on v17 (WS6.2, 20260604200000); full CREATE OR REPLACE that SUBSUMES
-- v15+v16+v17 - apply last in the stitch chain (or alone). REQUIRES the WS-A backfill (20260604210000) applied
-- FIRST so mix_weight = split_pct/100 (sum 1.0/pod); without it every multi-variant pod would mis-split.
--
-- PROBLEM (WS-A): apply_mix_weight_recommendation writes product_mapping.mix_weight, but the fan-out read
-- split_pct, so confirmed recommendations never reached the plan. This switches every split_pct math site in
-- stitch to mix_weight (the 0-1 fraction summing to 1.0/pod) so recommendations are live and stitch agrees with
-- get_procurement_demand (also switched).
--
-- FIX (diff-gated to source + scale only, vs v17): in all three split blocks - the REFILL/ADD pull fan-out
-- (pull_raw/pull_norm/pull_base), the WS1b REMOVE physical-fallback split (remove_phys_*), and the
-- deviation+procurement_alert blocks (m_raw/n/b, pm_per_row/demand):
--   * source  : pm.split_pct -> pm.mix_weight AS split_pct (column name kept so downstream CTE refs are
--               untouched; the column now carries the 0-1 fraction). Alert default 20 -> 0.20.
--   * scale   : pod_qty*norm_split/100.0 -> pod_qty*norm_split ; qty*split_pct/100.0 -> qty*split_pct.
--   * fallback: total_split=0 -> 1.0/variant_n (was 100.0/variant_n).
-- After the backfill (mix_weight = split_pct/100) this is numerically identical to v17; a confirmed
-- apply_mix_weight_recommendation then shifts the splits. PRESERVED byte-for-byte: the inline confirm gate
-- (write_refill_plan ok -> confirm, else skipped_write_failed), the WS6.2 0-stock-anywhere pull_lines filter,
-- the WS1b physical-fallback variant resolution, the internal_transfer fan-out invariant, role gate, app.via_rpc.
-- engine_version -> 'v18_wsa_mix_weight_canonical'. split_pct stays a read-only mirror until 2026-07-04.
...
BEGIN
  ...
  pull_norm AS (
    SELECT pnp.*,
           CASE WHEN total_split=0 THEN 1.0/variant_n ELSE COALESCE(pnp.split_pct,0) END AS norm_split
      FROM pull_norm_pre pnp
  ),
  pull_base AS (
    SELECT pn.*,
           FLOOR(pod_qty*norm_split)::int AS base_qty,
  ...
  pull_lines AS (
    SELECT plan_date,machine_id,shelf_id,pod_product_id,action,
     ...
     WHERE variant_final>0
       -- WS6.2: drop a resolved variant with literally 0 stock ANYWHERE (wh_avail = SUM over ALL warehouses,
       -- Active, non-quarantined; NOT the §1 serving-WH gate). Warehouse-sourced only; vox/internal exempt.
  ...
  -- WS1b: physical-fallback REMOVE/M2W (no Active pod_inventory match) resolved to CONCRETE boonz
  -- variant(s) via product_mapping (mirrors the REFILL pull path), splitting the physical shelf qty by
  -- split_pct (even fallback), remainder ±1. qty-0 planned REMOVE skipped (a.qty>0). Unmapped pods drop.
  remove_phys_base AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           a.machine_name,a.shelf_code,a.pod_product_name,
           a.source_origin, a.from_machine,
  ...
  ELSE
    -- MERGED: phaseF_stitch_gate_confirm_on_write_ok (2026-06-03). Only confirm when the write
    -- actually succeeded; otherwise leave the pod plan 'approved' (retryable + visible). Without
    -- this the pod flips to 'stitched' on a write validation_error while refill_plan_output stays
    -- empty -> silent whole-machine dispatch loss (the VML failure). DO NOT remove on re-baseline.
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v18_wsa_mix_weight_canonical',
