-- PRD-122 (lane-grain-priority) T3 + T3b / R3 + R3b / D4-D8 + D6b.
--
-- T3: p_score becomes a weighted sum (R3): w_runout*s_runout_hero + w_gap*s_gap +
-- w_holes*s_holes + w_expiry*s_expiry + w_stale*s_stale. Bundles R4's view-internal
-- additions (s_runout_hero, hero_runway_days, worst-three breadth_runout mean) into
-- this same migration, since R3's formula textually depends on s_runout_hero and
-- step 5 of the build order requires T3 alone to already reproduce the final tier
-- table -- see the commit history for the full reasoning (T4 is left with only the
-- pick_machines_for_refill change, which is a genuinely separate object).
-- w_holes 0.30->0.15, w_stale 0.13->0.08 (Q2) so the five weights sum to 1.00.
-- New P1 override: s_gap >= p1_gap_min (40). New reasons: capacity_gap,
-- quasi_empty_heavy. p1_threshold 50->32, p2_threshold 25->18. urgency keeps the old
-- GREATEST(...) untouched (Q1) as a rollback comparison column.
--
-- get_machine_health (D7): urgency_breakdown chips become the literal addends of
-- p_score (runout/gap/holes/expiry/stale), dropping the leftover-equals-runout
-- subtraction and the capacity/empty/lowfill/intents chips (those signals are no
-- longer score inputs -- see Q5, the five params they read go inert here).
--
-- check_priority_surface_consistency (T3b/R3b/D6b): realigned to the new chip shape
-- so this currently-green invariant does not go red the moment T3 ships (F11, Q11).
-- Exactly five edits inside its VALUES list: urgency_breakdown_sum's canonical moves
-- to mp.p_score; chip_capacity and chip_intents rows dropped; chip_runout's canonical
-- drops the leftover subtraction; a chip_gap row is added. The ch CROSS JOIN LATERAL
-- extraction block is left completely untouched -- chip_gap's health-value side is
-- inlined directly in its own VALUES tuple instead, keeping the diff to exactly the
-- VALUES list as R3b specifies ("no more").
--
-- Two real bugs found and fixed during verification, both before this file was
-- committed:
--  1. p_score's first draft rounded the raw weighted SUM once at the end
--     (round(w_runout*s_runout_hero + w_gap*s_gap + ..., 2)), while
--     get_machine_health's chips round each of the five terms individually and sum
--     those. Sum-of-rounded-parts can legitimately differ from round-of-sum by
--     +/-0.01 -- not a coincidence, a real discrepancy on 3 of 32 machines the first
--     time this was tested. Fixed by making p_score the sum of the same five
--     individually-rounded terms the chips display, so A8's "chips sum exactly to
--     p_score" holds by construction, not by chance.
--  2. That fix alone left 2 machines still off by 0.01: p_score's internal calc was
--     rounding each term from FULL-PRECISION mscore signals, while get_machine_health
--     reads the ALREADY-ROUNDED (numeric(6,2)) s_gap/s_runout_hero/etc columns
--     v_machine_priority exposes. Rounding 18.6166... differs from rounding the
--     pre-rounded 18.62. Fixed by rounding each input signal to 2dp first, inside
--     pscore, before multiplying by its weight -- matching exactly what
--     get_machine_health reads.
--
-- Cody: Approve (see the T1-T5 combined review, plus this file's own two fixes,
-- which are pure arithmetic-precision corrections with no design-intent change).
-- Article 12 (forward-only, no signature changes), Article 16 (v_machine_priority
-- and get_machine_health remain the sole canonical objects, edited in place).
--
-- Verified live: check_priority_surface_consistency() returns 0 rows (A11, the hard
-- gate) after both fixes. No NULL p_score across all 32 in-scope machines (A7).
-- ACTIVATEMCC-1037-0000-L0 (A1's worked example) still P1_RESTOCK at 56.31.
--
-- Full 32-machine tier table pulled and diffed against Section 7 (PRD measured
-- 2026-09-13; this was applied 2026-09-14, one day later): 11 of 32 machines differ
-- in tier. All 6 that dropped OUT of P1 relative to Section 7 (MINDSHARE-1009-4500-O1,
-- VML-1003-0400-O1, WAVEMAKER-1006-4100-O1, NOVO-1023-0000-W0, USH-1008-0000-W1,
-- VOXMCC-1005-0201-B0) show days_since_visit=0 today -- they were refilled overnight,
-- which correctly zeroed their urgency signals; this is the system working as
-- designed on genuinely new data, not a defect. The 5 that moved INTO P1/P2
-- (AMZ-1068-2401-O1, AMZ-1046-2406-O1->P2, MC-2004-0100-O1, OMDCW-1021-0100-W0->P2,
-- VOXMCC-1011-0101-B0) are consistent with continued unattended drain since the
-- Section 7 snapshot. Separately: A3/A6's literal wording ("JET-1016 must never rank
-- above any s_gap>=40 machine") does not actually hold even in Section 7's own
-- reference numbers (JET=15.98 already ranks above ALJLT-1015-0100-B1=13.31, whose
-- own s_gap=44.38 per Q9) -- flagged as a PRD-authoring imprecision in the final
-- report, not something this migration attempts to force.
