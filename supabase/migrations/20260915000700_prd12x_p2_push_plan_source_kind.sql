-- PRD-124 #38 / PRD-125 D3 -- source_kind mapped at push.
--
-- push_plan_to_dispatch's ordinary Remove and Refill/Add-New inserts never
-- set source_kind at all (only the M2M branch did, explicitly 'm2m') --
-- every other row defaulted to 'unknown'. Added v_source_kind, computed once
-- per line from source_origin (warehouse->wh, vox_at_venue->venue,
-- internal_transfer->m2m), and threaded it into both non-M2M INSERTs
-- (including the ON CONFLICT DO UPDATE SET for the Refill/Add-New upsert
-- path). rpc_version bumped to v18_prd125_p2_source_kind_mapping.
--
-- VERIFIED FALLOUT (PRD-124 G8d): after this change plus the 000600
-- migration's backfill of the 09-14/09-15 rows, validate_refill_plan's G8
-- check on 2026-09-15 dispatch went from 7 violations to 0 -- every one of
-- them was a venue line the CENTRAL-stock check was wrongly counting as
-- demand, because is_venue (derived from source_kind) always read false.
-- No change to validate_refill_plan itself was needed for G8d.
...
BEGIN
  ...
  LOOP
    ...
    IF v_action IN ('Remove','Machine To Warehouse') THEN
      ...
      INSERT INTO refill_dispatching (
        ...
        dispatch_date, action, quantity, include, comment,
        from_warehouse_id, from_wh_inventory_id, expiry_date, pinned_at_plan_time,
        source_origin, from_machine_id, pod_lot_id, source_kind,
        packed, picked_up, dispatched, returned, item_added
      ) VALUES (
    ...
    INSERT INTO refill_dispatching (
      ...
      dispatch_date, action, quantity, include, comment,
      from_warehouse_id, from_wh_inventory_id, expiry_date, pinned_at_plan_time,
      source_origin, from_machine_id, source_kind,
      packed, picked_up, dispatched, returned, item_added
    ) VALUES (
      v_machine_id, v_shelf_id, v_pod_product_id, v_boonz_product_id,
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v18_prd125_p2_source_kind_mapping'
  );
END $function$
