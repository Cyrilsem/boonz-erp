-- Loop 2026-09-25 (CS ADD TO LOOP, R3): push_plan_to_dispatch plain Remove/M2W lot binding fix.
--
-- DEFERRED, NOT YET APPLIED. push_plan_to_dispatch is a dispatch function; this loop's own hard
-- rule restricts migrations touching dispatch functions to the 22:00-06:00 Dubai window. Drafted
-- and investigated now so it is ready the moment the window opens. Queue order for tonight:
-- R1, R3, R2, R4, R5a, R5c.
--
-- This closes an open issue this loop already named but explicitly left out of scope back in A4
-- (20260925020000_loopv2_a4_m2m_lot_flavour_match.sql): "the equivalent shelf-only lookup for the
-- plain (non-M2M) Remove/Machine To Warehouse leg further down in push_plan_to_dispatch untouched,
-- out of A4's named scope."
--
-- Root cause confirmed live before writing anything: the plain (non-M2M) Remove/Machine To
-- Warehouse leg's lot lookup selects the earliest-expiry Active pod_inventory row on the shelf via
-- v_pod_inventory_latest, filtered only on machine_id, shelf_id, status, current_stock, with no
-- boonz_product_id filter -- the exact same unfiltered shape A4 fixed on the M2M leg. Confirmed on
-- the named evidence: ADDMIND-1007-0000-W0 shelf A16, 2026-09-25, three sibling non-M2M Remove
-- lines (Zero Lemon, Zero peach, Antioxidant) all bound to pod_lot_id 421b4db9 (the Antioxidant
-- lot, expiry 2026-09-27) regardless of which flavour each line actually removes.
--
-- Also checked before writing anything: repair_remove_leg_shelf_lot (the function
-- repair_remove_leg_shelf_lot_bulk calls, itself auto-run at the end of every push) already
-- filters by boonz_product_id correctly. It only repairs rows where pod_lot_id IS NULL, so it
-- never corrected these three ADDMIND lines -- push had already (wrongly) filled pod_lot_id in via
-- the unfiltered fallback, so the auto-repair's own NULL-only WHERE clause never touched them.
-- bind_dispatch_fefo was also checked and ruled out: it only binds Refill/Add/Add New lines, never
-- Remove, so it was never a candidate for this bug.
--
-- Fix: add the same boonz_product_id filter A4 already applies to the M2M leg, and
-- repair_remove_leg_shelf_lot already applies on its own repair path. No matching lot for this
-- product on the shelf now falls back to NULL (SELECT INTO with no matching row), exactly as A4
-- established, never another flavour's lot. Nothing else in this function changes.
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(line.source_origin::text,'warehouse') = 'internal_transfer' THEN
      ...
      BEGIN
        ...
        -- matching lot for this product on the shelf now falls back to NULL, never to another
        -- flavour's lot.
    ...
    IF v_action IN ('Remove','Machine To Warehouse') THEN
      ...
      -- Loop 2026-09-25 R3: filter to the SAME boonz_product_id being removed (v_boonz_product_id),
      -- matching A4's M2M-leg fix and repair_remove_leg_shelf_lot's own already-correct pattern.
      -- Confirmed live before this fix: this plain non-M2M lookup ignored product entirely,
      -- binding ADDMIND-1007 A16's Zero Lemon and Zero peach Remove lines to the shelf's
      -- earliest-expiry lot (Antioxidant, 421b4db9) regardless of flavour. No matching lot for
      -- this product on the shelf now falls back to NULL, never another flavour's lot.
      SELECT pil.expiration_date, pil.pod_inventory_id
        INTO v_remove_lot_expiry, v_remove_lot_id
        FROM public.v_pod_inventory_latest pil
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v22_loopv2_r3_plain_remove_lot_flavour_match'
  );
END $function$;
