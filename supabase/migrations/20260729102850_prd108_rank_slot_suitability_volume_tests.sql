...
-- PRD-108 Volume-Driven Size-Up, step 2/4. Source: docs/prds/PRD-108-EXECUTION-LOG.md Phase 2
-- (2.0 call-site audit, 2.1 Cody review, 2.2 ppad blocker, 2.3 applied); MIGRATIONS_REGISTRY.md:708.
-- This is the authoritative DROP+CREATE that produced the function's current shape: replaces the
-- machine-relative `proven_machine_pctile >= 0.80` size-up test with T1 velocity floor (reads
-- v_shelf_sales_identity.dvel, never the ppad selling-day fallback that overstated calendar
-- velocity 23.6x mean / 90x max - Article 16), T2 overflow, T3 opportunity-cost vs the
-- rank-1-BY-SUITABILITY newcomer (not MAX(exp_vel), which fails the star case), and a machine
-- floor (v_machine_velocity). `sizeup_rationale jsonb` appended last to RETURNS TABLE (safe:
-- one caller, named-column access per the 2.0 call-site audit). DROP+CREATE used because the
-- return type changes; no overload created (pg_proc verified 1 row post-apply).
-- This is the HIGHEST-CONFIDENCE reconstruction of rank_slot_suitability among the three
-- migrations that touch it in this apply order (084422, 084619, 102850) - it is the one that
-- actually produced today's cumulative shape via the mechanism documented in the execution log.

DROP FUNCTION IF EXISTS public.rank_slot_suitability(date, uuid, uuid, uuid, integer, uuid[]);
