-- PRD-114 §3.2 - the disposition writer.
--
-- The driver taps Sold / Remove / Exists / Skip on a sanity-check row. That tap
-- writes ONE day_close_events row of the new kind 'expiry_check' and NOTHING
-- else. No stock moves here. Stock moves at day-close acknowledge, through the
-- existing canonical archival path, and only when CS clicks (§4.1, §4.2).
--
-- v3 compatibility (§4.3): day_close_events keeps its shape. One value is added
-- to the kind CHECK; no column, no table, no cron.

ALTER TABLE public.day_close_events DROP CONSTRAINT IF EXISTS day_close_events_kind_chk;
ALTER TABLE public.day_close_events ADD CONSTRAINT day_close_events_kind_chk
  CHECK (kind = ANY (ARRAY[
    'substitution'::text, 'not_filled'::text, 'shortfall'::text,
    'spot_buy'::text, 'stock_unverified'::text, 'expiry_check'::text]));

-- Acceptance 2: "changing a disposition before acknowledge updates the SAME
-- event, never duplicates". The RPC re-taps by UPDATE, and this index is what
-- makes that a guarantee rather than an intention - two concurrent taps on the
-- same batch cannot both insert.
CREATE UNIQUE INDEX IF NOT EXISTS day_close_events_expiry_check_uniq
  ON public.day_close_events (event_date, ((payload ->> 'pod_inventory_id')))
...
BEGIN
  ...
  -- (PRD-016B). Leave them as we found them.
...
COMMENT ON FUNCTION public.record_expiry_check(uuid, text, date, uuid) IS
  'PRD-114 §3.2. Records one driver disposition (exists|sold|remove|skip) against one pod_inventory batch as a day_close_events row of kind expiry_check. Writes NO stock: pod_inventory is read (FOR UPDATE row lock) and never modified here. Re-tapping updates the same event. An expired batch accepts sold|remove only.';

REVOKE ALL ON FUNCTION public.record_expiry_check(uuid, text, date, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.record_expiry_check(uuid, text, date, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.record_expiry_check(uuid, text, date, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.record_expiry_check(uuid, text, date, uuid) TO service_role;
