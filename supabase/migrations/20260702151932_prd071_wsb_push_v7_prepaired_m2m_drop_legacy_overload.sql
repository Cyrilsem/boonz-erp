-- PRD-071 WS-B: push_plan_to_dispatch v6 -> v7 (autopair M2M) + drop legacy overload.
--
-- 1. DROP public.push_plan_to_dispatch(p_machine_name text, p_plan_date date):
--    prod-only relic (no migration ever created it; none dropped it). Its
--    presence made every PostgREST/named-notation call fail with 42725
--    "function is not unique", silently breaking the FE approve->push flow.
--    CLAUDE.md overload foot-gun rule + Amendment 005 (single canonical writer).
--
-- 2. v7 of the canonical (p_plan_date date, p_machine_name text) writer.
--    Base body = the LIVE prod v6 (prosrc md5 3a1438d7a169b08b4338beec74331c02,
--    full-def md5 d3acc78161d3ca44020cf9242317bbfd - NOT reconstructible from
--    git before this file; this file also closes that PRD-057 drift).
--    Changes, all anchored insertions, v6 logic otherwise byte-identical:
--    a. internal_transfer plan lines become PRE-PAIRED M2M writes: the dest
--       line (Add New/Refill carrying from_machine_id) creates the source
--       Remove leg(s) (FEFO batch split at the source machine, PRD-053
--       EXPIRY-TO-CONFIRM remainder) and the dest leg atomically, all with a
--       shared m2m_transfer_id + crosslinked m2m_partner_id + is_m2m=true,
--       source_kind='m2m'. Satisfies block_orphan_internal_transfer (PRD-070):
--       orphan legs are impossible. Source-side Remove lines defer to the dest
--       push (counted m2m_transfer_deferred). Unroutable / unmatched / failed
--       lines skip-log to monitoring_alerts and NEVER fail the push.
--       Zero warehouse involvement (from_warehouse_id NULL, packed=true,
--       matching swap_between_machines conventions).
--    b. post-loop safety net: pair_internal_transfer_m2m(p_plan_date, caller)
--       (idempotent PRD-070 D-2 pairer) runs same-txn after the loop; failure
--       logs and continues. Provenance GUCs restored after the call (PRD-016B
--       leak lesson).
--    c. rpc_version v7_prd071_autopair_m2m + m2m_* counters in the response.
--
-- Constitution: Articles 1, 3, 4, 6 (no warehouse_inventory.status writes),
-- 8 (via_rpc/rpc_name provenance), 12; Amendments 003 (protected
-- refill_dispatching via reviewed SECURITY DEFINER RPC) and 005.
-- Dara-designed, Cody-reviewed (PRD-071 execution log). Forward-only.
-- engine_add_pod / engine_swap_pod untouched (md5 gate in execution log).
-- swaps_enabled untouched (false). Idempotent: re-push same date+machine is a
-- no-op (dispatched flag + m2m_transfer_id null-filters).

DROP FUNCTION IF EXISTS public.push_plan_to_dispatch(text, date);
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v7_prd071_autopair_m2m'
  );
END $function$;

COMMENT ON FUNCTION public.push_plan_to_dispatch(date, text) IS
'v7_prd071_autopair_m2m (PRD-071 WS-B). Pushes approved refill_plan_output lines to refill_dispatching. internal_transfer lines are created as pre-paired M2M legs (shared m2m_transfer_id, dest-driven, source lines defer); post-loop pair_internal_transfer_m2m safety net; pairing failure never fails the push. Legacy (text,date) overload dropped (42725 ambiguity). Articles 1,3,4,6,8,12; Amendments 003/005.';
