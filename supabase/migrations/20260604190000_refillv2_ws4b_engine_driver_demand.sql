-- Refill reliability / WS4b - engine_add_pod ingests driver_feedback as a demand floor. STATUS: DRAFT - NOT
-- APPLIED. Cody-reviewed. Verbatim reproduction of engine_add_pod v12 (v12_wh_avail_s1_suppress), diff-gated to:
--   1. candidates: LEFT JOIN v_driver_feedback_demand -> + driver_req_qty (the in-window unresolved ask).
--   2. clamped: driver_req_qty added to the raw_qty / final_qty GREATEST (a floor: a driver ask forces the
--      line above velocity targets, still capped by capacity and still suppressed when wh_avail=0).
--   3. clamp_reason: new 'driver_request' branch when the driver ask dominates.
--   4. reasoning jsonb: + driver_req_qty.
--   5. after the plan write: mark ingested feedback resolved (decay closure - the 14d window + resolved flag
--      stop it from boosting forever).
--   6. engine_version -> 'v13_driver_feedback_demand'.
-- Unchanged: role gate, app.via_rpc, the _assert_gate_zero precheck, strategic-intent blocking, the PRD-017
-- §1 wh_avail suppression, signal/visual floors, gap_rows. driver_feedback is not an Appendix-A protected
-- entity; the resolve UPDATE runs inside the DEFINER with app.via_rpc set, consistent with the engine's
-- existing direct writes to its pod_refills staging table.
...
BEGIN
  ...
  candidates AS (
    SELECT
      ...
      -- PRD-017 BUG-A: §1 Available (serving WH per machine, Active, non-quarantined, in-date,
      -- non-reserved-elsewhere; never consumer_stock). Replaces the fleet-wide v_warehouse_pod_rollup.
      COALESCE((
        ...
        WHERE pm.pod_product_id = sl.pod_product_id
          AND pm.status = 'Active'
          AND (pm.machine_id IS NULL OR pm.machine_id = sl.machine_id)
      ), 0)::integer                                          AS wh_avail,
      -- WS4: in-window unresolved driver ask for this (machine, pod), 0 if none.
    ...
    JOIN public.slot_lifecycle sl
      ON sl.machine_id = p.machine_id AND sl.archived = false AND sl.is_current = true
    JOIN public.shelf_configurations sc
      ON sc.shelf_id = sl.shelf_id AND sc.is_phantom = false
    ...
    LEFT JOIN public.v_driver_feedback_demand dfd
      ON dfd.machine_id = sl.machine_id AND dfd.pod_product_id = sl.pod_product_id
  ...
  with_floor AS (
    SELECT s.*,
      ...
      CASE WHEN s.signal IN ('STAR','DOUBLE DOWN','KEEP GROWING','KEEP')
        THEN GREATEST(CEIL(s.max_stock * 0.50)::int - s.current_stock, 0)
        ELSE 0 END AS visual_floor_qty,
      (s.max_stock - s.current_stock) AS gap_units
    FROM sized s
  ),
  clamped AS (
    SELECT wf.*,
      CASE WHEN wf.blocking_intent_id IS NOT NULL THEN 0
        ELSE GREATEST(COALESCE(wf.velocity_raw_qty, 0), wf.signal_floor_qty, wf.visual_floor_qty, COALESCE(wf.driver_req_qty, 0))
      END AS raw_qty,
      -- PRD-017 BUG-A: §1 Available = 0 -> suppress (final_qty 0); gap_rows still emits the procurement_gap.
      -- WS4: COALESCE(driver_req_qty,0) joins the demand GREATEST (a driver ask floors the line above velocity).
      CASE WHEN wf.wh_avail = 0 THEN 0 ELSE LEAST(
        CASE WHEN wf.blocking_intent_id IS NOT NULL THEN 0
          ELSE GREATEST(COALESCE(wf.velocity_raw_qty, 0), wf.signal_floor_qty, wf.visual_floor_qty, COALESCE(wf.driver_req_qty, 0))
        END,
      ...
      CASE
        ...
        WHEN wf.wh_avail = 0                                                                   THEN 'blocked_no_wh'
        WHEN COALESCE(wf.driver_req_qty,0) > 0
             AND COALESCE(wf.driver_req_qty,0) >= GREATEST(COALESCE(wf.velocity_raw_qty,0), wf.signal_floor_qty, wf.visual_floor_qty) THEN 'driver_request'
        WHEN wf.velocity_raw_qty IS NULL AND wf.signal_floor_qty = 0 AND wf.visual_floor_qty = 0 THEN 'skipped_signal'
        ...
        WHEN COALESCE(wf.velocity_raw_qty,0) = 0 AND wf.v30 > 0
             ...
             AND wf.signal_floor_qty = 0 AND wf.visual_floor_qty = 0                           THEN 'skipped_runway_ok'
        WHEN COALESCE(wf.velocity_raw_qty,0) = 0 AND wf.signal_floor_qty = 0 AND wf.visual_floor_qty = 0 THEN 'skipped_no_demand'
  ...
  inserted AS (
    ...
    SELECT p_plan_date, c.machine_id, c.shelf_id, c.pod_product_id,
      ...
      jsonb_build_object(
        ...
        'signal_boost',     CASE c.signal WHEN 'STAR' THEN 2.0 WHEN 'DOUBLE DOWN' THEN 1.5 ELSE 1.0 END,
  ...
  gap_rows AS (
    SELECT c.official_name, c.shelf_code, c.pod_product_name, c.signal,
      c.v30::numeric(6,2) AS v30, c.current_stock, c.max_stock, c.target_stock,
      (c.target_stock - c.current_stock) AS gap_units, c.runway_days
    FROM clamped c
    WHERE c.wh_avail = 0 AND c.v30 > 0 AND c.current_stock < c.target_stock
  ...
  SELECT (SELECT COUNT(*) FROM inserted),
    COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'max_stock', gap_rows.max_stock, 'target_14d', gap_rows.target_stock,
      'target_signal', gap_rows.target_stock, 'gap_units', gap_rows.gap_units,
      'runway_days', gap_rows.runway_days
    ) ORDER BY gap_rows.gap_units DESC), '[]'::jsonb)
  INTO v_refills, v_procurement_gaps FROM gap_rows;

  -- WS4 decay closure: mark resolved the driver feedback that actually produced a refill line this plan.
  UPDATE public.driver_feedback df
     SET resolved = true, resolved_at = now(), resolved_by_engine = 'engine_add_pod_v13'
   WHERE df.resolved = false
     AND df.feedback_id IN (
       ...
       JOIN public.pod_refills pr
         ON pr.machine_id = dfd.machine_id
        AND pr.pod_product_id = dfd.pod_product_id
        AND pr.plan_date = p_plan_date
        AND pr.qty > 0
     );
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version',     'v13_driver_feedback_demand',
