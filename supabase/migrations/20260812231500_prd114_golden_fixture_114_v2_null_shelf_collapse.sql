-- PRD-114 - golden fixture 114, SUPERSEDING 20260812225200.
--
-- Why there is a v2: the first cut planted TWO Active batches of the same product
-- on the SAME shelf, to prove that v_machine_expiry_batches collapses siblings and
-- that the checklist must not. Seq 0 refused it:
--
--   duplicate key value violates unique constraint "idx_pod_inv_active_shelf"
--
-- `idx_pod_inv_active_shelf` is UNIQUE on (machine_id, shelf_id, boonz_product_id)
-- WHERE status = 'Active'. A shelf-bound sibling pair CANNOT EXIST. The scenario
-- was not merely unproven, it was impossible - and had seq 0 not been there, every
-- count assertion below would have read its `-1` sentinel against an expectation
-- and gone red for the wrong reason, or worse, read 0 against an expected 0.
--
-- The collapse is real, but it lives one predicate over. A btree unique index
-- treats NULLs as DISTINCT, so the index does NOT constrain `shelf_id IS NULL`
-- rows, while v_machine_expiry_batches partitions on
-- COALESCE(shelf_id::text,'noshelf') and keeps ONE row per partition
-- (ORDER BY snapshot_date DESC, pod_inventory_id - it keeps the NEWEST SNAPSHOT,
-- it does not take the MIN expiry; the first cut's comment said MIN and was wrong).
-- So N off-shelf batches of one product on one machine publish as ONE.
--
-- Live count of that class today: ZERO (PRD-059 cleaned 61 of them, PRD-105 RC-4
-- swept the rest). The collapse is therefore INERT IN PRODUCTION AND STRUCTURALLY
-- LIVE - which is precisely why it is worth a fixture rather than a paragraph.
-- This is the honest version of Cody's C-1: the checklist reads pod_inventory
-- directly not because the canonical view is wrong today, but because a
-- resolution rule cannot promise never to return FEWER rows than the ledger holds,
-- and on a safety worklist a silently hidden row is a pack of expired food left in
-- a machine. Over-inclusion is the correct failure mode here; silent omission is
-- not. Seq 4 proves the divergence, seq 26 proves the two surfaces still agree on
-- the shelf-bound set, so this is a reconciled disjointness and not a fork.
...
VALUES (
  114,
  'PRD-114: the dispatch list grows a final "Sanity checks - expired products" category. The read RPC returns EVERY stock-bearing batch inside the 7-day window at BATCH grain, including the off-shelf pair that v_machine_expiry_batches collapses to one - which is why it reads the ledger directly. The driver dispositions each row and one day_close_events row per batch is written, updated in place on a re-tap, never duplicated. No stock moves on a tap. CS acknowledge is what archives: Remove -> expired_writeoff, Sold -> sold_through_<date>, Exists/Skip -> nothing. A second acknowledge writes nothing. Warehouse is untouched by all four.',
  'PRD-114 s1, CS 2026-08-12: "the refill plan tells the driver what to PUT IN; nothing tells him what to CHECK". Live fleet at the time: 14 batches inside the window across 6 machines, 5 of them already expired, none surfaced anywhere in dispatching.',
  'P0',
  '2030-04-25',
  true,
  'passing',
...
DECLARE
  ...
  v_b1 uuid; v_b2 uuid; v_b3 uuid; v_b4 uuid; v_b5 uuid; v_b5b uuid; v_b6 uuid;
  ...
  v_b6_in int := -1; v_red_before_amber boolean := false;
  v_rpc_offshelf int := -1; v_view_offshelf int := -1;
  v_shelfbound_rpc int := -1; v_shelfbound_both int := -1;
...
BEGIN
  BEGIN
    ...
    -- THE COLLAPSE PAIR. Two off-shelf batches of one product on one machine.
    -- idx_pod_inv_active_shelf is UNIQUE on (machine, shelf, product) WHERE
    -- Active, but a btree treats NULLs as DISTINCT, so this pair is legal where a
    -- shelf-bound pair is not. v_machine_expiry_batches partitions on
    -- COALESCE(shelf_id::text,'noshelf') and keeps ONE row per partition, so it
    -- publishes ONE of these two. The driver has to put his hands on BOTH.
    INSERT INTO public.pod_inventory
      (machine_id, shelf_id, boonz_product_id, snapshot_date, current_stock, expiration_date, status)
    VALUES (v_machine, NULL, v_dark, v_today, 5, v_today + 2, 'Active')
    RETURNING pod_inventory_id INTO v_b5;

    INSERT INTO public.pod_inventory
      (machine_id, shelf_id, boonz_product_id, snapshot_date, current_stock, expiration_date, status)
    VALUES (v_machine, NULL, v_dark, v_today, 7, v_today + 4, 'Active')
    RETURNING pod_inventory_id INTO v_b5b;
    ...
    -- TRIPWIRE: the schema fact the first cut of this fixture got wrong. A second
    -- Active batch of the same product on the SAME shelf is refused outright, so
    -- the shelf-bound sibling scenario is impossible and must never be asserted.
    BEGIN
      INSERT INTO public.pod_inventory
        (machine_id, shelf_id, boonz_product_id, snapshot_date, current_stock, expiration_date, status)
      VALUES (v_machine, v_shelf_a, v_dark, v_today, 1, v_today + 1, 'Active');
      v_probe_sibling := 'NOT_BLOCKED';
    EXCEPTION WHEN OTHERS THEN v_probe_sibling := SQLERRM; END;
    ...
    SELECT count(*),
      ...
      INTO v_n_rows, v_n_expired, v_n_expiring, v_b6_in
    FROM public.get_expiry_sanity_checks(v_machine) s
    WHERE s.pod_inventory_id IN (v_b1, v_b2, v_b3, v_b4, v_b5, v_b5b, v_b6);

    -- C-1, measured rather than argued: what the checklist shows for the off-shelf
    -- pair vs what the canonical batch view publishes for the same pair.
    SELECT count(*) INTO v_rpc_offshelf
    FROM public.get_expiry_sanity_checks(v_machine) s
    WHERE s.pod_inventory_id IN (v_b5, v_b5b);

    SELECT count(*) INTO v_view_offshelf
    FROM public.v_machine_expiry_batches b
    WHERE b.pod_inventory_id IN (v_b5, v_b5b);

    -- ...and the reconciliation that keeps this a disjointness rather than a fork:
    -- on the SHELF-BOUND set the two surfaces agree row for row.
    SELECT count(*) INTO v_shelfbound_rpc
    FROM public.get_expiry_sanity_checks(v_machine) s
    WHERE s.pod_inventory_id IN (v_b1, v_b2, v_b3, v_b4);

    SELECT count(*) INTO v_shelfbound_both
    FROM public.get_expiry_sanity_checks(v_machine) s
    JOIN public.v_machine_expiry_batches b ON b.pod_inventory_id = s.pod_inventory_id
    WHERE s.pod_inventory_id IN (v_b1, v_b2, v_b3, v_b4);
  ...
  INSERT INTO golden.scratch (fixture_id, key, value) VALUES
    ...
    (v_fx, 'read',    jsonb_build_object('n_rows', v_n_rows, 'n_expired', v_n_expired,
                                         'n_expiring', v_n_expiring,
                                         'b6_in_window', v_b6_in, 'red_before_amber', v_red_before_amber)),
    (v_fx, 'grain',   jsonb_build_object('rpc_offshelf', v_rpc_offshelf,
                                         'view_offshelf', v_view_offshelf,
                                         'shelfbound_rpc', v_shelfbound_rpc,
                                         'shelfbound_both', v_shelfbound_both)),
...
$do$;
$fx$,
  'PRD-114 acceptance 1-4. v2: the collapse pair is OFF-SHELF, because idx_pod_inv_active_shelf makes a shelf-bound Active sibling pair impossible (seq 27 pins that). The ENTIRE scenario runs inside one deliberately rolled-back subtransaction: zero rows left in pod_inventory and zero in day_close_events. expiration_date is anchored to the REAL Dubai clock (the 7-day window is), event_date to the synthetic 2030 date. Seq 14-17, 24, 27 are the tripwires; seq 18-21 are Cody C-3, C-5, C-2; seq 4 and 26 are C-1.'
)
...
VALUES
(114, 0, 'The scenario ran to its deliberate rollback and not into a real error. This assertion is not ceremony: it is what caught the v1 fixture planting a row idx_pod_inv_active_shelf forbids, in a run where every count below would otherwise have failed for the wrong reason.',
...
(114, 1, 'The checklist returns every planted batch inside the 7-day window and nothing else (acceptance 1)',
 $a$SELECT value->>'n_rows' FROM golden.scratch WHERE fixture_id=114 AND key='read'$a$, 'eq', '6', true, 'P0'),
...
(114, 3, 'Four batches are inside today..today+7 and are classified AMBER - a batch expiring on day 7 is still amber, not red',
 $a$SELECT value->>'n_expiring' FROM golden.scratch WHERE fixture_id=114 AND key='read'$a$, 'eq', '4', true, 'P0'),

(114, 4, 'GRAIN (Cody C-1), measured not argued: the checklist shows BOTH off-shelf batches of one product while v_machine_expiry_batches publishes ONE of them. A btree unique index treats NULLs as distinct, so idx_pod_inv_active_shelf does not constrain off-shelf rows, but the view partitions on COALESCE(shelf_id,noshelf) and keeps the newest snapshot. On a safety worklist a silently dropped row is a pack of expired food left in a machine, which is why this RPC reads the ledger.',
 $a$SELECT (value->>'rpc_offshelf') || '/' || (value->>'view_offshelf')
     FROM golden.scratch WHERE fixture_id=114 AND key='grain'$a$, 'eq', '2/1', true, 'P0'),
...
(114, 26, 'RECONCILED (Cody C-1, second half): on the SHELF-BOUND set the checklist and the canonical batch view agree row for row. Without this, seq 4 would license a fork; with it, the divergence is confined to the off-shelf class and is a documented disjointness rather than a second answer to a registered question.',
 $a$SELECT (value->>'shelfbound_rpc') || '/' || (value->>'shelfbound_both')
     FROM golden.scratch WHERE fixture_id=114 AND key='grain'$a$, 'eq', '4/4', true, 'P0'),

(114, 27, 'TRIPWIRE / SCHEMA FACT: two Active batches of one product on one SHELF are refused by idx_pod_inv_active_shelf. This is pinned because the first cut of this fixture asserted a scenario the schema forbids, and a later leg reading only the C-1 prose would rebuild the same impossible test.',
 $a$SELECT value->>'shelf_sibling' FROM golden.scratch WHERE fixture_id=114 AND key='probes'$a$,
 'contains', 'idx_pod_inv_active_shelf', true, 'P0');
