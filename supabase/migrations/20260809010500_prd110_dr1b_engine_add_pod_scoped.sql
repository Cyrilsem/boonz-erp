-- PRD-110 DR-1b · leg 161 · Object 2 of 4
-- engine_add_pod (v19) — machine-scoped so it stops planning, and stops WIPING, the clusters
-- that are authoritative for v3.
--
-- TWO surgical predicates, both reading is_cluster_authoritative_v3 (DR-1, STABLE, machine-grain):
--   1. the pod_refills DELETE, which used to take the WHOLE plan_date. ⛔ This is the real
--      blocker DR-1 missed: the parking lot recorded "both ADD engines are whole-plan-date
--      scoped", which is true of the READ. The unscoped DELETE is what actually made two
--      engines unable to coexist on one plan_date.
--   2. the `picked` CTE, so v19 no longer sizes machines it no longer owns.
--
-- ⭐ FLAG-OFF PROOF (what makes this safe to ship today): with 0 of 10 clusters authoritative,
--    is_cluster_authoritative_v3 returns false for EVERY machine, so NOT(false)=true everywhere
--    and both predicates degenerate to the statements they replace. Behaviour is byte-identical.
--    Fixture 75 pins it as a whole-table md5 of pod_refills across the change.
--
-- ⛔ pronargdefaults MUST stay 2 (Wave-2 rule). The body below is the LIVE definition captured
--    via pg_get_functiondef, so `p_plan_date date DEFAULT (CURRENT_DATE + 1), p_days_cover
--    integer DEFAULT 14` and `#variable_conflict use_column` are carried verbatim. Asserted
--    after the replace rather than trusted.
...
DECLARE
  ...
  v_ndef  int;
BEGIN
  SELECT p.prosrc, p.pronargdefaults
    INTO v_src, v_ndef
  ...
  IF v_src IS NULL THEN
    RAISE EXCEPTION 'DR-1b: engine_add_pod(date,integer) not found after replace';
  END IF;

  -- The defaults are load-bearing: _build_draft_core_v3 and cron 13 both call this by name.
  IF v_ndef <> 2 THEN
    RAISE EXCEPTION 'DR-1b: engine_add_pod pronargdefaults is %, expected 2', v_ndef;
  END IF;

  -- Exactly one overload may exist. An accidental second signature is the repurpose_machine
  -- foot-gun the project has a standing rule against.
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public' AND p.proname = 'engine_add_pod') <> 1 THEN
    RAISE EXCEPTION 'DR-1b: engine_add_pod is overloaded; exactly one signature is allowed';
  END IF;

  -- Both scope predicates actually landed.
  IF position('is_cluster_authoritative_v3(pr.machine_id)' in v_src) = 0 THEN
    RAISE EXCEPTION 'DR-1b: the scoped pod_refills DELETE did not land';
  END IF;
  IF position('is_cluster_authoritative_v3(mtv.machine_id)' in v_src) = 0 THEN
    RAISE EXCEPTION 'DR-1b: the scoped picked CTE did not land';
  END IF;

  -- And the unscoped wipe is GONE. This is the assertion that matters most.
  IF position('DELETE FROM public.pod_refills WHERE plan_date = p_plan_date' in v_src) > 0 THEN
    RAISE EXCEPTION 'DR-1b: the unscoped whole-date pod_refills DELETE survived the replace';
  END IF;

  -- #variable_conflict must survive, or every unqualified column reference changes meaning.
  IF position('#variable_conflict use_column' in v_src) = 0 THEN
    RAISE EXCEPTION 'DR-1b: #variable_conflict use_column was lost';
  END IF;

  RAISE NOTICE 'DR-1b: engine_add_pod scoped; pronargdefaults=%, one signature, wipe scoped', v_ndef;
END $guard$;
