-- Procurement demand: box-size aware suggested qty.
--
-- boonz_products.units_per_box (added in 20260603...units_per_box) holds the
-- supplier case size. Suggested qty now rounds the buffered gap UP to the next
-- full box so procurement orders in whole cases (matches the weekly-procurement
-- skill's box-minimum rules). units_per_box NULL -> no rounding (raw ceil).
-- Also returns units_per_box so the FE can show the case size.
--
-- DROP+CREATE: the RETURNS TABLE shape gains a units_per_box column.
