-- PRD-CLEAN-02 M1 (DDL): refresh_correlation_pod day-bucketing fixed to Asia/Dubai.
-- Only change vs the prior version (docs/prds/rollback/refresh_correlation_pod_2026-07-11.sql):
-- daily buckets and the day spine use (.. AT TIME ZONE 'Asia/Dubai')::date instead of
-- UTC ::date / now()::date. Thresholds, pairing, and writes are unchanged.
...
BEGIN
  ...
  WITH daily_sales AS (
    SELECT machine_id, pod_product_id,
           (transaction_date AT TIME ZONE 'Asia/Dubai')::date AS day,
  ...
  days AS (
    SELECT ((now() AT TIME ZONE 'Asia/Dubai')::date - g)::date AS day
  ...
  WITH daily_sales AS (
    SELECT m.location_type, vshr.pod_product_id,
           (vshr.transaction_date AT TIME ZONE 'Asia/Dubai')::date AS day,
  ...
  days AS (
    SELECT ((now() AT TIME ZONE 'Asia/Dubai')::date - g)::date AS day FROM generate_series(0, p_window_days - 1) AS g
  ),
  loc_pods AS (
