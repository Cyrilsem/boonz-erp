...
--
-- push_plan_to_dispatch's Remove/Machine-To-Warehouse path (both the ordinary
-- single-machine branch and the M2M cross-machine transfer's source leg) used
-- to resolve shelf_id as COALESCE(lot_shelf_id, plan_shelf_id) -- the
-- pod_inventory lot's shelf WON over the shelf the plan actually named. When
-- no lot existed on the plan's own shelf, it fell back to a global,
-- non-shelf-scoped search across ALL of the machine's Active lots for that
-- product, and if that still found stock on a DIFFERENT shelf, it silently
-- re-pointed the product too ("[FLAVOR-CORRECTED]"), and if nothing existed
-- anywhere it wrote a 0-quantity "[NO LOT ON SHELF]" row that a driver could
-- not act on. This is exactly the class of bug PRD-125 D2 exists to kill:
-- pod_inventory deciding placement, quantity, or emptiness instead of WEIMI.
--
-- Fix: shelf_id is always the plan's own shelf. pod_inventory is read
-- SHELF-SCOPED, for expiry_date and pod_lot_id only -- never a different
-- shelf, never a different product, never split into multiple legs (a Remove
-- line always lands as exactly one dispatch row, matching D2's "no split
-- legs" proof requirement). When no Active lot sits on that shelf, the row
-- still lands with the full planned quantity, expiry_date NULL, and the
-- existing [EXPIRY-TO-CONFIRM] comment -- never a zeroed, unactionable row.
-- The [NO LOT ON SHELF] zeroing branch is deleted entirely, in both the
-- ordinary Remove path and the M2M source-leg path (same bug, same function,
-- not explicitly named in D2's examples but the same doctrine applies).
--
-- Everything else in this 36KB function (M2M destination-leg grouping, FEFO
-- pin on Refill/Add New, procurement-gap alerting, conservation/leakage
-- guard, slot guard, auto-repair, fefo-bind) is untouched. Diffed against the
-- pre-change live body: only the two Remove-emitting blocks changed shape;
-- every other line matches verbatim. rpc_version bumped to
-- v17_prd125_p1_weimi_shelf_truth so callers can see which body they hit.
--
-- Cody: DDL is CREATE OR REPLACE on an existing signature (no arg/return
-- change), forward-only (Article 12). SECURITY DEFINER unchanged, still sets
-- app.via_rpc / app.rpc_name at the top (Article 4). Canonical writer
-- allowlist membership unchanged (still the same function name).
--
-- Verified live: applied via apply_migration, pg_get_functiondef confirms
-- rpc_version = 'v17_prd125_p1_weimi_shelf_truth' is live. Functional proof
-- (rebuild of the 09-15 IFLYMCC A08 / MPMCC-1058 A02 Removes inside a
-- rolled-back transaction) recorded in OVERNIGHT-REPORT-2026-09-15.md, Phase 1.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v17_prd125_p1_weimi_shelf_truth'
  );
END $function$
