-- Refill System v2 / Phase 2 (#10, stage 2a: feed engine) - engine_swap_pod consumes signals.
--
-- Closes the "feeding the engine" half of #10: engine_swap_pod (v9_3 -> v9_4) now SUPPRESSES a
-- substitute that CS has rejected >= 3 times in the last 30 days on that machine (the deterministic
-- 3-in-30d rule CS chose). Reads refill_edit_signals (swap_rejected). Verbatim repro of v9_3,
-- diff-gated. Safe at zero signal volume (the temp table is empty -> no suppression).
-- NOTE: the remaining two feedback mechanisms (per-shelf qty bias in engine_add_pod; raise
-- repeatedly-missed items, which is structurally a placement-layer concern not an engine_add_pod
-- refill concern) are the documented stage-2b follow-ups.
-- APPLIED 2026-06-01 (CS sign-off; Cody-approved Articles 1,4,12; diff-gated; verified pg_proc v9_4).
...
BEGIN
  ...
  SELECT p_plan_date, tr.machine_id, tr.shelf_id_final, tr.pod_out,
    ...
    jsonb_build_object('pass','1','source','strategic_machine_tag','tag_id', tr.tag_id,
                       ...
                       'engine_version','v9_4_signal_feedback')
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_cooldown', v_skip_cooldown, 'engine_version','v9_4_signal_feedback',
