-- PRD-110 · P3.1e · migration D — stitch_v3's blocked-row marker now tells the truth
--
-- Every Blocked row stitch_v3 has written since P3.1c carried
--   'blocked_demand_promotion' : 'source=stitch writer lands with the live cutover (P3.1e)'
-- P3.1e has now landed, so that string is false in every row written from here on. A stale
-- promise inside the data is worse than no promise: the next reader would go looking for an
-- unbuilt writer that in fact exists.
--
-- ⭐ SPLICED from the live definition; the ONLY change is the value of one jsonb key in the
--    Blocked-row payload. No control flow, no quantity, no conservation arithmetic is touched.
--    ⛔ pronargdefaults preserved BY CONSTRUCTION (RISK 101), verified after apply.
