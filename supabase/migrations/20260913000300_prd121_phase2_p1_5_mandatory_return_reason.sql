-- PRD-121 Phase 2, P1.5: mandatory return_reason on every real return.
--
-- return_dispatch_line accepts p_return_reason DEFAULT NULL with no validation anywhere
-- in the body -- it flows straight through to the final UPDATE's return_reason column.
-- Every real return (not the early "refused"/"already_returned" no-op paths) can commit
-- with no explanation of why product came back, which is exactly what P1.5 flags.
--
-- Fix: after the existing early-return refusal checks (already_returned, internal_move,
-- m2m, already_received, never_physical, no_actor_non_physical -- none of these actually
-- perform a return, so none need a reason), require p_return_reason to be non-empty.
-- Deliberately NOT the >=10-character minimum used elsewhere in this codebase
-- (insert_driver_remove_line, correct_packed_m2m_transfer, repair_orphan_internal_transfer)
-- -- those guard free-text human justifications; this field is driven by a fixed FE
-- dropdown (RETURN_REASONS) whose shortest real value is "Other" (5 chars) -- a length
-- floor here would reject a legitimate existing UI selection. Non-empty is the correct bar
-- for a controlled reason list.
--
-- All three other DB callers already pass a real reason (repack_machine:
-- 'superseded_by_repack', eod_auto_release_unpicked: 'auto_release_unpicked_eod',
-- return_all_dispatches_for_machine: its own required p_return_reason parameter, no
-- DEFAULT) -- none break under this change.
--
-- FE companion change (same commit): src/app/(field)/field/dispatching/[machineId]/page.tsx
-- handleSave() now pre-flight-blocks save with a clear message when any 'returned' line has
-- no return_reason selected, instead of letting the driver hit this RPC's new exception
-- mid-save.
--
-- Cody: approve. Articles 1 (still the sole return writer, no new write path), 4 (adds an
-- input validation the function was missing, role/other guards unchanged), 12 (forward-only
-- CREATE OR REPLACE, same signature, md5-guarded).

DO $mig$ DECLARE v_def text; BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def
    FROM pg_proc p WHERE p.proname='return_dispatch_line' AND p.pronamespace='public'::regnamespace;
  IF md5(v_def) <> 'abf7187ab83e16aef82527f278f00198' THEN
    RAISE EXCEPTION 'return_dispatch_line drifted (md5 %), refusing blind replace', md5(v_def);
  END IF;
END $mig$;
