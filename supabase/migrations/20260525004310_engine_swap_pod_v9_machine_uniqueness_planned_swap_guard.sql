-- PRD-010 AC#2: engine_swap_pod v9
-- Two guards on autonomous Pearson Pass 2:
--   (a) machine-level uniqueness: a single swap cycle must not introduce the
--       same pod_product_id onto two different shelves of the same machine.
--   (b) planned_swaps priority: shelves with a status='pending' row in
--       planned_swaps are owned by the strategic-swap path and must be
--       excluded from autonomous Pass 2 substitution.
--
-- Implementation notes:
-- - Existing code already excludes products currently present on the machine
--   (v_pod_inventory_latest LEFT JOIN ... WHERE pil.machine_id IS NULL). That
--   handles the "across shelves" case for products already deployed. The new
--   guard (a) extends this to candidates picked WITHIN this same Pass 2 batch,
--   which the join cannot see.
-- - Pass 1 (strategic_machine_tags) is uncapped per design and intentionally
--   coexists with planned_swaps; the guard applies only to Pass 2 autonomous.
-- - planned_swaps stores shelf_code (text), so we translate to shelf_id via
--   shelf_configurations.
-- - engine_version bumps v8_runway_gate -> v9_swap_dedup_planned_priority.
--
-- Article 12 forward-only CREATE OR REPLACE; function identity, args, return
-- shape unchanged. No schema migration.

CREATE OR REPLACE FUNCTION public.engine_swap_pod(
  p_plan_date date,
  p_max_swaps_per_machine integer,
  p_min_pearson numeric,
...
BEGIN
  ...
  -- v9 AC#2(b): shelves with a pending planned_swap are owned by the strategic-swap
  -- path; autonomous Pass 2 must not substitute them. Translate planned_swaps.shelf_code
  -- to shelf_id via shelf_configurations.
  CREATE TEMP TABLE _planned_swap_shelves ON COMMIT DROP AS
  SELECT DISTINCT sc.machine_id, sc.shelf_id
    FROM public.planned_swaps ps
    JOIN public.shelf_configurations sc
      ON sc.machine_id = ps.machine_id
     AND sc.shelf_code = ps.shelf_code
     AND sc.is_phantom = false
   WHERE ps.status = 'pending';

  -- ============================================================
  -- PASS 1 — Strategic machine tags (UNCAPPED, CS overrides)
  -- ============================================================
  WITH picked AS (
  ...
  tag_targets AS (
    SELECT smt.tag_id, smt.strategic_intent_id AS intent_id,
           smt.machine_id, smt.pod_product_id AS pod_out,
           smt.action_directive, smt.substitute_pod_product_id AS pod_in,
           smt.priority,
    ...
    LEFT JOIN public.slot_lifecycle sl
      ON sl.machine_id = smt.machine_id
     AND sl.pod_product_id = smt.pod_product_id
     AND sl.archived = false AND sl.is_current = true
    WHERE smt.status = 'approved'
      AND smt.action_directive IN ('swap_out_with_substitute','swap_out_m2w')
  ),
  ...
  INSERT INTO public.pod_swaps(
    ...
    qty_out, qty_in, reason, substitute_source, substitute_score,
    linked_intent_id, reasoning)
  SELECT
    p_plan_date, tr.machine_id, tr.shelf_id_final, tr.pod_out,
    CASE WHEN tr.action_directive='swap_out_with_substitute' THEN tr.pod_in ELSE NULL END,
    COALESCE((SELECT GREATEST(SUM(pil.current_stock),1)::int FROM public.v_pod_inventory_latest pil
      JOIN public.product_mapping pm ON pm.boonz_product_id = pil.boonz_product_id AND pm.status='Active'
       WHERE pm.pod_product_id = tr.pod_out AND pil.machine_id = tr.machine_id
         AND pil.shelf_id = tr.shelf_id_final AND pil.status='Active'),1)::int,
    ...
    jsonb_build_object('pass','1','source','strategic_machine_tag',
                       'tag_id', tr.tag_id, 'tag_directive', tr.action_directive,
                       'tag_priority', tr.priority, 'engine_version','v9_swap_dedup_planned_priority')
  FROM tag_resolved tr
  WHERE tr.shelf_id_final IS NOT NULL
  ON CONFLICT (plan_date, machine_id, shelf_id, pod_product_id_out) DO NOTHING;

  UPDATE public.strategic_machine_tags smt
     SET status = 'consumed',
         consumed_at = now(),
         consumed_at_plan_date = p_plan_date,
         consumed_via_swap_id = ps.swap_id
    FROM public.pod_swaps ps
   WHERE ps.plan_date = p_plan_date
     AND (ps.reasoning->>'source') = 'strategic_machine_tag'
     AND (ps.reasoning->>'tag_id')::uuid = smt.tag_id
  ...
  -- ============================================================
  -- PASS 2 — Autonomous Pearson — v9 dedup + planned_swap guard
  -- ============================================================
  WITH picked AS (
    SELECT mtv.machine_id, m.location_type
    FROM public.machines_to_visit mtv
    JOIN public.machines m ON m.machine_id = mtv.machine_id
    WHERE mtv.plan_date=p_plan_date AND mtv.status IN ('picked','cs_added')
  ),
  pass1_used AS (
    SELECT machine_id, COUNT(*)::int AS used
      FROM public.pod_swaps WHERE plan_date=p_plan_date GROUP BY machine_id
  ),
  already_swapped AS (
    SELECT machine_id, shelf_id FROM public.pod_swaps WHERE plan_date=p_plan_date
  ),
  candidates_raw AS (
    SELECT
      p.machine_id, p.location_type, sl.shelf_id, sl.pod_product_id AS pod_out,
      sl.signal,
      pp.product_category AS cat_out,
      pp.pod_product_name,
      sc.shelf_code,
    ...
    LEFT JOIN _shelf_live_stock sls
      ON sls.machine_id = sl.machine_id AND sls.shelf_id = sl.shelf_id
    ...
    -- v9 AC#2(b): exclude shelves with pending planned_swaps.
    LEFT JOIN _planned_swap_shelves pss
      ON pss.machine_id = p.machine_id AND pss.shelf_id = sl.shelf_id
    WHERE a.shelf_id IS NULL
      AND pss.shelf_id IS NULL
  ),
  candidates_gated AS (
    SELECT
      c.*,
      (c.signal IN ('WIND DOWN','ROTATE OUT')
        AND c.v30 > 0
        AND c.current_stock < c.target_stock) AS is_healthy_skip
    FROM candidates_raw c
  ),
  skipped AS (
    SELECT machine_id, shelf_id, pod_out, signal, v30, current_stock, target_stock, runway_days,
           pod_product_name, shelf_code
    FROM candidates_gated WHERE is_healthy_skip = true
  ),
  candidates AS (
    SELECT machine_id, location_type, shelf_id, pod_out, signal, cat_out, pass1_used_count
    FROM candidates_gated WHERE is_healthy_skip = false
  ),
  ranked AS (
    SELECT c.*, ROW_NUMBER() OVER (PARTITION BY c.machine_id
  ...
  -- v9 AC#2(a): also exclude candidates whose pod_in is already targeted by
  -- a Pass 1 ADD_NEW for the same machine (existing pod_swaps with pod_product_id_in
  -- set). This catches the Pass 1 -> Pass 2 conflict before Pass 2 picks one.
  pass1_added_products AS (
    SELECT DISTINCT machine_id, pod_product_id_in AS pod_in
    FROM public.pod_swaps
    WHERE plan_date = p_plan_date AND pod_product_id_in IS NOT NULL
  ),
  sub_candidates AS (
    SELECT c.machine_id, c.shelf_id, c.pod_out, c.signal, c.cat_out,
           sub.pod_product_id AS pod_in, fv.avg_v30 AS sub_v30,
           per_m.pearson AS p_machine, per_l.pearson AS p_loc,
           CASE WHEN per_m.pearson IS NOT NULL THEN 'pearson_per_machine'
                WHEN per_l.pearson IS NOT NULL THEN 'pearson_per_loc'
                ELSE 'category_fallback' END AS sub_source,
    ...
    -- v9 AC#2(a): exclude Pass 1 already-introduced products on the same machine.
    LEFT JOIN pass1_added_products p1
      ON p1.machine_id = c.machine_id AND p1.pod_in = sub.pod_product_id
    WHERE pil.machine_id IS NULL
      AND ad_in.intent_id IS NULL
  ...
  -- v9 AC#2(a): batch-level dedup. If two shelves on the same machine both
  -- pick the same pod_in candidate, keep only the highest-correlation one and
  -- demote the other to NULL (it falls through to M2W).
  pass2_picks AS (
    SELECT
      ppr.*,
      ROW_NUMBER() OVER (
        PARTITION BY ppr.machine_id, ppr.pod_in
        ORDER BY COALESCE(ppr.p_machine, ppr.p_loc, ppr.sub_v30) DESC, ppr.shelf_id
      ) AS dedup_rank
  ...
  pass2_combined AS (
    SELECT c.machine_id,c.shelf_id,c.pod_out,c.signal,c.cat_out,
           ...
           pp.sub_v30, pp.sub_source, pp.p_machine, pp.p_loc,
           pp.dedup_rank
    FROM capped_with_cooldown c
  ...
  pass2_inserted AS (
    ...
    SELECT p_plan_date, p2.machine_id, p2.shelf_id, p2.pod_out, p2.pod_in,
      COALESCE((SELECT GREATEST(SUM(pil.current_stock),1)::int FROM public.v_pod_inventory_latest pil
        JOIN public.product_mapping pm ON pm.boonz_product_id = pil.boonz_product_id AND pm.status='Active'
         WHERE pm.pod_product_id = p2.pod_out AND pil.machine_id = p2.machine_id
           AND pil.shelf_id = p2.shelf_id AND pil.status='Active'),1)::int,
      ...
      CASE WHEN p2.pod_in IS NULL THEN 'm2w'
           ...
           ELSE 'rotate_out' END,
      COALESCE(p2.sub_source, 'm2w'),
      COALESCE(p2.p_machine, p2.p_loc, p2.sub_v30),
      NULL::uuid,
      jsonb_build_object('pass','2','source','autonomous','signal',p2.signal,'cat_out',p2.cat_out,
        ...
        'engine_version','v9_swap_dedup_planned_priority')
  ...
  -- v9 AC#2(b): count shelves owned by planned_swaps that Pass 2 skipped.
  ...
  RETURN jsonb_build_object(
    ...
    'max_swaps_per_machine',p_max_swaps_per_machine,
    'min_pearson',p_min_pearson,
    'days_cover',p_days_cover,
    'pass1_tag_swaps', v_tag_swaps,
    'pass1_tag_m2w', v_tag_m2w,
    'pass2_autonomous_swaps', v_auto_swaps,
    'pass2_autonomous_m2w', v_auto_m2w,
    'pass2_skipped_runway', v_skip_runway,
    'pass2_skipped_planned_swap', v_skip_planned,
    'pass2_demoted_duplicates', v_skip_duplicate,
    'skipped_swaps', v_skipped_swaps,
    'total_swaps', v_tag_swaps + v_auto_swaps,
    'total_m2w', v_tag_m2w + v_auto_m2w,
    'skipped_cooldown', v_skip_cooldown,
    'engine_version','v9_swap_dedup_planned_priority',
