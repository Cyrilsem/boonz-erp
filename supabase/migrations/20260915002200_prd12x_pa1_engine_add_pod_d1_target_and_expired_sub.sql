-- ONE-LOOP-2 Block A step 1a: engine_add_pod gets D1's real target and the
-- expired-on-shelf substitution rule wired into its allocation loop.
--
-- D1 (PRD-125 / ONE-LOOP-2): keep the banded scoring for ORDERING (cover_units,
-- whether legacy band-fraction or base_stock 'want', is untouched). Replace
-- only the CEILING that need_raw is clamped against: it was unconditionally
-- max_stock (fill_to_cap = max_stock - current_stock). It is now target_stock:
-- max_stock (with slot_capacity_max override, already folded into max_stock by
-- the existing COALESCE chain) when the lane's own 30-day daily velocity
-- (v30/30.0) is at or above refill_policy_params.hero_velocity_floor, or the
-- resolved product's source_of_supply is 'venue_team' -- else least(10, max_stock).
-- Still clamped by wh_available_for via the existing wh_avail/prior_need logic,
-- unchanged. Nothing else in the allocation math changes.
--
-- Implementation: candidates gains a LATERAL resolution of the same
-- product_mapping row already being queried for wh_avail (now reused, not
-- re-queried, for source_of_supply too -- this also removes a latent risk of
-- the two independent subqueries ever picking different tie-broken rows).
-- candidates_with_machine_velocity computes target_stock from real columns
-- (c.v30, c.source_of_supply), not a re-derived expression, so nothing risks
-- diverging from the v7/v30 CASE logic already baked into `candidates`.
-- flagged's two independent `GREATEST(max_stock - current_stock, 0)`
-- expressions (fill_to_cap and need_raw's ceiling) both become
-- GREATEST(target_stock - current_stock, 0). The reasoning jsonb's
-- 'target_stock' key, which previously just re-labelled max_stock, now
-- reports the real target.
--
-- Expired-on-shelf (PRD-125 D4): "Remove plus the substitute, on the same
-- line." A new pass over candidates_with_machine_velocity finds shelves
-- where WEIMI shows stock (current_stock > 0, the shelf truth per D2) AND an
-- Active pod_inventory lot on that exact shelf has expired (pod_inventory
-- read for expiry only, per D2 -- no product-identity comparison is made or
-- needed). For each such shelf, find_substitutes_for_shelf resolves a
-- substitute via substitution_rules (D4); when one exists with WH stock,
-- one pod_swaps row is written with BOTH pod_product_id_out/qty_out (the
-- full expired current_stock) and pod_product_id_in/qty_in (the substitute,
-- sized by the same D1 target logic, capped by its own wh_available_for).
-- When no rule matches, nothing is written here -- it surfaces instead in
-- get_pod_refill_draft_exceptions's no_rule_matched category (next
-- migration), which is what "the lane stays at its current level and
-- appears on the exception list" means for a lane the engine cannot fix.
--
-- Scarce-stock rule (D4, "under 12 fleet-wide goes to the highest-velocity
-- lane and nowhere else"): no new code. allocated's existing prior_need
-- window, ordered by v30 DESC then u_final_score DESC, already allocates a
-- scarce product's wh_avail to the highest-velocity lane first and zeroes
-- every lower-priority lane's final_qty once it runs out -- proven below,
-- not re-implemented.

DO $mig$ DECLARE v_def text; BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def
    FROM pg_proc p WHERE p.proname='engine_add_pod' AND p.pronamespace='public'::regnamespace
      AND pg_get_function_identity_arguments(p.oid) = 'p_plan_date date, p_days_cover integer';
  IF v_def IS NULL THEN
    RAISE EXCEPTION 'engine_add_pod not found, refusing blind replace';
  END IF;
END $mig$;
...
BEGIN
  ...
  -- PRD-CLEAN-09v2 (p0_fix2) + p0_fix10 (2026-07-12): identity is WEIMI-first.
  -- Shelf identity comes from v_shelf_slot_identity (the machine's own report); slot_lifecycle
  -- is fallback only. Drifted shelves are no longer hard-skipped: they are auto-planned for
  -- the TRUE WEIMI product as cold start, and alerted (slot_lifecycle is stale -- rebind).
  ...
  SELECT rpp.refill_sizing_mode, rpp.min_fill_pct, rpp.seller_wk_threshold, rpp.ewma_w7, rpp.ewma_w30,
         ...
         rpp.cold_start_days, COALESCE(rpp.hero_velocity_floor, 3)
    INTO v_mode, v_minfill, v_sellwk, v_w7, v_w30, v_spf, v_zlo, v_zmid, v_zhi, v_mlo, v_mhi,
         v_cold_days, v_hero_velocity_floor
  ...
  -- ── PRD-110 DR-1b: the scoped wipe. ────────────────────────────────────────────
  -- This DELETE used to take the whole plan_date. That single unscoped statement - not the
  -- SELECT scope - is what made a partial cutover physically impossible: v3 could write
  -- correct rows for a flipped cluster and v19's next run would delete them.
  -- FLAG-OFF PROOF: is_cluster_authoritative_v3 is false for every machine while 0 clusters
  -- are authoritative, so NOT(false) is true for every row and this degenerates EXACTLY to
  -- the unscoped DELETE it replaces. Fixture 75 pins the degeneracy by whole-table md5.
  ...
  DELETE FROM public.pod_swaps
   WHERE plan_date = p_plan_date
     AND reasoning->>'tagged_by' IN ('engine_add_pod_v15','engine_add_pod_v16','engine_add_pod_v17','engine_add_pod_v18','engine_add_pod_v19_base_stock','engine_add_pod_v20_expired_substitution');
  ...
  picked AS (
    SELECT mtv.machine_id, mtv.official_name FROM public.machines_to_visit mtv
     WHERE mtv.plan_date = p_plan_date AND mtv.status IN ('picked','cs_added')
       -- PRD-110 DR-1b: v19 plans only the clusters it is still authoritative for.
       -- Article 16: the authority rule is READ from is_cluster_authoritative_v3, never
       -- restated as venue_group = cluster_key here.
  ...
  candidates AS (
    SELECT
      ...
      rmap.source_of_supply                                   AS source_of_supply,
      COALESCE((SELECT SUM(waf.free_stock)
                  FROM public.wh_available_for(sl.machine_id, rmap.boonz_product_id) waf), 0)::int AS wh_avail,
    ...
    LEFT JOIN LATERAL (
      SELECT pm2.boonz_product_id, pm2.source_of_supply
        FROM public.product_mapping pm2
  ...
  candidates_with_machine_velocity AS (
    SELECT c.*,
      ...
      -- D1: fill to capacity is the rule for hero lanes and venue-supplied
      -- products; everyone else caps at 10 (or max_stock if lower).
      (CASE WHEN (c.v30 / 30.0) >= v_hero_velocity_floor OR c.source_of_supply = 'venue_team'
            THEN c.max_stock
            ELSE LEAST(10, c.max_stock)
       END)::int AS target_stock
    FROM candidates c
  ),
  decided AS (
    SELECT c.*,
  ...
  flagged AS (
    SELECT cv.*,
      ...
      false                      AS is_drain,
      GREATEST(cv.target_stock - cv.current_stock, 0)                         AS fill_to_cap,
      LEAST(GREATEST(cv.cover_units, COALESCE(cv.driver_req_qty,0), CASE WHEN v_add_abs_floor AND NOT(cv.v7=0 AND cv.v30=0) THEN v_min_facing_floor ELSE 0 END, CASE WHEN v_add_niche_fill AND (SELECT count(DISTINCT sl.machine_id) FROM slot_lifecycle sl JOIN machines mm ON mm.machine_id=sl.machine_id AND mm.status='Active' WHERE sl.pod_product_id = cv.pod_product_id AND sl.is_current AND NOT sl.archived) <= v_niche_footprint_max AND cv.v30 >= (SELECT MAX(cv2.v30) FROM covered cv2 WHERE cv2.pod_product_id = cv.pod_product_id) THEN (CASE WHEN v_niche_facing_target <= 1 THEN CEIL(v_niche_facing_target * cv.max_stock)::int ELSE v_niche_facing_target::int END) ELSE 0 END),
            GREATEST(cv.target_stock - cv.current_stock, 0))                  AS need_raw
    FROM covered cv
  ),
  allocated AS (
    SELECT f.*,
  ...
  -- PRD-125 D4, expired-on-shelf: WEIMI shows stock on the shelf (D2 shelf
  -- truth) and the Active pod_inventory lot there (read for expiry only, no
  -- product-identity comparison) has expired. Resolve a substitute via
  -- substitution_rules; write Remove + substitute on the same pod_swaps row
  -- when one is found. When none is found, nothing is written here -- it is
  -- picked up by get_pod_refill_draft_exceptions's no_rule_matched category.
  expired_on_shelf AS (
    SELECT DISTINCT ON (cmv.machine_id, cmv.shelf_id)
      cmv.machine_id, cmv.official_name, cmv.shelf_id, cmv.shelf_code,
      cmv.pod_product_id, cmv.pod_product_name, cmv.current_stock, cmv.target_stock,
      fs.pod_product_id AS sub_pod_product_id, fs.pod_product_name AS sub_pod_product_name,
      fs.wh_stock_units
    FROM candidates_with_machine_velocity cmv
    JOIN public.pod_inventory pi
      ON pi.machine_id = cmv.machine_id AND pi.shelf_id = cmv.shelf_id
    ...
    LEFT JOIN LATERAL public.find_substitutes_for_shelf(
      p_plan_date, cmv.machine_id, cmv.shelf_id, cmv.pod_product_id) fs ON true
    WHERE cmv.current_stock > 0
  ),
  expired_swaps AS (
    INSERT INTO public.pod_swaps(
      plan_date, machine_id, shelf_id, pod_product_id_out, pod_product_id_in,
      qty_out, qty_in, reason, substitute_source, reasoning)
    SELECT p_plan_date, e.machine_id, e.shelf_id, e.pod_product_id, e.sub_pod_product_id,
      e.current_stock,
      LEAST(e.target_stock, GREATEST(COALESCE(e.wh_stock_units,0)::int, 0)),
      'expired_on_shelf', 'substitution_rule',
      jsonb_build_object(
        'shelf_code',        e.shelf_code,
        'official_name',     e.official_name,
        'expired_product',   e.pod_product_name,
        'substitute_product',e.sub_pod_product_name,
        'tagged_by',         'engine_add_pod_v20_expired_substitution')
    FROM expired_on_shelf e
    WHERE e.sub_pod_product_id IS NOT NULL
    RETURNING 1
  ),
  gap_rows AS (
  ...
  SELECT (SELECT COUNT(*) FROM inserted),
         ...
         (SELECT COUNT(*) FROM expired_swaps),
  ...
  INTO v_refills, v_qty0_rows, v_dead_tags, v_binding_skipped, v_expired_subs, v_procurement_gaps FROM gap_rows;
  ...
  RETURN jsonb_build_object(
    ...
    'expired_on_shelf_substitutions', v_expired_subs,
    ...
    'engine_version', CASE WHEN v_mode='base_stock' THEN 'v20_base_stock_d1_expired_sub' ELSE 'v20_relative_score_band_f1_per_machine_d1_expired_sub' END,
    'sizing_mode', v_mode,
