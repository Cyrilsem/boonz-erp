...
-- original addition of a v_slot_binding_drift-scoped guard block to engine_add_pod().
-- CHANGELOG (~line 2897, 2911) documents that this guard was itself patched the same evening
-- by p0_fix2_engine_add_pod_scoped_drift_skip (unscoped fleet-halt RAISE -> scoped
-- monitoring_alert, activates the per-row binding_drift hard-skip) and p0_fix10 (WEIMI-first
-- identity; binding_drift no longer skips -- drifted shelves plan the TRUE WEIMI product and
-- are alerted instead). Both p0_fix* migrations are already committed to git. The pre-fix
-- 2026-07-12 01:29 body is not recoverable; the body below is the CURRENT live cumulative
-- state (confirmed via pg_get_functiondef, contains v_slot_binding_drift guard logic --
-- binding_drift count scoped to machines_to_visit picked/cs_added for p_plan_date, raised as
-- a critical monitoring_alert 'engine_add_pod_binding_drift'). Reapplying this file is a
-- no-op against current prod.
