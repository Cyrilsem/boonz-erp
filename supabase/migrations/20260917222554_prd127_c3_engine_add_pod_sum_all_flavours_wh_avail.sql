-- PRD-127 Block C bug #3: engine_add_pod's wh_avail already wraps
-- wh_available_for in SUM(...), but the boonz_product_id it sums over comes
-- from a LATERAL ... ORDER BY ... LIMIT 1 join (rmap) that picks exactly ONE
-- representative flavour mapped to the shelf's pod product. If that one
-- flavour happens to be out of stock while a sibling flavour of the same pod
-- product has plenty, wh_avail reports 0 and the whole lane gets
-- blocked_no_wh even though the pod product itself is fillable -- 13 lanes
-- falsely blocked on 09-16.
--
-- Fixed by summing wh_available_for over every DISTINCT Active
-- boonz_product_id mapped to that pod product for that machine, each still
-- resolved with the same machine-specific-over-global-default preference
-- (so the same flavour is never counted twice), instead of collapsing to a
-- single row before the sum. Every other line of the function body is
-- byte-for-byte identical to the live version (verified via
-- pg_get_functiondef diff before this migration was written) -- only the
-- wh_avail expression inside the `candidates` CTE changed.
...
BEGIN
  ...
  candidates AS (
    SELECT
      ...
      COALESCE((
        SELECT SUM(waf.free_stock)
        FROM (
          SELECT DISTINCT ON (pm3.boonz_product_id) pm3.boonz_product_id
          FROM public.product_mapping pm3
          WHERE pm3.pod_product_id = wid.identity_pod AND pm3.status = 'Active'
            AND (pm3.machine_id IS NULL OR pm3.machine_id = sl.machine_id)
          ORDER BY pm3.boonz_product_id, (pm3.machine_id = sl.machine_id) DESC NULLS LAST, pm3.is_global_default DESC
        ) all_flavours
        CROSS JOIN LATERAL public.wh_available_for(sl.machine_id, all_flavours.boonz_product_id) waf
      ), 0)::int AS wh_avail,
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version', CASE WHEN v_mode='base_stock' THEN 'v21_prd127c3_sum_all_flavours_wh_avail' ELSE 'v21_relative_score_band_f1_per_machine_d1_expired_sub_prd127c3' END,
    'sizing_mode', v_mode,
