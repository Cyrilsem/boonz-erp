-- CC-00 hotfix: door_statuses is the top-level cabinets array, not an object with a 'cabinets' key.
-- jsonb_typeof(door_statuses) = 'array', so iterate it directly.
-- One line changed:
--   WAS: LATERAL jsonb_array_elements(ls.door_statuses -> 'cabinets') cabinet(value),
--   NOW: LATERAL jsonb_array_elements(ls.door_statuses) cabinet(value),
DROP VIEW IF EXISTS public.v_live_shelf_stock;

CREATE VIEW public.v_live_shelf_stock AS
...
door_statuses is the top-level cabinets array (not an object with a cabinets key).
