-- ============================================================================
-- RC-06 FIX (2026-07-18): engine_swap_pod — version-agnostic dead-tag filters
-- ----------------------------------------------------------------------------
-- ROOT CAUSE (forensic audit 2026-07-16, re-verified live 2026-07-18):
--   engine_swap_pod's opening DELETE on pod_swaps preserves only unresolved
--   dead-tag rows whose reasoning->>'tagged_by' IN
--   ('engine_add_pod_v15','engine_add_pod_v16'), and the Pass-2a
--   dead-resolution loop filters on the same stale literals. engine_add_pod
--   has written tagged_by='engine_add_pod_v18' (normal mode) and
--   'engine_add_pod_v19_base_stock' (base_stock mode) since PRD048
--   (2026-06-22). Consequence inside build_draft_for_confirmed (add -> swap):
--   swap's opening DELETE destroys the fresh dead tags add just wrote, and
--   any survivor is invisible to the dead-resolution loop — the substitution
--   pipeline has been severed since 2026-06-22.
--
-- FIX (minimal diff — exactly two clauses changed, nothing else):
--   Both filters become version-agnostic:
--     reasoning->>'tagged_by' LIKE 'engine_add_pod%'
--   This matches every past and future engine_add_pod version (v15, v16,
--   v18, v19_base_stock, vNN...), so the swap engine can never again be
--   severed by an engine_add_pod version bump.
--
-- Source of truth: live body pulled 2026-07-18 from prod
--   (md5 ce685e14b6164257fb52301cdf5c8e7a — see rollback file for verbatim
--   restore). SECURITY DEFINER / SET search_path / attributes preserved
--   exactly.
--
-- NOT in scope (separate gated change): engine_add_pod candidate-universe /
--   slot_lifecycle visibility issue. engine_finalize_pod and
--   build_draft_for_confirmed verified clean — no tagged_by references.
-- ============================================================================
