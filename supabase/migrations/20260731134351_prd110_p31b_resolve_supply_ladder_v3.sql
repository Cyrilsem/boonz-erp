-- PRD-110 P3.1b · resolve_supply_ladder_v3 — the BUILD-SPEC line 88 substitution ladder.
--
...
--
-- SECURITY INVOKER, deliberately. Cody class (c) prefers INVOKER for a read-only helper, and
-- P3.1a's find_substitutes_for_shelf_v3 -- which this delegates to -- is already INVOKER/STABLE.
-- INVOKER also dissolves the role-gate question: RLS applies as the caller, so no operator_admin
-- check is needed (and a DEFINER here would silently widen read access to WH stock).
--
-- SHAPE: READ-ONLY and ADVISORY. It is STABLE and writes nothing -- no plan rows, no
-- blocked_demand rows, no protected-entity touch. LAW 4 (shadow, don't switch) and LAW 12
-- (never touch live plan tables) are therefore satisfied BY CONSTRUCTION rather than by
-- care. The writer half lands with stitch_v3; this unit is the decision, not the effect.
-- It has NO consumer yet, deliberately: engine_swap_pod still consumes v1 and P3.1b has
-- not earned the right to change swap behaviour in the same unit (LAW 3, carried from D-22).
--
-- TWO FINDINGS FROM LIVE DATA ARE LOAD-BEARING HERE, both measured before this was written:
--
--  (1) SENTINELS ARE NOT SUPPLY. v_wh_pickable carries 40 VOXSOURCE rows / 39,463 phantom
--      units (999 each, expiry 2099-12-31) minted by P0.4 to unblock venue-sourced planning.
--      Fade Fit shows 7,992 such units on the anchor machine and ZERO real ones. A ladder
--      that sums v_wh_pickable naively "rescues" that gap with stock nobody can pick.
--      Sentinels are counted SEPARATELY and REPORTED, never spent.
--
--  (2) STOCK IS NOT AVAILABILITY. The engine allocates a shared WH pool fleet-wide in
--      priority order. Live proof: VOXMCC-1005 / Vitamin Well recorded wh_avail=13 against
--      need_raw=8 and still clamped blocked_no_wh, because reasoning->>'prior_need_pool'
--      was 40 -- earlier shelves had already claimed the pool. wh_available_pod is GROSS
--      pod stock, not what remains. Every WH rung here nets off what this plan_date has
--      already committed, or the ladder would re-offer units the plan has spent
--      (the double-allocation / phantom-mint class of bug).
...
BEGIN
  ...
  IF NOT v_sat1 THEN
    ...
    BEGIN
      SELECT s.pod_product_id, s.pod_product_name, s.product_category, s.category_match,
       ...
       ORDER BY s.rank
       LIMIT 1;
      v_sat2 := (v_sub.pod_product_id IS NOT NULL AND COALESCE(v_sub.wh_stock_units, 0) > 0);
  ...
  ELSIF v_sat2 THEN
    ...
    v_qty := LEAST(p_qty_needed, v_sub.wh_stock_units)::int;
    v_payload := jsonb_build_object('substitute_pod_product_id', v_sub.pod_product_id,
                                    'substitute_name', v_sub.pod_product_name,
                                    'product_category', v_sub.product_category,
                                    'category_match', v_sub.category_match,
                                    'requires_cs_review', v_sub.requires_cs_review,
                                    'unit_margin', v_sub.unit_margin,
  ...
  v_ladder :=
    jsonb_build_array(
      ...
      jsonb_build_object(
        ...
        'satisfiable', v_sat2, 'qty_available', CASE WHEN v_sat2 THEN LEAST(p_qty_needed, v_sub.wh_stock_units)::int ELSE 0 END,
        'reason', CASE
          ...
          WHEN v_sat2 THEN format('find_substitutes_for_shelf_v3 rank 1 = %s (%s), category_match=%s',
                                  v_sub.pod_product_name, v_sub.product_category, v_sub.category_match)
...
-- S-57 applied FORWARD: tight from birth. Supabase's default privileges would otherwise
-- leave this anon-executable, which is how v1 find_substitutes_for_shelf ended up exposed.
REVOKE ALL ON FUNCTION public.resolve_supply_ladder_v3(date,uuid,uuid,uuid,integer,integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.resolve_supply_ladder_v3(date,uuid,uuid,uuid,integer,integer) FROM anon;
GRANT EXECUTE ON FUNCTION public.resolve_supply_ladder_v3(date,uuid,uuid,uuid,integer,integer) TO authenticated, service_role;

COMMENT ON FUNCTION public.resolve_supply_ladder_v3(date,uuid,uuid,uuid,integer,integer) IS
'PRD-110 P3.1b. BUILD-SPEC line 88 substitution ladder: variant -> substitute -> alt_wh -> m2m -> spot_buy -> blocked_demand. READ-ONLY advisory (STABLE, writes nothing). Logs all six rungs with reasons; asserts LAW 5 (silent qty-0 forbidden). Excludes VOXSOURCE sentinel stock and nets WH availability off prior claims on the same plan_date. Proven by golden fixture 40. No consumer yet - wiring into stitch_v3 is a later unit (D-22).';
