...
-- PRD-106b2 Evian-1L swap-in exclusion. Source: docs/architecture/CHANGELOG.md:2963 and
-- docs/architecture/MIGRATIONS_REGISTRY.md:697.
-- This migration's semantic delta: `universe` CTE excludes pod 990461ff-ff92-4b3f-aedb-cde0e951aaf8
-- (Evian - 1L) from the swap-in candidate universe — a standing CS guardrail that recommend_swaps_for_machine
-- already honored, surfaced as a gap by the prd106b probe (Evian ranked #3 on MC A15). Post-apply probe clean.
--
-- CANNOT BE RECOVERED EXACTLY: same caveat as the preceding migration 20260729084422
-- (prd106b_sizeup_requires_double_down) — this function was further DROP+CREATE'd by
-- 20260729102850 (prd108_rank_slot_suitability_volume_tests) the same day, replacing the
-- percentile size-up test with T1/T2/T3 volume tests. The pre-PRD-108 intermediate body is not
-- recoverable. This file carries the CURRENT CUMULATIVE body (pg_get_functiondef against live
-- prod); the Evian exclusion this migration introduced is visible in the `universe` CTE below
-- (`-- PRD-106b2: Evian - 1L is never a swap-in`). CREATE OR REPLACE is idempotent across the
-- repeated re-application in this and the neighboring migrations.
