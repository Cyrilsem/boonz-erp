-- PRD-127 Block C bug #2: the M2M-parent search inside
-- insert_driver_remove_line excludes any candidate Remove leg with
-- item_added=true. The true writer of item_added=true on both legs of an
-- M2M pair is approve_m2m_transfer (via receive_dispatch_line's blanket
-- UPDATE ... SET item_added = true), not "push pairing" as first reported --
-- traced in full in DECISIONS-2026-09-17.md D-004. A warehouse manager can
-- run approve_m2m_transfer before the driver has physically reconciled a
-- mixed-flavour lane, permanently blocking the multi-variant split.
--
-- Fix (D-005): drop the item_added filter from the M2M-parent search. This
-- is the option CS's own bug report explicitly offered ("drop item_added
-- from that predicate") -- the alternative ("stop the pairing writing it")
-- is not available without breaking approve_m2m_transfer's own idempotency
-- guard, which legitimately needs to set item_added=true at approval time.
--
-- Disclosed residual risk (D-005): this now lets a driver reduce an
-- already-approved leg's quantity after the fact via this RPC's own
-- `UPDATE refill_dispatching SET quantity = quantity - p_quantity`, and the
-- warehouse-side reconciliation that already ran used the OLD quantity and
-- will not be recomputed. CS's own report (a real driver blocked by this
-- exact case) reads as a deliberate judgment that this is the smaller cost.
