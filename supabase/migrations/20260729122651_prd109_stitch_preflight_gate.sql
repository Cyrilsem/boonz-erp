...
-- PRD-109: wires preflight_refill_plan into stitch_pod_to_boonz as a commit gate (blocking by
-- default, superseded within minutes by prd109_preflight_enforcement_warn_mode which added
-- refill_policy_params.preflight_enforcement and defaulted it to 'warn'). Adds p_force/p_force_reason
-- params (DROP 2-arg / CREATE 4-arg) and an append-only preflight_override_log audit table.
-- The function body below is stitch_pod_to_boonz's CURRENT live definition (2026-08-20), which is a
-- superset: later migrations outside this batch (PRD-110/111/etc.) have also touched this function
-- since 2026-07-29. This file captures it in full because it is the canonical current object;
-- do not treat it as a byte-exact diff of the 07-29 gate addition alone.

CREATE TABLE IF NOT EXISTS public.preflight_override_log (
  ...
  invariant_versions jsonb,
  source             text NOT NULL DEFAULT 'stitch'
);
-- Append-only per Article 7: no UPDATE/DELETE policy is granted; this reconstruction does not
-- attempt to recover the exact original RLS/grant set, only the column shape.

CREATE OR REPLACE FUNCTION public.stitch_pod_to_boonz(p_plan_date date DEFAULT (CURRENT_DATE + 1), p_dry_run boolean DEFAULT true, p_force boolean DEFAULT false, p_force_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_variant_subs jsonb := '[]'::jsonb;
  v_unfilled jsonb := '[]'::jsonb;
  v_unfilled_dev_n integer := 0;
  ...
  v_pf record;   -- PRD-109 pre-flight verdict
  v_enforce text;
BEGIN
  ...
  -- ══ PRD-109 PRE-FLIGHT GATE ═════════════════════════════════════════════
  -- Perfect refill is a property the system PROVES before commit, not an
  -- outcome anyone hopes for. Read-only; it can only report.
  ...
  IF v_pf.verdict = 'FAIL' AND NOT p_dry_run AND v_enforce = 'block' THEN
    IF NOT p_force THEN
      RETURN jsonb_build_object(
        ...
        'message','Commit refused: the plan violates one or more refill invariants. Each violation carries a fix path. Fix them and re-run, or re-run with p_force := true plus a p_force_reason of at least 10 characters - the override is audited.');
    END IF;
    IF p_force_reason IS NULL OR length(trim(p_force_reason)) < 10 THEN
      RAISE EXCEPTION 'stitch_pod_to_boonz: p_force requires p_force_reason of at least 10 characters';
    END IF;
    INSERT INTO public.preflight_override_log
      (plan_date, overridden_by, reason, verdict, violation_count, violations, invariant_versions)
    VALUES (p_plan_date, v_user_id, trim(p_force_reason), v_pf.verdict,
            jsonb_array_length(v_pf.violations), v_pf.violations, v_pf.invariant_versions);
  END IF;
  ...
  pull_raw AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           ...
           (EXISTS (SELECT 1 FROM public.v_shelf_variant_identity pil
           ...
           ROW_NUMBER() OVER (
             ...
             ORDER BY (pm.machine_id=a.machine_id) DESC NULLS LAST,
                      pm.is_global_default DESC, a.qty DESC, pm.boonz_product_id
           ) AS rnk
      FROM approved a
  ...
  pull_overlaid AS (
    SELECT pv.*,
           ...
           COALESCE((SELECT SUM(vp.warehouse_stock)::int FROM public.v_wh_pickable vp WHERE vp.boonz_product_id = pv.boonz_product_id), 0) AS wh_avail_variant
  ...
  pull_ranked AS (
    SELECT psr.*, ROW_NUMBER() OVER (
      PARTITION BY plan_date,machine_id,shelf_id,pod_product_id
      ORDER BY remainder_score DESC, (COALESCE(on_shelf,false)) DESC, norm_split DESC, boonz_product_id
  ...
  -- p0_fix11: WH-aware sibling redistribution. A warehouse-sourced variant's
  -- allocated share is capped at its pickable WH stock (v_wh_pickable is one row
  -- per wh_inventory_id, so the SUM is dedupe-safe); the shortfall moves to
  -- sibling variants of the same pod line that still have WH headroom
  -- (allocated in descending pickable stock), preserving the pod total.
  -- Driver-pinned quantities are never reduced. Residual remains only when NO
  -- sibling has stock — surfaced via unfilled_shortfalls, never silently dropped.
  pull_redist_base AS (
    SELECT pt.*,
           (COALESCE(source_origin,'warehouse') = 'warehouse'
            AND action IN ('REFILL','ADD_NEW')) AS wh_bound,
           CASE WHEN COALESCE(source_origin,'warehouse') = 'warehouse'
                     AND action IN ('REFILL','ADD_NEW')
                     AND pin_qty = 0
                THEN GREATEST(variant_target - COALESCE(wh_avail_variant,0), 0)
                ELSE 0 END::int AS donate_qty
      FROM pull_target pt
  ),
  pull_redist_pool AS (
    SELECT prb.*,
           (variant_target - donate_qty)::int AS kept_qty,
           SUM(donate_qty) OVER (PARTITION BY plan_date,machine_id,shelf_id,pod_product_id)::int AS sub_pool,
           CASE WHEN wh_bound AND donate_qty = 0
                THEN GREATEST(COALESCE(wh_avail_variant,0) - (variant_target - donate_qty), 0)
                ELSE 0 END::int AS headroom,
           string_agg(boonz_product_name, ', ') FILTER (WHERE donate_qty > 0)
             OVER (PARTITION BY plan_date,machine_id,shelf_id,pod_product_id) AS donor_names,
           (min(boonz_product_id::text) FILTER (WHERE donate_qty > 0)
             OVER (PARTITION BY plan_date,machine_id,shelf_id,pod_product_id))::uuid AS donor_boonz_id
      FROM pull_redist_base prb
  ),
  pull_redist AS (
    SELECT prp2.*,
           CASE WHEN wh_bound AND sub_pool > 0 AND headroom > 0
                THEN LEAST(headroom,
                       GREATEST(sub_pool - COALESCE(SUM(headroom) OVER (
                         PARTITION BY plan_date,machine_id,shelf_id,pod_product_id
                         ORDER BY (CASE WHEN headroom > 0 THEN COALESCE(wh_avail_variant,0) ELSE -1 END) DESC, boonz_product_id
                         ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0), 0))
                ELSE 0 END::int AS recv_qty
      FROM pull_redist_pool prp2
  ),
  pull_with_wh AS (
    SELECT pr2.*,
           pr2.wh_avail_variant AS wh_avail,
           (pr2.kept_qty + pr2.recv_qty)::int AS variant_final
      FROM pull_redist pr2
  ),
  pull_lines AS (
    SELECT plan_date,machine_id,shelf_id,pod_product_id,action,
           ...
           recv_qty AS variant_sub_recv,
           CASE WHEN recv_qty > 0 THEN donor_names END AS variant_sub_from
      FROM pull_with_wh
  ...
  remove_lines_raw AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
      ...
      FROM approved a
      JOIN public.v_shelf_variant_identity pil
  ...
  remove_lines AS (
    SELECT plan_date,machine_id,shelf_id,pod_product_id,action,
           ...
           0::int AS variant_sub_recv,
           NULL::text AS variant_sub_from
      FROM remove_lines_raw
  ),
  remove_lines_filtered AS (
    SELECT * FROM remove_lines WHERE variant_final > 0
  ),
  -- PRD-053-A FIX: physical fallback sizes from the pod PLAN total (a.qty), never live shelf stock.
  remove_phys_base AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           a.machine_name,a.shelf_code,a.pod_product_name,
           a.source_origin, a.from_machine,
           a.qty::int AS plan_qty
      FROM approved a
  ...
  remove_phys_map AS (
    SELECT b.plan_date,b.machine_id,b.shelf_id,b.pod_product_id,b.action,
           ...
           b.source_origin,b.from_machine,b.plan_qty,
           pm.boonz_product_id, bp.boonz_product_name, pm.split_pct AS split_pct,
           ROW_NUMBER() OVER (
  ...
  remove_phys_qty AS (
    SELECT s.*,
           FLOOR(plan_qty*norm_split)::int AS base_qty,
           (plan_qty*norm_split)-FLOOR(plan_qty*norm_split)::numeric AS remainder_score
      FROM remove_phys_split s WHERE norm_split>0
  ),
  remove_phys_rem AS (
    SELECT q.*, plan_qty - SUM(base_qty) OVER (PARTITION BY machine_id,shelf_id,pod_product_id)::int AS slot_remainder
  ...
  -- p0_fix11: pod lines whose donated shortfall could not be fully absorbed by
  -- siblings (pool leftover after redistribution) — explicit, never silent.
  pull_unfilled AS (
    SELECT plan_date, machine_id, shelf_id, pod_product_id,
           MAX(machine_name) AS machine_name,
           MAX(shelf_code)   AS shelf_code,
           MAX(pod_product_name) AS pod_product_name,
           MAX(pod_qty)::int AS pod_qty,
           MAX(donor_names)  AS donor_names,
           (MAX(sub_pool) - COALESCE(SUM(recv_qty),0))::int AS unfilled_units,
           'insufficient_wh_no_sibling_stock'::text AS reason
      FROM pull_redist
     WHERE wh_bound
     GROUP BY plan_date, machine_id, shelf_id, pod_product_id
    HAVING MAX(sub_pool) - COALESCE(SUM(recv_qty),0) > 0
  ),
  -- p0_fix11: pod lines that previously vanished entirely (no residual-eligible
  -- variant at all, e.g. total WH stockout on a known-variant REFILL shelf).
  pull_stockout AS (
    SELECT plan_date, machine_id, shelf_id, pod_product_id,
           MAX(machine_name) AS machine_name,
           MAX(shelf_code)   AS shelf_code,
           MAX(pod_product_name) AS pod_product_name,
           MAX(pod_qty)::int AS pod_qty,
           string_agg(DISTINCT boonz_product_name, ', ') AS donor_names,
           MAX(residual_pool)::int AS unfilled_units,
           'total_wh_stockout'::text AS reason
      FROM pull_norm
     WHERE action IN ('REFILL','ADD_NEW')
       AND COALESCE(source_origin,'warehouse') = 'warehouse'
     GROUP BY plan_date, machine_id, shelf_id, pod_product_id
    HAVING BOOL_AND(NOT is_residual_variant)
       AND MAX(residual_pool) > 0
  ),
  pull_underfill AS (
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'variant_sub_units', COALESCE(al.variant_sub_recv,0),
    'variant_sub_from', al.variant_sub_from,
    'comment', NULLIF(CONCAT_WS(' ', CASE
      ...
      ELSE NULL
    END,
    -- p0_fix11: explicit marker on every line that received units shifted from
    -- an out-of-stock sibling variant.
    CASE WHEN COALESCE(al.variant_sub_recv,0) > 0
      THEN '[variant_sub] ' || al.variant_sub_recv::text || ' units '
           || COALESCE(al.variant_sub_from,'(unknown)') || ' -> ' || boonz_product_name
           || ' (' || COALESCE(al.variant_sub_from,'(unknown)') || ' insufficient WH)'
    END), '')
  )) AS lines_json
  FROM all_lines al
  ...
  unfilled_agg AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
             'machine',          u.machine_name,
             'shelf_code',       u.shelf_code,
             'pod_product_name', u.pod_product_name,
             'from_sku',         COALESCE(u.donor_names,'(unknown)'),
             'to_sku',           NULL,
             'units',            u.unfilled_units,
             'reason',           u.reason,
             'machine_id',       u.machine_id,
             'shelf_id',         u.shelf_id,
             'pod_product_id',   u.pod_product_id,
             'pod_qty',          u.pod_qty
           ) ORDER BY u.machine_name, u.shelf_code), '[]'::jsonb) AS unfilled_json
      FROM (SELECT * FROM pull_unfilled UNION ALL SELECT * FROM pull_stockout UNION ALL SELECT * FROM pull_underfill) u
  )
  SELECT la.lines_json, ua.unfilled_json
    INTO v_lines, v_unfilled
  FROM lines_agg la CROSS JOIN unfilled_agg ua;

  v_line_count := COALESCE(jsonb_array_length(v_lines), 0);
  v_unfilled := COALESCE(v_unfilled, '[]'::jsonb);

  -- p0_fix11: explicit report of units shifted between sibling variants
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'machine',    r->>'machine_name',
           'shelf_code', r->>'shelf_code',
           'from_sku',   COALESCE(r->>'variant_sub_from','(unknown)'),
           'to_sku',     r->>'boonz_product_name',
           'units',      (r->>'variant_sub_units')::int,
           'reason',     'insufficient_wh'
         ) ORDER BY r->>'machine_name', r->>'shelf_code'), '[]'::jsonb)
    INTO v_variant_subs
    FROM jsonb_array_elements(COALESCE(v_lines,'[]'::jsonb)) r
   WHERE COALESCE((r->>'variant_sub_units')::int, 0) > 0;

  -- p0_fix11: unfilled shortfalls are NEVER silent — stamp the existing
  -- deviation mechanism (deviation_type 'wh_shortage' is in the table CHECK).
  ...
  SELECT p_plan_date,
         (u->>'machine_id')::uuid,
         (u->>'shelf_id')::uuid,
         (u->>'pod_product_id')::uuid,
         'REFILL',
         COALESCE((u->>'pod_qty')::int, (u->>'units')::int),
         COALESCE((u->>'pod_qty')::int, (u->>'units')::int) - (u->>'units')::int,
         jsonb_build_object('requested', COALESCE((u->>'pod_qty')::int, (u->>'units')::int)),
         jsonb_build_object('delivered', COALESCE((u->>'pod_qty')::int, (u->>'units')::int) - (u->>'units')::int,
                            'unfilled', (u->>'units')::int),
         'wh_shortage',
         '[p0_fix11] ' || (u->>'units') || ' unit(s) of ' || COALESCE(u->>'pod_product_name','(unknown)')
         || ' on ' || COALESCE(u->>'machine','?') || ' shelf ' || COALESCE(u->>'shelf_code','?')
         || ' unfilled: ' || COALESCE(u->>'reason','insufficient_wh')
         || ' (variants: ' || COALESCE(u->>'from_sku','(unknown)') || ')'
    FROM jsonb_array_elements(v_unfilled) u
   WHERE (u->>'machine_id') IS NOT NULL
     AND (u->>'shelf_id') IS NOT NULL
     AND (u->>'pod_product_id') IS NOT NULL;
  GET DIAGNOSTICS v_unfilled_dev_n = ROW_COUNT;
           ...
           CASE
             WHEN a.action IN ('REFILL','ADD_NEW') THEN
               CASE
                 ...
                 -- p0_fix11: EXISTS (not JOIN) so each wh_inventory_id row is counted
                 -- once even when global + machine-specific mapping rows coexist.
             ...
             WHEN a.action IN ('REMOVE','M2W') THEN
               CASE
                 WHEN NOT EXISTS (
                   SELECT 1 FROM public.v_shelf_variant_identity pil
                    WHERE pil.machine_id=a.machine_id AND pil.shelf_id=a.shelf_id
  ...
  -- drift-kill P1: WEIMI slot guard on every stitch (dry-run: 'check' = zero writes)
  v_slot_guard := public.assert_weimi_slot_match(p_plan_date, CASE WHEN p_dry_run THEN 'check' ELSE NULL END);
  PERFORM set_config('app.rpc_name', 'stitch_pod_to_boonz', true);

  RETURN jsonb_build_object(
    'plan_date',p_plan_date,'dry_run',p_dry_run,
    'engine_version','v30_p0fix11_variant_substitution',
    'lines_built',v_line_count,'deviations',v_deviation_n,'procurement_alerts',v_alert_n,
    'substitution_alerts', v_subs_alert_n,
    'variant_substitutions', v_variant_subs,
    'unfilled_shortfalls', v_unfilled,
    'unfilled_shortfall_deviations', v_unfilled_dev_n,
    ...
    'preflight_verdict', v_pf.verdict,
    'preflight_violation_count', jsonb_array_length(v_pf.violations),
    'preflight_warning_count', jsonb_array_length(v_pf.warnings),
    'preflight_violations', v_pf.violations,
    'preflight_forced', (p_force AND v_pf.verdict = 'FAIL' AND v_enforce = 'block'),
    'preflight_enforcement', v_enforce,
