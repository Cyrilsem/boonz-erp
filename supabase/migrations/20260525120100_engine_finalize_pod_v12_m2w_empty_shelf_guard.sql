-- PRD-011 Bug 5: engine_finalize_pod v12 — M2W empty-shelf guard
--
-- v11 already tags M2W/REMOVE rows with no paired ADD_NEW with a
-- 'warning': 'empty_shelf_after_removal' note in their reasoning jsonb.
-- It does NOT prevent them from being committed. The MC-2004 B08
-- (Organic Larder Rice Cake) incident on 2026-05-25 made it clear that
-- advisory warnings are insufficient: an unattended M2W with no
-- replacement leaves the shelf empty after the visit.
--
-- v12 adds an executable guard. For each M2W/REMOVE in the finalized
-- draft:
--   1. If there is NO same-shelf ADD_NEW or REFILL with qty > 0, AND
--   2. The shelf is not tagged for decommission via strategic_machine_tags
--      (action_directive = 'swap_out_m2w', status = 'approved', matching
--      machine_id + pod_product_id),
-- then set status='superseded' with reasoning.auto_suppressed and capture
-- the row in a new return key m2w_no_replacement_warnings. The same
-- entries are also appended to capacity_mismatch_warnings so existing
-- consumers of that field see them.
--
-- engine_version: v11_capacity_mismatch_warnings -> v12_m2w_empty_shelf_guard.
-- Article 1: existing canonical writer extended in place. No new function.
-- Article 12: forward-only CREATE OR REPLACE; identity + args unchanged.
...
BEGIN
  ...
  -- ==========================================================================
  -- PRD-011 Bug 5: M2W/REMOVE auto-suppress when no replacement and no
  -- approved decommission tag. Capture the suppressed rows for the return
  -- payload BEFORE the UPDATE flips status.
  -- ==========================================================================
  ...
  decom_tagged AS (
    -- Approved swap_out_m2w directive == the operator has explicitly chosen
    -- to retire this product from this machine; leaving the shelf empty is
       ...
       SET status     = 'superseded',
           reasoning  = COALESCE(prp.reasoning, '{}'::jsonb)
                        || jsonb_build_object(
                             ...
                             'auto_suppressed_by', 'engine_finalize_pod_v12'),
           updated_at = now()
     WHERE prp.plan_date = p_plan_date
       AND prp.status    = 'draft'
       AND prp.action    IN ('M2W','REMOVE')
  ...
  -- ==========================================================================
  -- v11 AC#4: capacity_mismatch_warnings (carried forward unchanged from v11)
  -- ==========================================================================
  ...
  -- PRD-011 Bug 5: merge m2w_no_replacement entries into the existing
  -- capacity_mismatch_warnings array so consumers that only read that
  -- field still surface the orphan M2Ws.
  IF v_m2w_suppressed_n > 0 THEN
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version', 'v12_m2w_empty_shelf_guard',
