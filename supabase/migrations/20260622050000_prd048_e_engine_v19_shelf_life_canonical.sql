-- PRD-048 Step 2: engine_add_pod v19 — converge shelf_life_days onto the canonical
-- v_product_shelf_life (Article 16). ONLY the base_stock shelf_life_days subquery changes
-- (inline warehouse_inventory read -> canonical view). Legacy branch byte-identical (gate-clean).
-- Forward-only CREATE OR REPLACE (Article 12). Dara ✅ -> Cody -> MCP. swaps_enabled untouched.
