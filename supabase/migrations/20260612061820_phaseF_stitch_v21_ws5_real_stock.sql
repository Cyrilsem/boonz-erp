-- PRD-027 WS5: stitch v20 -> v21, emit REAL current_stock / max_stock.
-- CS explicitly green-lit this second stitch rewrite within 24h of v20
-- (Hard Rule 10) on 2026-06-12, to ship BEFORE the PRD-024 section-2 re-stitch
-- so the rebuilt 06-13 plan carries real stock fields.
-- Problem: v19/v20 hardcoded 'current_stock',0,'max_stock',0 on every emitted
-- refill_plan_output line, so FE comparisons rendered every row "above cap".
-- Diff surface vs v20 (body otherwise verbatim from the applied
-- phaseF_stitch_split_pct_normalize):
--  1. two new CTEs shelf_stock (live per-shelf stock via v_live_shelf_stock)
--     + shelf_caps (v_shelf_max_stock max) appended after all_lines
--  2. emit: current_stock/max_stock = COALESCE(real, 0) - shelves absent from
--     both views keep 0 (the WS4 backfill population stays visible, not invented)
--  3. final emit FROM gains the two LEFT JOINs
--  4. engine_version -> v21_ws5_real_stock
-- Rollback: redeploy the v20 body (repo file 20260612200000_*.sql).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v21_ws5_real_stock',
