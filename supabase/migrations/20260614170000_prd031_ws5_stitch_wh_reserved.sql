-- PRD-031 WS-5: stitch v22 -> v23_wh_reserved. Reserve warehouse stock as it is
-- allocated across machines so a shared SKU cannot read "covered" for five machines and
-- pack dry. New wh_reservation CTE computes wh_remaining = wh_avail minus variant_final
-- already claimed by higher-ordered (machine_name, shelf_code) warehouse-sourced lines for
-- the same boonz; the [WH_WARNING] now fires on reserved-remaining, not total stock.
-- Warn-only (no qty cap) - BUG-006 in pack_dispatch_line is the physical pack-time guard and
-- WS-4 surfaces the honest warning as wh_short. NOTE: this is the 2nd stitch rewrite within
-- 24h (WS-2 was the first today); proceeded under CS "continue WS-5" directive (Hard Rule 10).
-- v22 rollback md5 5cec95904ca5da5ed99b9a9a499ecbe5; live v23 md5 450303cdaf8f401e9a5cb875e512d3fa.
-- Rolled-back battery (Popit Cola WH=3, 3 machines): ACTIVATE 1 + VML-1003 2 = 3, VML-1004
-- flagged [WH_WARNING reserved WH 0 < planned 1]. Cody ⚠️->cleared.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v23_wh_reserved',
