-- PRD-128 rollback snapshot, part 2: get_machine_health() BEFORE PRD-128. NOT APPLIED.
-- Captured via pg_get_functiondef against the live database on 2026-09-19 ~21:15 UTC.
-- To roll back step 06, run this CREATE OR REPLACE FUNCTION verbatim (it also restores the
-- original RETURNS TABLE column list, which prd128_06 changes -- a plain CREATE OR REPLACE
-- cannot narrow the column list back, so a rollback of 06 needs DROP FUNCTION first, exactly
-- as 06 itself does going forward).
