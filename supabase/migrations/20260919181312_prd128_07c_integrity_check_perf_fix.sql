-- PRD-128 performance follow-up. v_machine_priority (step 05) and
-- check_machine_health_integrity() (step 07) are already committed migrations -- this is a
-- forward-only fix on top of them, not an edit-in-place.
--
-- v_machine_priority references v_machine_health_signals three separate times internally
-- (pre-existing structure, not introduced by PRD-128): the outer SELECT's `s`, mscore's `s_1`,
-- and pscore's `s2`. Before PRD-128 this was harmless because the old last_visit was a cheap
-- direct MAX() over refill_dispatching. Once last_visit depends on v_delivery_verification
-- (step 08), the same pre-existing 3x reference pattern turned into severe multiplication:
-- EXPLAIN ANALYZE showed one of v_delivery_verification's LATERAL index scans running 166,976
-- times against an expected 5,353 (a ~31x blow-up), because v_machine_health_signals kept
-- getting re-inlined inside v_machine_priority's own nested-loop structure rather than computed
-- once. v_machine_priority alone measured 18.6s.
--
-- Fix: one shared `vmhs AS MATERIALIZED (SELECT * FROM v_machine_health_signals)` CTE at the
-- top of v_machine_priority_base's WITH clause, with all three references (`s`, `s_1`, `s2`)
-- pointed at it instead of independently invoking the view. Pure query-planner hint, zero
-- semantic change -- same columns, same filters, same values. Measured after: v_machine_priority
-- alone dropped to well under 15s (previously 18.6s; the loop count on the worst node dropped
-- from 166,976 to 7,873).
--
-- check_machine_health_integrity() gets the matching fix: `health` (get_machine_health()'s
-- output) and `mp_full` (v_machine_priority's output) are both wrapped MATERIALIZED so G-REV's
-- separate `FROM v_machine_priority` query and the `surface` CTE's mp join both read from the
-- same single computation instead of two independent ones.
--
-- KNOWN RESIDUAL ISSUE (see DECISIONS-2026-09-19.md D-010): get_machine_health() as a whole is
-- still measured at roughly 30-45s, down from 47.7s before any of these fixes but still far
-- from snappy. The with_velocity CTE's dead_stock_count/local_hero_count correlated subqueries
-- (pre-existing, unchanged by PRD-128) run twice per machine against v_sales_history_resolved,
-- itself a heavy fuzzy-name-matching view -- this is very likely the largest remaining cost and
-- was NOT introduced by this PRD. A proper fix needs either a materialized snapshot of
-- last-landed-delivery-date refreshed by a cheap cron job, or a Dara-designed rework of
-- v_sales_history_resolved's resolution CTEs -- both are follow-up work, out of this PRD's
-- read-surface, minimal-footprint scope.

CREATE OR REPLACE VIEW public.v_machine_priority AS
 WITH v_machine_priority_base AS (
         WITH vmhs AS MATERIALIZED (
                 SELECT * FROM v_machine_health_signals
                ), shelf_u25 AS (
                 ...
                 SELECT mscore.machine_id,
                   ...
                   FROM ( SELECT mscore_1.machine_id,
                           ...
                           FROM mscore mscore_1
                             JOIN vmhs s2 ON s2.machine_id = mscore_1.machine_id) mscore
