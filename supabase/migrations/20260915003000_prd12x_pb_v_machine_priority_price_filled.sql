-- ONE-LOOP-2 Block B: v_machine_priority's AED scoring reads
-- v_current_price_filled instead of v_current_price, closing the price gap
-- that blocked PRD-126 A3/A5/A6/A7 (D-008, last night). Two CTEs change:
-- lane_price now joins v_current_price_filled directly by (machine_id,
-- pod_product_id) -- the product_mapping resolution v_current_price_filled
-- already did is not repeated here. expiry_agg_aed joins it by
-- (machine_id, boonz_product_id) via a LATERAL LIMIT 1, since
-- pod_inventory only carries boonz_product_id and multiple pod_products
-- could in principle map to the same boonz_product on one machine (the
-- prior v_current_price join had the same latent fan-out risk; this makes
-- it explicit and safe rather than leaving it implicit). Every other CTE,
-- column and the final SELECT are byte-identical to prd12x_p5.
...
WITH v_machine_priority_base AS (
  ...
  lane_price AS (
    ...
    LEFT JOIN public.v_current_price_filled vcf
      ON vcf.machine_id = lg.machine_id AND vcf.pod_product_id = lg.pod_product_id
  ),
  lane_agg_aed AS (
  ...
  price_by_boonz AS (
    -- Collapsed to one row per (machine, boonz_product): pod_inventory only
    -- carries boonz_product_id, and this must be evaluated ONCE and
    -- hash-joined, not re-evaluated per pod_inventory row (a per-row
    -- LATERAL here made v_machine_priority time out -- caught live, see
    -- DECISIONS-2026-09-15.md).
    SELECT DISTINCT ON (machine_id, boonz_product_id)
      machine_id, boonz_product_id, effective_price_aed
    FROM public.v_current_price_filled
    ORDER BY machine_id, boonz_product_id, effective_price_aed DESC NULLS LAST
  ),
  expiry_agg_aed AS (
    SELECT pi.machine_id,
      SUM(pi.current_stock * COALESCE(vcf.effective_price_aed,0)) FILTER (WHERE pi.expiration_date < CURRENT_DATE) AS expired_value_aed,
      SUM(pi.current_stock * COALESCE(vcf.effective_price_aed,0)) FILTER (WHERE pi.expiration_date >= CURRENT_DATE AND pi.expiration_date <= CURRENT_DATE + 3) AS expiring3d_value_aed
    FROM public.pod_inventory pi
    LEFT JOIN price_by_boonz vcf
      ON vcf.machine_id = pi.machine_id AND vcf.boonz_product_id = pi.boonz_product_id
