-- PRD-034 Phase B: guard receive_dispatch_line so venue_team (VOX) REMOVE receipts
-- are logged but NOT credited to warehouse_inventory. NOT APPLIED. Author-only;
-- apply after CS sign-off and after Phase A (vox_return_log) is applied.
--
-- Based byte-for-byte on the LIVE body (pg_get_functiondef, 2026-06-18). Only the
-- action='Remove' branch changes, plus one DECLARE (v_supply) and one additive key
-- in the shared RETURN jsonb (wh_credit_skipped). Refill/Add New branches and the
-- entire WH-credit logic are unchanged. Keeps app.via_rpc/app.rpc_name (Article 4/8),
-- never writes warehouse_inventory.status (Article 6). Forward-only CREATE OR REPLACE.
...
DECLARE
  ...
  v_supply text;  -- PRD-034: product_mapping.source_of_supply for the Remove branch
BEGIN
  ...
  ELSIF v_dispatch.action = 'Remove' THEN
    ...
    -- PRD-034: resolve product supply. Per-machine mapping row preferred, else the
    -- global default. venue_team (VOX) returns are NOT credited to Boonz warehouse.
    ...
    IF v_supply = 'venue_team' THEN
      -- Boonz never owned these units: skip the ENTIRE WH-credit block (no
      -- warehouse_inventory write of any kind) and record the return for VOX
      -- reconciliation. Pod archive + dispatch-received update still run below.
  ...
  RETURN jsonb_build_object('dispatch_id', p_dispatch_id, 'action', v_dispatch.action, 'filled_quantity', p_filled_quantity, 'planned_quantity', v_planned, 'return_delta', v_return_delta, 'overfill', v_overfill, 'pod_inventory_id', v_pod_id, 'pod_archived', v_pod_archived, 'prior_active_merged', v_prior_active_merged, 'consumer_drained', v_consumer_drawn, 'path', v_path, 'effective_expiry', v_effective_expiry, 'received_by', p_received_by, 'credit_summary', v_credit_summary, 'wh_credit_skipped', CASE WHEN v_supply = 'venue_team' THEN 'venue_team' ELSE NULL END, 'status', 'received');
END;
$function$;
