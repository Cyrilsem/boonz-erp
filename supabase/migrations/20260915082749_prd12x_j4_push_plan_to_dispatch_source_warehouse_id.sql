-- ONE-LOOP-3 Job 4 (mid-turn CS instruction): grepped every INSERT INTO
-- refill_dispatching for the same class of bug the CS hotfix fixed in
-- insert_driver_remove_line (source_kind='wh' written without deriving
-- source_warehouse_id, violating refill_dispatching_source_consistency_chk).
--
-- Checked: set_dispatch_source, add_dispatch_row (both take an explicit,
-- validated p_source_warehouse_id -- safe), convert_removes_to_m2m_transfer,
-- repair_orphan_internal_transfer, correct_packed_m2m_transfer (all hardcode
-- source_kind='m2m' with a real source_machine_id, never 'wh' -- safe),
-- audit_m2m_dispatch_changes (a trigger, only reads NEW.source_kind for a
-- log payload, never writes refill_dispatching -- not a writer).
--
-- Found the same bug class in push_plan_to_dispatch's two 'wh'-eligible
-- INSERTs (the Remove/Machine To Warehouse leg and the Refill/Add New leg):
-- both compute v_source_kind='wh' whenever line.source_origin is 'warehouse'
-- (or NULL, via a COALESCE default), but neither INSERT's column list ever
-- included source_warehouse_id. Fixed by deriving it from the same warehouse
-- variable each branch already uses for from_warehouse_id
-- (v_primary_warehouse_id for the Remove/M2W leg, v_line_wh_id for the
-- Refill/Add New leg), falling back to source_kind='unknown' if that
-- warehouse id is ever NULL. Verified in a rolled-back transaction against
-- real historical data (NISSAN-0804-0000-L0, 2026-05-10): new rows now carry
-- source_kind='wh' with a matching source_warehouse_id instead of NULL.
  ...
  LOOP
    ...
    IF v_action IN ('Remove','Machine To Warehouse') THEN
      ...
      INSERT INTO refill_dispatching (
        ...
        v_remove_lot_id,
        -- ONE-LOOP-3 Job 4: same class of bug the CS hotfix fixed in
        -- insert_driver_remove_line -- v_source_kind='wh' with no
        -- source_warehouse_id would violate refill_dispatching_source_consistency_chk.
        -- Fall back to 'unknown' if the warehouse this line actually pins to
        -- (v_primary_warehouse_id) is somehow NULL.
    ...
    INSERT INTO refill_dispatching (
      ...
      COALESCE(line.source_origin, 'warehouse'::public.source_origin_enum),
      CASE WHEN line.source_origin='internal_transfer' THEN line.from_machine_id ELSE NULL END,
      -- ONE-LOOP-3 Job 4: same fix as the Remove/M2W insert above -- derive
      -- source_warehouse_id from the line's own resolved warehouse
      -- (v_line_wh_id) instead of leaving it unset whenever v_source_kind='wh'.
