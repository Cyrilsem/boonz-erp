-- PRD-031 WS-2 + WS-2b: stitch v21 -> v22_onshelf_scoped.
-- WS-2b: machine-scoped mapping is authoritative (pull_raw + pm_per_row): admit global
--   rows only when NO scoped row exists for (pod,machine). Curated SKU sets become exclusive.
-- WS-2: off-shelf redistribution. Residual distributes only across variants present on the
--   shelf (v_pod_inventory_latest); off-shelf units redistribute via PRD-024's split window
--   (no drop). ADD_NEW swap-ins exempt (variant not yet on shelf); empty shelf keeps full set.
-- Deviation block left verbatim (vacuous: variant_target===variant_final; WS-4 replaces it).
-- v21 rollback md5 52a6d3b139fc5cb5542ab733f848a01e. Cody-approved (revision #1 ADD_NEW folded in).
-- APPLIED 2026-06-14 to prod (in-DB transform of live v21 stored source, 6 verified replacements).
-- Live v22 md5 5cec95904ca5da5ed99b9a9a499ecbe5. Rolled-back battery (synthetic date 2099-06-14)
-- green: WS-2b scoped-only emit, WS-2 off-shelf zeroed, ADD_NEW exemption, empty-shelf no-starve.
...
BEGIN
  ...
  pull_raw AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           ...
           -- PRD-031 WS-2: is this boonz variant physically on the target shelf?
      ...
      JOIN public.product_mapping pm
       ...
       -- PRD-031 WS-2b: machine-scoped mapping is authoritative. Use scoped rows for
       -- this (pod,machine); admit global rows ONLY when no scoped mapping exists.
  ...
  pull_resid AS (
    SELECT pt.*,
           ...
           -- PRD-031 WS-2: does the shelf have a KNOWN on-shelf variant for this REFILL pod?
           ...
           -- residual-eligible: on-shelf, OR an ADD_NEW swap-in (variant not yet on shelf, Cody #1),
           -- OR the shelf's variant set is unknown (empty inventory -> never starve).
           (pin_qty = 0 AND COALESCE(split_pct,0) > 0
            AND ( COALESCE(on_shelf,false)
                  OR action <> 'REFILL'
                  OR NOT (bool_or(COALESCE(on_shelf,false) AND action='REFILL')
                            OVER (PARTITION BY plan_date,machine_id,shelf_id,pod_product_id)) )
  ...
  WITH pm_per_row AS (
    ...
    JOIN public.product_mapping pm
     ...
     AND pm.status = 'Active'
     -- PRD-031 WS-2b: scoped-authoritative for demand too (match emit's SKU set).
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v22_onshelf_scoped',
