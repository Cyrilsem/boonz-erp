-- PRD-087 dashboard v2: machines.status uses 'Active' (not 'Live');
-- also expose trading_machines = distinct machines with a sale in 30d.
...
sales AS (
  SELECT (sh.transaction_date AT TIME ZONE 'Asia/Dubai')::date AS d,
         sh.machine_id, sh.qty, sh.paid_amount, sh.cost_amount
