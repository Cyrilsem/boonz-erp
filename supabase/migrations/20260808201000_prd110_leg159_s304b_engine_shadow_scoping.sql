-- PRD-110 leg 159 - S-304b - engine_add_pod_v3 stops letting v19's output delete v3's input.
--
-- WHAT CHANGES: the `NOT EXISTS (approved refill_plan_output row)` predicate is removed from
-- all FOUR sites that carried a copy of it (scope count, empty-machine count, the picked CTE
-- that drives the write, and the RISK-75 self-proving coverage guard). Nothing else in the
-- function moves - proven by md5 on both images.
--
-- WHY: engine_add_pod_v3 writes exactly ONE table, public.pod_refills_shadow (verified live:
-- its body contains a single INSERT and no UPDATE/DELETE against any live plan table). The
-- exclusion therefore protected no live plan; it only removed from v3's scope the very
-- machines v19 had just planned. cron 45 fires 21:22 UTC. Live approval times for the last
-- seven real dates: 21:59, 22:31, 22:16, 20:54, 06:09, 09:03 - only 2026-08-04's landed after
-- the cron, and 2026-08-04 is the ONLY real date in engine_forecast_error_v3 carrying a v3
-- series. DR-1's cutover rule is defined on WMAPE over exactly that evidence base.
--
-- LAW 4 is untouched: v3 still writes shadow tables only and no cutover flag exists.
-- LAW 1: fixture 37 seq 33/34/35 shipped RED in 20260808200000 before this file.

DO $guard$
DECLARE v_pre text;
BEGIN
  v_pre := md5(pg_get_functiondef('public.engine_add_pod_v3(date,integer)'::regprocedure));
  IF v_pre = 'fd4930b534d14424c524e646d0c6fbee' THEN
    RAISE EXCEPTION 'leg159 M2: engine_add_pod_v3 already carries the S-304b image; refusing to re-apply';
  END IF;
  IF v_pre <> '06859fd71c2bf6702d96ff9d7f01eb78' THEN
    RAISE EXCEPTION 'leg159 M2: engine_add_pod_v3 pre-image md5 is % (expected 06859fd71c2bf6702d96ff9d7f01eb78); the body drifted, re-diff offline before shipping', v_pre;
  END IF;
END $guard$;
    ...
    FROM (
      SELECT DISTINCT mtv.machine_id, mtv.official_name
       ...
       WHERE mtv.plan_date = p_plan_date
         ...
         -- S-304 (leg 159). The approved-LIVE-plan exclusion that used to stand here is
         -- GONE, and its absence is the point. engine_add_pod_v3 writes exactly ONE table,
         -- public.pod_refills_shadow, and never a live plan table, so excluding a machine
         -- whose refill_plan_output row is already 'approved' protected nothing - it only
         -- deleted v3's input using v19's own output. cron 45 fires at 21:22 UTC; on
         -- 2026-08-07 the live plan was approved at 20:54:13 UTC, 28 minutes earlier, so
         -- all 7 picked machines were excluded, v3 planned NOTHING, and the runner logged
         -- 'ok'. The one non-vacuous v3 date in the entire cutover evidence base
         -- (2026-08-04) survives only because that night's approval landed 37 minutes LATE.
         -- Fixture 37 seq 33/34/35 pin this. LAW 4 is untouched: still shadow-only.
    ) p
    ...
    SELECT p.machine_id
      FROM (
        SELECT DISTINCT mtv.machine_id, mtv.official_name
         ...
         WHERE mtv.plan_date = p_plan_date
           AND mtv.status IN ('picked','cs_added')
         -- S-304 (leg 159): approved-live-plan exclusion removed here too - see the note above.
      ) p
  ...
  SELECT count(*) INTO v_uncovered
    FROM (
      SELECT DISTINCT mtv.machine_id, mtv.official_name
       ...
       WHERE mtv.plan_date = p_plan_date
         AND mtv.status IN ('picked','cs_added')
   -- S-304 (leg 159): the coverage guard MUST use the same scope as the write above,
   -- so its copy of the exclusion goes with them or the guard starts lying.
    ) p
    JOIN public.v_shelf_state s
      ON s.machine_id = p.machine_id AND s.pod_product_id IS NOT NULL
   WHERE NOT EXISTS (
...
DO $proof$
DECLARE v_post text; v_def text;
BEGIN
  v_def  := pg_get_functiondef('public.engine_add_pod_v3(date,integer)'::regprocedure);
  v_post := md5(v_def);
  IF position('refill_plan_output rpo' in v_def) > 0 THEN
    RAISE EXCEPTION 'leg159 M2: the approved-plan exclusion is still present in the post-image';
  END IF;
  IF (length(v_def) - length(replace(v_def, 'S-304 (leg 159)', ''))) / length('S-304 (leg 159)') <> 4 THEN
    RAISE EXCEPTION 'leg159 M2: expected exactly 4 S-304 markers, one per removed site';
  END IF;
  IF v_post <> 'fd4930b534d14424c524e646d0c6fbee' THEN
    RAISE EXCEPTION 'leg159 M2: post-image md5 is % (expected fd4930b534d14424c524e646d0c6fbee) - the shipped body is not the image diffed offline', v_post;
  END IF;
END $proof$;
