-- Smoke test results (rolled back, 2026-09-25 00:xx Dubai, before this file was applied):
-- 1. push_plan_to_dispatch M2M pair test (WAVEMAKER-1006-4100-O1 A01 -> AMZ-1046-2406-O1 A13,
--    Coca Cola Zero, synthetic plan_date 2031-06-01): both legs came back packed=true,
--    picked_up=false. Green.
-- 2. cancel_m2m_transfer happy path (synthetic M2M pair, dispatch_date 2031-06-02,
--    convert_source_to_return=true): both legs correctly zeroed (quantity=0, skipped=true,
--    include=false); a new plain Remove row was created via add_dispatch_row with
--    source_kind='wh', source_warehouse_id=4bebef68-9e36-4a5c-9c2c-142f8dbdae85 (WAVEMAKER's own
--    primary_warehouse_id, which is WH_CENTRAL). Three real, unrelated constraint/NULL gaps
--    found and fixed during this test, none of them assumed: refill_dispatching_edit_log's
--    edited_by_role is NOT NULL (COALESCE to 'system'); edit_kind and the paired
--    state-coherence check are both closed enumerations needing a new branch for
--    'cancel_m2m_transfer'; add_dispatch_row's own edit_log insert passes p_edit_role with no
--    COALESCE (worked around locally in this function's own call, add_dispatch_row itself not
--    touched, out of scope). Green after all three fixes.
-- 3. Guard test: a transfer with driver_confirmed_at set on one leg was correctly refused with
--    an exception, dry_run=true. Green.
--
-- Loop 2026-09-25 A1 (G2): M2M lock. push_plan_to_dispatch currently sets packed=true AND
-- picked_up=true on the source (Remove) leg of an M2M transfer at push time, before any driver
-- has touched it. Confirmed live (v19_prd12x_j4_source_warehouse_id_fix): the source leg's INSERT
-- sets (packed, picked_up, dispatched, returned, item_added) = (true, true, true, false, false).
-- The destination (Add New) leg already inserts with picked_up=false -- only the source leg had
-- this problem. skip_dispatch_line (confirmed live) refuses any row with picked_up=true
-- ("already picked up - too late to skip"), so an M2M transfer could never be skipped or
-- cancelled between push and the driver actually starting it. Window rule: this migration touches
-- push_plan_to_dispatch, applied inside 22:00-06:00 Dubai (confirmed via `select now() at time
-- zone 'Asia/Dubai'` before this apply, see STATE.md).
--
-- Fix 1: the source leg's picked_up literal changes from true to false. Nothing else in that
-- INSERT changes shape; the rest of the function is untouched, forward-only.
--
-- Fix 2: new RPC cancel_m2m_transfer(p_transfer_id, p_reason, p_convert_source_to_return default
-- true, p_dry_run default true). Allowed only if neither leg has driver_confirmed_at,
-- driver_outcome, returned=true, or filled_quantity>0 -- the same "driver has not started this"
-- bar A1 exists to create. Zeroes quantity and skips both legs using the exact same column set
-- skip_dispatch_line already uses (skipped, skipped_at, skipped_by, skip_reason, include=false,
-- edit_count+1, last_edited_by/role/at), plus quantity=0 (skip_dispatch_line does not touch
-- quantity, but a cancelled transfer leg should not remain a non-zero, non-actionable rump).
-- Logs one refill_dispatching_edit_log row per leg. When p_convert_source_to_return is true,
-- inserts a plain Remove to the source machine's own primary_warehouse_id via
-- add_dispatch_row(..., 'Remove', ..., 'wh', v_primary_wh, ...) -- the exact insert path
-- add_dispatch_row already uses for a plain warehouse-bound Remove, not reimplemented here. Using
-- the source machine's own primary_warehouse_id rather than a hardcoded WH_CENTRAL id means this
-- resolves correctly for a VOX machine too (its primary_warehouse_id is already its venue
-- warehouse); for a non-VOX machine this resolves to WH_CENTRAL (4bebef68-9e36-4a5c-9c2c-142f8dbdae85)
-- as named in the task, without hardcoding it.
--
-- Also adds 'cancel_m2m_transfer' to enforce_canonical_dispatch_write's allowlist -- otherwise
-- this new, legitimate canonical writer's own UPDATEs on refill_dispatching would log as bypass
-- violations in bypass_violation_log every time it runs (that trigger does not block, only logs
-- and warns, but a legitimate new RPC should not permanently show up as a bypass).
--
-- Also widens refill_dispatching_edit_log_edit_kind_check. Found while smoke-testing: the live
-- constraint only allows edit_kind IN ('qty','shelf','product','source','add','remove',
-- 'decline_swap') -- none of those cleanly describes cancelling a whole M2M transfer, and
-- reusing 'remove' would blur this new, distinguishable audit category into an existing one.
-- Forward-only add of 'cancel_m2m_transfer' as an eighth allowed value, same pattern as the
-- movement_kind CHECK widen used elsewhere this session.
...
ALTER TABLE public.refill_dispatching_edit_log ADD CONSTRAINT refill_dispatching_edit_log_edit_kind_check
  CHECK (edit_kind = ANY (ARRAY['qty','shelf','product','source','add','remove','decline_swap','cancel_m2m_transfer']));

-- A second constraint, found in the same smoke test: refill_dispatching_edit_log_state_coherence
-- enumerates each edit_kind's required before/after shape explicitly (an OR of exact branches, not
-- a general rule) -- 'cancel_m2m_transfer' needs its own branch, same shape as
-- qty/shelf/product/source/decline_swap (both before_state and after_state required), since
-- cancel_m2m_transfer always logs a real before and after image.
...
ALTER TABLE public.refill_dispatching_edit_log ADD CONSTRAINT refill_dispatching_edit_log_state_coherence
  CHECK (
    ...
    OR ((edit_kind = ANY (ARRAY['qty','shelf','product','source','decline_swap','cancel_m2m_transfer']))
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(line.source_origin::text,'warehouse') = 'internal_transfer' THEN
      ...
      BEGIN
        ...
        -- ONLY. This is the sole functional change in this migration to this INSERT.
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v20_loopv2_a1_m2m_not_picked_up_at_push'
  );
END $function$;
...
BEGIN
  ...
  -- Same column set skip_dispatch_line already uses, plus quantity=0 -- a cancelled transfer
  -- leg must not remain a non-zero, non-actionable rump on the dispatch board.
  ...
  IF p_convert_source_to_return THEN
    ...
    -- add_dispatch_row's own edit_log insert passes p_edit_role straight through with no
    -- COALESCE, and edited_by_role is NOT NULL -- found while smoke-testing this call with no
    -- authenticated session (v_role NULL). Passing COALESCE(v_role,'system') here, not fixing
    -- add_dispatch_row itself (out of A1's surgical scope; other callers may already rely on its
    -- current behavior with a real session role, where this never fires).
...
-- Add the new canonical writer to the allowlist so its own UPDATEs on refill_dispatching are not
-- logged as bypass violations every time it runs.
