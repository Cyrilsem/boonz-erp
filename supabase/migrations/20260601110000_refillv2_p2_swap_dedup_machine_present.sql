-- Refill System v2 / Phase 2 (#2 dedup-guard) — engine_swap_pod machine-present dedup.
--
-- ROOT CAUSE (investigated 2026-06-01): pass-2 substitute selection only excluded a substitute
-- if it had an Active row in v_pod_inventory_latest (the ERP's belief, via product_mapping). It
-- never checked slot_lifecycle placement or physical WEIMI stock. So a product physically on a
-- shelf but with a mapping/inventory gap looked "absent" and got swapped onto a second shelf of
-- the same machine (e.g. OMDBB-1020 Barebells A12 -> duplicated; dedup_demoted_to_m2w=false).
--
-- NOTE on the goal's #1: the v_live_shelf_stock aisle off-by-one is real but lives only in the
-- UNUSED aisle_code field (aisle_code 0-A11 == slot_name A12). Every engine caller joins on
-- slot_name, not aisle_code, so the off-by-one never bites and no view rewrite is needed. CS
-- confirmed: skip the view rewrite, fix the dedup. (memory: B2 drift already 727/727, 0 drift.)
--
-- FIX (verbatim reproduction of engine_swap_pod with 3 intended changes, diff-gated before apply):
--   (A) new temp table _machine_present_pods = slot_lifecycle is_current UNION v_live_shelf_stock
--       physical (current_stock>0), per (machine_id, pod_product_id).
--   (B) sub_candidates LEFT JOINs _machine_present_pods on the SUBSTITUTE (sub.pod_product_id) and
--       requires mpp.pod_product_id IS NULL. A substitute already present anywhere on the machine
--       is dropped from the pool, so the dead shelf takes a non-duplicate substitute or falls to
--       M2W (pod_in NULL). Never duplicated.
--   (C) engine_version 'v9_1_product_match_planned_swap' -> 'v9_2_machine_present_dedup' (3x).
-- No schema change, no role/gate change, no new raw write path.
-- APPLIED 2026-06-01 (CS sign-off; Cody-approved Articles 1,4,8,12,14; diff-gated; verified pg_proc).
...
BEGIN
  ...
  SELECT p_plan_date, tr.machine_id, tr.shelf_id_final, tr.pod_out,
    ...
    jsonb_build_object('pass','1','source','strategic_machine_tag','tag_id', tr.tag_id,
                       ...
                       'engine_version','v9_2_machine_present_dedup')
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_cooldown', v_skip_cooldown, 'engine_version','v9_2_machine_present_dedup',
