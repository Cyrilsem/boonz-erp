-- Refill reliability (PRD docs/prds/PRD_refill_reliability_2026-06-03.md) / WS6.2 - suppress 0-stock-anywhere variant rows.
--
-- STATUS: DRAFT - NOT APPLIED. Staged for CS apply. Cody-reviewed below. Builds on stitch v16 (WS1b
-- variant resolution, 20260604120000); this is a full CREATE OR REPLACE and SUBSUMES v15+v16 - apply this
-- last in the stitch chain (or alone). Requires the WS1b variant resolution it contains.
--
-- PROBLEM (postmortem RC4 / PRD WS6): the engine recommends every mapped variant of a pod regardless of
-- stock, so a multi-variant pod (Vitamin Well: Care / Antioxidant / Zero Peach with 0 inventory while
-- Upgrade has stock) still produces packing rows the driver can only skip. engine_add_pod's PRD-017 §1
-- suppression is at the POD level (serving WH); it does not drop an individual 0-stock VARIANT inside a
-- pod that is otherwise stocked.
--
-- FIX (this migration): in pull_lines, drop a resolved REFILL/ADD variant whose wh_avail = 0. Note stitch's
-- wh_avail is SUM(warehouse_stock) over ALL warehouses (Active, non-quarantined) = "stock anywhere", so this
-- is the literally-zero-anywhere suppression the PRD asks for, NOT the §1 serving-WH availability gate
-- (explicit non-goal). Only warehouse-sourced rows are subject to it; vox_at_venue / internal_transfer are
-- venue/truck-stocked and pass through. A multi-variant pod keeps its stocked variants and drops only the
-- 0-anywhere ones. Bounded: the engine's §1 pod-level suppression already removed 0-serving-WH pods upstream.
--
-- Verbatim reproduction of stitch v16 (20260604120000), diff-gated to: the pull_lines WHERE (+ the 0-anywhere
-- suppression) + engine_version -> 'v17_ws6_suppress_zero_anywhere'. The WS1b remove resolution, mapped remove
-- path, deviation/alert/diagnostic logic, role gate, app.via_rpc, and write/confirm flow are byte-for-byte
-- unchanged. WS6.1 (Red Bull / Vitamin Well per-machine pod_inventory repin) is a separate CS-signed data fix.
...
BEGIN
  ...
  remove_phys_split AS (
    SELECT n.*,
           CASE WHEN total_split=0 THEN 100.0/variant_n ELSE COALESCE(n.split_pct,0) END AS norm_split
      FROM remove_phys_norm n
  ),
  remove_phys_qty AS (
    SELECT s.*,
           FLOOR(phys_stock*norm_split/100.0)::int AS base_qty,
           (phys_stock*norm_split/100.0)-FLOOR(phys_stock*norm_split/100.0)::numeric AS remainder_score
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v17_ws6_suppress_zero_anywhere',
