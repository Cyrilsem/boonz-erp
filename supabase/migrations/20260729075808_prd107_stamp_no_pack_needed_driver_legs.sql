...
-- PRD-107 Pack-stage truth, step 5/5 (Phase 3 apply order). Source:
-- docs/prds/PRD-107-EXECUTION-LOG.md Phase 3 Finding 2; docs/architecture/MIGRATIONS_REGISTRY.md:687.
-- One BEFORE INSERT OR UPDATE trigger stamps no_pack_needed on any leg that draws nothing from
-- the warehouse (Remove/M2W, and M2M legs already packed at push), covering all five
-- packed=true writers uniformly instead of patching each one. pack_dispatch_line's
-- 'packed_no_pick' branch (the live class-B leak) is additionally patched to COALESCE its own
-- outcome, via an assertion-guarded substitution against the single occurrence of that branch.

CREATE OR REPLACE FUNCTION public.tg_default_pack_outcome_driver_legs()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
  IF NEW.pack_outcome IS NULL THEN
    -- Driver-side legs draw nothing from the warehouse. Terminal at write time,
    -- whatever their packed flag - this is R2 "auto-resolve at push".
    IF upper(trim(COALESCE(NEW.action, ''))) IN ('REMOVE','MACHINE TO WAREHOUSE','M2W') THEN
      NEW.pack_outcome := 'no_pack_needed'::public.pack_outcome_enum;
    -- M2M destination legs are pre-packed at push because the stock comes from
    -- another machine, not the WH. confirm_packed_transferred later upgrades this
    -- to 'packed_transferred' once the physical move is confirmed.
    ELSIF COALESCE(NEW.packed, false) = true AND COALESCE(NEW.is_m2m, false) = true THEN
      NEW.pack_outcome := 'no_pack_needed'::public.pack_outcome_enum;
    END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_default_pack_outcome_driver_legs ON public.refill_dispatching;
CREATE TRIGGER trg_default_pack_outcome_driver_legs
  BEFORE INSERT OR UPDATE ON public.refill_dispatching
  FOR EACH ROW EXECUTE FUNCTION public.tg_default_pack_outcome_driver_legs();

-- pack_dispatch_line: surgical patch to the packed_no_pick branch (the class-B leak source).
-- Full current body recovered via pg_get_functiondef; the load-bearing delta vs. pre-PRD-107
-- is the COALESCE(pack_outcome, 'no_pack_needed') on the UPDATE below.
...
BEGIN
  ...
  FOR v_pick IN SELECT * FROM jsonb_array_elements(v_resolved) LOOP
    ...
    IF v_first_pick THEN
      ...
      v_first_pick := false;
    ELSE
      INSERT INTO refill_dispatching (machine_id, shelf_id, pod_product_id, boonz_product_id, dispatch_date, action, quantity, filled_quantity, include, packed, picked_up, dispatched, returned, item_added, expiry_date, from_wh_inventory_id, pack_outcome, source_origin) VALUES (v_dispatch.machine_id, v_dispatch.shelf_id, v_dispatch.pod_product_id, v_pick_bpid, v_dispatch.dispatch_date, v_dispatch.action, v_pick_qty, v_pick_qty, true, true, false, false, false, false, v_wh_row.expiration_date, v_wh_row.wh_inventory_id, 'packed', v_dispatch.source_origin) RETURNING dispatch_id INTO v_new_child_id;
    END IF;
