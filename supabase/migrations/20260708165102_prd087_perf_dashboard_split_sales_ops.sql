-- PRD-087 PERF: split the dashboard aggregate.
--  · get_dashboard_sales(p_include_vox) — sales-derived blocks only
--    (KPIs, tops, trading count). Fast (~100ms); refetched on VOX toggle.
--  · get_dashboard_ops() — heavy VOX-independent blocks (machine health
--    ~2-5s via canonical v_machine_priority, stock overview, refill today,
--    expiry, procurement, leads). Fetched once, server-cacheable.
-- get_dashboard_summary(boolean) dropped — FE is its only consumer.
DROP FUNCTION IF EXISTS public.get_dashboard_summary(boolean);
...
sales AS MATERIALIZED (
  ...
  WHERE sh.delivery_status IN ('Success','Successful')
    ...
    AND (p_include_vox OR upper(m.official_name) NOT LIKE 'VOX%')
...
SELECT jsonb_build_object(
  ...
  'kpis', jsonb_build_object(
    'today', (SELECT jsonb_build_object(
        ...
        'default_rate', COALESCE(round(100*(1 - sum(paid_amount)/NULLIF(sum(total_amount),0)),2),0))
  ...
  'top_machines', jsonb_build_object(
  ...
  'top_products', jsonb_build_object(
    'today', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
...
'PRD-087 perf split: fast sales-derived dashboard blocks (KPIs, tops, trading count). Read-only.';
COMMENT ON FUNCTION public.get_dashboard_ops() IS
'PRD-087 perf split: heavy VOX-independent dashboard blocks (health, stock, refill today, expiry, procurement, leads). Server-cacheable. Read-only.';
