-- PRD-113 A7 — the guard belongs on the CREDIT WRITER, not only on the approve RPCs.
--
-- A3 refused an in-machine move in wh_approve_remove_receipt,
-- wh_approve_remove_receipt_multivariant and approve_stuck_remove. Those are the three
-- functions the PRD names. They are not the three ways a Remove leg gets credited.
--
-- Found by asking who else calls receive_dispatch_line — the function all three delegate
-- their "credit/archival logic" to:
--
--   receive_all_dispatches_for_machine(machine, date, use_filled)
--     loops over EVERY dispatched, unreceived, included row for a machine/date —
--     Remove legs included — and calls receive_dispatch_line directly. It is wired to
--     the "Mark All" bulk action on /refill (DailyDispatchingTab.tsx:381). Live, reachable,
--     and it walks straight past all three A3 guards.
--
--   src/app/(field)/field/trips/[machineId]/page.tsx also calls receive_dispatch_line
--   directly per line.
--
-- Guarding N call sites is the disease this PRD's canonical predicate exists to cure. The
-- guard goes on the single writer that actually moves the stock, so every present and
-- future caller inherits it. The three approve RPCs keep their own guard and still fire
-- first, with a message aimed at the warehouse manager rather than at a developer.
--
-- The bulk loop SKIPS such legs rather than aborting: a RAISE inside it would take down the
-- whole "Mark All" for every other line on the machine. Skipped legs are named in the return
-- value, so the operator is told rather than left to infer (LAW 5).
--
-- Article 12: forward-only. ADDITIVE ONLY: two CREATE OR REPLACE, same signatures, no drops.
-- The receive_dispatch_line body below is the live definition with ONE guard inserted
-- immediately after its existing already-received check; nothing else is touched.
...
BEGIN
  ...
  -- ── PRD-113 A7 ────────────────────────────────────────────────────────────
  -- This is the canonical credit writer, and therefore the only place the
  -- in-machine-move guard cannot be routed around. A3 guarded the three approve
  -- RPCs; it did not guard THIS, and there are live paths that reach here without
  -- passing through any of them:
  --   • receive_all_dispatches_for_machine loops over every dispatched, unreceived,
  --     included row for a machine/date — Remove legs included — and is wired to the
  --     "Mark All" action on /refill (DailyDispatchingTab).
  --   • the field trips page calls receive_dispatch_line directly.
  -- The three approve RPCs still refuse first, with a friendlier message; this is the
  -- floor under them.
  IF v_dispatch.action = 'Remove'
     AND COALESCE(public.is_internal_move_dispatch(p_dispatch_id), false) THEN
    RAISE EXCEPTION 'receive_dispatch_line: dispatch % is an INTERNAL MOVE - nothing to credit. These units moved to another shelf of the SAME machine (%) and never reached the warehouse; crediting them would create phantom warehouse stock. If this really is a warehouse return, call clear_internal_move_flag(%, <reason>) first.',
...
-- The bulk receiver skips in-machine moves instead of dying on the first one.
...
DECLARE
  ...
  v_one jsonb;
  v_qty numeric;
  v_skipped_moves jsonb := '[]'::jsonb;   -- PRD-113 A7
BEGIN
  PERFORM set_config('app.via_rpc',  'true', true);
  PERFORM set_config('app.rpc_name', 'receive_all_dispatches_for_machine', true);
  ...
  FOR v_row IN
    ...
    ORDER BY created_at
  LOOP
    -- PRD-113 A7: an in-machine move has no warehouse leg. receive_dispatch_line now
    -- refuses it outright, so skip it here rather than let one move leg abort the whole
    -- bulk receive for every other line on the machine.
    IF v_row.action = 'Remove'
       AND COALESCE(public.is_internal_move_dispatch(v_row.dispatch_id), false) THEN
      v_skipped_moves := v_skipped_moves || jsonb_build_object(
        'dispatch_id', v_row.dispatch_id,
        'shelf_id',    v_row.shelf_id,
        'quantity',    v_row.quantity,
        'reason',      'internal move - units go to another shelf of this machine, nothing to credit');
      CONTINUE;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'results', v_results,
    -- LAW 5: never silently do nothing. If legs were skipped, say which and why.
    'skipped_internal_moves', jsonb_array_length(v_skipped_moves),
    'internal_move_legs', v_skipped_moves
  );
END;
$function$;
