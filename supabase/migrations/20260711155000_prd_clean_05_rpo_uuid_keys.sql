-- PRD-CLEAN-05 M1: refill_plan_output gets real keys (additive, non-breaking).
-- 1) four nullable uuid columns + index
-- 2) write_refill_plan g8 populates them from the validated names
-- 3) push_plan_to_dispatch v9 prefers them, falls back to name matching (historical rows)
