-- Loop 2026-09-25 (CS HOLD feedback F1-F5): pick_machines_v12 fixes.
--
-- picker_config.picker_version stays 'shadow' throughout this migration; nothing here changes
-- which picker is authoritative.
--
-- F1 CAP: pick_machines_v12 returned more rows than p_cap (12 rows at p_cap=8 for 2026-09-27,
-- confirmed live before this fix) because the cap only ever governed non-P1 rows, with no total
-- cap at all. Fixed: the cap now applies to the whole output. P1 is still never trimmed (PRD-133
-- "always picked"); if P1 alone reaches or exceeds the cap, ONLY P1 rows are returned (no P2/P3),
-- and every P1 row's reasons gets a 'p1_overflow: cap exceeded, all P1 rows returned, no P2 or P3
-- filled' entry appended, so an overflow is visible in the output, never silently absorbed.
--
-- F2 CLUSTER: root cause confirmed live and fixed by data (see the prior migration in this loop,
-- 20260925110000_loopv2_f2_seed_building_id.sql): pick_machines_v12 derived a building_code from
-- the official_name naming convention, and many unrelated machines share the placeholder "0000"
-- second segment, colliding into the same fallback code. This migration switches the function to
-- read machines.building_id (via v_machine_priority.building_id, a direct passthrough, confirmed
-- from the view's own definition) instead of deriving anything, so clustering only ever happens
-- on the real, now explicitly seeded building groups. A cluster add (a machine whose own tier
-- would otherwise not include it) is not possible any more: cluster_role='cluster' is now only
-- ever layered on TOP of a tier a machine already has on its own merits (P1, P2, its own P3
-- ride-along need, or donor value) when another machine in the same real building also qualifies;
-- it never manufactures a tier and never changes the tier a machine already has.
--
-- F3 DONOR: tightened to the four conditions CS gave. (a) v_wh_pickable = 0 specifically at the
-- donor machine's own primary_warehouse_id (not summed across every warehouse). (b) donor lane
-- velocity < 0.4/day AND WEIMI current_stock >= 4 (was velocity only). (c) a receiver lane of the
-- same pod product exists in the TOP QUARTILE OF FLEET VELOCITY (percent_rank over every eligible
-- lane, not just lanes of the same product, which could trivially put the single fastest of a
-- 2-machine product group in its own "top quartile") with fill < 50%. (d) the receiver machine's
-- own tier is P1 or P2. donor_value_aed now counts only units the receiver can actually take
-- (receiver max_stock - receiver current_stock), not the donor's full on-hand quantity, and is
-- deduplicated one best receiver per (donor, pod_product) as already fixed in B2.
--
-- F4 EXPIRY PHANTOM LOTS: an expiring/expired pod_inventory row now only triggers P1 when WEIMI
-- currently shows that SAME resolved pod product on that machine with stock > 0 (checked via
-- v_shelf_slot_identity, which already resolves the physical lane's product through
-- weimi_product_alias and its own fuzzy matching). Root cause confirmed live: IRIS-1070-0000-O1
-- has a stale Active pod_inventory batch of Activia Mix & Go, expiring 2026-09-25, on a shelf
-- (A01) that WEIMI currently reports as Keen Health Dipped Crackers -- the Activia batch is a
-- phantom, no longer physically on that shelf. Phantom lots excluded by this fix are listed in
-- the loop report; none are archived or otherwise modified here (that is a separate cleanup, out
-- of this fix's scope).
--
-- F5 FILL<50% P1: this trigger now additionally requires at least one lane on the machine with
-- velocity >= 0.3/day (the machine is not simply, deliberately run at low stock fleet-wide) and
-- daily_revenue_aed at or above the fleet's own 25th percentile among eligible machines. Root
-- cause confirmed live: GRIT-1022-0100-W0 triggered P1 on fill<50% alone at roughly 6 AED/day of
-- revenue, a machine CS deliberately half-fills. When these two gates are not met, the fill<50%
-- signal now contributes to P2 instead of being dropped entirely.
...
BEGIN
  ...
  -- cluster_role='cluster' only when the machine already has SOME tier of its own (P1/P2/P3, or
  -- the donor-forced P3 above) AND shares a real, non-null building_id with another machine that
  -- also already has a tier. Never fires on machines.building_id NULL (F2's actual bug).
  with_cluster AS (
    SELECT f.*,
      CASE
        WHEN f.is_donor THEN 'donor'
        WHEN f.tier IS NOT NULL AND f.building_id IS NOT NULL
             AND EXISTS (
  ...
  scored AS (
    SELECT w.*,
      ROUND(COALESCE(w.daily_revenue_aed,0)
            * GREATEST(COALESCE(w.rhythm_days,7) - COALESCE(w.runway_days,0), 0)
            / GREATEST(COALESCE(w.rhythm_days,7),1), 2) AS sales_saved_aed,
      ROUND(COALESCE((
