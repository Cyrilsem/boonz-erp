-- PRD-110 leg 121 - S-227 FIX (forward-only, Article 12): the failure branch of every
-- stress suite is unreachable-until-it-is-not, and when it fires it raises 22P02.
--
-- `v_fails := v_fails || 'A02 ...'` is AMBIGUOUS: with v_fails typed text[], the untyped
-- literal is resolved as an ARRAY literal, not an element, so Postgres reports
--   22P02 malformed array literal: "A02 fewer than 10000 deltas processed"
--   DETAIL: Array value must start with "{" or dimension information.
-- and the whole suite ABORTS instead of reporting its failures. Measured live this leg on
-- golden.stress_s2_v1's first validation run (p_rounds => 2, where A02 legitimately fails).
--
-- ⚠️ THE SAME SHAPE IS IN golden.stress_s6_v1 (leg 117) AND golden.stress_s4_v1 (leg 120).
-- Neither has ever executed it: S6 finished 16/0 and S4 33/0, so the ELSE branch was dead
-- code in both. A red S4 or S6 would not report - it would crash with an array-literal error
-- pointing at a line that looks correct. That is recorded as S-227 in the PARKING-LOT and is
-- parked as its own unit rather than folded into this one: both suites are banked green and
-- carry md5s on the roll, so re-shipping them is a deliberate act, not a drive-by.
--
-- This migration re-ships golden.stress_s2_v1 with an explicit ::text cast on all 25 failure
-- appends. Nothing else in the body changes; 20260804050000 is left exactly as applied.
...
BEGIN
  ...
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A01 rounds_run <> p_rounds+1'::text; END IF;

  v_ok := (v_metric->>'deltas_processed')::bigint >= 10000;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A02 fewer than 10000 deltas processed'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE COALESCE((r.value->>'neg_est')::int, 1) <> 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A03 negative est_qty seen in some round'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE COALESCE((r.value->>'conf_oob')::int, 1) <> 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A04 confidence left [0,1] in some round'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE (r.value->>'r')::int > 0
                         AND COALESCE((r.value->'res'->>'sensor_above_capacity')::int, 0) = 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A05 a storm round raised no count>capacity'::text; END IF;

  v_ok := COALESCE((v_metric->'anom_by_kind'->>'count_above_capacity')::int, 0) >= 30 * p_rounds;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A06 too few count_above_capacity anomalies'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE (r.value->>'r')::int > 0
                         AND COALESCE((r.value->'res'->>'rises_flagged_anomaly')::int, 0) = 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A07 a storm round flagged no unexplained rise'::text; END IF;

  v_ok := COALESCE((v_metric->'anom_by_kind'->>'count_rise_unexplained')::int, 0) > 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A08 no count_rise_unexplained anomaly'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE (r.value->>'r')::int > 0
                         AND COALESCE((r.value->'res'->>'rises_auto_venue_fill')::int, 0) = 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A09 a storm round auto-attributed no venue fill'::text; END IF;

  v_ok := COALESCE((v_metric->'ev_by_kind'->>'venue_fill')::int, 0) > 0
      AND (v_metric->>'venue_fill_off_role')::int = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A10 venue_fill missing, or landed off the VENUE role'::text; END IF;

  v_ok := (v_metric->>'venue_role_rise_anoms')::int = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A11 a VENUE-role rise was flagged instead of attributed'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE (r.value->>'r')::int > 0
                         AND COALESCE((r.value->'res'->>'count_drops_allocated')::int, 0) = 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A12 a storm round allocated no ordinary decrement'::text; END IF;

  v_ok := COALESCE((v_metric->'ev_by_kind'->>'derived_decrement')::int, 0) > 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A13 no derived_decrement event written'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE (r.value->>'r')::int > 0
                         AND COALESCE((r.value->'res'->>'unallocatable_residuals')::int, 0) = 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A14 a storm round produced no unallocatable residual'::text; END IF;

  v_ok := COALESCE((v_metric->'anom_by_kind'->>'negative_delta_unallocatable')::int, 0) > 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A15 no negative_delta_unallocatable anomaly'::text; END IF;

  -- LAW 7
  v_ok := (v_metric->>'dd_on_expired')::int = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A16 EXPIRY IRON RULE broken: a derived_decrement consumed an expired bucket'::text; END IF;

  v_ok := (v_metric->>'law7_witnesses')::int > 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A17 A16 is vacuous: no shelf both held expired stock and took a decrement'::text; END IF;
  ...
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A18 a planted expired bucket moved during the soak'::text; END IF;

  v_ok := (v_metric->>'negraw_sellable')::numeric = 0
      AND (v_metric->>'negraw_negative')::int = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A19 a raw-negative WEIMI count did not floor to an empty sellable shelf'::text; END IF;

  v_ok := (v_metric->>'conf_regressions')::int = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A20 confidence rose across an unexplained-delta round'::text; END IF;

  v_ok := NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_metric->'rounds') r
                       WHERE COALESCE((r.value->'res'->>'cold_start_not_conserved')::int, 1) <> 0);
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A21 a cold-start seed failed to conserve the WEIMI count'::text; END IF;

  v_ok := v_residue = 0;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A22 probe rows survived the rollback'::text; END IF;
  ...
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1;
    v_fails:=v_fails||'A23 a probe-written table moved with zero probe residue - a concurrent writer (cron 44) committed inside the window; re-run outside :38-:45'::text; END IF;
  ...
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A24 a protected table moved - the estimator must touch none of them'::text; END IF;

  v_ok := EXTRACT(EPOCH FROM (clock_timestamp() - v_started)) * 1000 < 600000;
  IF v_ok THEN v_pass:=v_pass+1; ELSE v_fail:=v_fail+1; v_fails:=v_fails||'A25 soak exceeded the 600 000 ms budget'::text; END IF;
