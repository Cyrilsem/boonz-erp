-- PRD-129R step 03b: applied moments after 03, as a separate migration, once dry-running
-- G-REMAINDER showed 799 hits instead of the expected ~208 (see 03's own header for the two
-- scope bugs found and fixed: source_event_id reused for unrelated pack-debit/manual-correction
-- events, and Remove/M2M/returned dispatches compared against a "remainder" concept that does
-- not apply to them).
--
-- This file's CREATE OR REPLACE is identical to the corrected function already shown in
-- 20260921213200_prd129r_03_guards_and_cron.sql -- that file was edited in place before this one
-- was written, so both now carry the same, correct body. This file exists only so the repo keeps
-- a 1:1 record against the two separate apply_migration calls actually made
-- (prd129r_03_guards_and_cron, then prd129r_03b_fix_g_remainder_scope), rather than silently
-- merging a real in-session bug-and-fix into one migration as if the bug never shipped.
