-- PRD-074 P1: priority single source of truth - get_machine_health v3 + get_stale_visit_signals v2.
--
-- D1 fixed: days_since_visit now passes through v_machine_health_signals (executed dispatch
-- evidence: picked_up OR returned OR dispatched+packed). The old MAX(approved plan_date)
-- notion is RENAMED to last_plan_date / last_plan_days (informational, never a visit) -
-- this is why FE cards said "3d ago" while the picker said 22d in the VOX outage.
-- D2 fixed: get_stale_visit_signals is a thin SELECT over the signals view; threshold =
-- pick_urgency_params.stale_override_days (private >10 literal removed). Output column
-- names kept for SignalsTab compatibility (semantic now: last VISIT, not last plan).
--
-- Backward compatibility (grep-proofed, PRD-074-EXECUTION-LOG.md): get_machine_health is
-- consumed ONLY by refill/page.tsx; all 32 existing output keys kept, 4 APPENDED:
-- last_plan_date, last_plan_days, urgency_breakdown, reasons_arr.
-- urgency_breakdown = [{label, pts}] with pts summing EXACTLY to v_machine_priority.urgency
-- (empty + low-fill terms exact from the view's s_empty/s_lowfill x params; the remaining
-- runout/capacity/expiry/stale terms are not exposed as view columns - PRD-073 gate forbids
-- modifying the view here - so they ship as one lumped 'core' chip = urgency residual.
-- HIGHLIGHTED in the log; a future PRD can add the 4 s_* columns to the view and split it).
-- WEIMI-direct remainder (no canonical or deliberately kept, see log): total_stock,
-- max_capacity, total_slots, slots_at_zero, slots_below_25pct, fill_pct, has_sensor_errors,
-- dead_stock_count, local_hero_count, daily_revenue-based label/strategy.

-- RETURNS TABLE gains 4 appended columns -> DROP + recreate in one transaction
-- (0-arg function, no overload; grants re-issued below).
  ...
  SELECT
    ...
    CASE WHEN mp.machine_id IS NULL THEN NULL ELSE
      (SELECT COALESCE(jsonb_agg(jsonb_build_object('label', t.l, 'pts', t.pts) ORDER BY t.pts DESC), '[]'::jsonb)
       FROM (VALUES
         ('core urgency (runout+capacity+expiry+stale)',
          round(COALESCE(mp.urgency,0)
                - round(pup.w_empty * COALESCE(mp.s_empty,0), 2)
                - round(pup.w_lowfill * COALESCE(mp.s_lowfill,0), 2), 2)),
         ('empty shelves', round(pup.w_empty * COALESCE(mp.s_empty,0), 2)),
         ('low-fill sellers', round(pup.w_lowfill * COALESCE(mp.s_lowfill,0), 2))
       ) t(l, pts)
       WHERE t.pts <> 0)  -- keep a nonzero (even -0.01 rounding) core so pts always sum to urgency
    END,
    mp.reasons_arr
  FROM with_expiry we
...
CREATE OR REPLACE FUNCTION public.get_stale_visit_signals()
 RETURNS TABLE(machine_name text, last_refill_date date, days_since_last_refill integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
AS $function$
  SELECT
    s.official_name,
    (CURRENT_DATE - s.days_since_visit)::date AS last_refill_date,
    s.days_since_visit AS days_since_last_refill
  FROM v_machine_health_signals s
  JOIN machines m ON m.machine_id = s.machine_id
  CROSS JOIN pick_urgency_params p
  WHERE m.status = 'Active'
    AND COALESCE(m.include_in_refill, true) = true
    AND s.days_since_visit > p.stale_override_days
  ORDER BY s.days_since_visit DESC;
$function$;

-- re-issue grants lost by the DROP (match prior surface: authenticated reads health)
GRANT EXECUTE ON FUNCTION public.get_machine_health() TO authenticated, service_role;
