-- PRD-075 WS-B: logged manual/field refills count as canonical visit evidence (the VOX fix).
-- CS ruling 2026-07-04. Event source: pod_inventory_audit_log rows written by
-- log_manual_refill (reference_id 'manual-refill-<machine>-<date>', source='refill').
-- last_visit = GREATEST(executed dispatch evidence (UNCHANGED definition), latest manual
-- refill event date). Postgres GREATEST ignores NULLs, so machines with only one kind of
-- evidence still resolve. get_machine_health v3 inherits by pass-through (verified: it
-- reads hs.days_since_visit, no cached copy). Body otherwise VERBATIM the live view.
CREATE OR REPLACE VIEW public.v_machine_health_signals AS
 WITH base AS (
         ...
         -- PRD-075 WS-B: logged manual/field refills (log_manual_refill writer)
         SELECT b_1.machine_id,
            max(pal.created_at::date) AS last_manual_refill_date
           FROM base b_1
             LEFT JOIN pod_inventory_audit_log pal ON pal.machine_id = b_1.machine_id AND pal.reference_id LIKE 'manual-refill-%'
          GROUP BY b_1.machine_id
        ), sales_recent AS (
         SELECT b_1.machine_id,
