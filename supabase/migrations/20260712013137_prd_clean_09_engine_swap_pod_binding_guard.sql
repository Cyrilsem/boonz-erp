...
-- original addition of a v_slot_binding_drift-scoped guard to engine_swap_pod(). CHANGELOG
-- (~line 2913) documents the same-evening patch p0_fix12_engine_swap_weimi_identity_scoped_drift
-- (fleet-wide PRD-CLEAN-09 halt -> scoped alert + per-shelf skip; WEIMI-first identity at all
-- three incumbent-resolution sites), already committed to git. The pre-fix 2026-07-12 01:31
-- body is not recoverable; the body below is the CURRENT live cumulative state (confirmed via
-- pg_get_functiondef: binding_drift count scoped to machines_to_visit picked/cs_added,
-- 'engine_swap_pod_binding_drift' critical monitoring_alert, and a _binding_drift temp table
-- used to per-shelf-skip all three swap passes). Reapplying this file is a no-op against
-- current prod.
