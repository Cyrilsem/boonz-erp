-- WS-A PRIMARY - get_procurement_demand attributes demand by mix_weight (must match stitch). STATUS: DRAFT -
-- NOT APPLIED. Cody-reviewed. Verbatim reproduction, diff-gated to exactly the attributed-demand line:
--   ROUND(ps.sales_14d * pm.split_pct / 100, 0)  ->  ROUND(ps.sales_14d * pm.mix_weight, 0)
-- mix_weight is the 0-1 fraction (sum 1.0/pod after the WS-A backfill), so no /100. After the backfill this is
-- numerically identical to today; going forward a confirmed apply_mix_weight_recommendation shifts both
-- procurement demand and stitch the same way. The split_pct output column is left as-is (display / FE compat);
-- only the demand attribution switches. Uses is_global_default mappings, unchanged.

CREATE OR REPLACE FUNCTION public.get_procurement_demand(p_lookback_days integer DEFAULT 14, p_buffer_pct numeric DEFAULT 0.10, p_source text DEFAULT 'boonz'::text)
 RETURNS TABLE(boonz_product_id uuid, boonz_product_name text, pod_product_name text, product_category text, split_pct numeric, sales_14d numeric, variant_demand_14d numeric, wh_stock numeric, gap numeric, suggested_qty numeric, units_per_box smallint, source_of_supply text)
 LANGUAGE sql
 SECURITY DEFINER
...
WITH
  ...
  contrib AS (
    SELECT
      ...
      ROUND(ps.sales_14d * pm.mix_weight, 0)       AS attributed,
