-- PRD-028 dispatch-state-integrity step 1: phaseF_dispatch_state_guards
-- APPLIED to prod 2026-06-12 via MCP as phaseF_dispatch_state_guards (version 20260612135850).
-- Forward-only; both canonical writers in one migration.
-- Rollback: docs/prds/prd-028-dispatch/ROLLBACK-pre-guards-functiondefs.sql
--   pack_dispatch_line   pre-guard prosrc md5 76be93344adda4b6c5df4cfd77cc4d73
--   return_dispatch_line pre-guard prosrc md5 8520614a6105aec6984534b701ade4ce
-- Guards (PRD section 2a/2b, Incidents A+B 2026-06-12):
--   pack: unconditional three-flag refusal (skipped / cancelled / include=false), error names flag + skip_reason.
--   return: three-flag refusal CONDITIONAL on packed=false AND picked_up=false (nothing physical),
--           because flagged-but-packed returns are the recovery path (Incident A) and the EOD sweep
--           contract (eod_auto_release_unpicked pass 1 returns packed=true picked_up=false lines).
--           PLUS system-actor refusal: no p_returned_by AND nothing physical.
...
BEGIN
  ...
  -- PRD-028 dispatch-state guards (Incident B, 2026-06-12): a line that is skipped,
  -- cancelled, or excluded AND was never packed nor picked up has nothing physical to
  -- return. Flagged lines that WERE packed stay returnable: that is the recovery path
  -- (Incident A) and the EOD sweep contract (packed=true, picked_up=false).
  ...
  END IF;
  -- System-actor guard: no human actor AND nothing physical = refuse. This is the
  -- Dispatch Complete auto-return burst (return_dispatch_line by: system x5).
  -- eod_auto_release_unpicked only targets packed=true lines, so it never hits this.
