-- PRD-122 (lane-grain-priority) T4 / R4 (remainder) / D8.
-- Swaps pick_machines_for_refill's 6 venue_group eligibility predicates (F6) to
-- svc_track: the v_vox_all_equip bool_and check, vox_sel, nonvox_sel, the VOX-off-day
-- gate inside ranked_primary's WHERE, sibling_ranked, and the 'vox_emergency_offday'
-- reason-tag CASE. vox_centroid's geography clustering stays on venue_group (F6,
-- D8 -- routing is a different concern from refill responsibility). The off-day gate
-- additionally swaps runway_days for hero_runway_days (R4's last sentence), which now
-- exists as of T3. days_until_next_vox_day is untouched.
--
-- Structurally this is a no-op today until T5 lands: svc_track is still computed as
-- CASE WHEN venue_group='VOX' THEN 'vox' ELSE 'main' END, so it currently mirrors
-- venue_group exactly. T5 is what actually changes svc_track's source to
-- machines.service_model, which is when the real behavior change (8 Boonz-filled
-- machines reclassifying from 'vox' to 'main') takes effect.
--
-- Cody: Approve (see the T1-T5 combined review). Article 1/4 (canonical writer of
-- machines_to_visit -- role gate, app.via_rpc/rpc_name, and the INSERT/ON CONFLICT
-- shape are completely untouched; only which column six predicates read changes).
-- Article 12 (forward-only, no signature change).
--
-- Verified in a rolled-back transaction (impersonating an operator_admin caller):
-- called for CURRENT_DATE (Sunday, a non-VOX day). ACTIVATEMCC-1037-0000-L0 --
-- currently svc_track='vox' since T5 hasn't landed -- now appears in the picker's
-- output with the 'vox_emergency_offday' reason tag, at visit_order 8, satisfying
-- A4/A5 even before T5 formally recategorizes it. VOXMM-1013-0101-B0 (a real VOX
-- machine) also appears via the same emergency gate given its severe urgency,
-- which is correct.
  ...
  ELSE
    ...
    ranked_primary AS (
      ...
      FROM scored sc WHERE sc.p_tier = 'P1_RESTOCK'
        AND ( v_is_vox_day
              OR sc.svc_track IS DISTINCT FROM 'vox'
              OR ( sc.svc_track = 'vox' AND COALESCE(sc.hero_runway_days, 999) < public.days_until_next_vox_day(p_plan_date) ) )
    ),
    primary_picks AS (
    ...
    ordered AS (
      SELECT fp.*,
        CASE WHEN fp.sibling THEN ARRAY_APPEND(fp.reasons_arr, 'sibling') WHEN fp.svc_track = 'vox' THEN ARRAY_APPEND(fp.reasons_arr, 'vox_emergency_offday') ELSE fp.reasons_arr END AS final_reasons,
        ROW_NUMBER() OVER (
