-- PRD-088: unify the plan clock with the visit clock (supersedes PRD-087's
-- dispatched=true narrowing).
--
-- CS principle: a manual refill IS an executed plan - same servicing event.
-- get_machine_health() no longer derives last_plan_date/last_plan_days from
-- refill_plan_output at all: the plan_data CTE and its pld join are DROPPED,
-- and both columns now mirror the canonical service clock (hs.days_since_visit
-- from v_machine_health_signals, already joined):
--   last_plan_date := CURRENT_DATE - days_since_visit (NULL when unknown)
--   last_plan_days := COALESCE(days_since_visit, -1)
-- => last_plan_days == days_since_visit for every machine, dispatched plans
-- and on-spot manual refills counting identically.
--
-- Everything else byte-identical to the live body (base md5
-- c25188bfb91f12244463cc16dd2a8d33 = PRD-087 v2). Read-only canonical reader:
-- no write path, no data change, days_since_visit itself untouched.
-- Cody-reviewed (Articles 12/14/16 - see PRD-088 execution log).
-- Forward-only. Rollback: restore the plan_data CTE + pld columns (PRD-087
-- migration 20260708163722 holds that body verbatim).
