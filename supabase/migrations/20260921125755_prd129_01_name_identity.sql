-- PRD-129 step 01 (D-011): get_machine_health() returned machine_name as
-- COALESCE(weimi_device_status.device_name, machines.official_name) while machine_id came
-- from machines. The default view looked clean only because the Active filter hides the two
-- drifted rows: a row labelled ALJLT-1015-0200-O1 that is actually ALJ-1014-0200-O1_OLD, and
-- one labelled JET-1016-0000-O1 that is actually ALHQ-1016-0000-O1 -- both Inactive, both
-- masked rather than fixed.
--
-- Fix: machine_name is now machines.official_name, unconditionally. weimi_device_name is
-- added as its own column carrying the WEIMI-reported name, so drift stays visible on the
-- surface instead of silently renaming one identity to the other.
--
-- swap_data also switches from a machine_name join to machine_id -- planned_swaps already
-- carries machine_id, and joining by name was the exact same class of drift risk this step
-- is fixing, one line away from the change already being made here.
