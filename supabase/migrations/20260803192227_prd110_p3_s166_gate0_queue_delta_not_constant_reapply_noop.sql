-- PRD-110 leg 97 · S-166 RE-APPLICATION NO-OP PROOF.
--
-- This is the SAME body as 20260803192110, applied a SECOND time on purpose to
-- prove the marker guard ('tripwires_before') makes re-application a no-op.
-- It is filed here because the DB records it as an applied migration, and the
-- owed set (DB \ disk) must stay empty -- a migration that ran and has no disk
-- file is filing debt, however harmless its body.
--
-- VERIFIED AFTER THIS RAN: marker still occurs exactly once in each of bodies
-- 11/50/51, the three assertions are still deltas expecting 0, and
-- golden.assertions still totals 1792. Nothing changed.
--
-- METHOD NOTE for later legs: idempotency is better proven by re-running the
-- body through /tmp/prd110_sql.sh (plain SQL, mints no schema_migrations row)
-- than by re-applying it as a migration, which is why this file exists at all.
