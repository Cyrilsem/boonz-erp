-- PRD-110 · D-45 PROOF LEG 2 — S3 sensors 18 and 20 re-based from PINNING the defect to
-- ASSERTING the ruled property. This is the second half of D-45; the composer moved in
-- 20260805235500_prd110_d45_compose_add_additive.
--
-- ⭐ THE RED THAT JUSTIFIES THIS MIGRATION IS BANKED, NOT NARRATED. S3 ran against the fixed
-- composer with the OLD sensors still in place and failed exactly where the CS ruling said it
-- would: golden.stress_runs `2ecddab8-fbdd-4d5a-92ef-3f870ab19a34`, passed=false, 33 pass / 2 fail:
--   · seq 18 hard_edits_present_in_composed_output   actual 5, expect "0 missing of 10"
--   · seq 20 add_composes_as_absolute_D45_sensor     actual 5, expect "0 mismatched of 5"
-- ⛔ DO NOT DELETE THAT ROW. Paired with the next green run it is the whole proof that the fix
-- landed, the same way leg 132 proved D-46 with a broken-binder row and a fixed-binder row that
-- both still exist. S-243 obeyed: this migration is written AFTER the evidence it cites.
--
-- ⭐ ASSERTION 18 MOVED TOO, AND THE GOAL COMMAND ONLY NAMED 20. `v_hard_bad` asserts
-- `s.qty = e.qty` over hard edits of kind set_qty OR add; 10 of the run's hard edits qualified and
-- 5 of them were adds, so half of assertion 18 was reading the absolute expectation. Re-basing 20
-- alone would have left S3 permanently red and looked like the composer fix was wrong.
--
-- ⛔ THE BASE IS READ FROM v_base, NOT FROM base_qty_at_edit. A HARD edit deliberately outranks a
-- base that moved after the edit was recorded, so the composer applies `base_qty + qty` against
-- the CURRENT base run. Asserting against the edit-time snapshot would re-red the suite the first
-- time a hard edit rode over a moved base — the exact property assertion 18 exists to protect.
--
-- ⭐ NEW seq 36 `D45_additive_assertion_is_load_bearing` — AN ANTI-VACUITY GUARD, not scope drift.
-- An `add` onto a base of 0 composes to `qty` under BOTH the old and the new reading (that is why
-- composer loop (b) needed no change). So a future S3 run whose adds all happened to land on
-- shelves the engine never planned would pass seq 20 while exercising nothing the ruling decided.
-- `v_add_diverge` was already computed and already carried that count; it was only ever reported
-- as a diagnostic. Seq 36 promotes it to an assertion: at least one applied add must sit on a
-- non-zero base. Consistent with the build's own S-173 family — a guard passed by inspection is
-- not a guard passed.
--
-- ⛔ S-103 OBEYED: name, expect AND description move together. Seq 18's name gains
-- `_at_D45_additive_qty` and seq 20 is renamed `add_composes_as_additive_D45_property`, so a
-- reader of a future report cannot mistake the new semantics for the old sensor. Neither assertion
-- was LOOSENED — both still demand exactly 0 bad.
--
-- ⭐ NOT SECURITY DEFINER (unchanged) and `SET search_path TO 'golden','public','pg_temp'` restated
-- verbatim from pg_get_functiondef. Harness-only object in the `golden` schema: no protected
-- entity, no live plan table, no engine. Article 12 forward-only, no signature change.

CREATE OR REPLACE FUNCTION golden.stress_s3_verify_v1(p_plan_date date DEFAULT '2030-11-03'::date, p_days_cover integer DEFAULT 7, p_edit_ids uuid[] DEFAULT NULL::uuid[], p_calls jsonb DEFAULT '[]'::jsonb, p_bank jsonb DEFAULT NULL::jsonb, p_record boolean DEFAULT true, p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- ⭐ D-45 EXECUTED: the expectation is now BASE-AWARE. `add` composes to
  --    base + qty, `set_qty` still composes to qty. The base is read from the
  --    SAME base run the composer used (v_base), NOT from base_qty_at_edit -
  --    a hard edit deliberately survives a base that moved after it was
  --    recorded, so the edit-time snapshot is the wrong yardstick here.
  SELECT count(*),
         count(*) FILTER (WHERE NOT EXISTS (
           SELECT 1 FROM public.pod_refills_shadow s
            WHERE s.run_id = v_planned AND s.shelf_id = e.shelf_id
              ...
              AND s.qty = CASE WHEN e.kind = 'add'
                               THEN COALESCE((SELECT b.qty FROM public.pod_refills_shadow b
                                               WHERE b.run_id = v_base
                                                 AND b.shelf_id = e.shelf_id
                                                 AND b.pod_product_id = e.pod_product_id), 0)
                                    + e.qty
                               ELSE e.qty END))
  ...
  -- ⭐ D-45 EXECUTED (CS ruling 2026-08-04, composer fixed leg 133). This was a
  --    SENSOR pinning the defect; it is now an ASSERTION of the ruled property.
  --    v_add_bad counts applied `add` edits whose composed line is not exactly
  --    base + e.qty, where base is that key's qty in the composer's own base run
  --    (0 when the base never planned the line - the loop-(b) case, which was
  --    already additive and is why it never moved).
  --    v_add_diverge is RETAINED and still counts the adds carrying a non-zero
  --    base: it is now the measure of how much of this assertion is LOAD-BEARING.
  --    If it ever reads 0 the suite is only exercising the trivial 0 + qty path
  --    and assertion 20 has stopped proving the ruling - which is what seq 20b
  --    below now refuses to let happen silently.
  SELECT count(*),
         count(*) FILTER (WHERE NOT EXISTS (
           SELECT 1 FROM public.pod_refills_shadow s
            WHERE s.run_id = v_planned AND s.shelf_id = e.shelf_id
              AND s.pod_product_id = e.pod_product_id
              AND s.qty = COALESCE((SELECT b.qty FROM public.pod_refills_shadow b
                                     WHERE b.run_id = v_base
                                       AND b.shelf_id = e.shelf_id
                                       AND b.pod_product_id = e.pod_product_id), 0)
                          + e.qty)),
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    FROM (VALUES
      ...
      (18, 'hard_edits_present_in_composed_output_at_D45_additive_qty',
           '0 missing of ' || v_hard_n::text,
      ...
      -- ⭐ D-45 EXECUTED. Was 'add_composes_as_absolute_D45_sensor', pinning the
      --    defect; it now asserts the ruled property: `add` composes to base + qty.
      --    S-103: name, expect AND description moved together, never loosened.
      (20, 'add_composes_as_additive_D45_property', '0 mismatched of ' || v_add_n::text,
           v_add_bad::text, v_add_n > 0 AND v_add_bad = 0, false),
      -- ⛔ THE ANTI-VACUITY GUARD ON 20. An `add` on a base of 0 composes to qty
      --    under BOTH readings, so a run whose adds all landed on unplanned
      --    shelves would pass seq 20 without ever exercising the ruling. This
      --    refuses that: at least one applied add must carry a non-zero base.
      (36, 'D45_additive_assertion_is_load_bearing', '>=1 add on a non-zero base',
           v_add_diverge::text, v_add_diverge >= 1, false),
