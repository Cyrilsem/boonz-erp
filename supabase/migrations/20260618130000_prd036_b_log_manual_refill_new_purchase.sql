-- PRD-036 Phase B: extend log_manual_refill with a per-line new_purchase branch.
-- Forward-only CREATE OR REPLACE (same signature; body change only, no _v2).
-- Based on the LIVE body fetched 2026-06-18 (pg_get_functiondef).
--
-- WHY: a field new-purchase places stock the WH never received. The existing path
-- FEFO-decrements existing WH stock (shortfall if none). The new branch RECEIVES
-- the purchase into WH first (creates a warehouse_inventory batch with the captured
-- expiry, audited) THEN consumes it into the pod, so a new purchase is fully
-- in-system: WH batch created with captured expiry + pod placement, nothing on paper.
--
-- Per line: {boonz_product_id, qty, expiration_date, shelf_code, new_purchase?}.
-- new_purchase=false (default) keeps the EXACT existing behavior (FEFO decrement of
-- existing WH stock -> pod). new_purchase=true: insert WH receipt batch, fully draw
-- it for the placement, then the same pod insert runs.
--
-- Protected entities written: warehouse_inventory (INSERT receipt + UPDATE draw),
-- pod_inventory (INSERT placement). NEVER writes warehouse_inventory.status (Art 6 -
-- new batch is created Active at INSERT, never UPDATE-d to a different status here).
-- DEFINER, role-gated, sets app.via_rpc/app.rpc_name/app.provenance_reason; audit
-- trigger on warehouse_inventory/pod_inventory fires via app.via_rpc (Art 8).
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    -- PRD-036 Phase B: a new purchase must carry the captured expiry (it creates the batch).
    ...
    IF v_new_purchase THEN
      -- PRD-036 Phase B: RECEIVE the purchase into WH (new batch, captured expiry), then draw it fully for placement.
      -- Provenance stays 'manual_adjust' (exempt from wh_provenance_event_required: a brand-new
      -- receipt has no prior source event). The NEW-PURCHASE batch_id + inventory_audit_log rows
      -- carry the provenance narrative.
      ...
      -- draw the full purchase for the pod placement
    ...
    ELSE
      -- EXISTING behavior (unchanged): FEFO-decrement existing WH stock at the source WH.
