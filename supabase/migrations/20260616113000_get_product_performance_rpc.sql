-- get_product_performance: read-only product-performance briefing RPC.
-- Per-product trailing-3-complete-month throughput + current-month MTD,
-- factor-adjusted expected demand, total units/revenue/avg_price, sourcing bucket.
-- SECURITY DEFINER (reads RLS-restricted v_sales_transactions/machines); gated to
-- operator_admin/superadmin/manager/warehouse via EXISTS. NO writes to any table.
-- Constitution: Art 1 (canonical reader), Art 3 (read-only), Art 4 (validates input+role),
-- Art 16 (net-new metric; this function IS the canonical object).
...
BEGIN
  -- Article 4: role validation (read access via EXISTS subquery).
  IF NOT EXISTS (
  ...
  -- Article 4: input validation (normalize unknown buckets to the default).
  ...
  -- product (pod name) -> dominant boonz category + dominant boonz_product_id
  cat_map AS (
