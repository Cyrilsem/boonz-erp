-- =====================================================================================
-- 20260608124000_refillv2_stitch_driver_overlay_shelfguard.sql
-- Refill v2 — stitch_pod_to_boonz: v18_wsa_mix_weight_canonical -> v19_driver_overlay_shelfguard
-- =====================================================================================
--
-- TWO surgical additions, everything else verbatim from the live v18 body.
--
-- ADDITION A — Driver SKU overlay (CS-confirmed semantics):
--   "Driver SKU first-claim, remainder by mix_weight; pod total unchanged."
--   For each (machine_id, pod_product_id) that has rows in resolve_driver_intent
--   (resolved, both ids present, qty > 0):
--     * Each driver-named boonz_product_id variant is PINNED to its driver_qty
--       (first claim), capped so the running sum of pins never exceeds pod_qty.
--     * The residual pool = GREATEST(pod_qty - total_pinned, 0) is distributed
--       across the OTHER (non-pinned) variants by mix_weight via the EXISTING
--       largest-remainder mechanism (norm_split / base_qty / slot_remainder rank).
--     * If a driver-named SKU is not currently a mapping variant for that pod, it
--       is added as an extra variant row (so it appears) and still honours the
--       first-claim / remainder rule.
--     * Net invariant preserved: SUM(variant_final) over a pod = pod_qty (unchanged).
--   Implementation: a `driver_overlay` CTE near the top of the main WITH, a
--   `pull_variants` CTE that unions mapping variants with any extra driver-only
--   variants, then pin/residual math threaded through pull_norm_* / pull_base /
--   pull_slot_rem / pull_ranked / pull_target. The final per-variant integer column
--   name `variant_final` is preserved so pull_lines / all_lines / v_lines are
--   unchanged. Overlaid lines are tagged '[DRIVER-SKU-OVERLAY]' in the line comment.
--   NO-DRIVER-ROWS PATH: when driver_overlay has no rows for a pod, every pinned
--   quantity is 0 and total_pinned is 0, so residual == pod_qty and the original
--   largest-remainder math runs over ALL variants exactly as before. Byte-identical
--   allocation; comment branch falls through to the original CASE (no overlay tag).
--
-- ADDITION B — Defensive shelf-code canonical guard:
--   All live shelf_configurations.shelf_code are already canonical A01..E16 (verified:
--   0 non-canonical rows). This wraps every emitted shelf_code in the jsonb line
--   objects with an inlined canonicaliser (CASE over A01.. / A1.. / WEIMI '0-A1'/'1-A1'),
--   collapsing any future leak to canonical 'A01'. A single RAISE NOTICE fires if any
--   emitted shelf_code did not already match '^[A-E][0-9]{2}$'. The internal WEIMI
--   de-pad JOIN key in remove_phys_base (LEFT(shelf_code,1)||(SUBSTR(...)::int) ->
--   v_live_shelf_stock.slot_name) is left untouched — it is an internal join key,
--   not an emit.
--
-- The returned engine_version is bumped to 'v19_driver_overlay_shelfguard'.
--
-- KEPT VERBATIM: role gate, GUCs, preconditions, source_origin disagreement audit,
--   deviation + procurement_alerts inserts, write_refill_plan + confirm_stitched_plan
--   delegation, the REMOVE/M2W 'stitch invariant violation' check, action-label
--   mapping, dry_run gate.
--
-- GOVERNANCE (canonical-writer-adjacent):
--   stitch_pod_to_boonz BUILDS lines only; write_refill_plan persists them and
--   confirm_stitched_plan flips status. No direct write to refill_plan_output here,
--   and NO write to warehouse_inventory.status (Article 6 untouched). Constitution
--   Articles 1 (canonical writers), 4 (provenance/source_origin), 5 (append-only
--   deviations + procurement_alerts via INSERT), 8 (SECURITY DEFINER + role gate),
--   12 (dry-run-first diff gate). Ship behind diff-gate + CS green light.
-- =====================================================================================
...
BEGIN
  ...
  -- ADDITION B (guard pre-check): count any approved shelf_code that is NOT already
  -- canonical 'A01'..'E16'. All live data is canonical so this is 0; emit a single
  -- NOTICE if a future non-canonical leak appears (the per-line emit normalises it).
  ...
  WITH
  -- ADDITION A: driver SKU overlay source. One row per (machine, pod, boonz) driver
  -- intent with a positive resolved qty. Empty for any pod with no driver intent ->
  -- overlay is a no-op for that pod (no-driver path is byte-identical).
  driver_overlay AS (
  ...
  pull_raw AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           ...
           pm.boonz_product_id,bp.boonz_product_name,pm.mix_weight AS split_pct,
           ROW_NUMBER() OVER (
  ...
  -- ADDITION A: build the full variant set per slot = existing mapping variants
  -- UNION any driver-named boonz SKUs that are NOT already mapping variants for this
  -- (machine,pod). The extra driver-only rows carry split_pct=0 so they take no share
  -- of the mix-weight residual; they are pinned to driver_qty only. is_overlay_pod
  -- flags any slot whose pod has at least one driver_overlay row (used to tag the
  -- comment and to gate the pin logic).
  pull_variants AS (
    -- (a) existing mapping variants
    SELECT p.plan_date,p.machine_id,p.shelf_id,p.pod_product_id,p.action,
      ...
      FROM pull p
    UNION ALL
    -- (b) extra driver-only variants not present in the mapping for this slot
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.action,
           a.machine_name,a.shelf_code,a.pod_product_name,a.qty AS pod_qty,
           a.source_origin,a.from_machine,
  ...
  -- ADDITION A: attach driver_qty + pod-level overlay flag to each variant row.
  pull_overlaid AS (
    SELECT pv.*,
  ...
  -- ADDITION A: cap the pins so cumulative pinned qty never exceeds pod_qty.
  -- Order driver variants by driver_qty DESC, boonz_product_id for a deterministic
  -- first-claim. running_pin_before = sum of pins ranked ahead of this row; the pin
  -- granted to this row is bounded by remaining headroom (pod_qty - running_pin_before).
  pull_pins AS (
    SELECT po.*,
  ...
  -- ADDITION A: total pinned per slot + residual pool for the non-pinned variants.
  pull_pins_tot AS (
    SELECT pp.*,
  ...
  pull_resid AS (
    SELECT pt.*,
           ...
           -- a variant participates in the mix-weight residual split only if it is
           -- NOT pinned (pin_qty = 0) AND has a positive mix_weight.
           (pin_qty = 0 AND COALESCE(split_pct,0) > 0) AS is_residual_variant
      FROM pull_pins_tot pt
  ),
  -- ADDITION A: norm_split is computed over the RESIDUAL variants only. When there
  -- are no driver pins for a pod, residual_pool = pod_qty, every variant has pin_qty=0,
  -- so is_residual_variant collapses to (split_pct>0) and norm_split is identical to v18.
  pull_norm_pre AS (
    SELECT pr.*,
  ...
  pull_norm AS (
    SELECT pnp.*,
           CASE
             ...
             ELSE COALESCE(pnp.split_pct,0)
           END AS norm_split
      FROM pull_norm_pre pnp
  ),
  -- ADDITION A: largest-remainder distributes residual_pool (NOT pod_qty) across the
  -- residual variants only. Pinned / non-residual variants get base_qty 0 here and
  -- pick up their pin in pull_target. No-driver path: residual_pool=pod_qty and all
  -- variants are residual, so this is the v18 floor/remainder math verbatim.
  pull_base AS (
    SELECT pn.*,
  ...
  -- ADDITION A: variant_target = pin (for pinned variants) OR base + remainder bump
  -- (for residual variants). The +1 largest-remainder bump only applies to residual
  -- variants, so pinned variants are never inflated. SUM over a pod = total_pinned +
  -- residual_pool = pod_qty (net invariant preserved).
  pull_target AS (
    SELECT pr.*,
           CASE
  ...
  remove_phys_split AS (
    SELECT n.*,
           CASE WHEN total_split=0 THEN 1.0/variant_n ELSE COALESCE(n.split_pct,0) END AS norm_split
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    -- ADDITION B: normalise shelf_code on emit (inlined public_canon_shelfcode).
    'shelf_code',
      CASE
    ...
    'comment', CASE
      ...
      -- ADDITION A: tag overlaid refill/add lines so the driver-claim is visible.
      ...
      WHEN action IN ('REFILL','ADD_NEW') AND is_overlay_pod
        THEN '[DRIVER-SKU-OVERLAY] driver SKU first-claim; remainder by mix_weight'
  ...
  m_raw AS (
    SELECT a.plan_date,a.machine_id,a.shelf_id,a.pod_product_id,a.qty AS pod_qty,
           pm.boonz_product_id,pm.mix_weight AS split_pct,
           ROW_NUMBER() OVER (
             PARTITION BY a.machine_id,a.shelf_id,a.pod_product_id,pm.boonz_product_id
             ORDER BY (pm.machine_id=a.machine_id) DESC NULLS LAST,
                      pm.is_global_default DESC, pm.boonz_product_id
  ...
  n AS (SELECT np.*, CASE WHEN total_split=0 THEN 1.0/variant_n ELSE COALESCE(np.split_pct,0) END AS norm_split FROM n_pre np),
  b AS (
  ...
  WITH pm_per_row AS (
    SELECT DISTINCT ON (prp.plan_date, prp.machine_id, prp.shelf_id, prp.pod_product_id, pm.boonz_product_id)
           ...
           pm.boonz_product_id,
           prp.qty,
           COALESCE(NULLIF(pm.mix_weight, 0), 0.20)::numeric AS split_pct
    FROM public.pod_refill_plan prp
    JOIN public.product_mapping pm
  ...
  demand AS (
    SELECT boonz_product_id, pod_product_id,
           SUM(FLOOR(qty * split_pct)::int) AS demand_pod_qty,
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v19_driver_overlay_shelfguard',
