-- PRD-087 dashboard v3 (CS round): p_include_vox scope toggle (VOX* machines
-- in/out — name-based, cinema fleet); per-period top machines AND top
-- products baked in (universal Today/7d/30d toggle, no nested filters);
-- refill_today gains per-machine status list; health.top_urgent ordered
-- P1 → P2 (skip tier excluded); KPIs drop margin, add default_rate
-- (1 − paid/total on POS-recorded sales) ; expiry rolled up for a KPI card.
...
refill_lines AS (
  SELECT rd.*, m.official_name AS machine_name
    ...
    'top_urgent', COALESCE((
      ...
      FROM (
        ...
        WHERE upper(machine_name) NOT LIKE 'WH%' AND include_in_refill
          AND priority_tier IN ('P1_RESTOCK','P2_MAINTAIN')
...
COMMENT ON FUNCTION public.get_dashboard_summary(boolean) IS
'PRD-087 v3: themed dashboard aggregate. p_include_vox toggles VOX* cinema machines in sales KPIs/tops. Per-period top machines+products; refill machine statuses; P1→P2 alerts; default_rate = 1 - paid/total (POS-recorded). Read-only.';
