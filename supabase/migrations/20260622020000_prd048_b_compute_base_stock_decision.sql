-- PRD-048 Step B: PURE base-stock sizing helper (IMMUTABLE, zero table lookups).
-- Implements policy spec PRD-048 §3 + edge cases §4. New NAME (not an overload of compute_refill_decision)
-- per CLAUDE.md function foot-gun rule. Dara (✅) -> Cody review -> MCP apply.
--
-- mu_day = w7*(v7/7) + w30*(v30/30); sigma=sqrt(mu_day)
-- DEAD (v7=0 AND v30=0 AND not cold): add=0 (unchanged)
-- cold_start (no velocity but RAMPING/new): seed target = ceil(min_fill_pct*cap)
-- else: S = mu_day*T + z*sigma*sqrt(T); spoilage cap S<=mu_day*shelf_life*spf;
--       floor = ceil(min_fill_pct*cap) ONLY if (mu_day*7)>=seller_wk_threshold else 0;
--       target = min(cap, max(round(S), floor)); want=clamp(target-oh,0,cap-oh); add=min(want,wh_pickable)
...
calc AS (
  SELECT sc.*,
    ...
    CASE
      WHEN sc.no_vel AND NOT sc.cold THEN 0                                              -- DEAD
      WHEN sc.no_vel AND sc.cold     THEN LEAST(sc.cap, ceil(sc.minfill*sc.cap)::int)    -- cold-start seed
      ELSE LEAST(sc.cap,
             GREATEST( round(sc.s_capped::numeric)::int,
                       CASE WHEN sc.is_seller THEN ceil(sc.minfill*sc.cap)::int ELSE 0 END ))
    END AS target
  FROM sc
),
fin AS (
...
COMMENT ON FUNCTION public.compute_base_stock_decision IS
  'PRD-048 pure service-level base-stock sizing (order-up-to S). All inputs passed in -> IMMUTABLE, unit-testable. Called by engine_add_pod only when refill_policy_params.refill_sizing_mode=base_stock.';
