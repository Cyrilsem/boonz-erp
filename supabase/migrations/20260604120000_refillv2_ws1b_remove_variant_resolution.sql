-- Refill reliability (PRD docs/prds/PRD_refill_reliability_2026-06-03.md) / WS1b - REMOVE/M2W concrete-variant resolution.
--
-- STATUS: DRAFT - NOT APPLIED. Staged for CS apply. Cody-reviewed below. Subsumes the v15 qty-0-skip
-- file (20260603120000); apply v15 then this, or this alone (it is a full CREATE OR REPLACE).
--
-- ROOT CAUSE (postmortem RC2, PRD WS1b): the stitch physical-fallback path (fired when a REMOVE/M2W has
-- physical shelf stock but no Active pod_inventory match - e.g. a swap just cleared pod_inventory) emitted
-- boonz_product_name = the POD name ("Evian", "Vitamin Well", "Krambals & Zigi"). write_refill_plan V5
-- (`boonz_product_name must exist in boonz_products`) rejected the whole batch -> validation_error -> the
-- machine's entire dispatch was lost (the VML Evian->Popit failure). Confirmed: "Evian" is not a
-- boonz_product_name; the Evian pod maps 1:1 to "Evian - Regular". Also confirmed push_plan_to_dispatch
-- skips any line whose boonz_product_name does not resolve, so a NULL-boonz remove never dispatches anyway.
--
-- FIX (this migration): resolve the physical-fallback REMOVE/M2W to CONCRETE boonz variant(s) via
-- product_mapping - the same machine-scoped/global-default resolution the REFILL path (pull_lines) already
-- uses - and split the physical shelf qty (v_live_shelf_stock.current_stock) across the mapped variants by
-- split_pct (even 100/variant_n fallback when no split_pct), distributing the remainder ±1. So:
--   * Evian (1 variant)    -> one line "Evian - Regular", full qty.
--   * Popit Mix (3 variants 40/30/30) -> three lines summing to the physical qty.
-- A REMOVE/M2W pod with NO active mapping resolves to nothing and is DROPPED (was already undispatchable;
-- now it no longer poisons the batch). qty-0 planned REMOVE is skipped (from v15). FEFO note: product_mapping
-- carries no per-variant expiry, so the split uses the pull-style remainder ordering; true FEFO applies on
-- the mapped (pil) path where expiry is known. This does NOT implement WH-availability gating (explicit
-- non-goal) - it only names the variant of stock physically being removed.
--
-- Verbatim reproduction of stitch v15 (20260603120000), diff-gated to: the remove_lines_physical_fallback
-- CTE (now resolves concrete variants) + engine_version -> 'v16_ws1b_remove_variant_resolution'. The mapped
-- remove path, pull path, deviation/alert/diagnostic logic, role gate, app.via_rpc, and write/confirm flow
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version','v16_ws1b_remove_variant_resolution',
