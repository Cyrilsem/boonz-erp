-- PRD-117 item L: warn (never block) when a planned Remove/Machine To Warehouse
-- quantity exceeds the lane's own most recent WEIMI stock reading.
-- Verified against md5 f0c3c97c67ae91469accd28315187493 of the prior body (v12_prd115_tombstone_guard).
...
BEGIN
  ...
  LOOP
    ...
    IF v_action IN ('Remove','Machine To Warehouse') THEN
      -- PRD-117 item L: warn (never block) when the planned Remove/M2W qty
      -- exceeds the lane's own most recent WEIMI stock reading (within 3 days).
