-- ============================================================================
-- PRD-REFILL-V2 · Item 5 · pick_machines_for_refill  v7 -> v8  "P1 + siblings"
-- ============================================================================
-- CHANGES vs v7 (diff-gate summary):
--   1. P1_RESTOCK band now MIRRORS get_machine_health.priority_tier='P1_RESTOCK'
--      (redesigned 2026-06-07, migration 20260607170000):
--        velocity>0 AND ( empty_shelf | runway<3 | units7>=50&runway<5 |
--                         units7>=100&runway<7 | units7>=50&fill<60 |
--                         under25>=2&units7>=30 )
--      v7 used runway<7 / runway<14&units>=20 / under25>=1&units>=20 (looser, over-picked).
--      NOTE: v_machine_health_signals does NOT expose daily_velocity, so v8 proxies
--      the velocity gate with units_last_7d > 0 (sold in the last 7d => velocity>0),
--      and uses units_last_7d for the unit thresholds (the view's available analog of
--      get_machine_health's daily_velocity*7). under25 still comes from the shelf_u25
--      CTE (the view has no slots_below_25pct column). Documented proxy, not exact.
--   2. P2_MAINTAIN gains the get_machine_health "(empty_shelf AND velocity=0)" branch
--      so an empty-but-not-selling unit falls to P2, never P1.
--   3. EXPLICIT warehouse/excluded exclusion (NEW): joins machines and requires
--      include_in_refill = true AND status NOT IN ('Warehouse','Inactive'). v7
--      relied entirely on the upstream view; the PRD wants this explicit (A8).
--   4. KEPT verbatim: planned_swaps auto-close, venue-sibling expansion
--      (r_cluster = COALESCE(venue_group, building_id, official_name)), the
--      machines_to_visit write, visit_order/route_cluster (page & route agree),
--      GUC/role gate, ON CONFLICT upsert.
--
-- AC8: picked set = P1 ∪ siblings; no warehouses / excluded. Page (machines_to_visit
--   read-back ORDER BY visit_order) and route (visit_order) remain the single source.
--
-- DARA: no schema change. CODY: canonical writer (machines_to_visit) -> diff-gate,
--   CS green light. Articles 1,4,5,8,12. Forward-only. No warehouse_inventory.status.
-- ============================================================================
...
BEGIN
  ...
  RAISE NOTICE 'pick_machines_for_refill v8: auto-closed % planned_swaps for plan_date %',
  ...
  scored AS (
    SELECT s.*,
      ...
      CASE
        -- P1_RESTOCK == get_machine_health bands (velocity proxied by units_last_7d>0)
        WHEN s.units_last_7d > 0 AND (
                 s.empty_shelves_count >= 1
              OR (s.runway_days IS NOT NULL AND s.runway_days < 3)
              OR (s.units_last_7d >= 50  AND s.runway_days IS NOT NULL AND s.runway_days < 5)
              OR (s.units_last_7d >= 100 AND s.runway_days IS NOT NULL AND s.runway_days < 7)
              OR (s.units_last_7d >= 50  AND s.fill_pct < 60)
              OR (COALESCE(u.under25,0) >= 2 AND s.units_last_7d >= 30)
             ) THEN 'P1_RESTOCK'
        WHEN s.dead_slot_pct >= 15 OR s.days_since_visit >= 14
          OR s.expired_skus_now >= 1 OR s.active_intent_count > 0
          OR (s.empty_shelves_count >= 1 AND COALESCE(s.units_last_7d,0) = 0)
          THEN 'P2_MAINTAIN'
        ELSE 'skip'
      END AS p_tier,
      (  (CASE WHEN s.empty_shelves_count >= 1 THEN 50 + LEAST((s.empty_shelves_count-1)*12, 36) ELSE 0 END)
       + (CASE WHEN s.units_last_7d >= 100 AND s.runway_days < 7 THEN 35
               WHEN s.units_last_7d >= 50  AND s.runway_days < 5 THEN 30
               WHEN s.runway_days < 3 THEN 28
               WHEN s.units_last_7d >= 50 AND s.fill_pct < 60 THEN 20 ELSE 0 END)
       + (CASE WHEN s.units_last_7d >= 30 THEN LEAST(COALESCE(u.under25,0)*8, 24) ELSE 0 END)
      ...
      ARRAY_REMOVE(ARRAY[
        ...
        CASE WHEN s.runway_days IS NOT NULL AND s.runway_days < 3 THEN 'runway_critical' END,
        CASE WHEN s.units_last_7d >= 50 AND s.runway_days IS NOT NULL AND s.runway_days < 5 THEN 'strong_seller_low_runway' END,
        CASE WHEN s.units_last_7d >= 50 AND s.fill_pct < 60 THEN 'strong_seller_low_fill' END,
        CASE WHEN s.units_last_7d >= 30 AND COALESCE(u.under25,0) >= 2 THEN 'shelves_under25' END,
    ...
    JOIN public.machines m
     ...
     AND COALESCE(m.status, 'Active') NOT IN ('Warehouse', 'Inactive')   -- explicit exclusion (A8)
  ...
  RAISE NOTICE 'pick_machines_for_refill v8 % -> % rows picked', p_plan_date, v_rows;
