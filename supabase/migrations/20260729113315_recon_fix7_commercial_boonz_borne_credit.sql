...
-- (2026-07-29) of the VOX SOA reconciliation wave (recon_fix1..5b, documented at
-- docs/architecture/CHANGELOG.md:2957-2959 and docs/architecture/RPC_REGISTRY.md:501, which
-- covered get_payment_default_summary / get_vox_consumer_report / get_vox_commercial_report
-- through recon_fix4/5a/5b). Confirmed applied via supabase_migrations.schema_migrations
-- (version 20260729113315, immediately after the PRD-108 wave and before recon_fix7b).
--
-- Reconstructed by inspecting get_vox_commercial_report's live body (pg_get_functiondef):
-- a `boonz_borne` computed column exists in the `txn_waterfall` CTE that was NOT present in the
-- recon_fix4/5a/5b description (which only covered refund sourcing). It credits the FULL
-- total_amount of a transaction as boonz-borne when the transaction is completely unmatched to
-- any settlement path (no Adyen psp_reference, no captured_refunded, no cash_recovered) -
-- i.e. a basket Boonz absorbs entirely rather than reporting as a plain "default". This
-- migration's inferred delta ("_credit"): introduce the `boonz_borne` CASE WHEN column and
-- expose it as `waterfall.boonz_borne_amount` in the JSON result.
-- GREEN — the concept is unambiguously present and coherent in the live function; classified
-- from prod state per the audit brief (no doc trace found, so not "guessed" — read off the code).
-- NOTE: this function is CUMULATIVE across many same-week migrations (recon_fix4, recon_fix5a,
-- recon_fix5b, recon_fix6 [not in this reconstruction set / owned elsewhere], recon_fix7 [this
-- file], recon_fix7b [next file]). The body below is the CURRENT prod state, i.e. it already
-- includes recon_fix7b's "_net" delta (boonz_borne folded into net_revenue_calc / boonz_share /
-- vox_share / vox_net_dues) — that fold-in is recon_fix7b's own delta, not this migration's;
-- see that file's header. CREATE OR REPLACE is idempotent across the repeated re-application.
...
BEGIN
  ...
  txn_final AS (
    SELECT *,
      -- recon_fix7b folds boonz_borne into the net formula (see that migration's header).
