-- PRD-129R step 01: receive_dispatch_line computes v_return_delta = planned - filled and
-- credits warehouse_stock with it inline (either through the v_consumer_row combined UPDATE, or
-- the v_wh_row fallback branches), but never sets remainder_credited. Its closing UPDATE then
...
-- remainder_credited still false and calls credit_dispatch_remainder(), crediting the identical
-- remainder a second time. 208 of 208 partial-fill events since 2026-07-06 doubled, 537 phantom
-- units across 59 products.
--
-- Fix: set remainder_credited = true in the same closing UPDATE that sets item_added = true.
-- Nothing else in this function changes. This is sufficient on its own:
-- tg_credit_dispatch_remainder_on_receive()'s own guard is
-- "AND NOT COALESCE(NEW.remainder_credited,false)" -- once this UPDATE sets both columns in one
-- statement, NEW.remainder_credited is already true by the time the trigger evaluates NEW, so
-- credit_dispatch_remainder() is never invoked from the trigger path for a fresh receive.
-- credit_dispatch_remainder() itself is untouched by this step (see prd129r_02 for its own,
-- separate fix) and still returns 'already_done' if called directly afterwards, since it checks
-- the same column at its own top.
--
-- Verified in a rolled-back transaction before applying: a line planned 8, filled 3, from a
-- pinned wh_inventory_id produced exactly one warehouse_stock increment of 5 (not 10), exactly
-- one refill_dispatching row with remainder_credited = true, and a direct call to
-- credit_dispatch_remainder() on that dispatch afterwards returned status 'already_done'. See
-- this migration's application record for the exact numbers.
