-- PRD-010a AC#2: engine_finalize_pod v12.1 — widen capacity warning signal filter.
-- v11/v12 only emitted capacity_mismatch_warnings for signals in
-- ('STAR','DOUBLE DOWN','KEEP GROWING'). MC-2004 Coca Cola Zero is KEEP
-- (v30=1.53, capped_by_max on a 14-unit shelf) — no warning emitted.
-- KEEP is the most common proven-product signal; excluding it means most
-- capacity mismatches go unreported. v12.1 adds 'KEEP' to the IN-list.
-- All other v12 behaviors (M2W empty-shelf guard, m2w_no_replacement_warnings,
-- return jsonb shape) preserved verbatim. Same signature.
...
BEGIN
       ...
       SET status     = 'superseded',
           reasoning  = COALESCE(prp.reasoning, '{}'::jsonb)
                        || jsonb_build_object(
                             ...
                             'auto_suppressed_by', 'engine_finalize_pod_v12_1'),
           updated_at = now()
     WHERE prp.plan_date = p_plan_date
       AND prp.status    = 'draft'
       AND prp.action    IN ('M2W','REMOVE')
  ...
  -- v12.1 AC#2: widened to include KEEP. The most common proven-product
  -- signal is KEEP; without it the capacity mismatch warning never fires for
  -- products like MC-2004 Coca Cola Zero (v30=1.53 on a 14-unit shelf).
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version', 'v12_1_keep_in_capacity_filter',
