-- PRD-010a AC#1: engine_swap_pod v9.1 — planned-swap guard product-name match.
-- v9 joined planned_swaps.shelf_code to shelf_configurations.shelf_code, but
-- planned_swaps stores WEIMI cabinet-prefixed codes ('0-A06') while
-- shelf_configurations uses logical codes ('A06'). The JOIN missed all
-- MC-2004 Plaay shelves. v9.1 matches by the product being removed instead,
-- using the same join chain as pick_machines_for_refill v6 (planned_swaps →
-- pod_products on remove_pod_product_name → slot_lifecycle on pod_product_id).
-- No signature change. Cody-approved against Articles 1, 4, 5, 8, 12.
...
BEGIN
  ...
  -- v9.1 AC#2(b): match by product being removed, not by shelf_code text.
  -- WEIMI-format cabinet codes in planned_swaps ('0-A06') do not match
  -- shelf_configurations logical codes ('A06'). Joining via slot_lifecycle on
  -- the current occupant is robust regardless of the format planned_swaps used.
  ...
  SELECT DISTINCT sl.machine_id, sl.shelf_id
    ...
    JOIN public.pod_products pp
      ON pp.pod_product_name = ps.remove_pod_product_name
    JOIN public.slot_lifecycle sl
      ON sl.machine_id = ps.machine_id
     AND sl.pod_product_id = pp.pod_product_id
  ...
  SELECT
    ...
    jsonb_build_object('pass','1','source','strategic_machine_tag',
                       ...
                       'tag_priority', tr.priority, 'engine_version','v9_1_product_match_planned_swap')
  ...
  pass2_inserted AS (
    ...
    SELECT p_plan_date, p2.machine_id, p2.shelf_id, p2.pod_out, p2.pod_in,
      ...
      jsonb_build_object('pass','2','source','autonomous','signal',p2.signal,'cat_out',p2.cat_out,
        ...
        'engine_version','v9_1_product_match_planned_swap')
