-- PRD-121 P1.1b: write_refill_plan surfaces the 9 merchandising gates as an early
-- warning, using validate_refill_plan's new p_source='plan_output' mode (P1.1a).
--
-- write_refill_plan already validates data SHAPE at write time (V1-V6: action, quantity,
-- shelf_code, machine_name, product_name, duplicate) and already returns ALL of those in
-- one response -- that part of P1.1 was already correct. What was missing is the same 9
-- merchandising gates validate_refill_plan runs (lane capacity, empty lane, no mapping,
-- Remove-with-no-replacement, CENTRAL stock, FEFO) -- those only ran at approve time
-- (P0.1), so an operator only discovered a bad plan after writing it.
--
-- Runs AFTER the INSERT loop (so it sees the rows just written), attached under a new
-- 'validation' key -- informational only, never blocks the write itself. The real hard
-- gate stays at approve_refill_plan (P0.1); this is strictly an earlier look at the same
-- canonical gate object (Article 16 -- one 9-gate validator, two call sites, not a second
-- divergent check). If nothing was actually written (validation_error / empty-plan early
-- returns), no validation call is made -- there is nothing yet to check.
--
-- Cody: approve. Articles 1 (write_refill_plan remains the sole writer of
-- refill_plan_output; validate_refill_plan is read-only, called after the write
-- completes), 4 (unchanged role/input guards), 12 (forward-only CREATE OR REPLACE, same
-- signature, md5-guarded), 16 (reuses validate_refill_plan's plan_output mode rather than
-- re-deriving the 9 gates a second way in this function).

DO $mig$ DECLARE v_def text; BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def
    FROM pg_proc p WHERE p.proname='write_refill_plan' AND p.pronamespace='public'::regnamespace;
  IF md5(v_def) <> '0f5206987e86dfa0820bf3d065fa9ec6' THEN
    RAISE EXCEPTION 'write_refill_plan drifted (md5 %), refusing blind replace', md5(v_def);
  END IF;
END $mig$;
...
BEGIN
  ...
  -- PRD-121 P1.1: early warning using the same 9-gate canonical validator approve_refill_plan
  -- enforces as a hard gate, run against the rows just written (plan_output, not dispatch).
