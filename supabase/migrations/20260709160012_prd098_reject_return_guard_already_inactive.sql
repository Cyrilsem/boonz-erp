-- PRD-098 forward-fix: reject_return must only call inactivate_warehouse_row when the row is
-- still Active (an already-Inactive quarantined row with stock>0 would otherwise error). Drain +
-- audit still apply; inactivate skipped when already Inactive (idempotent write-off).
