-- PRD-110 · D-43 EXECUTE (both halves) + S-193 FIX — the warehouse role self-serves a repack,
-- and a repack actually re-creates the rows it returned.
--
-- CS RULING (2026-08-04): "D-43 CLOSED: warehouse SELF-SERVES repacks. Add `warehouse` to
-- push_plan_to_dispatch's role list (option a). The pre-flight fix (repack_machine must check push
-- authorisation BEFORE returning a single row) remains mandatory and independent, per the original
-- finding."
--
-- ⛔ WHY S-193 RIDES IN THE SAME MIGRATION, AND WHY THAT IS NOT SCOPE DRIFT (finding S-248).
--   Today the packing role hits S-191: repack half-completes and RAISES `push_failed` — LOUD.
--   D-43 half 1 alone authorises that role for push, so it stops erroring and lands on S-193
--   instead: repack returns status='ok' with ZERO fresh dispatch rows and a plan re-stamped
--   dispatched=true. ⛔ Shipping half 1 by itself would hand the exact role that reported the
--   original incident a SILENT freeze in place of a loud error. The parking lot has said since
--   leg 107 that "S-193 is independent of D-43 and must be fixed regardless"; it is the
--   precondition that makes D-43 safe, and both defects live in the same function.
--
-- ⭐ S-193's MECHANISM, in one line: push's RC-01 §5(5b) multi-wave idempotency probe excluded
--   skipped / cancelled / is_m2m rows but NOT `returned` ones. A row that repack had just RETURNED
--   still matched, so push "preserved" the plan line against a dead row, pushed nothing, and
--   re-stamped dispatched=true. The partial unique index and prevent_duplicate_unstarted_dispatch
--   BOTH already exclude returned=true — this predicate was the only place that did not, which is
--   why the fresh INSERT succeeds the moment the predicate agrees with them.
--
-- ⛔ THE CHANGE IS FOUR NAMED SUBSTITUTIONS ON `pg_get_functiondef` OUTPUT FOR push (diff-verified
-- to exactly 4 hunks / 6 changed lines) AND ONE INSERTED BLOCK FOR repack (1 hunk). Nothing else
-- in either body moved.
--
-- ⭐ THE ROLE LIST BECOMES A NAMED OBJECT, and that is the point of half 2. S-248: once `warehouse`
-- joins push's list, repack's gate and push's list are IDENTICAL SETS, so the pre-flight is a
-- branch NO ROLE CAN REACH — it cannot be proven by a role replay. `push_dispatch_authorized_roles()`
-- makes the relation assertable as DATA rather than as a prosrc substring, so fixture 9 can state
-- `repack_roles ⊆ push_roles` and catch the divergence class that caused this incident, instead of
-- passing the guard by inspection (the S-173 sin).
--
-- ⚠️ DELIBERATELY NOT CHANGED, so the next leg does not read silence as an oversight:
--   · repack_machine's own gate literal ('warehouse','operator_admin','superadmin','manager') —
--     fixture 9 seq 1 pins it and option (a) leaves it intact.
--   · repack_machine's search_path ('public' alone, no pg_temp) — fixture 9 seq 4 pins that
--     unhardened shape as a known fleet-scale item (S-198). Hardening it here would be an unruled
--     second change and would move seq 4 for the wrong reason.
--   · S-192 (a repack's own returns permanently block the next repack) is UNTOUCHED and still
--     true; fixture 9 seq 31-34 continue to pin it.
--
-- ⛔ FIXTURE 9 IS EXPECTED TO GO RED ON THIS MIGRATION (seq 2, 9-11, 13, 16-22, 27-28). That red is
-- the proof the fix landed and is banked BEFORE the sensors are re-based in the follow-up
-- migration, exactly as D-45 and D-46 proved themselves.
--
-- Cody articles: 1 (canonical writer preserved) · 3 (RPC only, no raw write) · 4 (GUC hygiene
-- unchanged) · 6 (no protected-entity semantics loosened — the fix makes push create the row it
-- was already supposed to create) · 8 (role gate tightened into a named object, never widened
-- beyond the CS ruling) · 12 (forward-only CREATE OR REPLACE, no signature change) · 16 (versioned
-- tell: rpc_version v10_rc01_single_writer -> v11_rc01_single_writer_d43_s193).

-- ---------------------------------------------------------------------------
-- 1. The shared role source of truth (NEW object, additive).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.push_dispatch_authorized_roles()
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
AS $function$
  -- D-43 (CS 2026-08-04): `warehouse` self-serves repacks. This is the ONLY place the
  -- push-authorisation role set is written down; push_plan_to_dispatch gates on it and
  -- repack_machine pre-flights against it, so the two can never silently diverge again.
  SELECT ARRAY['operator_admin','superadmin','manager','warehouse']::text[]
$function$;

REVOKE ALL ON FUNCTION public.push_dispatch_authorized_roles() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.push_dispatch_authorized_roles() FROM anon;
-- ⛔ Cody revision (leg 134, Article 3 / least privilege): NO `authenticated` grant. Both callers
-- are SECURITY DEFINER owned by postgres, so the EXECUTE check inside them resolves as the definer
-- and the grant would only publish the privileged-role list to every logged-in user for no caller
-- that needs it. Re-grant if DR-6's refusal affordance ever needs to read it client-side.
GRANT EXECUTE ON FUNCTION public.push_dispatch_authorized_roles() TO service_role;

-- ---------------------------------------------------------------------------
-- 2. push_plan_to_dispatch — D-43 half 1 (gate) + S-193 (returned rows never serve a line).
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    -- RC-01 §5(5a): preserve a LIVE manual-edit row only (skipped/cancelled excluded).
    SELECT rd.dispatch_id INTO v_existing_edit_id
     ...
     WHERE rd.machine_id     = v_machine_id
       ...
       AND COALESCE(rd.returned,  false) = false  -- S-193: a RETURNED row never serves a plan line
    ...
    -- RC-01 §5(5b): multi-wave idempotency for warehouse Refill/Add New.
    ...
    -- RC-01 §7 / B1(c): resolve route warehouse (cold->central, ambient->primary).
  ...
  RETURN jsonb_build_object(
    ...
    'rpc_version','v11_rc01_single_writer_d43_s193'
  );
END $function$;

-- ---------------------------------------------------------------------------
-- 3. repack_machine — D-43 half 2: pre-flight the push authorisation BEFORE the
--    first destructive act (the return_dispatch_line loop). No savepoint exists,
--    so this refusal is the only thing standing between a half-authorised caller
