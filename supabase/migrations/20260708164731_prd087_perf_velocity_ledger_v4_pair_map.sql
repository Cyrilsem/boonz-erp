-- PRD-087 PERF: boonz-level ledger was ~2.1s because the mapping LATERAL
-- (with NOT EXISTS) ran per SALES ROW (~30k). v4 resolves the mapping once
-- per (pod_product, machine) PAIR (~hundreds) and joins. Same output.
