...
-- pack_dispatch_line v2: venue-supplied (source_origin = 'vox_at_venue') dispatch lines may only
-- draw from 2099-expiry venue-placeholder warehouse_inventory rows; real batches are protected
-- from being consumed by VOX venue-team stock, and a missing placeholder fails soft (bind_failed)
-- rather than silently substituting a real batch.
