-- PRD-087 v3 — adds p_level ('pod' | 'boonz') to the velocity ledger.
-- 'boonz' distributes mixed pod-product sales (Chocolate Bar, Coca Cola Mix,
-- Soft Drinks Mix, Krambals & Zigi, ...) into their boonz products using
-- normalized product_mapping split_pct (machine-specific mappings win over
-- global defaults). Sales rows whose pod name cannot be resolved keep their
-- pod name so nothing silently disappears. Pod mode unchanged from v2.
...
-- boonz distribution: machine-specific Active mappings win; split normalized
dist AS (
  SELECT r.pod_name, r.machine_id, r.machine_name, r.txn_date,
         CASE
           WHEN p_level = 'boonz' AND pm.boonz_product_id IS NOT NULL
             THEN bp.boonz_product_name
           ELSE r.pod_name
         END AS pname,
         CASE
           WHEN p_level = 'boonz' AND pm.boonz_product_id IS NOT NULL
             THEN r.qty * pm.split_pct / NULLIF(pm.tot_split, 0)
           ELSE r.qty
         END AS qty
  FROM raw r
  LEFT JOIN LATERAL (
    SELECT pm2.boonz_product_id, pm2.split_pct,
           sum(pm2.split_pct) OVER () AS tot_split
    FROM product_mapping pm2
    WHERE p_level = 'boonz'
      AND pm2.pod_product_id = r.pod_product_id
      AND pm2.status = 'Active'
      AND (
        (pm2.machine_id = r.machine_id)
        OR (
          (pm2.machine_id IS NULL OR pm2.is_global_default)
          AND NOT EXISTS (
            SELECT 1 FROM product_mapping p3
            WHERE p3.pod_product_id = r.pod_product_id
              AND p3.machine_id = r.machine_id
              AND p3.status = 'Active'
          )
        )
      )
  ) pm ON p_level = 'boonz'
  LEFT JOIN boonz_products bp ON bp.product_id = pm.boonz_product_id
  WHERE (p_level <> 'boonz') OR (pm.boonz_product_id IS NOT NULL) OR (r.pod_product_id IS NULL)
),
scoped AS (
  SELECT pname, machine_name, qty, txn_date FROM dist WHERE qty > 0
),
firsts AS (
...
COMMENT ON FUNCTION public.get_product_velocity_ledger(integer, text, text) IS
'PRD-087 v3: velocity ledger over last N complete Dubai weeks. p_level=pod (as-sold shelf names) or boonz (mixed shelves distributed to boonz products via normalized product_mapping split_pct, machine mappings win). Refund-excluded; current partial week separate.';
