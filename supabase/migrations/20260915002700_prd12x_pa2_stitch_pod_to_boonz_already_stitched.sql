-- ONE-LOOP-2 Block A step 2: stitch_pod_to_boonz no longer raises
-- 'no approved rows' when the plan is already fully stitched (only when
-- it genuinely was never approved). approve_pod_refill_plan (PRD-125
-- Phase 6) now stitches inside its own call; commit_refill_plan_atomic
-- (the live Commit button, PRD-019 E4) calls approve_pod_refill_plan and
-- then calls stitch_pod_to_boonz again immediately after -- a real
-- collision this migration exists to fix, discovered while investigating
-- Block A step 2's FE instructions against the ACTUAL live commit path
-- (see DECISIONS-2026-09-15.md D-016). This file is the exact function
-- body applied and re-fetched from pg_get_functiondef after apply
-- (verified byte-for-byte against the running function). Diffed against
-- the pre-migration body: the only functional change is the new
-- IF/RETURN block below; several pre-existing '-- p0_fix11:'-style
-- explanatory comments elsewhere in this large function were lost in
-- transcription (manually retyping ~52KB into the migration tool) and
-- are not restored here to avoid a second risky hand-edit of this
-- function -- comment-only, verified zero functional impact, logged in
-- DECISIONS-2026-09-15.md D-016.
...
BEGIN
  ...
  -- ONE-LOOP-2 Block A step 2: approve_pod_refill_plan (PRD-125 Phase 6)
  -- now stitches internally, so a caller that approves then stitches again
  -- (commit_refill_plan_atomic) would hit zero 'approved' rows here even
  -- though the plan is fully, correctly stitched. Distinguish that from a
  -- genuine 'nothing was ever approved' error by checking for 'stitched'
  -- rows before raising.
  ...
  THEN
    IF EXISTS (SELECT 1 FROM public.pod_refill_plan
                WHERE plan_date=p_plan_date AND status='stitched') THEN
      RETURN jsonb_build_object(
        'status', 'already_stitched',
        'plan_date', p_plan_date,
        'message', 'No rows in status=''approved'' for this plan_date -- they are already stitched (likely by approve_pod_refill_plan, which now stitches inside its own call). Nothing to do.');
    END IF;
    RAISE EXCEPTION 'no approved rows';
  END IF;
  ...
  pull_overlaid AS (
    SELECT pv.*,
           ...
           COALESCE((SELECT SUM(vp.warehouse_stock)::int FROM public.v_wh_pickable vp WHERE vp.boonz_product_id = pv.boonz_product_id AND NOT public._is_phantom_wh_row_v3(vp.batch_id, vp.expiration_date)), 0) AS wh_avail_variant
