-- PRD-031 WS-6: drop the dead always-0 refill_plan_output.sold_7d placeholder.
-- The column was never populated (stitch emitted 0) and actively misled direct queries.
-- The real 7d-sales display value is computed by get_refill_plan_output_enriched from
-- v_sales_history_attributed (canonical sales source). Step 1: write_refill_plan no longer
-- writes the column. Step 2: drop it. Forward-only; Cody class(a/b) cleared.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'preflight_version', 'g7'
  );
END;
$function$
;

ALTER TABLE public.refill_plan_output DROP COLUMN IF EXISTS sold_7d;
