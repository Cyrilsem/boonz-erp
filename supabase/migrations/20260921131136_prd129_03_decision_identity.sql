...
--
-- supabase_migrations.schema_migrations records this exact version (20260921131136) under the
-- name "prd129_03_decision_identity", applied live. It was superseded 6 minutes later by
-- 20260921131742 (prd129_03b_decision_identity_facings_fix) after a real bug was found during
-- verification: this version summed v_sales_history_resolved by pod_product_id with no facings
-- division, so a product spanning multiple physical slots on one machine got the same
-- whole-machine sales total repeated on every one of its slots, inflating final_score by roughly
-- the facings count. Caught before merge, never shipped in a PR.
--
-- Unlike the other superseded migrations in this reconciliation, the exact text of this one is
-- available verbatim: it is the SQL this same session sent to the migration tool for this exact
-- named call, before the facings-normalization fix was written. Sourced from that record, not
-- from "the current live definition" (the live function is the corrected 03b version).
--
-- This file intentionally reproduces a since-superseded, since-buggy intermediate state. Do not
-- apply it. The live, correct definition is 20260921131742_prd129_03b_decision_identity_facings_fix.sql.
...
BEGIN
  ...
  WHERE sh.machine_id = p_machine_id
    ...
    AND sh.pod_product_id = v_pod_id;
