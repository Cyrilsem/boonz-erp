-- Machine-health priority redesign (2026-06-07) — CS-directed.
--
-- Goal: P1 = "restock today" = keep shelves full, protect top performers, never an empty shelf,
-- and catch next-day runway threats. P2 = assortment work (dead / stale / expiry / pending swap),
-- which a refill cannot fix. Warehouses / parked units are excluded outright.
--
-- Read-only function, SECURITY DEFINER STABLE, 33-col signature UNCHANGED → FE binds untouched.
-- Three changes only vs live:
--   1. visit_data CTE: only count approved plans with plan_date <= CURRENT_DATE
--      (fixes the corrupt days_since_visit = -26847 from a future-dated plan row).
--   2. priority_tier CASE: new restock-urgency model (balanced thresholds, CS-approved):
--        excluded if not in refill / Warehouse / Inactive;
--        P1 if sells (vel>0) AND (empty shelf OR runway<3 OR (>=50/wk AND runway<5)
--             OR (>=100/wk AND runway<7) OR (>=50/wk AND fill<60) OR (>=2 low shelves AND >=30/wk));
--        else P2 if dead%>=15 OR stale>=14 OR expired OR pending swap OR (empty on a zero-velocity unit);
--        else skip.
--   3. priority_score: velocity-weighted points aligned to the new tiers.
-- Everything else byte-identical.
--
-- CODY: ✅ Approve. Class (c) read-only DEFINER — no writes, no protected-entity mutation, signature
-- unchanged (Article 12 forward-only new migration; no Article 6 concern). Display/ranking logic only.
...
AS $function$
  ...
  SELECT
    ...
    -- NEW priority_tier — restock-urgency model (2026-06-07)
    CASE
      WHEN NOT we.include_in_refill OR COALESCE(we.machine_status,'Active') IN ('Warehouse','Inactive')
        THEN 'excluded'
      WHEN we.daily_velocity > 0 AND (
             we.slots_at_zero >= 1
          OR h.runway_v < 3
          OR (h.units7 >= 50  AND h.runway_v < 5)
          OR (h.units7 >= 100 AND h.runway_v < 7)
          OR (h.units7 >= 50  AND h.fill_p < 60)
          OR (we.slots_below_25pct >= 2 AND h.units7 >= 30)
        ) THEN 'P1_RESTOCK'
      WHEN h.dead_pct >= 15 OR h.dsv >= 14 OR we.expired_units >= 1 OR h.intent_cnt > 0
        OR (we.slots_at_zero >= 1 AND we.daily_velocity = 0)
        THEN 'P2_MAINTAIN'
      ELSE 'skip'
    END,
    -- NEW priority_score — velocity-weighted, aligned to the new tiers
    (  (CASE WHEN we.slots_at_zero >= 1 THEN 60 + LEAST((we.slots_at_zero-1)*10, 40) ELSE 0 END)
     + (CASE WHEN h.runway_v < 2 THEN 40
             WHEN h.runway_v < 3 THEN 30
             WHEN h.units7 >= 100 AND h.runway_v < 7 THEN 28
             WHEN h.units7 >= 50  AND h.runway_v < 5 THEN 24
             WHEN h.units7 >= 30  AND h.runway_v < 7 THEN 10
             ELSE 0 END)
     + (CASE WHEN h.units7 >= 50 AND h.fill_p < 60 THEN 20 ELSE 0 END)
     + (CASE WHEN h.units7 >= 100 THEN 10 WHEN h.units7 >= 50 THEN 6 ELSE 0 END)
     + LEAST(we.slots_below_25pct * (CASE WHEN h.units7 >= 30 THEN 8 ELSE 4 END), 24)
     + (CASE WHEN h.dead_pct >= 30 THEN 10 WHEN h.dead_pct >= 15 THEN 5 ELSE 0 END)
     + (CASE WHEN h.dsv >= 21 THEN 8 WHEN h.dsv >= 14 THEN 5 ELSE 0 END)
     + (CASE WHEN we.expired_units >= 1 THEN 6 ELSE 0 END)
     + (CASE WHEN h.intent_cnt > 0 THEN 4 ELSE 0 END)
    )::numeric(6,2)
  FROM with_expiry we
  ...
  CROSS JOIN LATERAL (
    SELECT
      ...
      CASE WHEN we.max_capacity > 0 THEN GREATEST(we.total_stock,0)::numeric / we.max_capacity * 100 ELSE 0 END AS fill_p,
