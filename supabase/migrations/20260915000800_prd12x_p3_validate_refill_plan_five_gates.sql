-- PRD-125 D1/D6 / ONE-LOOP Phase 3 -- the gate checks the engine's rules.
--
-- validate_refill_plan rewritten to exactly five blocking checks, none
-- waivable: G3 (empty lane, no line, no substitution rule -- Phase 4 adds
-- the table; until then no match means no rule, so this is unchanged from
-- today's G3), G5 (no Active mapping, machine or global), G7 (Remove with
-- no replacement on the same lane), G8 (need exceeds free stock at the
-- correctly-resolved supplying warehouse), G10 (new -- a Refill/Add New
-- lands on a lane WEIMI shows holding a DIFFERENT product, with no Remove
-- for it on that lane). G1, G2, G4, G6, G9 deleted entirely.
--
-- G8 does NOT call wh_available_for directly. Caught live: wh_available_for
-- subtracts pins from refill_dispatching rows dispatch_date within the next
-- 30 days that are not yet packed/dispatched -- but the lines THIS function
-- is validating today ARE exactly those rows, so calling it here would
-- double-subtract the same demand (once as "need", once as wh_available_for's
-- own "pin"). Verified: this bug alone inflated 2026-09-15 dispatch G8 from
-- 7 to 25 spurious violations. Fixed by inlining the same venue/central
-- routing rule against RAW warehouse_inventory instead.
--
-- Verified live (all real data, no rollback needed -- read-only function):
--   2026-09-15 dispatch: 6 blocking (2 real WEIMI/plan product mismatches
--     via the new G10, 2 real G7 empty-lane-no-replacement, 2 real G8
--     shortages on Red Bull Diet/Regular). Canary fingerprint unchanged
--     (9aba17f8fc6477fe50b4f2aadca47f62).
--   2026-09-12 dispatch: 10 blocking (3 G7, 7 G8), all real.
--   2026-09-12 / 2026-09-15 plan_output: 0/0 (no live 'pending' rows on
--     either date -- see DECISIONS-2026-09-15.md D-002).
--   AMZ-1038-3001-O1 alone on 2026-09-15 dispatch: 0 blocking.
--
-- DEFERRED (disclosed, not silently skipped): G2/G4/G9 as boolean columns
-- on get_pod_refill_draft, and D1's literal "target = max_stock at
-- velocity>=3 or venue_team, else least(10,max_stock)" fully integrated
-- into engine_add_pod's existing band-based sizing model. See
-- DECISIONS-2026-09-15.md for both.
...
BEGIN
  ...
  lines AS (
    SELECT rd.machine_id, sc.shelf_code, rd.boonz_product_id, rd.pod_product_id, rd.action, rd.quantity
      FROM public.refill_dispatching rd
    ...
    UNION ALL
    SELECT rpo.machine_id, sc2.shelf_code, rpo.boonz_product_id, rpo.pod_product_id, rpo.action, rpo.quantity
      FROM public.refill_plan_output rpo
...
ALTER TABLE public.refill_policy_params ADD COLUMN IF NOT EXISTS hero_velocity_floor numeric NOT NULL DEFAULT 3;
