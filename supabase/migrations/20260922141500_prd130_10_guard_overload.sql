-- PRD-130 step 10: G-OVERLOAD, a permanent guard against the exact class of bug prd130_09
-- fixed (add_dispatch_row briefly had two overloads because CREATE OR REPLACE with a changed
-- argument count creates a new function instead of replacing the old one). Checks every
-- canonical writer CS named for more than one overload in pg_proc. Expect 0.
