-- Forward fix of p0_fix8: warehouse_inventory has no updated_at column.
-- Idempotency guard now blocks only duplicate PENDING proposals; an old
-- confirmed Inactive proposal from a prior cycle (row since reactivated)
-- must not block a fresh sweep of a currently-Active zero-stock row.
