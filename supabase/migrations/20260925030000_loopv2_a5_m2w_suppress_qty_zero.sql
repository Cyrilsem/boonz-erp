-- Loop 2026-09-25 A5 (G6): engine_finalize_pod must not leave auto-suppressed M2W/REMOVE rows
-- with qty>0 marked superseded. It must write qty 0 decision rows instead.
--
-- Root cause confirmed live before writing anything: the auto-suppress UPDATE in
-- engine_finalize_pod (the "IF v_m2w_suppressed_n > 0 THEN ... UPDATE public.pod_refill_plan"
-- block) sets status='superseded' and stamps the reasoning with auto_suppressed metadata, but
-- never touches qty. Confirmed on the named evidence: VML-1004-0500-O1 A03, Red Bull, plan_date
-- 2026-09-25, action M2W, qty 12, status superseded, reasoning.auto_suppressed = 'no replacement
-- for shelf'. Any downstream consumer scanning pod_refill_plan for open M2W quantity by qty alone
-- (rather than also filtering status) sees 12 units still queued to leave the shelf on a row the
-- engine itself already decided was a no-op.
--
-- Fix: the same UPDATE now also sets qty = 0, so a superseded auto-suppressed row plainly carries
-- zero quantity, matching what "auto suppressed, no replacement" actually means (decided not to
-- move any units), not just a status flag next to a stale nonzero qty.
--
-- Surgical: only the one UPDATE statement in the v_m2w_suppressed_n > 0 branch changes (adds
-- qty = 0 to its SET list) and the auto_suppressed_by / engine_version tags are bumped so this
-- version is distinguishable in reasoning and output. No other logic in the function changes.
-- Plan_date 2026-09-25's own already-superseded VML-1004 A03 row is NOT touched by this migration
-- (per the hard rule against touching that plan_date outside step A2); this is a forward-only fix
-- for future finalize runs.
--
-- Smoke call (rolled back, 2026-09-25 before apply): ran the fixed function body inside
-- BEGIN/ROLLBACK against a synthetic plan_date with a seeded orphan M2W row (qty 9, no
-- replacement shelf) and confirmed the row lands with status='superseded' AND qty=0. Result in
-- STATE.md.
...
BEGIN
  ...
  inserted AS (
    ...
    SELECT u.plan_date, u.machine_id, u.shelf_id, u.pod_product_id, u.action,
           u.qty, u.reasoning, u.decision, u.linked_refill_pk, u.linked_swap_id, 'draft',
           -- v15.1: machine-scoped Active mappings, when present, are the
           -- authoritative variant set (globals ignored); global defaults apply
           -- only when the machine has no scoped rows for this pod product.
           -- vox_at_venue only when ALL rows in the chosen set are venue_team.
           COALESCE((
             SELECT CASE WHEN bool_and(x.source_of_supply = 'venue_team')
                         THEN 'vox_at_venue'::public.source_origin_enum
                         ELSE 'warehouse'::public.source_origin_enum END
               FROM (
                 SELECT pm.source_of_supply
                   FROM public.product_mapping pm
                  WHERE pm.pod_product_id = u.pod_product_id
                    AND pm.status = 'Active'
                    AND CASE WHEN EXISTS (
                               SELECT 1 FROM public.product_mapping pm2
                                WHERE pm2.pod_product_id = u.pod_product_id
                                  AND pm2.status = 'Active'
                                  AND pm2.machine_id = u.machine_id)
                         THEN pm.machine_id = u.machine_id
  ...
  IF v_m2w_suppressed_n > 0 THEN
    ...
    -- Loop 2026-09-25 A5: qty = 0 added to this SET list. A row the engine has decided has no
    -- replacement shelf and is being auto-suppressed must carry zero quantity, not its original
    -- nonzero qty next to a status flag -- confirmed live on VML-1004-0500-O1 A03 (Red Bull,
    -- 2026-09-25, qty 12, status superseded, still reading qty 12 after suppression).
    UPDATE public.pod_refill_plan prp
       SET status     = 'superseded',
           ...
           reasoning  = COALESCE(prp.reasoning, '{}'::jsonb)
                        || jsonb_build_object(
                             ...
                             'auto_suppressed_prior_qty', prp.qty),
           updated_at = now()
     WHERE prp.plan_date = p_plan_date
       AND prp.status    = 'draft'
       AND prp.action    IN ('M2W','REMOVE')
  ...
  RETURN jsonb_build_object(
    ...
    'engine_version', 'v15_2_loopv2_a5_m2w_qty_zero',
