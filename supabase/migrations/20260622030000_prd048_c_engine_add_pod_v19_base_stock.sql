-- PRD-048 Step C: engine_add_pod v19_base_stock — flag-gated base-stock ADD sizing.
-- SCOPE: covered/flagged CTEs sizing only. NO change to engine_swap_pod, picker, stitch, dispatch.
-- INVARIANT: refill_policy_params.refill_sizing_mode='legacy' => output byte-identical to v18 (gate-clean).
--   Achieved by: (a) compute_refill_decision left UNTOUCHED; (b) every base_stock-only column/string
--   is CASE-gated on v_mode and short-circuits in legacy; (c) DEAD rule unchanged.
-- Dara (✅ inputs) -> Cody review -> Supabase MCP apply. NOT git-committed in this pass. NO prod enable.
...
DECLARE
  ...
  -- PRD-048 base-stock params (read once; legacy path ignores all of them)
  v_mode             text;
  v_minfill          numeric;
  v_sellwk           numeric;
  v_w7               numeric;
...
BEGIN
  ...
  -- PRD-048: read sizing mode + params (default legacy if table/row absent)
  ...
  candidates AS (
    SELECT
      ...
      -- PRD-048 base-stock inputs (CASE-gated -> legacy short-circuits, no eval, no fan-out)
      ...
      CASE WHEN v_mode = 'base_stock' THEN (
             SELECT NULLIF(GREATEST((MIN(wi.expiration_date) - CURRENT_DATE), 0), 0)::numeric
             FROM public.product_mapping pm
             JOIN public.warehouse_inventory wi
               ON wi.boonz_product_id = pm.boonz_product_id AND wi.status = 'Active' AND wi.quarantined = false
              AND wi.expiration_date >= CURRENT_DATE
              AND wi.warehouse_id = ANY (ARRAY[mwh.primary_warehouse_id, mwh.secondary_warehouse_id])
             WHERE pm.pod_product_id = sl.pod_product_id AND pm.status = 'Active'
           )
    ...
    SELECT r.*,
      CASE WHEN v_mode = 'base_stock' THEN
        public.compute_base_stock_decision(
          r.v7, r.v30, r.current_stock, r.max_stock,
      ...
      ELSE
        CASE
          ...
          ELSE GREATEST(ROUND(COALESCE(b.u_velocity,0) * p_days_cover
                 * (CASE b.machine_band WHEN 1 THEN 1.00 WHEN 2 THEN 0.60 ELSE 0.30 END))::int, 1)
        END
      END AS cover_units
    FROM bs b
  ),
