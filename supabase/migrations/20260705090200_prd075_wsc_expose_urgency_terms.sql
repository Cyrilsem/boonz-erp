-- PRD-075 WS-C: expose s_runout / s_capacity / s_expiry / s_stale as v_machine_priority
-- output columns (values already computed in mscore - COLUMN EXPOSURE ONLY, no logic
-- change; p_tier + urgency proven byte-identical for every machine in a rolled-back
-- full-fleet diff, see PRD-075-EXECUTION-LOG.md). Closes the PRD-074 lumped-core-chip
-- carry-forward. Body otherwise VERBATIM the PRD-073 v2 body. View md5 change EXPECTED
-- and recorded in MIGRATIONS_REGISTRY (before a49cd7d37e1ebf088f36351d54f646ac).
...
SELECT
  ...
  -- PRD-075 WS-C: pure column EXPOSURE of terms already computed in mscore
