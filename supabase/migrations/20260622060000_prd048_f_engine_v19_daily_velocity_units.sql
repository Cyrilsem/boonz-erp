-- PRD-048 Step 2 fix: engine_add_pod v19 — velocity UNITS correction.
-- slot_lifecycle.velocity_7d/velocity_30d are DAILY rates (verified: velocity_7d = units_7d/7).
-- compute_base_stock_decision (PRD-048 3) expects WINDOW TOTALS (it divides by 7 and 30).
-- So the engine must pass v7*7 and v30*30. WITHOUT this, mu_day was ~7-10x too small and
-- base_stock badly under-sized (scratch proof: 1 refill vs legacy 28 on seller-heavy machines).
-- ONLY the base_stock bs CTE call changes vs the prior v19. Legacy branch byte-identical (gate-clean).
-- Forward-only CREATE OR REPLACE (Article 12). Cody -> MCP. swaps_enabled untouched.
...
BEGIN
  ...
  candidates AS (
    SELECT
      ...
      -- PRD-048 Step 2: shelf-life now reads the CANONICAL v_product_shelf_life (Article 16),
      -- not an inline warehouse_inventory predicate.
    ...
    SELECT r.*,
      CASE WHEN v_mode = 'base_stock' THEN
        public.compute_base_stock_decision(
          r.v7 * 7.0, r.v30 * 30.0, r.current_stock, r.max_stock,   -- velocity_7d/30d are DAILY rates; helper expects WINDOW TOTALS (PRD-048 §3)
