...
export default function PackingDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const fifoMap: Record<string, { allocations: BatchAllocation[]; primary_expiry: string | null }> = {}
    ...
    const mapped: PackLine[] = dispatchLines.map((line) => {
      ...
      const fifo = fifoMap[line.dispatch_id] ?? { allocations: [], primary_expiry: null }
      ...
      return {
        ...
        packed_qty: (line.filled_quantity as number | null) ?? (line.quantity ?? 0),
  ...
  function updatePackedQty(dispatchId: string, value: number) {
    setLines((prev) =>
      prev.map((l) => l.dispatch_id === dispatchId ? { ...l, packed_qty: Math.max(0, value) } : l)
    )
  }

  function handleMarkAllPacked() {
    setLines((prev) =>
      prev.map((l) => ({ ...l, action: 'packed' as LineAction, packed_qty: l.recommended_qty }))
    )
  }
            ...
            {shelfLines.map((line) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mb-0.5 text-sm font-medium">{line.pod_product_name}</p>
                  ...
                  {!hasStock ? (
                    ...
                    <p className="mb-2 text-xs">
                      <span className="font-bold">Qty: {line.allocations[0].qty}</span>
                      {'  '}
                      <span className="font-bold">Expiry:</span>{' '}
                      <span className={getExpiryStyle(line.allocations[0].expiry_date).qtyColor}>
                  ...
                  <p className="mb-2 text-xs">
                    <span className={stockColor(line.warehouse_stock, line.recommended_qty)}>
                  ...
                  <div className="mb-2 flex items-center gap-2">
                    <label className="text-xs text-neutral-500">Packed qty:</label>
                    <input
                      type="number"
                      min={0}
                      value={line.packed_qty}
                      onChange={(e) =>
                        updatePackedQty(line.dispatch_id, parseFloat(e.target.value) || 0)
                      }
                      disabled={isReadOnly}
