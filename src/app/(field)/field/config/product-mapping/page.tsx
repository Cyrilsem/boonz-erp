...
let _k = 0;
const nk = () => `k${++_k}`;
const aKey = (podId: string, machineId: string | null) =>
  `${podId}|||${machineId ?? "__global__"}`;

const MAPPING_SEL =
  "mapping_id, pod_product_id, boonz_product_id, machine_id, split_pct, status, pod_products!inner(pod_product_name), boonz_products!inner(boonz_product_name), machines(official_name)";
...
export default function ProductMappingPage() {
  ...
  const [addSplits, setAddSplits] = useState<
    { key: string; boonz_product_id: string; split_pct: number }[]
  >([]);
  ...
  const loadMappingsForMachine = useCallback(
  ...
  const byProductGroups = useMemo<PodGroup[]>(() => {
    ...
    for (const r of mappings.filter((r) => r.status === "Active")) {
      ...
      if (g) {
        g.total_pct += r.split_pct;
        g.split_count++;
      } else
        map.set(r.pod_product_id, {
          pod_product_id: r.pod_product_id,
          pod_product_name: r.pod_product_name,
          total_pct: r.split_pct,
          split_count: 1,
        });
    }
    return [...map.values()].sort((a, b) =>
      a.pod_product_name.localeCompare(b.pod_product_name),
    );
  }, [mappings]);
  ...
  const filteredProductGroups = useMemo(() => {
    ...
    return byProductGroups.filter((g) =>
      g.pod_product_name.toLowerCase().includes(q),
    );
  ...
  const byMachineGroups = useMemo<MachineSection[]>(() => {
    const machineMap = new Map<
      string,
      {
        machine_id: string | null;
        machine_name: string;
        pods: Map<string, PodGroup>;
      }
    >();
    for (const r of mappings.filter(
      (m) => m.status === "Active" && m.machine_id !== null,
    )) {
      ...
      if (g) {
        ...
        section.pods.set(r.pod_product_id, {
          pod_product_id: r.pod_product_id,
          pod_product_name: r.pod_product_name,
          total_pct: r.split_pct,
          split_count: 1,
    ...
    const sections: MachineSection[] = [...machineMap.values()].map((s) => ({
      ...
      pod_groups: [...s.pods.values()].sort((a, b) =>
        a.pod_product_name.localeCompare(b.pod_product_name),
      ),
    }));
  ...
  const filteredMachineSections = useMemo(() => {
    ...
    return byMachineGroups
      .map((s) => ({
        ...s,
        pod_groups: s.pod_groups.filter((g) =>
          g.pod_product_name.toLowerCase().includes(q),
        ),
      }))
  ...
  const filteredPodProducts = useMemo(
    () =>
      podProducts.filter((p) =>
        p.pod_product_name.toLowerCase().includes(addPodSearch.toLowerCase()),
      ),
  ...
  function toggleAccordion(podId: string, machineId: string | null) {
    ...
    const rows =
      machineId === null
        ? mappings.filter(
            (r) => r.pod_product_id === podId && r.machine_id === null,
          )
        : mappings.filter(
            (r) => r.pod_product_id === podId && r.machine_id === machineId,
          );
    setSplitDrafts((prev) => ({
      ...prev,
      [key]: rows.map((r) => ({
  ...
  function patchSplit(
    draftKey: string,
    splitKey: string,
    patch: Partial<SplitDraft>,
  ) {
    setSplitDrafts((prev) => ({
      ...prev,
      [draftKey]: prev[draftKey].map((s) =>
        s.key === splitKey ? { ...s, ...patch } : s,
      ),
    }));
  }
  ...
  async function saveMapping(podId: string, machineId: string | null) {
    ...
    try {
      ...
      const freshRows: MappingRow[] = (
        (freshRaw ?? []) as unknown as RawRow[]
      ).map((r) => ({
        mapping_id: r.mapping_id,
        pod_product_id: r.pod_product_id,
        pod_product_name: r.pod_products.pod_product_name,
  ...
  async function handleAddCreate() {
    if (!addPodId) {
      setAddError("Select a pod product");
      return;
    }
    if (addSplits.length === 0) {
      setAddError("Add at least one split row");
      return;
    }
    if (addSplits.some((s) => !s.boonz_product_id)) {
      setAddError("All rows must have a boonz product selected");
      return;
    }
    ...
    async function prefetch() {
      ...
      if (data && data.length > 0) {
        setAddSplits(
          data.map((r) => ({
            key: nk(),
            boonz_product_id: r.boonz_product_id as string,
            split_pct: r.split_pct as number,
          })),
        );
        setAddIsUpdate(true);
      } else {
        setAddSplits([
          {
            key: nk(),
            boonz_product_id: boonzProducts[0]?.product_id ?? "",
            split_pct: 100,
          },
        ]);
  ...
  const getMachineName = (machineId: string | null) =>
    machineId === null
      ? "Global (all machines)"
      : (machines.find((m) => m.machine_id === machineId)?.official_name ??
        machineId);
        ...
        {isOpen && (
          <div className="space-y-3 border-t border-neutral-100 px-4 pb-4 pt-3 dark:border-neutral-800">
            <p className="text-xs text-neutral-500">
              {machineId
                ? `Machine: ${getMachineName(machineId)}`
              ...
              {activeDrafts.map((s) => (
                <div key={s.key} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      patchSplit(key, s.key, {
                        split_pct: parseFloat(e.target.value) || 0,
                      })
                    }
              ...
              {deletedDrafts.map((s) => (
                <div key={s.key} className="flex items-center gap-2 opacity-40">
                  <span className="flex-1 truncate text-xs line-through text-neutral-400">
                    {boonzProducts.find(
                      (b) => b.product_id === s.boonz_product_id,
                    )?.boonz_product_name ?? s.boonz_product_id}
                  </span>
                  <button
              ...
              {bulkOpen && (
                <div className="mt-2 space-y-2">
                  <p className="text-xs text-neutral-400">
                    Select machines to copy these splits to (replaces existing):
                  </p>
                  {machines
                  ...
                  {bulkSelected.size > 0 && !bulkConfirm && (
                    ...
                    >
                      Apply to {bulkSelected.size} machine
                  ...
                  {bulkConfirm && (
                    <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 dark:border-amber-700 dark:bg-amber-900/20">
                      <p className="text-xs font-medium text-amber-800 dark:text-amber-300">
                        Apply these splits to {bulkSelected.size} machine
                        {bulkSelected.size !== 1 ? "s" : ""}? This will replace
            ...
            <>
              ...
              <ul className="space-y-2 px-4">
                ...
                {filteredProductGroups.map((g) =>
                  renderPodRow(g, selectedMachineId),
                )}
              </ul>
            </>
          )}
                ...
                <div key={section.machine_id ?? "__global__"}>
                  ...
                  <div className="mb-2 flex items-center gap-3">
                    ...
                    <span className="shrink-0 rounded-full bg-neutral-100 px-2 py-0.5 text-xs text-neutral-500 dark:bg-neutral-800">
                      {section.pod_groups.length} product
                      {section.pod_groups.length !== 1 ? "s" : ""}
                    </span>
                    <div className="h-px flex-1 bg-neutral-200 dark:bg-neutral-800" />
                  </div>
                  <ul className="space-y-2">
                    {section.pod_groups.map((g) =>
                      renderPodRow(g, section.machine_id),
                    )}
                  </ul>
                </div>
              ))}
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setAddSplits((prev) =>
                            prev.map((r) =>
                              r.key === s.key
                                ? { ...r, boonz_product_id: e.target.value }
                                : r,
                            ),
                          )
                        }
