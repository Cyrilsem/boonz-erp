...
export default function ProductNamingPage() {
  ...
  const officialGroups = useMemo<OfficialGroup[]>(() => {
    ...
    return [...map.entries()]
      .map(([official_name, originals]) => ({ official_name, alias_count: originals.size }))
  ...
  const filteredGroups = useMemo(() => {
    ...
    return officialGroups.filter(g => g.official_name.toLowerCase().includes(q))
  ...
  async function addAlias(officialName: string) {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle()
    if (existing) { setAddAliasError('Already exists'); setAddingAlias(false); return }
  ...
  async function renameOfficialName(oldName: string) {
    ...
    const clash = officialGroups.some(
      g => g.official_name.toLowerCase() === newName.toLowerCase() && g.official_name !== oldName
    )
    if (clash) { setRenameError('An official name with this value already exists'); return }
    ...
    if (error) { setRenameError(error.message); setRenameSaving(false); return }
  ...
  async function handleAddCreate() {
    ...
    if (!name) { setAddError('Official name is required'); return }
    const exists = officialGroups.some(g => g.official_name.toLowerCase() === name.toLowerCase())
    if (exists) { setAddError('Already exists'); return }
    setAdding(true)
    setAddError(null)
    const today = new Date().toISOString().slice(0, 10)
    const supabase = createClient()
    const rows: { original_name: string; official_name: string; mapped_at: string }[] = [
      { original_name: name, official_name: name, mapped_at: today },
    ]
    if (addFirstAlias.trim()) {
      rows.push({ original_name: addFirstAlias.trim(), official_name: name, mapped_at: today })
    }
  ...
  return (
    <div className="pb-24">
      <FieldHeader
        ...
        rightAction={
          <button
            onClick={() => { setShowAdd(true); setAddOfficialName(''); setAddFirstAlias(''); setAddError(null) }}
      ...
      <div className="space-y-2 px-4 py-3">
        ...
        <p className="text-xs text-neutral-400">
          {officialGroups.length} official name{officialGroups.length !== 1 ? 's' : ''}
        </p>
      </div>
        ...
        {filteredGroups.map(g => {
          ...
          return (
            ...
            >
              ...
              >
                <p className="max-w-[70%] truncate text-sm font-semibold">{g.official_name}</p>
              ...
              {isOpen && (
                <div className="space-y-4 border-t border-neutral-100 px-4 pb-4 pt-3 dark:border-neutral-800">
                  ...
                  {!renaming ? (
                    <button
                      onClick={() => { setRenaming(true); setRenameValue(g.official_name); setRenameError(null) }}
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium text-amber-600">
                        This will update all {aliases.length} alias{aliases.length !== 1 ? 'es' : ''} to use the new name.
                      </p>
                  ...
                  <div className="space-y-1">
                    {aliasLoading ? (
                      <p className="text-xs text-neutral-400">Loading aliases…</p>
                    ) : aliases.length === 0 ? (
                      ...
                      aliases.map(a => (
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-xs font-medium">{a.original_name}</p>
                            {a.mapped_at && (
                              <p className="text-xs text-neutral-400">{a.mapped_at.slice(0, 10)}</p>
                            )}
                          </div>
                          <button
                  ...
                  {/* Add alias */}
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-neutral-500">Add alias</p>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={newAlias}
                        ...
                        onKeyDown={e => { if (e.key === 'Enter') addAlias(g.official_name) }}
                        className="flex-1 rounded border border-neutral-300 px-2 py-1.5 text-sm dark:border-neutral-600 dark:bg-neutral-900"
                      />
                      <button
          ...
          <div className="relative z-10 max-h-[85vh] overflow-y-auto rounded-t-3xl bg-white px-4 pb-10 pt-5 dark:bg-neutral-900">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mb-1 text-xs text-neutral-400">If this name appears differently in raw data, add it here.</p>
                <input
                  type="text"
                  value={addFirstAlias}
