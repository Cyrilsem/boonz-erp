...
function getGroupKey(row: PodRow, groupBy: Exclude<GroupBy, "none">): string {
  switch (groupBy) {
    case "machine":
      return row.machines?.official_name ?? "—";
    case "product":
      return row.boonz_products?.boonz_product_name ?? "—";
    case "category":
      return row.boonz_products?.product_category ?? "Uncategorised";
  }
}
    ...
    >
      ...
      <div className="min-w-0 flex-1 pr-3">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          ...
          {style.label && (
            <span
              className={`rounded-full ${style.badgeBg} px-3 py-1 text-sm font-semibold ${style.badgeText}`}
            >
              {style.label}
            </span>
          )}
...
function rowProps(
  groupBy: GroupBy,
): Omit<PodRowItemProps, "row" | "isPending" | "onClick"> {
  switch (groupBy) {
    case "machine":
      return { showProduct: true, showMachine: false, showCategory: true };
    case "product":
      return { showProduct: false, showMachine: true, showCategory: false };
    case "category":
      return { showProduct: true, showMachine: true, showCategory: false };
    case "none":
      return { showProduct: true, showMachine: true, showCategory: false };
  }
}
...
export default function PodInventoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (r) =>
          (r.boonz_products?.boonz_product_name ?? "")
            .toLowerCase()
            .includes(q) ||
    ...
    return [...result].sort((a, b) => {
      ...
      if (sortField === "expiry") {
        ...
        if (da === null && db === null)
          return (a.machines?.official_name ?? "—").localeCompare(
      ...
      if (sortField === "qty") {
        ...
        return (a.boonz_products?.boonz_product_name ?? "—").localeCompare(
          b.boonz_products?.boonz_product_name ?? "—",
        );
      }
      // sortField === 'product'
      const cmp = (a.boonz_products?.boonz_product_name ?? "—").localeCompare(
        b.boonz_products?.boonz_product_name ?? "—",
      );
  ...
  const machineOptions = useMemo((): string[] => {
    const names = new Set(
      filtered.map((r) => r.machines?.official_name ?? "—"),
    );
  ...
  const machineFiltered = useMemo((): PodRow[] => {
    ...
    return filtered.filter(
      (r) => (r.machines?.official_name ?? "—") === selectedMachine,
    );
  ...
  const groups = useMemo((): DisplayGroup[] => {
    ...
    const built: DisplayGroup[] = Array.from(map.entries()).map(
      ([key, items]) => {
        const sorted = [...items].sort((a, b) => {
          if (a.expiration_date === null && b.expiration_date === null) {
            return groupBy === "product"
              ? (a.machines?.official_name ?? "—").localeCompare(
                  b.machines?.official_name ?? "—",
                )
              : (a.boonz_products?.boonz_product_name ?? "—").localeCompare(
                  b.boonz_products?.boonz_product_name ?? "—",
                );
          }
          ...
          <div className="relative z-10 max-h-[90vh] overflow-y-auto rounded-t-2xl bg-white px-4 pt-5 pb-10 shadow-xl dark:bg-neutral-900">
            ...
            <div className="mb-4">
              <p className="text-base font-bold">
                {selectedRow.boonz_products?.boonz_product_name ?? "—"}
              </p>
              <p className="text-sm text-neutral-500">
                {selectedRow.machines?.official_name ?? "—"}
              </p>
              <p className="mt-1 text-xs text-neutral-400">
                {selectedRow.current_stock} units · expires{" "}
                {formatDate(selectedRow.expiration_date)}
              </p>
            </div>
            ...
            <div className="mb-4 space-y-2">
              ...
              >
                ...
                <p className="text-xs text-neutral-500">
                  Product is present and not expired
                </p>
              </button>
              {editType === "in_stock" && (
                <div className="ml-4">
                  ...
                  />
                  <p className="mt-1 text-xs text-neutral-400">
                    Current stock — adjust if needed
                  </p>
                </div>
              )}
              ...
              >
                ...
                <p className="text-xs text-neutral-500">
                  All units purchased by a customer
                </p>
              </button>
              {editType === "sold" && (
                <div className="ml-4">
              ...
              >
                ...
                <p className="text-xs text-neutral-500">
                  Product is damaged or unusable
                </p>
              </button>
              {editType === "damaged" && (
                <div className="ml-4 space-y-3">
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-neutral-400">
                      Enter how many units were damaged
                    </p>
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-neutral-500">
              ...
              >
                ...
                <p className="text-xs text-neutral-500">
                  Item removed from machine due to expiry
                </p>
              </button>
              {editType === "expired" && (
                <div className="ml-4">
              ...
              >
                ...
                <p className="text-xs text-neutral-500">
                  Remove from machine — return stock to warehouse
                </p>
              </button>
              {editType === "return_to_warehouse" && (
                <div className="ml-4">
                  ...
                  />
                  <p className="mt-1 text-xs text-neutral-400">
                    (pre-filled — full stock being returned)
                  </p>
                </div>
              )}
