...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setProfile(data ? { full_name: data.full_name, role: data.role } : { full_name: null, role: 'field_staff' })
      setLoading(false)
    }
    load()
  }, [])
    ...
    <div>
      ...
      <div className="p-4 space-y-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 capitalize">{profile?.role?.replace('_', ' ') ?? 'Field Staff'}</p>
        </div>
        <button
