...
export default function DispatchingDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (dispatchLines) {
      ...
      const allResolved = mapped.length > 0 && mapped.every((l) => l.action !== null)
  ...
  async function runInventoryUpdates(line: {
    ...
    try {
      ...
      for (const batch of batches ?? []) {
        ...
        log('WH: deducted ' + deduct + ' from batch expiry=' + batch.expiration_date)
      }
    } catch (err) {
  ...
  function updateReturnReason(dispatchId: string, reason: string) {
    setLines((prev) =>
      prev.map((l) => l.dispatch_id === dispatchId ? { ...l, return_reason: reason } : l)
    )
  }

  function updateFilledQty(dispatchId: string, value: number) {
    setLines((prev) =>
      prev.map((l) => l.dispatch_id === dispatchId ? { ...l, filled_qty: value } : l)
    )
  }
  ...
  function handleMarkAllAdded() {
    setLines((prev) =>
      prev.map((l) => ({ ...l, action: 'added' as LineAction, filled_qty: l.quantity }))
    )
  }
  ...
  return (
    <div className="px-4 py-4 pb-40">
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-2 flex items-start justify-between gap-2">
                    <p className="text-sm font-medium">{line.pod_product_name}</p>
                    {line.expiry_date && (
                      <span className={`shrink-0 text-xs ${expiryStyle.qtyColor}`}>
                  ...
                  {invWarning && (
                    <p className="mb-2 text-xs text-amber-600 dark:text-amber-400">{invWarning}</p>
                  )}

                  {/* Qty */}
                  <div className="mb-2 flex items-center gap-2">
                    <span className="text-xs text-neutral-500">Planned: {line.quantity}</span>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateFilledQty(line.dispatch_id, parseFloat(e.target.value) || 0)
                      }
                      disabled={isReadOnly}
                  ...
                  {!isReadOnly && (
                    <div className="mb-2 flex gap-2">
                      ...
                      <button
                        onClick={() => updateAction(line.dispatch_id, 'returned')}
                        className={`flex-1 rounded-lg border py-1.5 text-xs font-semibold transition-colors ${
                          line.action === 'returned'
                  ...
                  {line.action === 'returned' && !isReadOnly && (
                    <div className="mb-2">
                      <label className="mb-0.5 block text-xs text-neutral-500">Return reason</label>
                      <select
                        value={line.return_reason}
                        onChange={(e) => updateReturnReason(line.dispatch_id, e.target.value)}
                  ...
                  {line.action === 'returned' && isReadOnly && line.return_reason && (
      ...
      <div className="fixed bottom-14 left-0 right-0 border-t border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
        {isReadOnly ? (
          <button
            onClick={() => { setSaved(false); setEditingAfterSave(true) }}
