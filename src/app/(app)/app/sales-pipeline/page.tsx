    ...
    >
      ...
      >
        {lead.next_follow_up_date ? (
          <div
            style={{ fontSize: 10, ...followUpStyle(lead.next_follow_up_date) }}
          >
            {overdueFollowUp ? "⚠️ " : "📅 "}Follow-up{" "}
            {fmt(lead.next_follow_up_date)}
          </div>
...
function LeadDrawer({
  ...
  const [form, setForm] = useState<
    Omit<SalesLead, "id" | "created_at" | "updated_at">
            ...
            {editMode && (
              <>
                ...
                <button
                  ...
                  style={{
                    background: saving ? "#d1d5db" : "#24544a",
                    border: "none",
                    borderRadius: 8,
                    padding: "6px 14px",
                    fontSize: 12,
            ...
            {editMode ? (
              <>
                <SelectField
                  ...
                  onChange={(v) => setField("funnel_stage", v)}
                />
                <SelectField
                  label="Engagement"
                  value={form.engagement_status}
                  options={ENGAGEMENT_STATUSES}
                  onChange={(v) => setField("engagement_status", v)}
                />
                <SelectField
                  label="Priority Order"
                  value={String(form.priority_order ?? 5)}
                  options={PRIORITY_ORDERS.map(String)}
                  onChange={(v) => setField("priority_order", Number(v))}
                />
                <SelectField
                  label="Lead Owner"
                  value={form.lead_owner ?? ""}
                  options={["CS", "RK", "CE", "HR", "CS/RK"]}
                  onChange={(v) => setField("lead_owner", v)}
                />
              </>
            ) : (
              <>
                <ViewField label="Funnel Stage">
                  {stageBadge(form.funnel_stage)}
                </ViewField>
                <ViewField label="Engagement">
                  {engagementBadge(form.engagement_status)}
                </ViewField>
                <ViewField label="Priority">
                  {PRIORITY_LABELS[form.priority_order ?? 5] ??
                    `Tier ${form.priority_order}`}
                </ViewField>
                <ViewField label="Lead Owner">
                  {ownerBadge(form.lead_owner)}
                </ViewField>
              </>
            )}
            ...
            {editMode ? (
              <>
                ...
                <InputField
                  ...
                  onChange={(v) => setField("relationship_type", v)}
                />
              </>
            ) : (
              <>
            ...
            {editMode ? (
              <>
                ...
                <InputField
                  ...
                  onChange={(v) => setField("location_type", v)}
                />
              ...
              <>
                ...
                <ViewField label="Location Type">
                  {form.location_type
                    ? `${LOCATION_ICONS[form.location_type] ?? "📍"} ${form.location_type}`
                    : "—"}
                </ViewField>
                <ViewField label="Address" style={{ gridColumn: "1 / -1" }}>
                  {form.location_address?.startsWith("http") ? (
                    <a
                      href={form.location_address}
                      target="_blank"
                      rel="noreferrer"
                      style={{ color: "#24544a", fontSize: 12 }}
                    >
                      📍 Open in Maps
                    </a>
                  ) : (
          ...
          >
            {editMode ? (
              <>
                <InputField
                  label="Est. Machines"
                  value={String(form.estimated_machines ?? "")}
                  onChange={(v) =>
                    setField("estimated_machines", v ? Number(v) : null)
                  }
                  type="number"
                />
                <InputField
                  label="Rev Share %"
                  value={String(form.rev_share ?? "")}
                  onChange={(v) => setField("rev_share", v ? Number(v) : null)}
                  type="number"
                />
                <InputField
                  label="Lead Ref"
                  value={form.lead_ref ?? ""}
                  onChange={(v) => setField("lead_ref", v)}
                />
              </>
            ) : (
              <>
                <ViewField label="Est. Machines">
                  {form.estimated_machines != null
                    ? `${form.estimated_machines} machine${form.estimated_machines !== 1 ? "s" : ""}`
                    : "—"}
                </ViewField>
                <ViewField label="Rev Share">
                  {form.rev_share != null ? `${form.rev_share}%` : "—"}
                </ViewField>
            ...
            {editMode ? (
              <>
                <InputField
                  ...
                  onChange={(v) => setField("date_initiated", v)}
                  type="date"
                />
                <InputField
                  label="Last Contact"
                  value={form.last_contact_date ?? ""}
                  onChange={(v) => setField("last_contact_date", v)}
                  type="date"
                />
                <InputField
              ...
              <>
                <ViewField label="Date Initiated">
                  {fmt(form.date_initiated)}
                </ViewField>
                <ViewField label="Last Contact">
                  {fmt(form.last_contact_date)}
                </ViewField>
                ...
                <ViewField label="Installation">
                  {fmt(form.installation_date)}
                </ViewField>
              </>
            )}
          </div>
              ...
              >
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <select
                    value={newAct.type}
                    onChange={(e) =>
                      setNewAct((p) => ({ ...p, type: e.target.value }))
                    }
                    style={{
                      border: "1px solid #e8e4de",
                      borderRadius: 6,
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewAct((p) => ({ ...p, created_by: e.target.value }))
                    }
                    style={{
                      border: "1px solid #e8e4de",
                      borderRadius: 6,
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    onChange={(e) =>
                      setNewAct((p) => ({ ...p, content: e.target.value }))
                    }
                        ...
                        >
                          {act.reminder_date && (
                            <span style={{ fontSize: 10, color: "#d97706" }}>
                              🔔 Reminder: {fmt(act.reminder_date)}
                            </span>
                          )}
                          <span style={{ fontSize: 10, color: "#9ca3af" }}>
...
export default function SalesPipelinePage() {
  ...
  const [selectedLead, setSelectedLead] = useState<SalesLead | "new" | null>(
    null,
  );
  ...
  const activeCount = leads.filter(
    (l) => l.engagement_status === "Active",
  ).length;
  const wonCount = leads.filter(
    (l) => l.engagement_status === "Closed-Won",
  ).length;
  const totalMachines = leads.reduce(
    (a, l) => a + (l.estimated_machines ?? 0),
    0,
  );
  ...
  const wonMachines = leads
    .filter((l) => l.engagement_status === "Closed-Won")
    .reduce((a, l) => a + (l.estimated_machines ?? 0), 0);
  ...
  const filtered = useMemo(() => {
    return leads.filter((l) => {
      ...
      if (
        filterEngagement.length < ENGAGEMENT_STATUSES.length &&
        !filterEngagement.includes(l.engagement_status)
      )
        return false;
      if (filterOverdue) {
        if (!l.next_follow_up_date) return false;
        if (new Date(l.next_follow_up_date + "T00:00:00") >= new Date())
          return false;
      }
      if (search) {
  ...
  const columnLabel = (col: string) => {
    if (activeTab === "order")
      return PRIORITY_LABELS[Number(col)] ?? `Tier ${col}`;
    return col;
  };
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ margin: "4px 0 0", color: "#6b6860", fontSize: 14 }}>
            {loading
              ? "Loading…"
              : `${totalLeads} leads tracked across all stages`}
          </p>
        </div>
        <button
      ...
      >
        ...
        <StatCard
          ...
          value={activeCount}
          sub={`${totalMachines - wonMachines} est. machines`}
          color="#24544a"
        />
        <StatCard
          label="Closed-Won"
        ...
        <StatCard
          ...
          value={overdueCount}
          sub="Need attention"
          color={overdueCount > 0 ? "#dc2626" : "#6b6860"}
        />
        <StatCard
          label="Total Est. Machines"
          value={totalMachines}
