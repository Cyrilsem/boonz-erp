...
export default function ProductsPage() {
  ...
  // Performance tab (read-only RPC)
  const [perfBucket, setPerfBucket] = useState<SourcingBucket>("boonz");
  ...
  // Performance tab: read-only RPC, re-query on bucket change.
  useEffect(() => {
...
function nextMonthLabel(labels: string[]): string {
  const last = labels[labels.length - 1];
  const idx = MONTH_ABBR.indexOf(last);
  if (idx < 0) return "MTD";
  return MONTH_ABBR[(idx + 1) % 12];
}
...
function PerformanceTab({
  ...
  setBucket: (b: SourcingBucket) => void;
  rows: PerfRow[];
  loading: boolean;
  error: string | null;
}) {
  const labels = rows[0]?.month_labels ?? ["", "", ""];
  const mtdLabel = rows[0] ? nextMonthLabel(labels) : "MTD";
  ...
  const totals = rows.reduce(
    ...
    { m3: 0, m2: 0, m1: 0, mtd: 0, expected: 0, total_units: 0, revenue: 0 },
  );
  const totalsAvg =
    totals.total_units > 0 ? totals.revenue / totals.total_units : 0;
  ...
  const muted = "#94a3b8";
  const expBg = "rgba(52,211,153,0.10)";
  const expText = "#6ee7b7";
  const expHeadBg = "rgba(52,211,153,0.16)";
  ...
  const toggleBtn = (b: SourcingBucket): React.CSSProperties => ({
    padding: "7px 18px",
    fontSize: 12,
    fontWeight: 700,
    letterSpacing: "0.04em",
    border: "none",
    background: bucket === b ? "#34d399" : "transparent",
    color: bucket === b ? "#06281d" : muted,
    cursor: "pointer",
    fontFamily: font,
    borderRadius: 7,
    ...
    <>
      ...
      >
        ...
        >
          <button
            type="button"
            onClick={() => setBucket("boonz")}
            style={toggleBtn("boonz")}
          >
            Sourced by Boonz
          </button>
          <button
            type="button"
            onClick={() => setBucket("vox")}
            style={toggleBtn("vox")}
          >
            Sourced by VOX
          </button>
        ...
        <span style={{ fontSize: 13, color: "#6b6860", fontFamily: font }}>
          {loading
            ? "Loading…"
            : `${rows.length} products · ${fmtInt(totals.total_units)} units · AED ${fmtAed(
                totals.revenue,
              )} (3mo + MTD)`}
        </span>
      </div>
          ...
          >
            <thead>
              <tr style={{ background: headBg }}>
                ...
                <th style={th}>{labels[0] || "—"}</th>
                <th style={th}>{labels[1] || "—"}</th>
                <th style={th}>{labels[2] || "—"}</th>
                <th style={th}>{mtdLabel} MTD</th>
                <th
                  style={{
                    ...th,
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td
                    colSpan={colCount}
                    style={{ ...tdLeft, color: muted, padding: "28px 14px" }}
                  >
                    Loading performance…
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  >
                    No sales in the trailing window for this bucket.
                  </td>
                </tr>
              ) : (
                <>
                  {rows.map((r) => (
                    ...
                    >
                      <td style={{ ...tdLeft, color: muted, fontSize: 12 }}>
                        {r.rank}
                      </td>
                      <td
                        style={{
                      ...
                      <td style={{ ...td, color: muted }}>{fmtInt(r.m3)}</td>
                      <td style={{ ...td, color: muted }}>{fmtInt(r.m2)}</td>
                      <td style={td}>{fmtInt(r.m1)}</td>
                      <td style={td}>{fmtInt(r.mtd)}</td>
                      <td
                        style={{
                          ...td,
                          background: expBg,
                  ...
                  <tr style={{ background: "#0b1220" }}>
                    ...
                    {(
                      [totals.m3, totals.m2, totals.m1, totals.mtd] as number[]
                    ).map((v, i) => (
                      <td
                        key={i}
                        style={{
      ...
      >
        Trailing 3 complete months plus current-month-to-date, ranked by total
        units. <span style={{ color: expText }}>Expected</span> projects
        month-end demand from the current daily run-rate, adjusted by active
        demand-context factors (events, weather). Revenue and avg price in AED.
      </p>
    </>
  );
}
