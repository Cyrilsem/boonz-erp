...
**Phase:** 3 (Hardening)
**Status:** Shipped 2026-05-25 (all three Phase 3 items now applied; original deferrals closed in the same session)
...
## Phase 3 closeout (P3.B + C.6 shipped after CS asked to close in-session)

| #                               | Deliverable                                                           | Files                                                                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P3.B prereq                     | `remove_pod_inventory_batch` canonical RPC + spelling fix             | `supabase/migrations/20260525180000_*.sql` + `20260525190100_*.sql`                        | Wraps the driver bulk-removals INSERT. Discovered that the live `pod_inventory.status` whitelist uses `'Removed/Expired'` (no spaces) but the FE+initial RPC used `'Removed / Expired'` (with spaces); forward fix corrected the spelling. Migrated `field/trips/[machineId]/removals/page.tsx:103` from direct INSERT to RPC call.                                                                                                                                                                                                                                                                     |
| P3.B                            | A.6 hard-block trigger on direct INSERT to `pod_inventory`            | `supabase/migrations/20260525190000_*.sql`                                                 | `trg_block_direct_pod_inventory_insert` BEFORE INSERT raises SQLSTATE 42501 unless `app.via_rpc='true'`. Pre-deploy audit confirmed all 6 INSERT-doing functions set the marker. Smoke test: direct INSERT blocked; canonical RPC succeeds; GUC-reset direct INSERT blocked again. The remaining direct UPDATEs on pod_inventory (5 sites in field/inventory/page.tsx) stay out of A.6 scope and remain for PRD-013.                                                                                                                                                                                    |
| C.6 schema + writer integration | inventory_control_attempt extension + approve/reject RPC modification | `supabase/migrations/20260525200000_*.sql`, `20260525200100_*.sql`, `20260525200200_*.sql` | Added `pod_inventory_id uuid` + `edit_id uuid` columns. Replaced legacy `ica_target_path_coherence` CHECK with a strict-wider per-target_path conditional. Widened `field_changed` whitelist for `pod_add_approved` and `pod_add_rejected`. Both approve and reject RPCs now lookup the caller's open inventory_control_session (started_by + status='open') and conditionally INSERT an attempt row attributing the decision. End-to-end smoke: session opened, propose, approve, attempt row landed with target_path='pod_by_id' + populated pod_inventory_id + populated edit_id + result='success'. |
| Amendment 009                   | Constitution paragraph for inventory_control_attempt widening         | `docs/architecture/01_constitution.html` (after Amendment 008 paragraph)                   | Documents Amendment 007 extension: warehouse-only → warehouse-plus-pod attribution surface. Cody-approved with revisions (added edit_id + per-target_path conditional CHECK + Amendment 009 paragraph).                                                                                                                                                                                                                                                                                                                                                                                                 |

## Originally deferred (resolved per CS request)
