...
Merged to `main` as `9cabb43` (`--no-ff`), after fast-forwarding `main` onto `8ecd62e`. The
branch carried 14 files and every one is a PRD-115 file; three of them
(`CHANGELOG.md`, `MIGRATIONS_REGISTRY.md`, `RPC_REGISTRY.md`) are shared architecture
registers that Cody C-1 requires be amended. Other PRDs' reports were dirty in the working
tree throughout and were never staged.

Post-merge on `main`, re-run rather than inherited from the branch:
...
### Production is running this commit

The deploy recorder wrote `2026-08-14T14:58:42.728Z | 9cabb43` to `docs/DEPLOYMENTS.md`.
...
`{postgres=arwdDxtm, anon=rxtm, authenticated=arwdDxtm, service_role=arwdDxtm}` with
`reloptions = NULL` — **byte-identical to `v_dispatch_pack_progress` and
`v_machine_priority`**, two views PRD-115 never opened. `CREATE OR REPLACE VIEW` moved the
grant posture in neither direction. (The `m` verb is PG17 `MAINTAIN`, which
`information_schema.role_table_grants` does not surface; the §1 pre-image is the same
posture read through a narrower lens, not a smaller one.)

**Acceptance 2, proven on the live fleet rather than on a fixture:**

| `pack_state`  | machines | `needs_reconfirm` | `max(unresolved_n)` |
| ------------- | -------- | ----------------- | ------------------- |
| `open`        | 927      | 0                 | 17                  |
| `completed`   | 231      | 0                 | **0**               |
| `in_progress` | 1        | 0                 | 0                   |

`WHERE pack_state='completed' AND unresolved_n > 0` returns **0 rows**. The combination that
put NISSAN-0804 into "already packed for 2026-08-14" AND "Finish - 1 to resolve"
simultaneously is now unrepresentable, and no machine is currently sitting in the new state.

**FE, walked over the real path.** `boonz-erp.vercel.app` with a hand-built
`sb-<ref>-auth-token` session cookie for a real logged-in user (warehouse, `bf32624e`), both
...
Five golden fixtures are red and stay red: 14 (estimator seed vs a capacity that moved),
16 (a staging premise live warehouse stock can no longer satisfy), 42 and 46 (unchanged
pre-existing), 74 (v3 horizon elapsed). All five are argued in §6 and none belong to this
PRD, but none of them fixed themselves either. They are real work for the estimator, pin
and DR-1 owners.
