...
- [x] 06-13 plan rebuilt 2026-06-12 (CS-gated runbook, Gates 1+2 passed). 65 stale drafts for the CS-dropped NISSAN-0804/NOOK-1019/VML-1003 superseded; re-pick (lifecycle v14 signals) -> 7 machines (6 picked + VOXMCC-1005 sticky cs_added); engines add v16 / swap v10.2 (3 dead tags cap-deferred, 2 explicit pearson fallbacks) / finalize v14; Gate-1 approve 117 rows (explicit machine subset); stitch v21 dry-run + FULL battery green (48 shelf-pods conserve, 0 single-SKU drift, Activia 60/40 -> 5/3, 42 zero-WH variants excluded vs FRESH post-transfer WH, deviations 0, noncanonical 0); Gate-2 commit wrote 102 lines / 117 stitched. Post-commit assert: 0 shelves where SUM(active dispatch variants) > pod qty (28 exact, 20 WH-limited under). Duplicate fills eliminated structurally: engine read live post-refill stock (today-refilled shelves drafted qty 0).
- [x] Dispatch coverage green: all 7 approved machines have active lines (16/18/25/2/21/13/7 = 102). The 103 stale pre-rebuild dispatch lines are off the driver plan: 22 true orphans deleted by cleanup_orphan_dispatching, remaining 81 auto-skipped (skipped=true) by the resilient bridge during approve. Drivers see only the corrected v21 plan with real current/max stock fields.
