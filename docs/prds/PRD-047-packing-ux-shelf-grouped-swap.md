...
**Status:** ✅ Shipped (v2 to prod fc03e37 2026-06-23, smoke green; v1 backend P0 swap_dispatch_shelf live 2026-06-21; FE Swap dialog (1b) committed to main (`c7ac999`) + build-verified. Prod deploy of `c7ac999` was **Vercel-rate-limited** ("retry in 24h", free-tier deploy cap from frequent deploys) - NOT a build error; it auto-promotes on the next successful Vercel build of main. B1 (PRD-045) + B2 (PRD-044) are LIVE (deploy `37ce14d`). swaps_enabled untouched (false).
...
## EXECUTION LOG (2026-06-23) - v2 (grouping fix + pod-level swap)

- **Investigation.** Grouped layout keyed cards on `shelf_code` and labelled the header with `shelfLines[0].display_name` (the first SKU/flavor), so multi-flavor pods read like per-product cards (e.g. "A09 Barebells - Cookies And Cream"). Data facts: a multi-flavor pod's variants all share one `shelf_id`/`shelf_code`; a single shelf can hold >1 pod (A01 = Ice Tea + Plaay; A06 = Plaay Tablets + Santiveri).
- **PHASE 1 (FE, `e0f33ee`).** Group by `shelf_id` (one card per physical shelf); header = `shelf_code` badge + distinct **pod name(s)** ("A09 - Barebells", "A01 - Plaay Truffles - Mix, Ice Tea"); one row per `boonz_product_id` with FEFO batches collapsed (Expired column shows the earliest-expiry batch date + "N batches · FEFO" caption, never sibling rows); one shelf total. Verified 375px on machine 69195812: A09 Barebells one card/5 rows/total 20, A10 Chocolate Bar one card/total 14, multi-pod shelves each one card. axe 0, 0 controls <44px, no h-scroll.
- **PHASE 2 backend (migration `20260623080000`, `b6e7c3d`).** `spread_pod_qty` (read-only INVOKER helper, replicates stitch v26 distribution) + `swap_shelf_pod` (DEFINER writer composing `add_dispatch_row`). Dara design / Cody review: ✅ Art 1 (canonical writer composed, no direct table write), Art 4 (via_rpc/rpc_name + input/role validation + reason>=10), Art 8 (audit via add_dispatch_row edit-log + generic trigger), Art 12 (forward migration), Art 16 (reads canonical v_wh_pickable). Whole-shelf semantics resolve the multi-pod case (clear the shelf, add the new pod at capacity). The stitch distribution is NOT cleanly extractable without refactoring the live engine (byte-equivalence, Art 12), so it is **replicated exactly** in `spread_pod_qty` and the two MUST be kept in sync (noted in the function comment).
- **Backend tests (BEGIN..ROLLBACK, DO-block + RAISE, synthetic seed on a clean future plan_date — no live rows touched).** T3 ✅ swap removed all 2 seeded old-pod lines + added 5 new-pod lines. T3b ✅ Add sum = 25 = shelf capacity; each Add qty == `spread_pod_qty` (== stitch distribution) and all `source_kind='wh'`. T4 ✅ atomic: a swap to an impossible pod raised the pre-validation guard and wrote 0 partial Remove/Add lines while the seeds stayed intact.
- **PHASE 2 FE (`b6e7c3d`).** Swap dialog is pod-level: Remove = auto-filled read-only current pod(s) on the shelf, Add New = searchable in-stock pod picker (pods with >=1 WH-available variant; 56 options on the test machine), **no qty field**; Confirm calls `swap_shelf_pod`. Per-shelf "Swap pod" button + header entry (header entry shows a shelf selector). Verified 375px: Remove="Chocolate Bar" read-only, no number input, pick "Barebells" -> Confirm enables; axe 0, 0 controls <44px, no h-scroll. Evidence `/tmp/prd047v2_p1_375.png`, `/tmp/prd047v2_p2_swap.png`.
- **T9/T10** (confirm mixed / swap-then-confirm) covered by construction: `swap_shelf_pod` emits standard `refill_dispatching` rows through the canonical `add_dispatch_row` (byte-identical shape to stitch output), so the live PRD-044 `confirm_machine_packed` contract handles them with no special-casing; not browser-executed to avoid mutating live packed rows.
- **HARD SAFETY honoured:** no picker/engine change; `engine_add_pod`/`stitch_pod_to_boonz` untouched (byte-equivalent); swaps_enabled stays false; no live dispatched/packed rows modified (all mutating tests rolled back); forward-only git.
