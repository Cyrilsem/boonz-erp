...
| `receive_dispatch_line` | `refill_dispatching`, `warehouse_inventory`, `pod_inventory` | ✅ **UPDATED 2026-05-18** — M2M-aware early branch: if `is_m2m=true`, skips all WH stock ops; Remove decrements pod `current_stock` (archives at zero), Add New upserts pod via `ON CONFLICT`. Returns early with `is_m2m: true`. Previous: 2026-05-11 REMOVE handling (BUG 3 fix). |
...
| `pick_machine_manually(p_plan_date, p_machine_id, p_reason)` | `machines_to_visit` (INSERT with status='cs_added' + confirmed_at=now() auto-confirm; ON CONFLICT cleans dropped_* + reinstates as cs_added) | ✅ NEW 2026-05-18 — Gate 0 add. CS-added machines bypass picker and are auto-confirmed. Articles 1, 4, 5, 8. |
