...
| `auto_decrement_pod_inventory`        | `pod_inventory`, `pod_inventory_audit_log` | ✅ A.5b — patched 2026-04-26 · **PRD-113 (2026-08-10, `prd113_a4_fifo_never_consumes_expired`): batches expired as of the current Dubai date are INELIGIBLE for FIFO sales consumption** — expired stock never exits by assumption, only by explicit manual write-off (CS iron rule / v3 LAW 7 / golden fixture 20). Ordering among eligible batches is unchanged. Sales it cannot absorb from non-expired stock, where expired stock of that product IS present, are reported to `monitoring_alerts` under `prd113_fifo_expired_overflow`; the sale is still marked `pod_inv_decremented`, as it already was when total stock fell short. Sole caller chain: `refresh-stage1` edge fn → `run_pod_inventory_decrement()` → here. No cron caller (verified across all 48 jobs), so cron parity holds by construction. |
...
### PRD-113 — in-machine moves are not warehouse returns (2026-08-10)

| Function                               | Writes to                                                        | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `is_internal_move_dispatch(uuid)`      | — (read-only, `STABLE`, `security_invoker`)                      | ✅ LIVE. **Canonical object, Article 16** — registered in `METRICS_REGISTRY.md`. `security_invoker` deliberately: `refill_dispatching` carries policy `authenticated_read FOR SELECT` with `qual = true`, so DEFINER would buy nothing and only add an escalation surface. Returns NULL for an unknown `dispatch_id`; every caller wraps in `COALESCE(..., false)`.                                                                                           |
| `mark_internal_move_legs(date, uuid)`  | `refill_dispatching.is_internal_move`                            | ✅ LIVE. DEFINER. Article 4: sets `app.via_rpc` + `app.rpc_name`, validates caller role (warehouse / operator_admin / superadmin / manager; a NULL uid is the trusted service_role/cron context). In the `enforce_canonical_dispatch_write` allowlist. Idempotent. Never touches a settled leg or one carrying `internal_move_cleared_at`. Alerts on `prd113_internal_move_flagged`.                                                                          |
| `clear_internal_move_flag(uuid, text)` | `refill_dispatching.is_internal_move`, `internal_move_cleared_*` | ✅ LIVE. DEFINER. **The canonical un-stamp writer — Articles 1, 3, 4.** Every PRD-113 refusal message points here rather than at a raw column write, which `authenticated` can otherwise execute (table-level UPDATE grant + a permissive policy for five roles). Requires an authenticated caller, one of the four roles, and a reason of 10+ characters. Durable: no writer re-stamps a cleared leg. Alerts on `prd113_internal_move_cleared` at `warning`. |
| `tg_mark_internal_move_pair()`         | `refill_dispatching.is_internal_move` (trigger-only)             | ✅ LIVE. AFTER INSERT on `refill_dispatching`. Stamps `app.rpc_name` around its own UPDATE and restores the prior value, so `write_audit_log` names the real writer instead of crediting the enclosing engine (S-160). Both arms implemented — a Remove arriving after its Add New stamps itself; an Add New arriving after its Remove stamps the Remove.                                                                                                     |

**Guards added to existing writers (bodies otherwise byte-identical):** `wh_approve_remove_receipt`, `wh_approve_remove_receipt_multivariant`, `approve_stuck_remove` each refuse a leg the predicate identifies, immediately after their existing PRD-070 `is_m2m` guard. `approve_stuck_remove` is included because it credits through `receive_dispatch_line` exactly as the other two do — without it, it is the back door.

**Known gap, explicitly out of scope for PRD-113:** `refill_dispatching` grants `authenticated` full table-level DML (the S-308 Supabase default) on top of policy `field_and_warehouse_update_dispatch`, which passes five roles with `qual = true`. Pre-existing; closing it would break live FE writers. Recorded here so it is not rediscovered as new.
...
**2026-08-19 (DF3):** `receive_purchase_order` and `cancel_po_line` now also UPDATE `driver_tasks` (`status`, `collected_at`, `outcome_comment`) — once zero actionable lines remain on the PO, the open task auto-closes as `collected` (≥1 line received) or `cancelled` (all not_purchased). Migrations `df3_driver_tasks_autoclose_on_po_settle` + `df3_receive_purchase_order_autoclose_task`. `cancel_po` inherits via delegation. See CHANGELOG 2026-08-19.
...
| `receive_dispatch_line`                                                                                                                                 | `refill_dispatching`, `warehouse_inventory`, `pod_inventory`                     | ✅ **UPDATED 2026-08-20** (PRD-016c `prd016c_receive_trusted_when_wh_approved`) — the 3 create-new-batch stamps are now conditional: `wh_approved_at IS NOT NULL` (manager verified via `wh_approve_remove_receipt`) ⇒ `dispatch_receive` (trusted, not quarantined); otherwise `dispatch_return_unverified` as before. Kills the swap-return double-quarantine. Prior **2026-05-31** (PRD-016B Migration 2, `phaseF_prd016_unverified_return_provenance`) — create-new-batch ELSE branches (`RETURN-%`, `REMOVE-RECEIVE-%`) now stamp `provenance_reason='dispatch_return_unverified'` (→ `quarantined=true`) immediately before the INSERT and restore `dispatch_receive` after; merge paths stay trusted. Prior 2026-05-18: M2M-aware early branch (is_m2m skips WH ops); 2026-05-11 REMOVE handling (BUG 3 fix). |
...
| `backfill_archive_pod_inventory_row(uuid, text, uuid, uuid)`                                        | `pod_inventory` (status→Inactive, stock=0, est=0, removal_reason)                                              | ✅ NEW 2026-05-25 (PRD-013 P2.C) — backlog-cleanup helper. Gated to superadmin + operator_admin. `p_reason` min 10 chars. FOR UPDATE lock. Idempotent (RAISE NOTICE on already-Inactive). Used once on 2026-05-25 to archive 9 stuck pod rows. Stays available for future ad-hoc backfills under per-row CS sign-off. Articles 1, 4, 8, 12. Cody ✅ (F1 — p_reason 5→10 chars). **PRD-114 (2026-08-12): SECOND CALLER — `acknowledge_day_close_event` calls this for an `expiry_check` event whose outcome is Remove (`removal_reason = 'expired_writeoff'`) or Sold (`sold_through_<event_date>`). That satisfies the "per-row CS sign-off" clause literally: the call fires only on CS's per-row acknowledge click, never on a driver tap and never on a schedule. PRD-114 added NO new `pod_inventory` writer. ⚠️ A third body writes this same Active→Inactive transition — `driver_confirm_expired_removal`, keyed on a `pod_inventory_edits` row queued by `sweep_expired_inventory`. PRD-114 cannot reach it (its flow has no edit row) and deliberately did not invent one. Two entry points converging on ONE writer is legal; a fourth body is not.** |
...
## PRD-114 — the driver's expiry sanity checklist (2026-08-12)

| Function                                                 | Writes to                                          | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_expiry_sanity_checks(uuid)`                         | — (read-only, `STABLE`, DEFINER)                   | ✅ NEW 2026-08-12 (PRD-114 §3.1). **Canonical object, Article 16** — registered in `METRICS_REGISTRY.md` as DISJOINT from `v_machine_expiry_summary` on grain. Active, stock-bearing `pod_inventory` batches with `expiration_date <= today(Dubai)+7`; `severity` = `expired` (strictly behind today) \| `expiring`. Roles field_staff / warehouse / operator_admin / superadmin / manager; anon refused BY NAME as its own statement. `.limit(10000)`. DEFINER where INVOKER would suffice (`pod_inventory` carries `authenticated_read_inventory_pod USING (true)`) — accepted on the `get_machine_expiry_detail` precedent, recorded by Cody so it is not cited as the house default. Grants: `authenticated`, `service_role`; `anon` and `PUBLIC` revoked explicitly (a new function is born EXECUTE-to-PUBLIC).                                                                                                                                                                                                                                                                                                                        |
| `record_expiry_check(uuid, text, date, uuid)`            | `day_close_events` (kind `expiry_check`) ONLY      | ✅ NEW 2026-08-12 (PRD-114 §3.2). DEFINER. **Writes no stock.** `pod_inventory` is read under `SELECT … FOR UPDATE` — a row lock that serializes two taps on one batch, not a write. Article 4: sets `app.via_rpc` + `app.rpc_name`, validates the outcome against the closed set `exists\|sold\|remove\|skip`, validates role, refuses anon by name, refuses a closed day, refuses an acknowledged (locked) check. `p_actor` is a recorded hint, never an authorization claim. **Severity is recomputed here from the ledger and the real clock — the client never sends it** — and an `expired` batch accepts `sold\|remove` only. Re-tap UPDATEs the same row; the partial unique index `day_close_events_expiry_check_uniq` on `(event_date, payload->>'pod_inventory_id') WHERE kind='expiry_check'` makes that a guarantee rather than an intention. ⛔ **LEAF WRITER (Cody C-2):** it clears `app.via_rpc` / `app.rpc_name` on exit, which is correct at top level and would disarm a caller's guard mid-transaction (S-160 / S-197 family). Nothing in `public` may call it internally; golden fixture 114 seq 21 pins the residue. |
| `acknowledge_day_close_event(uuid, text)` — **EXTENDED** | `day_close_events`, `pod_inventory` (via delegate) | ✅ EXTENDED 2026-08-12 (PRD-114 §3.3). The PRD-112 substitution branch is carried byte-for-byte; one `ELSIF kind='expiry_check'` added. Remove → `backfill_archive_pod_inventory_row(pod, 'expired_writeoff', …)`; Sold → same writer, `sold_through_<event_date>`; Exists → no stock write, stamps `verified_at`/`verified_by`; Skip → no stock write. An already-`Inactive` batch returns `already_archived` and is NOT re-archived (that would overwrite a truer `removal_reason`). Idempotent — a second acknowledge writes nothing. **Article 6: `warehouse_inventory` is untouched in all four outcomes**, pinned by an md5 fingerprint over `status` + `warehouse_stock`, not a row count (Cody C-3). ⛔ **Role `manager` is refused BY NAME on a stock-moving outcome**, because `backfill_archive_pod_inventory_row` admits only superadmin / operator_admin. Cody C-7: do not close that gap by widening the archival gate, and do not convert the raise into a silent no-op — a stock write that quietly does not happen is worse than one that loudly refuses.                                                                  |

## 2026-08-14 — PRD-115 mid-pack plan edit safety

| RPC / object                                                                                                                       | Writes                                                                                                                                                                                                          | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `remove_dispatch_row(p_dispatch_id, p_edit_role, p_reason, p_conductor_session)` — **EXTENDED, now a `refill_plan_output` writer** | `refill_dispatching` (UPDATE include=false) + edit_log INSERT kind='remove' **+ `refill_plan_output` (UPDATE `operator_status`, `operator_comment`, `reviewed_at` — ONLY `WHERE dispatch_id = p_dispatch_id`)** | ✅ EXTENDED 2026-08-14 (PRD-115 §2.1, migration `prd115_remove_dispatch_row_tombstone`). **Cody C-1: this is the entry that makes it a legal ninth writer to `refill_plan_output` (Appendix A protected).** Column scope is exactly the three named above; the predicate is exactly `dispatch_id = p_dispatch_id` and nothing wider. Sets `operator_status='rejected'` and APPENDS `removed_at_dispatch_by <uid> <utc-ts>` to `operator_comment`. ⛔ **Never `comment`** — that field is engine-authored and `push_plan_to_dispatch` copies it verbatim onto the dispatch row. Same transaction as the `include=false` write, so a dispatch row cannot be removed without its plan parent being tombstoned. `plan_rows_tombstoned=0` is the honest report for an operator-added row with no plan parent. Every prior guard (role, `p_edit_role`, `FOR UPDATE`, picked_up refusal) is byte-identical — the UPDATE sits inside them. Articles 1, 4, 5, 8, 12, 15. Golden fixture 115 seq 1-5, 28. |
| `push_plan_to_dispatch(p_plan_date date, p_machine_name text)` → **v12_prd115_tombstone_guard**                                    | `refill_dispatching`, `refill_plan_output` (dispatched/dispatch_id link)                                                                                                                                        | ✅ UPDATED 2026-08-14 (PRD-115 §2.2, migration `prd115_push_plan_tombstone_guard`). Surgical anchored splice on the v11 body (base md5 `5f858899eecef1e75e6ae6d00fcc1c8b` → v12 `487f33107819ce8335251e235ac7405e`), following the `drift_kill_p1` pattern: base-md5 gate, three anchor-uniqueness gates, idempotent re-run. **Proven byte-identical:** reversing the three insertions reproduces the v11 md5 exactly. Adds ONE guard at the head of the plan-row loop — a plan row whose `dispatch_id` points at a dispatch row that is `include=false` AND carries an append-only `edit_kind='remove'` entry never produces a dispatch row on that `plan_date`, **regardless of `operator_status`**. Belt to §2.1's braces: `operator_status` is mutable, the edit log is not. Response gains `lines_tombstoned`. Conservation stop-ship, RC-01 §5(5a)/§5(5b), M2M pairing, FIFO walk and the duplicate-unstarted `ON CONFLICT` are untouched. Golden fixture 115 seq 13-16, 25-27.           |
| `v_machine_pack_status` — **EXTENDED**                                                                                             | — (view)                                                                                                                                                                                                        | ✅ EXTENDED 2026-08-14 (PRD-115 §2.3, migration `prd115_pack_status_needs_reconfirm`). Canonical object for "Machine pack/pickup/dispatch readiness" (`METRICS_REGISTRY.md`). `pack_state` gains `needs_reconfirm` = a FINAL confirm exists AND `ready_to_pack_close` is false. `'completed'` now IMPLIES `ready_to_pack_close`, so "already packed" + "N to resolve" is unrepresentable. `'open'` and `'in_progress'` (a deliberate `final=false` Save & come back) are unchanged — gating on `c.final` is what keeps a mid-pack save from being relabelled a fault. Two additive trailing columns: `needs_reconfirm boolean`, `unresolved_n int` (from `v_dispatch_pack_progress`, the same object `confirm_machine_packed` gates on, so whatever unblocks Finish clears the state). Grants and `reloptions` verified identical pre/post (Cody C-7). Golden fixture 115 seq 17-22.                                                                                                            |

## PRD-022 (2026-08-14 → 2026-08-15) - total-first PO pricing, advisory flags, review verdicts

Migrations `po_price_total_first_flag_only`, `procurement_events_allow_price_flag_raised`,
`po_pricing_status_v1`, `po_pricing_status_v1_rpcs`, `po_pricing_status_v1_view_and_backfill`,
`po_pricing_status_v1_receive_path`. Cody ⚠️ approve with revisions, 4 required changes + 1
statement deleted, all applied before DDL. Articles 1, 2, 3 + S-308, 4, 5, 7, 8, 12, 13, 15.

| function                                                               | writes                                                                                                                         | notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_po_addition_v2(text,uuid,numeric,numeric,date,text,text,text)` | `po_additions` (INSERT), `write_audit_log`                                                                                     | ✅ Canonical INSERT writer on `po_additions`. SECURITY DEFINER, `search_path=public,pg_temp`, roles `field_staff / warehouse / operator_admin / superadmin / manager`. **8-arg; the 7-arg signature was dropped** (see `deprecated.md`). `p_pricing_status` contract, all returning structured jsonb and never raising: `free_goods` requires total null/0 (`FREE_GOODS_WITH_PRICE`), `priced` requires total > 0 (`PRICED_WITHOUT_TOTAL`), `unpriced` is refused as an input (`BAD_PRICING_STATUS`) because it is system-assigned. ACL `{postgres=X,authenticated=X,service_role=X}` — anon revoked. |
| `review_price_flag_v1(text,uuid,text,text)`                            | `po_additions` **or** `purchase_orders` (`price_flag` → NULL), `write_audit_log`, `procurement_events` (`price_flag_reviewed`) | ✅ NEW. SECURITY DEFINER, roles `operator_admin / superadmin / manager` only. `p_verdict in ('confirmed_correct','corrected')`, `p_note` ≥ 10 chars. Refuses a row carrying no flag. Dual audit. **Does not reset `pricing_status`** — `unpriced` is a statement about the money on the row, and reviewing a flag does not put money there. Clearing does not re-arm the flagger: the UPDATE moves neither price nor qty. ACL `{postgres=X,authenticated=X,service_role=X}`.                                                                                                                          |
| `correct_procurement_unit_price_v1(text,uuid,numeric,text,boolean)`    | `po_additions` / `purchase_orders` (price), `write_audit_log`                                                                  | Unchanged body. **Anon `EXECUTE` revoked** under `po_pricing_status_v1_rpcs` (S-268). `p_dry_run` defaults true and the Price Review page requires a preview before Apply is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `receive_purchase_order(text,jsonb,jsonb)`                             | `purchase_orders`, `warehouse_inventory`, `po_additions`, `inventory_audit_log`, `procurement_events`                          | Line payload now accepts `total_price_aed` (preferred) and `pricing_status`. `price_per_unit_aed` still accepted for back-compat and surfaced as `legacy_unit_entry` / `legacy_unit_lines` in the `goods_received` payload so the soak can see whether the old shape is still in use. When a total arrives, **both** price columns are written explicitly, which leaves the trigger's derivation clauses disarmed and the RPC authoritative. `p_additions` accepts an optional `pricing_status`.                                                                                                      |
| `_mirror_po_addition_line_v1(uuid,uuid)`                               | `purchase_orders` (INSERT), `procurement_events`, `write_audit_log`                                                            | Carries `pricing_status` onto the mirrored line. Without it a `free_goods` addition mirrors into a `priced` zero-total PO line and the trigger's INSERT path stamps it `UNPRICED_RECEIPT`. **ACL still exactly `{postgres=X,service_role=X}`** — verified post-apply; `CREATE OR REPLACE` preserves grants.                                                                                                                                                                                                                                                                                           |
| `procurement_price_sync_and_flag()` _(trigger)_                        | `price_flag`, `pricing_status`, `price_per_unit_aed`, `total_price_aed` on the NEW row; `procurement_events`                   | BEFORE INSERT OR UPDATE on both tables. **Never raises.** See MIGRATIONS_REGISTRY for the load-bearing block ordering.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

⛔ **The "two INSERT writers is the ceiling" ruling is untouched.** PRD-022 adds no INSERT path to
`purchase_orders`. `review_price_flag_v1` only ever UPDATEs `price_flag` to NULL.
...
## PRD-119 D4 CORRECTION (2026-09-08) — `check_expiry_pull_candidates` + `v_expiry_pull_candidates`

Migration `20260908072418_prd119_d4_correction_pull_candidates_view.sql`. Read-only. Gives
`expiry_pull_horizon` (seeded by `prd119_d4_expiry_pull_horizon_table`) its intended job after the
load-time guard that briefly read it (`prd119_d4_k1_reads_expiry_pull_horizon`) was reverted the
same day — see MIGRATIONS_REGISTRY and `docs/prds/PRD-119-CLOSEOUT-FOLLOWUP-REPORT.md`.

| function/view                    | reads                                                                                                         | notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v_expiry_pull_candidates`       | `v_machine_expiry_batches`, `boonz_products`, `expiry_pull_horizon`, `v_shelf_sales_identity`, `pod_products` | ✅ NEW read-only helper view. `within_horizon` = category's `pull_days_before_expiry` puts the lane in its pull window; `is_pull_candidate` = within that window AND the lane's `units_on_lane / (units_7d/7)` days-to-clear exceeds `days_to_expiry`. **`is_pull_candidate` is `NULL`, not a fabricated `true`/`false`, when the boonz_product_id↔pod_product_id identity bridge can't resolve** — measured ~3% resolution fleet-wide (documented in the migration header). Article 16: velocity read is `v_shelf_sales_identity.units_7d` (canonical), never re-aggregated from `sales_history`. NOT a gate — `approve_refill_plan` does not read this view. |
| `check_expiry_pull_candidates()` | `v_expiry_pull_candidates`                                                                                    | ✅ NEW, same family as `check_expiry_unvalidated`/`assert_sales_names_resolved`. Cron `check_expiry_pull_candidates_nightly` (20:10 UTC). Alerts via `safe_monitoring_alert('expiry_pull_candidates','warning',...)` only on confirmed `pull_candidates`; returns `needs_review` (unresolved-identity, within-horizon rows) separately, never folded into the alert count.                                                                                                                                                                                                                                                                                     |
...
## 2026-09-08 — `assert_priority_runout_triggers_p1`

| function                               | reads                                                         | notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `assert_priority_runout_triggers_p1()` | `pick_urgency_params`, `pg_get_viewdef('v_machine_priority')` | ✅ NEW regression fixture (not a live-data health check) for the `v_machine_priority` GREATEST rewrite — see METRICS_REGISTRY. Two checks: structural (the view body must still contain `GREATEST(ms.s_runout, ms.s_empty, ms.s_lowfill, ms.s_expiry, ms.s_stale, ms.s_holes)` ≥3 times) and arithmetic (`GREATEST(50,0,0,0,0,0)` must clear the live `p1_threshold`). RAISEs on failure rather than alerting via `safe_monitoring_alert` — deliberate, matches this repo's DO-block fixture convention over its nightly-health-check convention. Scheduled nightly (20:15 UTC) anyway so a future reversion surfaces via `cron.job_run_details` within a day. |

## 2026-09-09 — PRD-120 closeout: Blocker 2, `push_plan_to_dispatch` v14/v15, bulk repair, nightly assertion

Full narrative: `docs/prds/PRD-120-REPORT.md`. Migrations: see MIGRATIONS_REGISTRY's 2026-09-09
PRD-120 section for the 4 files/SHAs.

| function                                                                                                                                                                | reads / writes                                                                    | notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `check_eg_resolvable(p_plan_date date)`                                                                                                                                 | `refill_plan_output`, `pod_inventory`, `boonz_products`                           | ✅ NEW, read-only `SECURITY INVOKER` (no DEFINER, no role gate — deliberate, see the migration header). Flags a Remove line whose planned flavor has no Active pod lot on that machine+shelf. Wired into Gate-1 (`src/components/RefillPlanReview.tsx`) as a non-blocking amber warning.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `push_plan_to_dispatch(p_plan_date date, p_machine_name text)` → **v14_prd120_l5_flavor_fallback → v15_prd120_l5_autorepair**                                           | `refill_dispatching`, `refill_plan_output`, `stitch_leakage`, `monitoring_alerts` | **v14 (L5 root cause):** both the general Remove/M2W branch and the M2M source branch now fall back to a shelf-wide (machine+shelf, no product filter) lot search when the exact planned flavor has zero lots, splitting across lots if the shelf holds several, tagging each corrected leg `[FLAVOR-CORRECTED: ...]`; a genuine zero-lot shelf gets a non-blocking `quantity=0` `[NO LOT ON SHELF]` leg instead of an unresolvable NULL-`pod_lot_id` leg. M2M destination leg rewritten to mirror the ACTUAL flavor(s) pulled at source (one dest leg per distinct resolved flavor, correctly `m2m_partner_id`-linked), not the originally planned flavor. **v15 (L5 item 2, defense-in-depth):** now auto-calls `repair_remove_leg_shelf_lot_bulk(p_plan_date, p_machine_name, ..., p_dry_run:=false)` right after `pair_internal_transfer_m2m`, same non-fatal `monitoring_alerts`-on-failure pattern. Response gains `remove_flavor_corrected`, `remove_no_lot_on_shelf`, `auto_repair_null_pod_lot`. |
| `repair_remove_leg_shelf_lot_bulk(p_plan_date date, p_machine_name text DEFAULT NULL, p_reason text, p_caller uuid DEFAULT NULL, p_dry_run boolean DEFAULT true)` — NEW | `refill_dispatching` (via the existing single-row `repair_remove_leg_shelf_lot`)  | ✅ NEW writer wrapper. SECURITY DEFINER, role check (operator_admin/superadmin/manager), `p_reason` ≥10 chars, sets `app.via_rpc`/`app.rpc_name`. Loops undelivered (`NOT picked_up AND NOT dispatched`, not cancelled/skipped/returned) Remove legs with NULL `pod_lot_id` for the date(+machine), calling the unmodified single-row `repair_remove_leg_shelf_lot` per row, catching per-row exceptions so one un-repairable leg doesn't abort the batch. The wrapped RPC's own exact-`boonz_product_id` lot lookup is untouched — a leg whose flavor has zero lots anywhere still reports `no Active pod lot found`; this wrapper only aggregates, it does not change what's repairable.                                                                                                                                                                                                                                                                                                                |
| `check_null_pod_lot_remove_legs()` — NEW                                                                                                                                | `refill_dispatching`                                                              | ✅ NEW, same family as `check_far_future_picked_visits()`. Cron `check_null_pod_lot_remove_legs_nightly` (20:25 UTC). Alerts via `safe_monitoring_alert('remove_leg_null_pod_lot','critical',...)` when any undelivered Remove leg carries a NULL `pod_lot_id` — the exact defect class L5 fixes at push time. Verified live 2026-09-09: reports `violation`, count=4 against real 2026-06 stragglers, all `packed=true`, deliberately left unrepaired and flagged to CS (see PRD-120-REPORT.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
