...
| Payment default / captured / gap (reconciliation)         | `get_payment_default_summary(from,to,venue_group,machine_ids)`                                                                  | ✅ LIVE — /app/performance ribbon+dark bar WIRED (PRD-028 WS4, 2026-06-12); consumer ribbons DEFERRED on CS semantics decision (see WS4 design memo: matched-only 1.27% vs total-exposure 21.03%) | ~~/app/performance ribbon + dark bar client calc~~ wired (one pdSummary call; refunds+cash own fields) · /refill/consumers + /consumers_vox ribbon stay on get_vox_commercial_report waterfall (PRD-023 binding) until CS picks option 1/2/3 in WS4 memo |
