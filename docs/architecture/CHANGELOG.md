...
- **Three states, because "total = 0" was answering two different questions.** `pricing_status` (`priced` / `free_goods` / `unpriced`) on both `po_additions` and `purchase_orders`. Before this, a receipt with no money recorded and a receipt that was genuinely free were the same row shape — both zero, both unflagged, because the flagger deliberately skips a zero unit price (it cannot be compared to a peer median). `free_goods` is a human assertion and is never flagged; `unpriced` is system-assigned on the receive transition and carries an advisory `UNPRICED_RECEIPT`. Backfill: 3 additions + 18 PO lines.
- **Nothing here can block a warehouse write, by construction.** The flagger never raises. The only refusals in the whole unit are `create_po_addition_v2` argument violations, and those return structured jsonb (`FREE_GOODS_WITH_PRICE`, `PRICED_WITHOUT_TOTAL`, `BAD_PRICING_STATUS`) rather than raising. `UNPRICED_RECEIPT` is advisory forever — a line with no price still saves.
- **The trigger's block ordering is load-bearing and was proven, not asserted.** free_goods short-circuit → unpriced→priced reset → the unpriced stamp, all placed **before** the existing `if not v_changed` early return, because a receive transition is a status change and moves neither price nor qty. The stamp keys on `v_recv OR v_prev_code = 'UNPRICED_RECEIPT'`, not on `pricing_status` alone: that is the difference between a reviewed flag staying closed and resurrecting on the next unrelated edit. Verified in a rolled-back transaction across six transitions, including qty 10 → 50 leaving the price untouched (the standing "qty edits never rescale prices" rule).
- **Cody ⚠️ approve with revisions — 4 required changes, 1 statement deleted, all applied before any DDL.** (a) The migration opened an unaudited flag-clearing path it did not close: `po_additions` carries `warehouse_update` (UPDATE, no `WITH CHECK`, no column restriction) atop the S-308 grant, so any warehouse user could have set `pricing_status='free_goods'` from the browser and — via the free_goods short-circuit — nulled a `price_flag` with no audit row, for the full week before the soak migration lands. Closed with a column-level `REVOKE UPDATE` + re-grant on the 14 pre-existing columns. `purchase_orders` needed no equivalent: `authenticated` holds SELECT only there. (b) The `begin;`/`commit;` wrapper came out — `apply_migration` already wraps the body and an inner COMMIT ends it early. (c) `receive_purchase_order_addition` added to the anon revoke; it is a second receive path on `po_additions` and fires the same new block. (d) The 7-arg `create_po_addition_v2` drop recorded in a new `docs/architecture/deprecated.md`.
- **⛔ The Vitamin Well - Care row was NOT marked `free_goods`, contrary to the PRD text.** Its total is 9.25 — Union Coop billed 1 pc and gave 7 free, and the line was already corrected on 2026-08-11 to carry that blended total. `create_po_addition_v2` in the same migration refuses to create such a row. A backfill must not seed a state its own canonical writer can neither reproduce nor repair (Article 1), and the standing blended-totals-for-mixed-lines decision is what the row already does. It stays `priced`, which is true: 9.25 was paid. If it is ever flagged, `review_price_flag_v1(verdict='confirmed_correct')` is the audited instrument.
- **⛔ Article 13 is currently unenforceable project-wide, and that is now written down.** `pg_stat_user_functions` returns NULL `calls` for **every** function including `receive_purchase_order`, which runs daily — `track_functions` is off, so the 90-day zero-call window Article 13 prescribes cannot be measured on any object. The 7-arg drop was waived on static evidence instead (0 hits in `src/`, migrations, `pg_stat_statements`, and other function bodies; the function was one day old). Recorded in `deprecated.md` with a standing open action to enable `track_functions`.
- **FE, one pass.** Receive screen: every price input is now "Total price ex-VAT (AED) — as printed on the bill" writing `total_price_aed`, with a read-only `= X.XX /unit` chip and the 90-day comparison inverted onto the derived unit. The pack-total warning inverted with it — under total-first the classic slip is a UNIT price typed into the total field, which lands a derived unit far _below_ the trailing average, so that is what now warns. Amber inline banner renders `price_flag` with the `suggested_unit_aed` "Did you mean X?" prompt. An explicit "Free goods — 0.00 is correct" checkbox is required before a zero-total line saves. The addition form routes to `create_po_addition_v2` (splitting the bill total across batches by qty, remainder to the last, so the rows sum back to what was billed) and **the last direct `.from('po_additions')` write is gone** — the four remaining call sites are all `.select`. New admin/manager page `/app/procurement/price-review` over `v_po_price_flags`: dry-run preview required before Apply, or confirm-correct with a note. Build green, `tsc --noEmit` clean.
- **The "Free / bonus units" field stays, but its arithmetic necessarily changed.** It no longer alters what is stored — under total-first the typed bill total already _is_ the cash for the billed units, so the derived unit spreads it across everything that landed, which is the same landed-cost treatment PRD-003 §12 wanted, reached without inflating then deflating the line. It now annotates the breakdown chip only.
- **Held, not applied:** `_HELD_prd022_po_additions_rpc_only.sql` drops the two direct-write policies on `po_additions`. Gated on a one-week clean soak (earliest 2026-08-22); the four preconditions and their queries are in the file header. Reads untouched.

## 2026-08-10 — PRD-113: an in-machine move is not a warehouse return, and expired stock is never assumed sold

- **The incident, read off live rows, not inferred.** MC-2004-0100-O1 plan 2026-08-07: two shelves swapped contents inside one machine (Pepsi Black `6a0a…`→`702bca73…`, Coke Zero the reverse). Both Remove legs sat in the warehouse return queue. Approving them would have credited the warehouse for units physically still inside the machine. The manager caught it; the previous occurrence was silently mishandled. The queue was `action='Remove' AND wh_approved_at IS NULL` and had a cross-machine exclusion (`is_m2m`, PRD-070) but no same-machine one.
- **One definition, four consumers (Article 16).** `is_internal_move_dispatch()` is read by `v_pending_wh_remove_confirmations` and by all three RPCs that can credit a Remove — `wh_approve_remove_receipt`, `wh_approve_remove_receipt_multivariant`, `approve_stuck_remove`. `approve_stuck_remove` is in the list because it credits through `receive_dispatch_line` exactly as the other two do; without it, it is the back door.
- **The stored column is deliberately NOT the safety net.** `refill_dispatching.is_internal_move` is a materialisation for FE labelling and audit. Every credit-bearing consumer calls the function, so a leg the writers missed is still caught live.
- **A trigger, not an edit to four writer families.** The pair is not knowable when the Remove leg is inserted — its Add New counterpart is normally written later in the same batch, so only a post-insert pass can see both legs. `stitch_pod_to_boonz` (51 KB) and `push_plan_to_dispatch` (24 KB) are therefore untouched. Same shape as `trg_flag_remove_with_transfer_intent`.
- **The false-positive direction is chosen, not overlooked.** A genuine return coinciding with a warehouse-sourced Add New of the same product on another shelf costs an _uncredited_ return (reversible); the opposite error costs _phantom stock_ (silent, and the incident). Every auto-flag lands a `prd113_internal_move_flagged` alert naming the leg (LAW 5).
- **Expired stock never exits by assumption (v3 LAW 7, golden fixture 20).** `auto_decrement_pod_inventory` ordered candidate batches `expiration_date ASC NULLS LAST` with **no expiry filter at all**, so it drained expired batches first — recording expired units as sold while they physically sat in the machine. Expired batches are now ineligible; ordering among eligible batches is unchanged. Verified across all 48 `cron.job` rows, `src/`, `supabase/functions/` and `pg_proc` that the only caller chain is `refresh-stage1` → `run_pod_inventory_decrement` → here, so "one shared implementation, not two copies" holds by construction and no edge function needed a redeploy.
- **Measured, not assumed.** 30-day repair report: 3 batches, 14 units consumed while expired (`docs/prds/PRD-113-expired-consumption-report.md`). Report only — no automatic restoration.
- **Cody ⚠️ Approve with revisions (Articles 1, 2, 3, 4, 5, 7, 8, 11, 12, 14, 15, 16), 7 conditions, all implemented before apply.** The sharpest: all three refusal messages had told the operator to "clear `refill_dispatching.is_internal_move` on the leg" — and `authenticated` holds table-level UPDATE on that table under a policy passing five roles with `qual = true`, so the instruction was executable and would have re-opened the exact credit gate the PRD exists to close. Canonical writer `clear_internal_move_flag` (authenticated caller, four roles, 10+ char reason, `warning`-level alert) replaced it, with `internal_move_cleared_at` making the human ruling durable against re-stamping.
- **Noted, out of scope:** `refill_dispatching` grants `authenticated` full table-level DML (S-308 default) atop a permissive UPDATE policy. Pre-existing; closing it would break live FE writers. Recorded so it is not rediscovered as new.

## 2026-08-08 — PRD-110 DR-10: Article 8 closed across the whole `*_proposals_v3` family

- **The gap, measured not inferred:** approving or rejecting a proposal minted no `write_audit_log` row anywhere. `write_audit_log` held **0** rows for the entire family; golden fixture 73's RED baseline (`f8664987`, 12 pass / 9 fail) recorded `no_trigger=5`, `wrong_fn=5`, `wrong_shape=5` and five real decisions producing zero audit rows. Coverage now begins at this migration — triggers are not retroactive, and the 83 pre-existing proposal rows have no audit history and never will.
- **The site list is FIVE, not the four the parking lot recorded.** Derived at apply time by SHAPE (a `public` table carrying all of `proposal_id` / `status` / `reviewed_by` / `reviewed_at`) rather than a hardcoded name list. The fifth is `reallocation_proposals_v3`, 92 live rows, which a paste of the note's four names would have left as the one divergent sibling — the exact rot DR-10 was raised to prevent. `20260808181000_prd110_dr10_proposals_family_audit_triggers` loops the shape predicate and asserts `no_trigger=wrong_fn=wrong_shape=0` in the same transaction, so a partial install cannot commit.
- **`audit_log_write('proposal_id')`, never the argument-less form.** The helper COALESCEs a missing `TG_ARGV[0]` to `'id'`, a column none of these five has, so the bare form would not error — it would log `row_pk = '?'` on every decision forever while every "is the trigger installed" check passed. Fixture 73 seq 9 is the standing guard.
- **Fixture 73 (21 assertions) decides a REAL pending proposal on each of the five inside a rolled-back PL/pgSQL subtransaction**, counting the audit row before the rollback (variables survive; rows do not). Structural assertions alone would prove a catalogue row, not an audit row (D-47 / S-173: a guard passed by inspection is not a guard passed). Verified green **21/0** (`8cd19e87`), zero `scenario_error`, and an independent post-run probe confirmed all five status populations, zero stray review notes, and the family's `write_audit_log` count byte-unchanged.
- Cody ✅ Approve (Articles 1, 2, 3, 7, 8, 12, 14, 16). `write_audit_log` verified append-only at the policy layer (`wal_no_update` / `wal_no_delete` both `USING (false)`) before adding five new write sources into it. No RPC signature moved.
...
## 2026-07-18 — Dispatch role CHECK constraints aligned to current role vocabulary (Cody PASS — Articles 1,2,7,12,14)

- **Bug:** the field packing "Swap the pod" flow (`swap_shelf_pod` → `add_dispatch_row`) failed with `new row for relation "refill_dispatching" violates check constraint "refill_dispatching_last_edited_role_chk"` for any caller whose `user_profiles.role` is `field_staff` or `warehouse` (i.e. the drivers/packers who actually do refills). Root cause: role-vocabulary drift — the app + `user_profiles` moved to `field_staff`/`warehouse`, but two dispatch CHECK constraints still only permitted the legacy `driver`/`warehouse_manager`. The canonical writer passes the caller's role straight through with no translation, so its legitimate output was rejected. Only operator_admin/superadmin/manager could swap. (Backend/MCP calls slipped through because they resolve to `system`/`operator_admin`.)
- **Fix (migration `phaseF_dispatch_role_chk_align_field_staff_warehouse`):** widened both `refill_dispatching_last_edited_role_chk` (last_edited_by_role) and `refill_dispatching_edit_log_edited_by_role_check` (edited_by_role) to additionally allow `field_staff` + `warehouse`; legacy `driver`/`warehouse_manager` retained for historical rows. Additive; RLS untouched; no audit-history rewrite; DROP+ADD is the sanctioned forward-only way to widen a CHECK (existing rows all hold allowed values, re-validation passes). Verified live: a `field_staff`-impersonated `swap_shelf_pod` on WH1-2002 A01 now succeeds — Remove + two Add New rows written with `last_edited_by_role='field_staff'`.
- Rollback: new forward migration restoring the prior two constraints (ARRAY without field_staff/warehouse).

## 2026-07-16 — Machine warehouse mapping: canonical writer + VOX invariant + data fix

- **Data fix:** NISSAN-0804-0000-L0 (INDEPENDENT) and NOVO-1023-0000-W0 (NOVO) were mis-mapped primary=WH_MCC (VOX staging); reset to primary=WH_CENTRAL, secondary=NULL. Effect had been: returns routed to WH_MCC and refills bound from_warehouse_id=WH_MCC (NISSAN 203u/82 lines, NOVO 155u/69 lines since 06-16). Reconciliation showed the real drift was cosmetic — 272u actually bound to WH_CENTRAL batches, 31u to real WH_MCC-staged batches (Central transfers/returns, NOT VOX 999 sentinels), 55u unbound (general capture-gap pattern). No inventory correction warranted.
- **Structural fix (migration `phasef_machine_warehouse_canonical_writer`, Cody PASS — Articles 1,2,4,8,12,14):** on `machines` (39 rows, all compliant, immediate validation): (1) FK machines_primary_wh_fk + machines_secondary_wh_fk → warehouses ON DELETE RESTRICT; (2) CHECK machines_nonvox_primary_central = `venue_group='VOX' OR primary_warehouse_id = WH_CENTRAL` (blocks the bug on ANY write path); (3) `set_machine_warehouse(machine_id, primary, secondary, reason)` SECURITY DEFINER canonical writer — role-gated (operator_admin/superadmin/manager), validates WH existence + primary≠secondary + VOX invariant, sets app.via_rpc/app.rpc_name. Verified present + negative test confirmed the CHECK blocks non-VOX→WH_MCC.
- Rollback: `DROP FUNCTION public.set_machine_warehouse(uuid,uuid,uuid,text); ALTER TABLE public.machines DROP CONSTRAINT machines_nonvox_primary_central, DROP CONSTRAINT machines_primary_wh_fk, DROP CONSTRAINT machines_secondary_wh_fk;`
- Follow-up: reconcile the 2 quarantined WH_MCC Freakin return units via manager approve_return (no auto-release, PRD-098). Full write-path retrospective: `BOONZ BRAIN/POSTMORTEM_backend_reliability_2026-07-16.md`.
...
- **WS-B (VOX fix):** logged manual/field refills are canonical visit evidence — `v_machine_health_signals.days_since_visit` = GREATEST(executed dispatch, latest `pod_inventory_audit_log` 'manual-refill-%' event). Proven: VOXMCC-1005 22d -> 0d in a rolled-back test. FieldCapturePanel hardened minimally: machine-scoped product list (mapped pods on that machine's shelves, with "fits N" hints), fill-to-cap qty default, offline-tolerant submit (lines never lost). METRICS_REGISTRY days_since_visit row updated.
- **WS-C:** `v_machine_priority` exposes s_runout/s_capacity/s_expiry/s_stale (exposure only — p_tier/urgency invariance proven 0 diffs; md5 a49cd7d3 -> 97e69fa0, expected+recorded). `get_machine_health` breakdown now shows the six real terms (runout carries the rounding residual; sum == urgency exactly fleet-wide). Guard v2 covers the four split chips; 0 diffs live. Closes PRD-073's and PRD-074's carry-forwards. Engines + picker md5 byte-identical.
...
- `prd072_p0/p1`: pack_dispatch_line live binding — pre-flight validation + live FEFO re-bind of stale picks, fail-soft `bind_fail_reason` on the line, and the whole-remainder `reserved_for_machine_id` pin write REMOVED (Dara: the warehouse_stock→consumer_stock move is already the qty-scoped commitment; `v_dispatch_availability` window math covers plan contention; cron-34 sweeper demoted to legacy-pin safety net). Fixtures (a) PO-after-bind re-bind, (b) pinned sibling, (c) quarantined-only, (d) Inactive-batch, (e) partial+remainder credit, plus golden not_filled/skip-guard paths: 8/8 PASS in an always-rollback prod dry run before apply.
- `prd072_p3`: `check_provenance_reason_registry()` + apply-time assert — writers-vs-`wh_provenance_reason_enum` drift is now self-detecting (the 07-03 partial-return outage class).
- `prd073_reweight_pod_splits`: canonical recommendation-driven split writer (rec x0.90 proportional, others share 0.10, dry-run default). Applied seeds from the 02-03/07 weekly doc: Activia rebuild NISSAN 45/45/10 + WH2-1018 (was sum-170) 45/45/5/5 with the missing 'Greek Yogurt Rasberry' mapping CREATED on both; Chocolate Bar VML-1003 (Bueno45/M&M25/Twix20), VML-1004 (Oreo38.56/Bueno32.14/Delice19.29 — Oreo+Delice mappings created), NISSAN (Delice51.43/Oreo38.57 — mappings created). SKIPPED: OMDCW + ALJLT-O1 Chocolate Bar (KitKat does not exist as a boonz product — create product+mapping then rerun), NISSAN Barebells (doc says C&C 45 but sole-rec formula yields 90 — CS to rule), NOOK/ALJLT-B1/AMZ-1057/AMZ-1038/Plaay/Zigi/McVities rows (ambiguous quantities or pod naming — listed in supabase/tests/prd073_gate1_runner.sql header).
- `weimi_alias_tier_v_live_shelf_stock`: tier-4 'alias' resolution — the 17 drifted WEIMI names now resolve (2 unmatched fleet-wide); 'Freakin Healthy Granola Bar' 7/7.
- B0 prod-sync: 8 MCP-applied migrations backfilled to git byte-equivalent with prod-registry version filenames (d0b0e26); registry versions for the 6 new wave-2 migrations realigned by UPDATE to the committed filenames (apply-time drift workaround).
- FE (B1): dispatch list physical-subset filter (not_filled/skipped/cancelled excluded), product-mapping + field-capture + SIM pickers Active-only, packing zero-pick branch routed through pack_dispatch_line (Article 3 residue removed). Riders already shipped earlier and verified: push v7 jsonb toast fix, PRD-020 Performance tab. `engine_add_pod`/`engine_swap_pod` md5 byte-identical throughout (ca074e57…/90f26896…).
...
**Applied to:** both (prod via MCP apply_migration, CS-signed; pushed to main)
**Migration name:** `prd063_p1_pick_urgency_params`, `prd063_p2_v_shelf_sales_identity`, `prd063_p3_v_machine_priority_urgency_rewrite` (+ held `_ROLLBACK_prd063_v_machine_priority_restore_prd058_body.sql`)
**Summary:** The 6am picker shortlists main-track P1 via `WHERE p_tier='P1_RESTOCK' ORDER BY p_score` from `v_machine_priority`, which was still PRD-058 machine-level logic (fill %, empty-shelf count, dead-slot %, machine runway). It wasted top slots on empty/dead machines (MC-2004 score 83, ALJLT-1015-0200 60, NOVO-1023) and missed real stockouts (ADDMIND-1007, a top seller ~1 day from empty, not even listed). PRD-063 rewrites `v_machine_priority` IN PLACE (no shadow) to a shelf-aware urgency model: per (machine, product-identity) shelf from new `v_shelf_sales_identity` — dvel=units30d/30, dos=stock/dvel, grade A≥0.5/B≥0.2/C>0/D=0; `urgency = 0.50·s_runout(gradeWt·clamp((H−dos)/H), worst+breadth) + 0.15·s_capacity(1−Σstock/Σcap over A/B/C) + 0.20·s_expiry((2·expired+1·exp≤3d)/6) + 0.15·s_stale(0@≤7d→100@21d)`; TIER overrides P1 if hero (A dos<H AND days_since_visit>1) OR stale (>14d) OR expired≥1 OR urgency≥50, else P2 if exp≤3d≥3 OR any A/B dos<H OR urgency≥25, else SKIP. All knobs live in new singleton `pick_urgency_params` (CROSS JOINed). Identity resolves sales (`sales_history.pod_product_name`, text) and shelf stock to a canonical pod identity via the same name tiers + a scoped pod alias (Hunter↔"Hunter Ridge", since both assorted pods sell the renamed line; pods are mixes so boonz_product_id is not a usable identity key); Pepsi Regular stays distinct from Black. New live main-P1 reproduces the CS-locked list: drops MC-2004/ALJLT-1015-0200/NOVO-1023, adds ADDMIND-1007 (hero, A-dos 0.86) + GRIT-1022 (overdue 26d), keeps AMZ-1029/USH-1008/WPP/HUAWEI/AMZ-1038 (expired override). Every prior output column preserved (sourced unchanged from v_machine_health_signals/machines/shelf_u25); appended urgency/soonest_a_dos/grade_a..d_count. Tiering is UNCAPPED in the view; the cap-8 stays a picker selection limit (main track), VOX parallel. Verified live: T1 reproduces list (full-fleet exec 100ms<800), T2 identity coverage 100% (≥95 gate), T3 `pick_machines_for_refill` reads view (column parity → mechanics intact), T4 `get_machine_health` reads view, T5 `engine_add_pod`/`engine_swap_pod` do not reference the view (byte-identical, swaps_enabled false), T6 single-row guard rejects id=2, T7 rollback file restores prior body verbatim. Picker/FE code untouched. Cody ⚠️→✅ (registry revisions at close).
**Rollback:** Apply `_ROLLBACK_prd063_v_machine_priority_restore_prd058_body.sql` (held; restores the exact captured PRD-058 body). `pick_urgency_params` + `v_shelf_sales_identity` then sit inert; drop separately for a full teardown. Forward-only otherwise.
...
## 2026-06-24 — PRD-059 expiry batch hygiene (NULL-shelf resolution + Inactive cleanup + drawer truth) APPLIED

**Phase / Article:** PRD-059 / Articles 1, 3, 5, 8, 12, 16 + Hard Rule 6 (no-destructive)
**Applied to:** prod (via MCP `apply_migration`, per-WS CS gate). NOT git-pushed to main (committed on the working branch; CS reconciles/deploys).
**Migrations (5, all forward-only, all idempotent, all audited via `tg_audit_pod_inventory` + `app.rpc_name`):**

- `prd059_ws2_resolve_shelf_backfill` — RESOLVE: backfilled `pod_inventory.shelf_id` NULL→resolved live shelf for **61** Active NULL-shelf batches that are live on the machine (resolution rule: boonz→pod_product via Active `product_mapping`, confirmed live in `v_live_shelf_stock`, shelf via `slot_lifecycle` single current shelf). Pointer-only, zero stock change. An anti-collision guard (`NOT EXISTS` Active row on the same machine+shelf+product) skips 74 "secondary expiry batch of an already-shelved product" rows that would violate `UNIQUE idx_pod_inv_active_shelf` — those are surfaced by WS6 display instead. (Of 158 RESOLVE: 61 backfilled, 74 collision-skipped, 23 ambiguous "Mix"/multi-shelf left NULL.)
- `prd059_ws3_no_mapping_inactive` — NO ACTIVE MAPPING: **23** NULL-shelf Active batches whose boonz_product has no Active mapping → `status='Inactive'`. Status transition only.
- `prd059_ws4_highlight_orphan_writeoff` — HIGHLIGHT orphans: **110** NULL-shelf Active batches mapped but not live on the machine (off-planogram / `_OLD` / WH staging) → `status='Removed/Expired'`, `removal_reason='orphan_not_on_machine'`.
- `prd059_ws5_inactive_cleanup` — Inactive cleanup: **1,901** Inactive rows with stock>0 (incl. the 23 from WS3, per CS "all inactive should be removed") → `status='Removed/Expired'`, `removal_reason='inactive_cleanup'`. `current_stock` PRESERVED (not zeroed). End state: zero Inactive-with-stock rows.
- `prd059_ws6_drawer_expiry_truth` — drawer truth (read-only RPCs): `get_machine_slots_with_expiry` gains `nearest_expiry_days`/`nearest_expiry_qty` (nearest-expiry batch per product, any horizon) so the drawer Exp Qty is never blank; NEW `get_machine_orphan_expiry(machine)` returns NULL-shelf Active batches not on any live slot (umbrella/ambiguous remainder) for the FE "Unassigned / orphan expiry" section.

**FE:** `src/app/(app)/refill/page.tsx` — Exp Qty cell falls back to `nearest_expiry_qty`; new "Unassigned / orphan expiry" section in the machine drawer (fed by `get_machine_orphan_expiry`). tsc + lint (0 errors) + production build green.
**Tests:** T1 (header == drawer-visible) holds — fleet-wide only **1** machine has a true orphan (2u/1 batch), surfaced by the orphan section; all other 96 remaining NULL-shelf batches fold into their live slot's product aggregate. T2 (card counts only Active) — 0 Inactive/Removed stock counted. T3 (no row deleted; rows in==out) — `pod_inventory` total **6,515 unchanged**; every WS3/4/5 row a reversible status transition. T4 (live-shelf parity) — WAVEMAKER Coca Cola Zero classifies RESOLVE (vs old `slot_lifecycle.is_current` miss). T5 — `engine_add_pod`/`engine_swap_pod` bodies unmodified; `swaps_enabled=false`.
**Rollback:** all reversible via `write_audit_log` (rpc_name per WS): WS2 `shelf_id`→NULL; WS3 →Active; WS4 →Active + removal_reason NULL; WS5 →Inactive + removal_reason NULL. WS6 RPCs: restore prior `get_machine_slots_with_expiry` body (git/migration history) + `DROP FUNCTION get_machine_orphan_expiry(text)`.

## 2026-06-24 — PRD-058 tunable P1/P2 priority weights + dead-stock dial APPLIED

**Phase / Article:** PRD-058 / Articles 2, 3, 12, 14, 16
**Applied to:** prod (via cowork `apply_migration`)
**Migration name:** `prd058_tunable_priority_weights` (file `20260624010000_prd058_tunable_priority_weights.sql`)
**Summary:** New single-row config table `refill_priority_params` (id smallint PK DEFAULT 1 CHECK id=1; 30 tunable `numeric` weight/threshold cols + `dead_stock_forces_p1 boolean` + updated*at/by) feeding the canonical Article-16 object `v_machine_priority`. The view was rewritten to `CROSS JOIN refill_priority_params p`, replacing every baked-in weight, tier gate, and dead-stock literal with `p.<col>` (score-internal comparison constants without a named param — runway<2/<5, units>=30, fill<40/<60, dead_slot>=15, days>=21/>=10 — stay literal per PRD; `reasons_arr` unchanged; `::numeric(6,2)` cast preserved). The dead dial: `dead_stock_forces_p1=false` + `w_expired_now=0` drops expired-only machines out of P1; raising `w_expired_now` reprioritizes them. Consumers (`get_machine_health`, `pick_machines_for_refill`) untouched — they read the view as-is. RLS: `rpp_prio_select` SELECT `USING(true)` (required — the view CROSS JOINs the single row, a role-scoped SELECT would silently empty the view); `rpp_prio_write` FOR ALL gated to operator_admin/superadmin/manager via the `user_profiles` + `(SELECT auth.uid())` join. Policy names `rpp_prio*_`to avoid colliding with`refill*policy_params`' `rpp*_`. Seeded EXACTLY to the literals baked into the view, so deploy is behavior-neutral. Dara ✅, Cody ✅ (Articles 2,3,12,14,16). **Validation (post-apply):** T1 GOLDEN live fleet md5 = `6bb5b9cbd44aa0f10f0519f7f6579dcb`(30 rows over machine_id||p_tier||p_score||reasons_arr) — byte-identical to the pre-change snapshot; seed-defaults-ok=true (all 32 cols); row_count=1 (T6 guard via PK+CHECK); both policies present; view_uses_params=true. T2/T3/T4 dial behavior proven arithmetically by the pre-apply inline diff (0/30 rows drift on defaults); T5 picker parity follows from T1 (picker reads the byte-identical view). swaps_enabled untouched (false); engine_add_pod / engine_swap_pod untouched.
**Rollback:** data-level —`UPDATE refill_priority_params SET <all cols> = <seeded defaults> WHERE id=1;`restores current behavior instantly (defaults are in the migration file + table comment). Full teardown: restore the prior`v_machine_priority`body via`CREATE OR REPLACE VIEW`(in Supabase migration history / git) then`DROP TABLE refill_priority_params;`.
...
**Applied to:** prod (MCP `apply_migration`; landed on main via prod-sync merge)
**Migration name:** `prd052_convert_removes_to_m2m_transfer` (file `20260623120000_prd052_convert_removes_to_m2m_transfer.sql`; prod schema_migrations version `20260623105700`)
...
**Applied to:** prod (eizcexopcuoycuosittm), CS-authorized ("apply B3-part1", "apply B4", "apply B3-part2"). Migrations `prd040_b3_product_landed_cost`, `prd040_b4_stitch_wh_pickable_unify`, `prd040_b3p2_engine_landed_cost_margin` (repo `20260620150000/160000/170000_*.sql`).
**Summary:** Closes the value-model cost gap and the last inline WH read.

- **B3-part1** `v_product_landed_cost` (read-only view): canonical landed cost = COALESCE(avg_30days_cost, avg_cost, AVG(product_mapping.avg_cost), AVG(pod_products.purchasing_cost), physical_type median). cost_basis flags sourced vs imputed. **307/307 coverage** (249 sourced, 58 category-median imputed). Registered as the canonical cost metric.
- **B4** `stitch_pod_to_boonz` v24 -> **v25_wh_pickable_unified**: the 3 inline WH reads (pull_overlaid.wh_avail_variant/pull_with_wh, diag resolved_no_wh_stock_warning, alert supply CTE) repointed onto canonical `v_wh_pickable` (adds in-date) in ONE pass. Authored via a guarded `pg_get_functiondef + regexp_replace + EXECUTE` DO-block (drift guards: abort if any `warehouse_inventory` remains or != 3 `v_wh_pickable` reads). **Replay (plan_date 2026-05-24, before/after dry-run): lines_built 165=165, diagnostics jsonb identical, procurement_alerts 38=38 / 0 row diff** (0 expired-but-Active batches today, so the in-date exclusion is currently a no-op; forward resilience). Closes the METRICS_REGISTRY WH-pickable stitch TODO.
- **B3-part2** `engine_swap_pod` v13 -> **v14_landed_cost_margin**: V() now consumes `v_product_landed_cost.landed_cost::numeric` (incumbent KEEP cap + candidate margin) in place of the inline `price - avg_30days_cost` proxy. DO-block surgical rewrite (guard: no `avg_30days_cost` left). **Replay (2026-06-21, baseline v13 vs landed v14, swaps forced true): 10=10 swaps, 7 changed (intended cost-coverage delta), all guardrails hold** (A1 uniqueness=1, R1 TCCC/coexistence leak=0, rate<=2, H1<=3). `engine_add_pod` v18 UNTOUCHED (does not consume margin) so **T12 holds, no re-baseline**. `swaps_enabled` stays false (Pass-3 no-op until Phase 3). Cody Approve all three (12/16, T12). Follow-up: imputed-cost (58) swaps to be watched in the Track-D supervised review; v_product_landed_cost may be re-applied numeric via DROP+CREATE in a future pass.
  **Rollback:** forward-only; B3-p2 -> CREATE OR REPLACE engine_swap_pod back to v13 (avg_30days_cost); B4 -> re-derive stitch v24 from migration; B3-p1 -> DROP VIEW v_product_landed_cost (no consumer once B3-p2 reverted).

## 2026-06-20 — PRD-040 B2: product-family taxonomy + Rule-2 family-keying APPLIED

**Phase / Article:** PRD-040 Track B2 / Articles 2, 12, 14, 16.
**Applied to:** prod (eizcexopcuoycuosittm), CS-authorized ("apply B2"). Migration `prd040_b2_family_taxonomy_rule2_narrow` (repo `supabase/migrations/20260620140000_*.sql`).
**Summary:** Closes the `product_family_id` 0/307 gap and family-keys coexistence Rule 2. (1) `coexistence_rules` gains `is_active` (supersede support; ADD COLUMN IF NOT EXISTS). (2) Populated `curated_product_families` (was empty; `boonz_products.product_family_id` FKs to it) with **136** families = pod-inherited names (from jaccard `product_families` via product_mapping) + 'BRAND: x' fallbacks. (3) Backfilled `boonz_products.product_family_id` **305/307** (2 brand-null stay NULL). (4) `_coexistence_blocks` CREATE OR REPLACE adds a `family_id` match type (the CHECK already allowed it) + an `is_active` filter on every rule join (it is the SOLE coexistence_rules reader). (5) Rule-2 NARROW flip: the 4 same-brand max-1 rows (groups 2/3/4/7) DEACTIVATED (`is_active=false`, supersede-not-delete), replaced by **13** `family_id` rules covering every family-pair each brand group occupies (preserves max-1-per-brand across sub-families). Groups 1/5/6 (cross-brand) + rule1 (TCCC) untouched. **Replay:** 0 newly-unblocked (max-1-per-brand preserved), 21 newly-blocked all Loacker (the intended max-1-Loacker-family fix the old cross-variant rule missed; CS opted in), 0 co-family leak. PRD-037 T1-T4 / PRD-039 R1 coexistence behaviour preserved. `swaps_enabled` false; engine_swap_pod/add untouched. Cody Approve (2/12/14/16); curated_product_families RLS confirmed; \_coexistence_blocks confirmed sole enforcer. **B1** (affinity metric): registered in METRICS_REGISTRY (doc); find_substitutes convergence is plan-only (not executed).
**Rollback:** forward-only; CREATE OR REPLACE `_coexistence_blocks` back to the pre-B2 body, `UPDATE coexistence_rules SET is_active=true WHERE rule_group IN (2/3/4/7 brand rows)`, delete the 13 family rows. boonz/curated backfill is additive (leave, or NULL the family column).

## 2026-06-20 — PRD-040 Track A: two built-but-unapplied migrations APPLIED

**Phase / Article:** PRD-040 Track A / Articles 1, 4, 6, 8, 12, 14, 16.
**Applied to:** prod (eizcexopcuoycuosittm), CS-authorized ("apply A1", "apply A2"). Repo files already present (`20260618130000_*`, `20260616130000_*`).
**Migration names:** `prd036_b_log_manual_refill_new_purchase`, `prd019c_compact_product_fallback_is_configured`.
**Summary:** Closed the two dormant migrations the 2026-06-20 prod-sync deliberately excluded (they were not yet applied). **A1** — `CREATE OR REPLACE log_manual_refill(text,uuid,date,jsonb,text)` (single overload) adds a per-line `new_purchase` branch: `false` (default) is byte-identical to the prior body (FEFO-decrement existing WH stock -> pod); `true` INSERTs a `warehouse_inventory` receipt batch (Active, `NEW-PURCHASE-<date>`, captured expiry, audited) then draws it fully into the pod, so a field purchase with no PO is fully in-system. CS-confirmed: field new-purchases receipt via this RPC, not procurement. Art-6: INSERT-with-initial-`status='Active'` is not a status mutation; the new batch's Active->Inactive on draw-to-0 is the pre-existing depletion trigger, not a new path; replay proved 0 pre-existing row status mutated. **A2** — `CREATE OR REPLACE VIEW v_refill_planning_compact` (read-only) adds a planogram fallback in the product COALESCE chain (0 row change in current data) + a new trailing `is_configured` column (528 true / 96 false). Replay: 624->624 rows, 0 diff on all 27 existing columns; no DB dependents. `swaps_enabled` stays false; `engine_add_pod` v18 untouched. Cody: A1 Approve (1/4/6/8/12), A2 Approve (12/14/16).
**Rollback:** forward-only; A1 -> CREATE OR REPLACE `log_manual_refill` back to the pre-036b body; A2 -> CREATE OR REPLACE VIEW back to the pre-019c body (drops `is_configured`; confirm no FE hard-depends on it first).
...
**Applied to:** prod (Supabase `eizcexopcuoycuosittm`) + repo migration file
**Migration name:** `prd031_ws3_engine_cover_capped` (`engine_add_pod` v16_no_silent_drop → v17_cover_capped)
**Summary:** CS decision (2026-06-14, Option B): cover is the target, capacity caps. The PRD's root-cause-C premise was stale — the live v16 already filled to capacity (`need_raw = GREATEST(max_stock−current_stock, driver_req)`), it did NOT use velocity×days_cover; the under-fill the PRD saw was the WS-2 stitch leak. v17 makes the stance-aware velocity cover the target: `need_raw = LEAST(GREATEST(cover_units, driver_req_qty), fill_to_cap)`, where `cover_units = CASE WHEN stance IN ('WIND DOWN','ROTATE OUT','DEAD') THEN 0 ELSE GREATEST(ROUND(velocity_target)::int, 1) END` and `velocity_target = compute_refill_decision.velocity_target = velocity×days_cover×cover_mult`. Slow-but-selling movers now fill to cover (lean), capacity binds only for fast movers, driver requests are still honored up to capacity, and the WH allocation (`prior_need` shared-SKU pool, WH cap) is unchanged. Drift fix: `compute_refill_decision` is now called with the engine's `p_days_cover` (was hardcoded 10). The engine consumes `velocity_target`, NOT `compute_refill_decision.refill_qty` — `refill_qty`/`target_units` carry an extra visual floor (`floor_pct×cap`, e.g. KEEP→70% cap) that CS's lean-slow-mover decision rejects; those fields are now advisory (METRICS_REGISTRY flags the follow-up to retire/rename). New `clamp_reason='cover_capped'`; `reasoning` gains `cover_units`+`velocity_target`; the version-tag housekeeping (delete-list, `dead_tags.tagged_by`, `driver_feedback.resolved_by_engine`, `engine_version`) bumped to v17. Applied via guarded in-DB transform (10 verified replacements). Read-only battery on the real 2026-06-15 picked plan (135 selling shelves): **v16 515 units → v17 324 units (−37%)**; 44 lean cover-capped (incl. barely-alive ROTATE OUT/WIND DOWN now 0 — Ice Tea 9→0, Nescafe 6→0, McVities 7→0, which v16 wastefully refilled to capacity), 53 capacity-binds shelves unchanged, fast movers identical. A live end-to-end run is gated by Gate-0 (8 picked-but-unconfirmed machines on the in-flight plan) — not run, to avoid touching a live plan; the read-only comparison uses the exact v17 formula against live `compute_refill_decision(...,14)`.
**Rollback:** `CREATE OR REPLACE` v16 (`need_raw = GREATEST(fill_to_cap, driver_req)`, `compute_refill_decision(...,10)`, version/tag strings → v16); live v17 md5 `53efb83f235af2518e43cbe1ea976e68`. Forward-only, no data migration.
...
**Phase / Article:** Phase F stitch / Constitution Articles 1, 4, 5, 8, 12, 14
**Applied to:** prod (Supabase `eizcexopcuoycuosittm`) + repo migration file
**Migration name:** `prd031_ws2_ws2b_stitch_onshelf_scoped` (`stitch_pod_to_boonz` v21_ws5_real_stock → v22_onshelf_scoped)
**Summary:** Pod intent was leaking between `pod_refill_plan` and the dispatched SKU two ways; this fixes both in the canonical Stage-3 writer. **WS-1 (mapping integrity) was a verified no-op:** the read-only audit found ZERO (pod,boonz,machine) duplicates and the UNIQUE + global-default partial-unique constraints already live — the "80 Red Bull rows" are 38 distinct per-machine scoped rows + global, not duplicates. No dedup was written (it would have deleted legitimate per-machine mappings); the PRD's duplicate premise was wrong. **WS-2b (machine-scoped authoritative):** `pull_raw` and procurement `pm_per_row` mapping joins move from the union `(pm.machine_id IS NULL OR pm.machine_id=a.machine_id)` to set-level precedence — a global default is admitted only via `pm.machine_id IS NULL AND NOT EXISTS (active scoped row for the same pod+machine)`. A curated machine now emits ONLY its scoped SKU set (fixes the Amazon Snack-Bar global leak: AMZ-1057 curated Delice+KitKat had been dispatched Delice+KitKat+McVities×2+Oreo). **WS-2 (off-shelf redistribution):** a new `on_shelf` bool on `pull_raw` (EXISTS in `v_pod_inventory_latest` for machine+shelf+boonz) and a windowed `shelf_has_known_variant` drive a revised `pull_resid.is_residual_variant` = `pin_qty=0 AND split_pct>0 AND (on_shelf OR action<>'REFILL' OR NOT shelf_has_known_variant)`. Because PRD-024's `total_split` window already sums split over residual-eligible variants only, excluding an off-shelf REFILL variant auto-renormalizes the on-shelf variants to 1.0 and the largest-remainder distributor hands them the full residual_pool — no unit is dropped, it redistributes to on-shelf variants. The `action<>'REFILL'` clause is Cody's required-revision #1: ADD_NEW swap-ins (variant not yet on shelf) stay eligible. An empty/unknown shelf (`shelf_has_known_variant=false`) keeps the full mapped set — identical to v21, no starvation. The deviation `m_raw`/`ex_final` block was left verbatim: it is structurally vacuous (`variant_target ≡ variant_final`, so `expected_qty > actual_qty` never fires — exactly PRD-031 root-cause D), and WS-4 replaces it as the real accuracy gate. Applied via in-DB transform of the live v21 stored source (6 character-exact replacements, each guarded). Rolled-back battery on synthetic date 2099-06-14 against real mapping+inventory: AMZ-1057 Snack Bar REFILL → Kinder Delice only (global McVities/Oreo excluded though on-shelf; off-shelf KitKat zeroed); the same Snack Bar as ADD_NEW → Delice+KitKat (exemption proven by contrast); Red Bull on a shelf with no Red Bull on-shelf → full set kept (no starvation). All synthetic rows rolled back (0 rows persisted on the date).
**Rollback:** `CREATE OR REPLACE` the v21 body (rollback md5 `52a6d3b139fc5cb5542ab733f848a01e`); live v22 md5 `5cec95904ca5da5ed99b9a9a499ecbe5`. Forward-only, no data migration.
...
**Applied to:** prod (Supabase `eizcexopcuoycuosittm`) + repo migration files
**Migration name:** `prd030_pack_outcome_enum_and_column` + `prd030_pack_dispatch_line_partial_notfilled`
**Summary:** Eliminates the dark stage where one out-of-stock line freezes a whole machine. New `pack_outcome_enum` (packed/partial/not_filled) + nullable `refill_dispatching.pack_outcome` column, set only by `pack_dispatch_line`. The writer now accepts a confirmed zero pick (empty `p_picks=[]`) as `not_filled`: no raise, no warehouse debit, no `from_wh_inventory_id` required, `packed` stays false, planned qty preserved in `original_quantity`. Partial picks set `pack_outcome='partial'` and preserve planned; `quantity` still equals total_picked so `conserve_split_dispatch_quantity` is untouched (Cody's required conservation-trigger check resolved via `original_quantity` instead of a trigger edit). BUG-006 `from_wh` guard intact for real picks; signature unchanged (no overload). Rolled-back battery green: not_filled left WH 25->25 / consumer 3->3 (no debit), partial filled=3/qty=3/original=5/outcome=partial, full=packed. Design + Cody verdict (⚠️ approve-with-revisions, both revisions resolved) in docs/prds/prd-030/state-model-design.md. Steps 3-5 (confirm_machine_packed + views + release_stale exclusion + FE + battery) follow.
**Rollback:** redeploy pack_dispatch_line v-before (md5 `63454d3d3f51d6a56e8a4852dc5c703c`); column/enum are additive (drop only if ever reverting the whole PRD).

## 2026-06-12 — PRD-029 dispatch-state-integrity steps 2+3: driver app + packing FE inertness, unskip writer
...
## 2026-06-12 — PRD-029 dispatch-state-integrity step 1: pack/return guards (skipped lines are inert)
