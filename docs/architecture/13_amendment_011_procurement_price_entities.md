...
| 3       | `purchase_orders` ✅ — `authenticated` holds SELECT only; no INSERT/UPDATE/DELETE policy exists. `po_additions` ✅ — **CLOSED 2026-09-07**: `field_staff_insert` and `warehouse_update` dropped via `20260907111412_prd022_po_additions_rpc_only.sql` after a clean soak (0 legacy_unit_lines across 17 events, 0 non-RPC INSERT rows since 2026-08-15). `create_po_addition_v2` (INSERT) and `receive_purchase_order` (UPDATE) are now the only write paths; `authenticated_read` (SELECT) is the only remaining policy, verified live. Fixture: a direct `authenticated`/`field_staff` INSERT now fails with `42501`; `create_po_addition_v2` still succeeds unaffected. |
...
1. ✅ **DONE 2026-09-07** — Apply `_HELD_prd022_po_additions_rpc_only.sql` after the soak — closes the Article 3 gap. Applied as `20260907111412_prd022_po_additions_rpc_only.sql`.
