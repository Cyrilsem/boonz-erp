...
2. **RC-05b — event semantics cleanup**: make reason-set writes with empty event GUC _clear_ `source_event_id` instead of inheriting; requires `approve_return` to stamp its own event (e.g. the return row's originating dispatch id) and a decision on `log_manual_refill` / `warehouse_expire_writeoff` row-id-as-event stamps.
