...
**Trigger event:** Resolution of refill-app issues #2, #8, #11. CS instruction (verbatim, 2026-05-04): _"in this case let's remove this constraint and auto inactive warehouse_inventory.status is manager-only condition. Actually, allow any change in the condition to be highlighted in the inventory for the warehouse manager to confirm. this would help in checks and balances and ensure if there's any error to be detected"_
...
The new rule preserves the manager as the sole gatekeeper of the _applied_ state, but lets automation surface the candidates for action. Specifically:
...
| `m3b_bind_warehouse_inventory_propose_triggers` _(pending tonight)_ | 6, 8                 | Binds the triggers AFTER UPDATE/INSERT on `warehouse_inventory` |
