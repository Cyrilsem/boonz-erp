-- Migration: phaseF_picker_v7_velocity_shelf_reweight
-- Purpose: Reweight pick_machines_for_refill toward velocity + shelf depletion.
--          Empty shelf = top P1. Dead items + long refill = P2. VOX parallel track.
-- Touches: machines_to_visit (additive cols, NOT in Appendix A) + SECURITY DEFINER fn.
-- Identity signature unchanged: pick_machines_for_refill(date). (Article 14)

-- 1) Schema additions (Dara proposal, additive, idempotent)
ALTER TABLE public.machines_to_visit
  ADD COLUMN IF NOT EXISTS service_track text NOT NULL DEFAULT 'main'
    CHECK (service_track IN ('main','vox')),
  ADD COLUMN IF NOT EXISTS priority_tier text
    CHECK (priority_tier IS NULL OR priority_tier IN ('P1_RESTOCK','P2_MAINTAIN'));

CREATE INDEX IF NOT EXISTS idx_mtv_plandate_track_tier_score
  ON public.machines_to_visit (plan_date, service_track, priority_tier, priority_score DESC)
  WHERE status = 'picked';

-- 2) Function rewrite (v7)
...
BEGIN
  ...
  -- Auto-close planned_swaps physically executed (unchanged from v6)
  WITH pending AS (
  ...
  -- Shelf-level depletion (under-25% non-empty) per machine, from live WEIMI stock
  WITH shelf_u25 AS (
  ...
  scored AS (
    SELECT s.*,
      ...
      -- Two-tier category: P1 = stockout/freshness of sellers; P2 = maintenance
      CASE
  ...
  sibling_picks AS (
    ...
    FROM scored sc
    WHERE sc.venue_group IS DISTINCT FROM 'VOX'           -- VOX not sibling-expanded (parallel daily track)
  ...
  ordered AS (
    SELECT fp.*,
      ...
      ROW_NUMBER() OVER (
        ORDER BY (fp.svc_track = 'vox'),                       -- main route first
  ...
  SELECT p_plan_date, o.machine_id, o.official_name, o.location_type, o.venue_group, o.building_id,
    ...
    -- keep legacy severity populated for back-compat: map tier->severity band by score
