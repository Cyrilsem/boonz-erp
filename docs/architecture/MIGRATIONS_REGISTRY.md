...
| `prd119_g2b_bind_dispatch_fefo_manually_quarantined` | 1,6,12 | ✅ Applied | 2026-09-07 | Close-out audit item 7: `bind_dispatch_fefo` was the one function (of 16 matching a `quarantined` grep) still missing the `manually_quarantined` check in its `_bind_tally` filter. Additive/restrictive only; verified against 0 packed lines on the live 2026-09-07 packing day. Closes PRD-118 G2b. Cody ✅. |
| `prd119_close_schedule_sentinel_overcommit_nightlies` | 11 | ✅ Applied | 2026-09-07 | Close-out audit item 8: `check_consignment_sentinel_integrity` and `check_dispatch_batch_overcommit` existed but were never scheduled (same gap as `check_expiry_unvalidated` before PRD-119 P4). Scheduling only — found 55 sentinel-bound dispatch rows and 54 overcommitted batches live, both real and unremediated, flagged for CS. Cody ✅. |
| `prd119_d5_record_variant_correction_exclude_field_staff` | 1,4 | ✅ Applied | 2026-09-07 | Close-out audit item 5: `record_variant_correction` wrongly authorized `field_staff` to reassign product identity, contradicting D5 doctrine (dates/qty yes, products no). Zero live callers confirmed repo-wide before fixing. Cody ✅. |
| `prd120_l2_substitute_dispatch_line_and_pin_assertion` | 1,4,11,12 | ✅ Applied | 2026-09-07 | New `substitute_dispatch_line` replaces `driver_substitute_dispatch_line`'s in-place mutation with an insert-and-supersede pattern (new `superseded_by` column), pinning the replacement via `pick_wh_batch_for_machine` instead of an ad-hoc FEFO query. New nightly assertion `check_unpinned_warehouse_dispatch_lines` (packed=false, dated>=today, Refill/Add New, NULL pin, source_origin=warehouse must be 0) — 17 violations found, all synthetic 2030 golden-fixture data. Verified end-to-end against a real 2026-09-07 dispatch row in a rolled-back transaction. Cody ✅. |
| `prd120_l3_assert_sales_names_resolved` | 11,16 | ✅ Applied | 2026-09-07 | New nightly assertion: sales_history rows in the last 14 days failing to resolve via `v_sales_history_resolved`, excluding an explicit ignore list (C4 Energy Drink). Fixture-verified: a trailing-space test name resolves correctly, a fake name is flagged. 0 real violations live. Cody ✅. |
| `prd120_l3_trim_sales_history_pod_product_name` | 12 | ✅ Applied | 2026-09-07 | New `BEFORE INSERT/UPDATE` trigger trimming `sales_history.pod_product_name` on write. No writer to this table exists in the repo (external WEIMI/VOX sync) — documented per the goal's own instruction for that case, DB-side default shipped instead. Defense-in-depth; the resolved view already trims at read time. Cody ✅. |

## 2026-09-07 — PRD-119b: first-week expiry fixes + three PRD-119 held items

| Migration name                                                       | Article(s) | Status                 | Date       | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------------------------------------------------- | ---------- | ---------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `prd119b_t1_remove_leg_resolves_against_shelf_lot`                   | 1,12       | ✅ Applied             | 2026-09-07 | E1: `push_plan_to_dispatch`'s two Remove-leg branches drop shelf-scoping (machine+product only), so a real dated lot on a different shelf than planned is no longer missed and mislabeled with a placeholder date. New `pod_lot_id` column pins the exact lot. `v_pod_inventory_latest` widened additively. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `prd119b_t1_repair_remove_leg_shelf_lot_rpc`                         | 1,4,12     | ✅ Applied             | 2026-09-07 | New audited `repair_remove_leg_shelf_lot` RPC, packed-aware (corrects expiry+pod_lot_id only on an already-packed row, respects `protect_packed_dispatch_row`). Repaired the 1/5 named E1 legs still open (4/5 had superseded to skipped=true). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `prd119b_t2_record_remove_leg_outcome`                               | 1,4        | ✅ Applied             | 2026-09-07 | E2: new `record_remove_leg_outcome` composes `apply_expiry_check` (the P3 tap's own writer) so driver "done"/"partial"/"not_done" on a Remove leg writes the lot decrement + WM line — one writer for both paths. Verified in a rolled-back transaction (lot 3→2, 1 disposition_events row). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `prd119b_t4_t5_lot_identity_and_lane_mismatch_orphans`               | 16         | ✅ Applied             | 2026-09-07 | E5/E6: `get_machine_slots_with_expiry` widened with `nearest_expiry_product_name` (the lot's own product, not the lane's current WEIMI product). `get_machine_orphan_expiry` gains a `lane_mismatch` reason class for lots stranded on a real live shelf whose product has since changed. DROP+CREATE (RETURNS TABLE shape change). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `prd119b_t5_orphan_lane_mismatch_require_resolved_lane_id`           | 16         | ✅ Applied             | 2026-09-07 | Follow-up: `lane_mismatch` must require the shelf's current live product to have resolved to a real `boonz_product_id` first — an unresolved WEIMI name (a separate, PRD-120 L3-tracked bug class) was flagging every lot on that shelf as a false-positive mismatch. Live: 852→729 genuine rows after the fix. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `prd119b_t5_nightly_orphan_shelf_lots_assertion`                     | 16         | ✅ Applied             | 2026-09-07 | New nightly `assert_no_orphan_shelf_lots`, same pattern as `assert_sales_names_resolved`. Live: 729 genuine stranded lots / 3168 units fleet-wide — a fleet-wide finding for CS, not remediated here (detection only). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `prd119b_t7_reconcile_disposition_line_writer`                       | 1,4,12     | ✅ Applied             | 2026-09-07 | E2+E4: new `create_reconcile_disposition_line` RPC + new `disposition_events.source='reconcile'` value + `v_wm_confirmations` widened to read it. 4 lines created for confidently-identified physical removals with no ledger record; nothing written to inventory by the RPC itself. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `prd119b_t8a_sweep_stale_delivered_lines`                            | 1,4,12     | ✅ Applied             | 2026-09-07 | Held 320/308-line sweep: new `driver_outcome='delivered_unconfirmed'` value + `sweep_stale_delivered_lines` RPC releases only the stale `warehouse_inventory.consumer_stock` reservation (no pod_inventory credit). 306/308 lines applied, 128 units released across 48 warehouse rows; 2 rows skipped (legacy `pack_outcome=NULL` data). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `prd119b_t8b_widen_unpinned_dispatch_monitor_to_packed_lines`        | 16         | ✅ Applied             | 2026-09-07 | T8(b): `check_unpinned_warehouse_dispatch_lines` (PRD-120 L2) filtered `packed=false`, structurally blind to `driver_substitute_dispatch_line`'s NULL-pin (which lands on already-packed lines). Widened to catch both pack states — 17→34 violations, all synthetic. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `prd119b_t8c_pack_dispatch_line_honours_manually_quarantined`        | 6,12       | ✅ Applied             | 2026-09-07 | G2b: `pack_dispatch_line`'s direct-pick re-validation checked `quarantined` but not `manually_quarantined` — a manager's hold could be bypassed by a direct pick. Fixed; falls through to the already-safe `v_wh_pickable` substitution path instead. Fixture-verified before/after. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `prd022_po_additions_rpc_only`                                       | 1,3,12     | ✅ Applied             | 2026-09-07 | Amendment 011 step 3: dropped `field_staff_insert`/`warehouse_update` on `po_additions` after a clean soak (0 legacy_unit_lines/17 events, 0 non-RPC INSERTs since 2026-08-15). `create_po_addition_v2`/`receive_purchase_order` are now the only writers; `authenticated_read` is the only remaining policy. Fixture: direct INSERT refused (42501), RPC still succeeds. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `followup3_1a_1b_rescope_overcommit_and_sentinel_assertions`         | 16         | ✅ Applied             | 2026-09-07 | Close-out follow-up 1a/1b: `check_dispatch_batch_overcommit` (54→0) and `check_consignment_sentinel_integrity` (55→0) re-scoped to `dispatched=false AND dispatch_date >= today`; 1b also gains a permanent `source_origin='vox_at_venue'` exemption (venue lines bind to the 2099 placeholder by design, matching PRD-118 K1's `65681b5` precedent). Both were 100% historical — nothing repinned. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `followup3_1c_expiry_unvalidated_by_machine`                         | 16         | ✅ Applied             | 2026-09-07 | Close-out follow-up 1c: `check_expiry_unvalidated` gains a `by_machine` breakdown (name+count, sorted) in its return/alert payload. Ask-list `docs/ops/expiry-ask-list-2026-09-07.md` produced for the 164 live rows (machine/shelf/product/qty, ordered by machine sales velocity). No dates invented, no rows touched. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `prd120_followup_get_machine_health_resolved_sales_names`            | 16,12      | ✅ Applied             | 2026-09-07 | Close-out follow-up item 2: `get_machine_health`'s `dead_stock_count`/`local_hero_count` moved from raw `sales_history.pod_product_name` text match to `v_sales_history_resolved` (pod_product_id). Byte-identical otherwise. Real alias case isolated in a rolled-back transaction: 0 rows → 1 row/20 qty. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `prd120_followup_get_machine_slots_with_expiry_resolved_sales_names` | 16,12      | ✅ Applied             | 2026-09-07 | Close-out follow-up item 2: `get_machine_slots_with_expiry`'s `product_velocity` CTE moved from raw name match to `v_sales_history_resolved` (pod_product_id), resolved via the function's own existing `pod_by_name` CTE. Live end-to-end fixture: ACTIVATE-2005-0000-W0 slot B6, a real `product_name_conventions` alias, `units_sold_7d` 0 → 15. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `prd120_followup_auto_generate_refill_plan_resolved_sales_names`     | 16,12      | ✅ Applied             | 2026-09-07 | Close-out follow-up item 2 (3 of 3, the live refill engine): `v_daily_avg` subquery moved from raw name match to `v_sales_history_resolved` on `v_slot.pod_product_id` (already carried by `v_live_shelf_stock`, no new CTE). This bug under-plans refill quantity, not just a report — live end-to-end fixture on ACTIVATE-2005-0000-W0 B06: before, no plan row at all for a real alias-recorded demand; after, correct REFILL to max_stock. Whole-fleet dry run post-apply: `status='ok'`. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `prd119_d3_receipt_shelf_life_and_duplicate_date_guards`             | 1,4,12,16  | ✅ Applied             | 2026-09-07 | D3 (close-out item 3): new `boonz_products.typical_shelf_life_days` (nullable), new `check_receipt_shelf_life_deviation`/`check_receipt_duplicate_expiry_dates` composed into `receive_purchase_order` (the actual warehouse-side receipt writer — not `create_po_addition_v2`) next to the existing `log_expiry_entry_suspect` call. Both warn via `safe_monitoring_alert`, never block. PRD-118 A's NULL-expiry hard refusal was already live; verified unbroken. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `prd119_d4_k1_reads_expiry_pull_horizon`                             | 1,4,5,12   | ⚠️ REVERTED 2026-09-08 | 2026-09-07 | D4: `approve_refill_plan`'s item-K Gate-2 short-dated guard reads `expiry_pull_horizon` by category instead of the hard-coded `plan_date+7`, falling back to the table's `default` row then to 7. Fixture (synthetic 2099-06-01, no live rows touched): dairy at +4d (horizon 5) refused, beverage at +20d (horizon 14) passed. **Flagged a real contradiction with PRD-119's own original D4 decision (§3, rejected category thresholds) — a CS call, not resolved here.** Cody ✅. **CS ruled on the flagged contradiction 2026-09-08: too aggressive as a load-time block (a 21-day Snacks/Confectionery horizon refused sellable chocolate on a weekly-visit machine) — see the two rows below.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `prd119_d4_correction_k1_visit_aware_floor`                          | 1,4,5,12   | ✅ Applied             | 2026-09-08 | **D4 CORRECTION.** Reverts the row above: item-K's short-dated branch no longer reads `expiry_pull_horizon`. New floor: `expiry_date <= plan_date + GREATEST(7, days_to_next_planned_visit + 3)`, next-visit read from `machines_to_visit` (`status IN ('picked','cs_added')`, nearest `plan_date > p_plan_date`); no future visit known → `GREATEST` ignores the NULL operand → falls back to the bare 7-day floor. `expiry_pull_horizon` itself untouched — moves to the pull-list job below. Fixture (rolled back, synthetic 2099-06-01, real machine ACTIVATE-2005-0000-W0, also re-run live post-apply): dairy at +4d refused regardless of cadence; chocolate at +20d with a visit in 7d passes (floor=10); chocolate at +20d with no visit known passes too (floor=7). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `prd119_d4_correction_pull_candidates_view`                          | 2,12,16    | ✅ Applied             | 2026-09-08 | **D4 CORRECTION, part 2.** New `v_expiry_pull_candidates`: category (`expiry_pull_horizon`) sets the deadline (`within_horizon`), the canonical `v_shelf_sales_identity.units_7d` (Article 16 — not the gated in-stock velocity object) decides whether the lane clears before it (`is_pull_candidate`). ⚠️ boonz_product_id↔pod_product_id identity bridge (same `pod_by_name` lowercase-match pattern as `get_machine_slots_with_expiry`) resolves only ~3% of rows fleet-wide — an unresolved row gets `is_pull_candidate = NULL` (LAW 5: never a fabricated true/false), surfaced by `check_expiry_pull_candidates()` (same family as `check_expiry_unvalidated`, nightly 20:10 UTC, `check_expiry_pull_candidates_nightly`) as `needs_review`, separate from confirmed `pull_candidates`. Fixture (rolled back, mutated a real Active `pod_inventory` row for AMZ-1038-3001-O1/"Al Ain Zero", units_7d=43): 10 units@+5d clears in 1.6d → not a candidate; 100 units@+5d needs 16.3d → IS a candidate; 100 units@+30d (outside the 14d default horizon) → not yet in the window. Live post-apply run: 0 confirmed candidates, 9 real needs-review rows (fleet's product-naming resolution gap — flagged for CS, not fixed here). Cody ✅. **Driver PULL screen not yet built — no existing FE surface consumes this view. Flagged for CS as a distinct follow-up.** |

## INCIDENT 2026-09-08/09 — K1 visit-floor regression (see `docs/architecture/INCIDENT_2026-09-08_K1_VISIT_FLOOR_REGRESSION.md`)

The D4 correction's visit-aware K1 floor (row above, `prd119_d4_correction_k1_visit_aware_floor`)
regressed live: `machines_to_visit` carried 64 `status='picked'` rows dated in 2030 (exploratory
`pick_machines_for_refill` calls, not typos), so the floor computed to ~1,500 days and
`approve_refill_plan` refused every warehouse line on the 12 affected machines. Hit live 2026-09-09,
~80 lines.

| version                                  | articles | status     | date       | note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------------------- | -------- | ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prd119_k1_cap_visit_floor_incident_fix` | 1,4,5,12 | ✅ Applied | 2026-09-08 | Caps the floor: `GREATEST(7, LEAST(COALESCE(next_visit_days+3,7), 21))`, and the next-visit lookup now ignores any `machines_to_visit` row more than 60 days past `plan_date`. The `COALESCE(...,7)` is load-bearing — a bare `GREATEST(7, LEAST(x+3,21))` would have collapsed the "no visit known" case to 21 instead of 7, since Postgres `LEAST`/`GREATEST` ignore NULL operands (`LEAST(NULL,21)=21`, not NULL). Reproduced live before the fix (17 real lines wrongly refused on today's plan across 4 machines; all 17 pass clean after). Fixture: a machine carrying a live 2030-01-04 pick row approves a normal expiry+10d line cleanly; dairy at +4d still refused; a legit 40-day-out visit caps the floor at 21 (a +25d line passes, which the old unbounded formula would have wrongly refused at 25≤43). Cody ✅. |
| `check_far_future_picked_visits_nightly` | 16       | ✅ Applied | 2026-09-08 | New `check_far_future_picked_visits()` (same family as `check_expiry_unvalidated`), nightly 20:20 UTC, alerts if any `status='picked'` row is ever >30 days ahead again. Detection-side containment for the root cause (below) until/unless it's fixed at the source. Verified clean post-cleanup, fires on a synthetic far-future row. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

**Fleet cleanup (data remediation, not a migration file):** all 64 poisoned rows unpicked via the
canonical `unpick_machine_to_visit(plan_date, machine_id, reason)` RPC — never a direct UPDATE.
Verified: all 64 calls returned `dropped:1`; 0 `status='picked'` rows remain past 30 days.

**Root cause, documented not fixed:** `pick_machines_for_refill(p_plan_date, ...)` validates
`p_plan_date < CURRENT_DATE - 7` but has no upper bound — any caller can INSERT real `picked` rows for
any future date. Recommended follow-up (reject `p_plan_date > CURRENT_DATE + 30` in that RPC) flagged
for CS, not done here — changes the automated picker's own behavior, deserves its own review.
...
## 2026-09-08 — `v_machine_priority` GREATEST rewrite + delivery-based `days_since_visit`

CS: replace the weighted-sum urgency score with the worst of the six real urgency signals (see
METRICS_REGISTRY's `v_machine_priority` row for the full note).

| version          | name                                                       | articles | note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------- | ---------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `20260908165128` | `v_machine_health_signals_delivery_based_days_since_visit` | 12,16    | `last_visit` join now requires `picked_up OR returned` — `packed`/`dispatched` alone no longer count as a visit. Only 2 consumers (`v_machine_priority`, `v_shelf_state`), no signature change. Measured live: 1/32 graded machines' value changes today, zero tier impact (already P1 either way). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `20260908165145` | `v_machine_priority_greatest_score_and_rank_in_tier`       | 12,16    | Replaces the weighted sum with `GREATEST(s_runout, s_empty, s_lowfill, s_expiry, s_stale, s_holes)` at all 5 sites (P1/P2 tier tests, `p_score`, `urgency`, `reasons_arr`'s `high_urgency`) in one `replace()` call. `s_capacity`/`s_intents` excluded from the GREATEST list per CS. Adds `rank_in_tier` (`ROW_NUMBER() OVER (PARTITION BY p_tier ORDER BY units_last_7d DESC)`) via a non-destructive outer wrapper — the already-5×-repeated `p_tier` CASE was not duplicated a 6th time. Fleet impact measured live before apply: P1=7/P2=1/P3=24 → P1=7/P2=2/P3=23; one tier change (`MPMCC-1058-0000-R0`, P3→P2, `s_stale=28.57` alone now clears `p2_threshold=25`, previously diluted to ~3.7 by `w_stale=0.13`). Cody ✅. |
| `20260908165200` | `assert_priority_runout_triggers_p1_regression`            | 16       | Regression fixture, nightly: `GREATEST(50,0,0,0,0,0)` must clear the live `p1_threshold`, AND the live view body must still contain the `GREATEST(...)` expression ≥3 times (structural guard against a future silent revert to a weighted sum). Verified fails pre-rewrite (0 occurrences), passes post-rewrite (5 occurrences, 50≥50). Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                  |

## 2026-09-09 PRD-120 closeout: Blocker 1, Blocker 2, L5 (flavor fallback + bulk repair)

CS goal: close PRD-120 completely (L4/L5 + the two blockers found 2026-09-09). Full narrative in
`docs/prds/PRD-120-REPORT.md`. L4 is FE-only, tracked separately (no migration).

| version          | name                                              | articles  | note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------- | ------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `20260909170426` | `blocker1_reject_far_future_plan_dates_at_source` | 1,4,12    | Root-cause fix for the 72 `status='cs_added'` 2030 `machines_to_visit` rows (all from `pick_machine_manually`, 2026-08-04). Adds `p_plan_date > CURRENT_DATE + 30` RAISE to `pick_machine_manually`, `add_machine_to_plan` (same defect, unexploited, fixed anyway), and `pick_machines_for_refill` (adds the missing upper bound alongside its existing 7-day lower bound). All 72 rows then unpicked via `unpick_machine_to_visit`. Fixture: all three RAISE on `2030-01-01`; a real near-term date still succeeds. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `20260909170909` | `blocker2_check_eg_resolvable_helper`             | 16        | `check_eg_resolvable(p_plan_date)`; read-only `SECURITY INVOKER` (plain SQL STABLE, no DEFINER/role-gate; mutates nothing, RLS already governs visibility), flags a `refill_plan_output` Remove line whose named flavor has no Active pod lot on that machine+shelf. Wired into the Gate-1 review screen (`src/components/RefillPlanReview.tsx`) as a non-blocking amber warning banner. Cody ✅.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `20260909173544` | `prd120_l5_push_plan_flavor_fallback`             | 1,4,12    | L5 root-cause fix in `push_plan_to_dispatch` (both the general Remove/M2W branch and the M2M source branch): when the exact-flavor lot lookup finds zero lots, fall back to a shelf-wide (machine+shelf, no product filter) search, splitting across lots/flavors if needed, tagged `[FLAVOR-CORRECTED: ...]`; if even that finds nothing, emit a non-blocking `quantity=0` `[NO LOT ON SHELF]` leg instead of an unresolvable NULL-`pod_lot_id` leg. M2M destination leg rewritten to mirror the ACTUAL flavor(s) pulled at source (one dest leg per distinct resolved flavor, correctly partner-linked), not the originally planned flavor. `rpc_version` v13→v14. Fixtures (a) wrong flavor named, only one real flavor on shelf, (b) two real flavors on shelf → two legs, (c) M2M with a flavor swap → destination adopts the pulled flavor; all 3 passed, plus an exact reproduction of the live HUAWEI-B16→AMZ-1029-A11 case. Cody ✅.                                                                                                                               |
| `20260909175530` | `prd120_l5_bulk_repair_and_auto_run`              | 1,4,12,16 | L5 items 2+4. `repair_remove_leg_shelf_lot_bulk(p_plan_date, p_machine_name, p_reason, p_caller, p_dry_run)` aggregates the existing single-row `repair_remove_leg_shelf_lot` over all undelivered NULL-`pod_lot_id` Remove legs, catching per-row failures. `push_plan_to_dispatch` now auto-calls it (`p_dry_run:=false`) after `pair_internal_transfer_m2m`, same defensive `monitoring_alerts`-on-failure pattern; `rpc_version` v14→v15. `check_null_pod_lot_remove_legs()` nightly at 20:25 UTC via `safe_monitoring_alert`. Verified live: bulk wrapper against 2026-06-03 (2 real legacy NULL-lot legs, pre-L5) aggregates 2 attempted/0 succeeded with per-row "no Active pod lot" errors caught, not aborted; against 2026-09-09 (today's pushed plan, post-L5) 0 attempted; role/reason guards both reject correctly; nightly assertion correctly reports `violation`, count=4 against the real fleet stragglers (all 2026-06, all `packed=true`, none `picked_up`/`dispatched`; left untouched, flagged to CS per the "don't touch packed rows" rule). Cody ✅. |

Fixture note: none of this section's fixtures or the bulk-repair verification calls mutated a
`packed=true`, `picked_up=true`, `dispatched=true`, or `plan_date >= 2026-09-09` **live** dispatch
row; every write-capable call above ran with `p_dry_run=true` against historical (2026-06) data, or
was a role/input-validation rejection that raised before any write. The 4 real historical
NULL-`pod_lot_id` Remove legs the nightly assertion flags are `packed=true` and were deliberately
left unrepaired this session. |
