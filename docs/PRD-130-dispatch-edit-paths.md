...
**Drafted and tested 2026-09-23** (`prd130_03_intra_machine_receive_no_wh_touch`, not yet applied, gated to after 22:00 Dubai -- this file was never written when the rest of PRD-130 shipped, since it touches `receive_dispatch_line`). Root cause confirmed live: `receive_dispatch_line` only checked `is_m2m`, with no awareness of `source_kind='intra_machine'` at all. Testing the fix in a rolled-back transaction surfaced a second, more fundamental finding: a live PRD-113 guard (`tg_block_internal_move_credit` / `is_internal_move_dispatch`) already unconditionally blocks any `action='Remove'` row from reaching `item_added=true` whenever it detects a same-machine Remove+AddNew pair -- it catches intra-machine legs too, by coincidence of its general "MC-2004 shape" heuristic, even though it has no idea `source_kind='intra_machine'` exists. That guard's `WHEN` clause is scoped to `action='Remove'` only, so it never touches an Add New row. This changes the fix's real shape: only the Add New leg is ever received (the Remove leg permanently stays a shadow record, already terminal at creation per `add_intra_machine_move`); the Add New leg's own `receive_dispatch_line` call now also archives the paired Remove leg's from-shelf `pod_inventory` row (found via the shared `m2m_transfer_id`), since the Remove leg can never independently trigger that archive itself. Verified green: to-shelf credited to `Active`, from-shelf archived to `Inactive`, no `warehouse_inventory` touch, no exception.

`protect_packed_dispatch_row` root-cause check (same migration): the live trigger locks only `boonz_product_id, pod_product_id, machine_id, shelf_id, dispatch_date` on a packed row -- it does not lock `quantity, expiry_date, skipped, cancelled` at all, for any row. The root cause as stated above does not hold; no change made to that function.
...
**Drafted and tested 2026-09-23** (`prd130_04_split_inherits_parent`, not yet applied, gated to after 22:00 Dubai). Root cause confirmed live: `wm_confirm_line_split` does not insert into `refill_dispatching` at all (only `warehouse_inventory`/`disposition_events` -- it is a warehouse-side receipt split, a different function than the driver's own split despite the overlapping name in this PRD's original text). `conserve_split_dispatch_quantity` already found a matching packed parent and decremented its quantity, but only fired when the caller had ALREADY set the child's `packed=true` correctly -- exactly the case that can be wrong, and never touched any other inherited field. Fixed: the trigger's guard is now `created_by_edit=true` (the same predicate G-SPLIT itself uses) instead of trusting `NEW.packed`, and it now force-inherits all nine listed fields from the parent, not just decrementing quantity. Verified green in a rolled-back transaction against a real packed parent row.
